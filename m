...
Received: from mail-bw0-f46.google.com ([209.85.214.46]:54650 "EHLO
	mail-bw0-f46.google.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1751412Ab0LYVWY (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Sat, 25 Dec 2010 16:22:24 -0500
Message-ID: <4D166089.8050908@gmail.com>
Date: Sat, 25 Dec 2010 22:22:17 +0100
...
Subject: Re: [RFC/PATCH] v4l: Add S5P MIPI-CSI2 interface subdev driver
References: <1293132375-26342-1-git-send-email-s.nawrocki@samsung.com> <201012241154.35159.hverkuil@xs4all.nl>
In-Reply-To: <201012241154.35159.hverkuil@xs4all.nl>
...
On 12/24/2010 11:54 AM, Hans Verkuil wrote:
 ...
 >> Hello,
 >>
 >> this patch adds a v4l2 subdev/platform device driver for S5P SoC 
MIPI-CSI2
...
Yes, I just noticed this too right after posting..
...
b/drivers/media/video/s5p-mipi_csis.c
 ...
 >> +MODULE_PARM_DESC(debug, "Enable module debug trace. Set to 1 to 
enable.");
 >> +
 ...
 >> +};
 >> +
 >> +static struct s5p_csis_state *to_s5p_csis_state(struct v4l2_subdev 
*sdev)
 >> +{
 ...
 >> +
 >> +
 >> +static void s5p_csis_enable_interrupts(struct s5p_csis_state 
*state, int on)
 >> +{
 ...
 >> +}
 >> +
 >> +static void s5p_csis_system_enable(struct s5p_csis_state *state, 
int on)
 >> +{
 >> +	u32 cfg;
 >> +
 ...
 >> +}
 >> +
 >> +static void s5p_csis_set_hsync_settle(struct s5p_csis_state *state, 
int settle)
 >> +{
 ...
 >> +	cfg = readl(state->regs + S5P_CSIS_CTRL);
 >> +	writel(cfg | S5P_CSIS_CTRL_UPDATE_SHADOW, state->regs + 
S5P_CSIS_CTRL);
 >> +}
 >> +
 >> +static void s5p_csis_clk_enable(struct s5p_csis_state *state, bool 
enable)
 >> +{
 >> +	int i;
 >> +
 ...
 >> +}
 >> +
 >> +static int s5p_csis_s_fmt(struct v4l2_subdev *sd, struct 
v4l2_mbus_framefmt *mf)
 ...
 >> +}
 >> +
 >> +static int s5p_csis_g_fmt(struct v4l2_subdev *sd, struct 
v4l2_mbus_framefmt *mf)
 >> +{
 >> +	struct s5p_csis_state *state = to_s5p_csis_state(sd);
 >> +
 ...
 >
 > Huh?
Ups, that really seems a bit pointless, will remove that line.
 >
 >> +
...
Yes, obviously more appropriate than returning -ENODEV.
 >
 >> +		goto p_err4;
 >> +	}
 >> +
...
Yes, that's indeed my omission. Will change, thanks.
 ...
 >> +
 >> +
 >> +	/* This allows to retrieve the platform device id by the host 
driver */
 ...
 >> +}
 >> +
 >> +static int s5p_csis_suspend(struct platform_device *pdev, 
pm_message_t msg)
 >> +{
 >> +	struct v4l2_subdev *sd = platform_get_drvdata(pdev);
 >> +
...
Thank you for the review. I am going to incorporate your comments
along with fixes for the few minor issues I've found.
