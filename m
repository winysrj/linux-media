...
Received: from gofer.mess.org ([88.97.38.141]:43957 "EHLO gofer.mess.org"
        ...
        id S1751381AbdJEIpc (ORCPT <rfc822;linux-media@vger.kernel.org>);
        Thu, 5 Oct 2017 04:45:32 -0400
From: Sean Young <sean@mess.org>
To: linux-media@vger.kernel.org
Subject: [PATCH v2 09/25] media: lirc: lirc interface should not be a raw decoder
Date: Thu,  5 Oct 2017 09:45:11 +0100
Message-Id: <83d0ce59fc62eb64adff8f78ec86be43b98f9d84.1507192752.git.sean@mess.org>
...
---
 ...
 10 files changed, 145 insertions(+), 243 deletions(-)
...
index efdcf2318448..9d2c68e883cb 100644
...
index ed78a141de34..94561d8b0c0b 100644
...
+	if (dev->send_mode == LIRC_MODE_SCANCODE) {
 		...
 		if (n != sizeof(scan))
@@ -202,13 +185,12 @@ static ssize_t ir_lirc_transmit_ir(struct file *file, const char __user *buf,
 	if (ret < 0)
 		goto out;
...
-
 		...
 		/*
@@ -233,20 +215,11 @@ static ssize_t ir_lirc_transmit_ir(struct file *file, const char __user *buf,
...
-
 	if (_IOC_DIR(cmd) & _IOC_WRITE) {
 		...
 		if (ret)
@@ -260,7 +233,7 @@ static long ir_lirc_ioctl(struct file *filep, unsigned int cmd,
...
@@ -273,7 +246,7 @@ static long ir_lirc_ioctl(struct file *filep, unsigned int cmd,
...
+		dev->send_mode = val;
 	...
 	/* TX settings */
@@ -307,7 +280,7 @@ static long ir_lirc_ioctl(struct file *filep, unsigned int cmd,
 			return -EINVAL;
...
@@ -317,7 +290,7 @@ static long ir_lirc_ioctl(struct file *filep, unsigned int cmd,
 		if (val <= 0)
 			return -EINVAL;
...
@@ -372,7 +345,7 @@ static long ir_lirc_ioctl(struct file *filep, unsigned int cmd,
 		if (!dev->timeout)
 			return -ENOTTY;
...
+		dev->send_timeout_reports = !!val;
 	...
 	default:
@@ -399,7 +372,7 @@ static const struct file_operations lirc_fops = {
 	.llseek		= no_llseek,
 };
...
+int ir_lirc_register(struct rc_dev *dev)
 {
 	...
 	int rc = -ENOMEM;
@@ -444,7 +417,6 @@ static int ir_lirc_register(struct rc_dev *dev)
...
@@ -458,12 +430,11 @@ static int ir_lirc_register(struct rc_dev *dev)
 		goto out;
...
+	dev->lirc_dev = ldev;
 ...
 out:
@@ -471,40 +442,8 @@ static int ir_lirc_register(struct rc_dev *dev)
 	return rc;
 }
