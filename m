...
Received: from mailout2.w1.samsung.com ([210.118.77.12]:17598 "EHLO
	...
	with ESMTP id S1758470Ab1GDRzs (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Mon, 4 Jul 2011 13:55:48 -0400
Date: Mon, 04 Jul 2011 19:55:07 +0200
From: Sylwester Nawrocki <s.nawrocki@samsung.com>
Subject: [PATCH v3 16/19] s5p-fimc: Add support for camera capture in JPEG
 format
...
Message-id: <1309802110-16682-17-git-send-email-s.nawrocki@samsung.com>
MIME-version: 1.0
...
@@ -696,6 +696,16 @@ static int fimc_cap_try_fmt_mplane(struct file *file, void *fh,
 	return 0;
 }
...
@@ -751,6 +761,9 @@ static int fimc_cap_s_fmt_mplane(struct file *file, void *priv,
...
+			clear_bit(ST_CAPT_RUN, &fimc->state);
 		...
 		if (++cap->buf_index >= FIMC_MAX_OUT_BUFS)
 			cap->buf_index = 0;
@@ -409,14 +416,12 @@ static irqreturn_t fimc_irq_handler(int irq, void *priv)
...
+			clear_bit(ST_CAPT_STREAM, &fimc->state);
 ...
 out:
@@ -588,9 +593,6 @@ int fimc_prepare_config(struct fimc_ctx *ctx, u32 flags)
...
@@ -645,7 +647,7 @@ static void fimc_dma_run(void *priv)
 		fimc_hw_set_mainscaler(ctx);
 		fimc_hw_set_target_format(ctx);
 		fimc_hw_set_rotation(ctx);
-		fimc_hw_set_effect(ctx);
+		fimc_hw_set_effect(ctx, false);
 	...
 	fimc_hw_set_output_path(ctx);
@@ -775,7 +777,7 @@ static int fimc_s_ctrl(struct v4l2_ctrl *ctrl)
...
@@ -1097,6 +1099,8 @@ static int fimc_m2m_s_fmt_mplane(struct file *file, void *fh,
...
@@ -1268,15 +1272,13 @@ static int fimc_m2m_s_crop(struct file *file, void *fh, struct v4l2_crop *cr)
...
@@ -670,7 +671,7 @@ void fimc_hw_en_irq(struct fimc_dev *fimc, int enable);
...
@@ -698,7 +699,8 @@ void fimc_adjust_mplane_format(struct fimc_fmt *fmt, u32 width, u32 height,
...
@@ -706,6 +708,7 @@ int fimc_prepare_addr(struct fimc_ctx *ctx, struct vb2_buffer *vb,
