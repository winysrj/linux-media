...
Received: from o1678950229.outbound-mail.sendgrid.net ([167.89.50.229]:4311
        "EHLO o1678950229.outbound-mail.sendgrid.net" rhost-flags-OK-OK-OK-OK)
        by vger.kernel.org with ESMTP id S966309AbeAOQin (ORCPT
...
Subject: [PATCH v5 4/9] v4l: vsp1: Convert display lists to use new body pool
Date: Mon, 15 Jan 2018 16:38:42 +0000 (UTC)
Message-Id: <eed07b9279d1273ca48c91463340c4b1b1ca05e2.1516028582.git-series.kieran.bingham+renesas@ideasonboard.com>
...
@@ -878,45 +781,12 @@ void vsp1_dlm_reset(struct vsp1_dl_manager *dlm)
 	dlm->pending = NULL;
 }
...
@@ -931,13 +801,26 @@ struct vsp1_dl_manager *vsp1_dlm_create(struct vsp1_device *vsp1,
...
@@ -954,12 +837,10 @@ void vsp1_dlm_destroy(struct vsp1_dl_manager *dlm)
 	if (!dlm)
 		return;
...
index 32771372bee2..a9cf874312c1 100644
...
+	.dims = { LUT_SIZE },
 ...
 /* -----------------------------------------------------------------------------
@@ -176,8 +178,16 @@ static void lut_configure(struct vsp1_entity *entity,
 	}
 }
...
+	.destroy = lut_destroy,
 ...
 /* -----------------------------------------------------------------------------
@@ -203,6 +213,15 @@ struct vsp1_lut *vsp1_lut_create(struct vsp1_device *vsp1)
 	if (ret < 0)
 		return ERR_PTR(ret);
