...
Received: from sj-iport-2.cisco.com ([171.71.176.71]:14860 "EHLO
	sj-iport-2.cisco.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S932481Ab1D1OaR (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Thu, 28 Apr 2011 10:30:17 -0400
From: Hans Verkuil <hansverk@cisco.com>
To: Yan@cisco.com, Yupeng <yyan@quicinc.com>
Subject: Re: [PATCH 1/1 v3] libv4l: Add plugin support to libv4l
Date: Thu, 28 Apr 2011 16:30:10 +0200
...
Message-Id: <201104281630.10868.hansverk@cisco.com>
...
> 
> Since you don't error check the close callback's return value in the
> single place you call it, you might just as well make it void
> 
> > +    int (*ioctl)(void *dev_ops_priv, int fd, unsigned long int request, 
void *arg);
...
v4l2_fd_open()
> >      in v4l2_dev_info's flags member, so care should be taken that the do 
not
...
>ioctl(devices[index].dev_ops_priv,
...
> >   		int saved_err = errno;
> >
> > @@ -277,7 +284,8 @@ static int v4l2_dequeue_and_convert(int index, struct 
v4l2_buffer *buf,
> >   		return result;
> >
> >   	do {
...
> >   			if (errno != EAGAIN) {
> >   				int saved_err = errno;
> > @@ -356,7 +364,8 @@ static int v4l2_read_and_convert(int index, unsigned 
...
buf_size);
> > +		result = devices[index].dev_ops-
>read(devices[index].dev_ops_priv,
...
> > +
> 
> This should be moved lower, to were the actual fd gets closed, if the fd
> was opened multiple times (through dup) the plugin stuff should not
> be cleaned up yet...
> 
...
> >   	/* Appearantly the kernel and / or glibc ignore the 32 most 
significant bits
> >   	   when long = 64 bits, and some applications pass an int holding the 
req to
> >   	   ioctl, causing it to get sign extended, depending upon this 
behavior */
> 
> You don't seem to use plugin_data consistently in the function, see below.
> 
...
> > +		result = devices[index].dev_ops->ioctl(plugin_data, fd, request, 
arg);
> >   		saved_err = errno;
> >   		v4l2_log_ioctl(request, arg, result);
> >   		errno = saved_err;
> 
> This is the 1 of places where you actually use plugin_data, either use it 
everywhere,
> or drop it.
> 
...
...)
> >
> >   		/* Do a real query even when converting to let the driver fill 
in
...
> >
> 
> This is the only other place you use plugin_data.
> 
...
> >   	pthread_mutex_lock(&devices[index].stream_lock);
> >
> >   	/* When not converting and the device supports read let the kernel 
handle
> >   	   it */
...
> >
> 
> I so no need for the plugin_data variable here, just use
> devices[index].dev_ops_priv directly when you call dev_ops->read.
> 
...
> > +
> 
> -1 is an error? Why would the default plugin always return error on close?
> Anyways as said before just make it void.
> 
> > +static int dev_ioctl(void *dev_ops_priv, int fd, unsigned long cmd, void 
*arg)
> > +{
...
> > +}
> > +
> > +static ssize_t dev_read(void *dev_ops_priv, int fd, void *buf, size_t 
len)
> > +{
> > +	return syscall(SYS_read, (int)(fd), (void *)(buf), (size_t)(len));
> > +}
> 
> Please use the SYS_IOCTL and SYS_READ macro's here for compat with freebsd.
> 
> > +
> > +
...
> > -		return fd;
> > -
> >   	/* Try to Register with libv4l2 (in case of failure pass the fd to 
the
> >   	   application as is) */
> >   	v4l2_fd_open(fd, 0);
> 
...
> > +++ b/lib/libv4lconvert/control/libv4lcontrol.c
> > @@ -701,7 +701,8 @@ static void v4lcontrol_get_flags_from_db(struct 
...
> > @@ -721,10 +722,12 @@ struct v4lcontrol_data *v4lcontrol_create(int fd, 
...
> > @@ -741,7 +744,8 @@ struct v4lcontrol_data *v4lcontrol_create(int fd, int 
always_needs_conversion)
...
no cost
> > @@ -749,7 +753,8 @@ struct v4lcontrol_data *v4lcontrol_create(int fd, int 
always_needs_conversion)
...
> >   				data->controls |= 1<<  i;
> >   		}
> > @@ -761,17 +766,20 @@ struct v4lcontrol_data *v4lcontrol_create(int fd, 
int always_needs_conversion)
...
> >   			break;
> >
> > @@ -788,7 +796,7 @@ struct v4lcontrol_data *v4lcontrol_create(int fd, int 
always_needs_conversion)
> >   	if (data->controls == 0)
...
> >   		goto error;
> >   	}
> > @@ -974,7 +982,8 @@ int v4lcontrol_vidioc_queryctrl(struct v4lcontrol_data 
*data, void *arg)
> >   		}
> >
...
> > @@ -1011,7 +1020,8 @@ int v4lcontrol_vidioc_g_ctrl(struct v4lcontrol_data 
*data, void *arg)
> >   			return 0;
> >   		}
> >
...
> > @@ -1032,7 +1042,8 @@ int v4lcontrol_vidioc_s_ctrl(struct v4lcontrol_data 
*data, void *arg)
> >   			return 0;
> >   		}
> >
...
> >   		free(data);
> >   		return NULL;
> > @@ -205,7 +210,8 @@ int v4lconvert_enum_fmt(struct v4lconvert_data *data, 
...
> > @@ -313,7 +319,8 @@ static int v4lconvert_do_try_format(struct 
...
> > @@ -381,7 +388,8 @@ int v4lconvert_try_format(struct v4lconvert_data 
*data,
...
> >   			*src_fmt = *dest_fmt;
> >   		return result;
> > @@ -1236,7 +1244,8 @@ static void v4lconvert_get_framesizes(struct 
v4lconvert_data *data,
> >
> >   	for (i = 0; ; i++) {
> >   		frmsize.index = i;
...
*/
> > @@ -1296,7 +1305,8 @@ int v4lconvert_enum_framesizes(struct 
v4lconvert_data *data,
> >   			errno = EINVAL;
> >   			return -1;
> >   		}
...
> > @@ -1332,7 +1342,8 @@ int v4lconvert_enum_frameintervals(struct 
v4lconvert_data *data,
> >   			errno = EINVAL;
> >   			return -1;
> >   		}
...
> >   			V4LCONVERT_ERR("%s\n", strerror(errno));
> >   		return res;
> > @@ -1377,7 +1388,8 @@ int v4lconvert_enum_frameintervals(struct 
v4lconvert_data *data,
