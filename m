...
Received: from [194.250.18.140] ([194.250.18.140]:36695 "EHLO tv-numeric.com"
	rhost-flags-FAIL-FAIL-OK-FAIL) by vger.kernel.org with ESMTP
	id S1753083AbZETNlB convert rfc822-to-8bit (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Wed, 20 May 2009 09:41:01 -0400
From: "Thierry Lelegard" <thierry.lelegard@tv-numeric.com>
To: "'Devin Heitmueller'" <dheitmueller@kernellabs.com>
Cc: <linux-media@vger.kernel.org>
Subject: RE : Hauppauge Nova-TD-500 vs. T-500
Date: Wed, 20 May 2009 15:40:44 +0200
Message-ID: <!~!UENERkVCMDkAAQACAAAAAAAAAAAAAAAAABgAAAAAAAAAJf2pBr8u1U+Z+cArRcz8PAKHAAAQAAAAFBoGmdchXU6WAjdqWZ5KogEAAAAA@tv-numeric.com>
MIME-Version: 1.0
In-Reply-To: <829197380905200605x49be3048u303109f56ab737c3@mail.gmail.com>
...
> >
> > So, from now on, I use only one antenna cable, in the top 
> aerial input.
> > Maybe this could be clarified somewhere (unless it is 
> already but I missed it).
> >
> > 3) There are still many uncorrectable errors (TS packets 
> with "transport
> > error indicator" set) in the input. The amount of 
> uncorrectable errors is
> > approximately 0.1% (depending on the frequency), while I do 
> not have any
> > with the T-500 using the same antenna.
> >
> > These errors occurs in groups of +/- 50 consecutive packets 
> with TEI set.
> > Sometimes, in the middle of packets with errors, one packet 
> has TEI clear
> > but it is still erroneous (invalid PID in this context for 
> instance).
> >
> > Note: I forgot to mention in the initial post that I set 
> the following option:
> >    options dvb_usb_dib0700 force_lna_activation=1
> >
> > 4) There is apparently a minor but annoying problem in the 
> driver concerning
> > the ioctl FE_GET_INFO. Its usage requires opening the 
> frontend device in
> > O_RDWR mode with the TD-500.
> >
> > Comparison of T-500 vs. TD-500.
> >
> > a) After boot, before the first tuning, if we open the 
> frontend in O_RDONLY
> >   more, the ioctl FE_GET_INFO succeeds with the TD-500 and 
> the T-500.
> >
> > b) While another process uses the frontend for packet 
> reception, the ioctl
> >   FE_GET_INFO succeeds with the TD-500 and the T-500, with 
> frontend in
> >   O_RDONLY mode.
> >
> > c) After the frontend has been used for packet reception 
> but no other process
> >   uses it any longer, the ioctl FE_GET_INFO fails with 
> errno "no such device"
> >   with TD-500 frontend in O_RDONLY mode. However, the ioctl 
> FE_GET_INFO succeeds
> >   with TD-500 frontend in O_RDWR mode. With the T-500, the 
> ioctl FE_GET_INFO
> >   succeeds in O_RDONLY mode.
> >
> >   Using O_RDWR mode for a simple ioctl FE_GET_INFO is a 
> problem since it
> >   fails with "device busy" when another process is using it 
> in O_RDWR mode
> >   for tuning and reception, which is normal.
> >
> >   I see here two inconsistencies and one "annoying feature":
> >   - Inconsistent behaviour of ioctl FE_GET_INFO in O_RDONLY 
> mode before and
> >     after the first tuning.
> >   - Incorrect error "no such device" because 
> /dev/dvb/adapter0/frontend0
> >     is still there and useable in O_RDWR mode.
> >   - Obligation to use O_RDWR mode for reading the 
...
> >> Hello,
> >>
> >> Like many others, I have some problems with the Hauppauge 
> Nova-TD-500.
> >>
...
> >>
> >> The two systems (the first one with one T-500 and the 
> second one with
...
> >>
> >> With the two TD-500, the 4 adapters are created under 
> /dev/dvb. Using
...
> >> packets are corrupted. This does not happen all the time. 
> Sometimes,
...
> >>
> >> I guess that most of you will say "most probably an 
> application pb"...
...
> >> 4) Windows + Terratec Cinergy T USB XE Rev 2 (not 
> supported on Linux)
> >>
> >> Of course, on Windows, the input module is a custom 
> DirectShow capture
...
> >> ioctl (frontend_fd, FE_GET_INFO, ...) returns "No such 
> device" errno.
> >> Example:
> >>
