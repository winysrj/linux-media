...
Received: from mail-pb0-f47.google.com ([209.85.160.47]:63866 "EHLO
	mail-pb0-f47.google.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1751537AbaENGqS (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Wed, 14 May 2014 02:46:18 -0400
...
Subject: [PATCH v2 1/4] [media] s5p-mfc: Add variants to access mfc registers
Date: Wed, 14 May 2014 12:16:02 +0530
Message-Id: <1400049965-1022-2-git-send-email-arun.kk@samsung.com>
...
---
 ...
 drivers/media/platform/s5p-mfc/s5p_mfc_opr_v6.c |  693 +++++++++++++++--------
 drivers/media/platform/s5p-mfc/s5p_mfc_opr_v6.h |    7 +-
 6 files changed, 707 insertions(+), 255 deletions(-)

diff --git a/drivers/media/platform/s5p-mfc/s5p_mfc.c b/drivers/media/platform/s5p-mfc/s5p_mfc.c
index 9959c9c..a80d23b 100644
--- a/drivers/media/platform/s5p-mfc/s5p_mfc.c
+++ b/drivers/media/platform/s5p-mfc/s5p_mfc.c
@@ -1225,6 +1225,7 @@ static int s5p_mfc_probe(struct platform_device *pdev)
...
@@ -369,16 +369,17 @@ static int s5p_mfc_set_dec_stream_buffer_v6(struct s5p_mfc_ctx *ctx,
...
+	WRITEL(start_num_byte, mfc_regs->d_cpb_buffer_offset);
 	...
 	mfc_debug_leave();
 	return 0;
@@ -390,6 +391,7 @@ static int s5p_mfc_set_dec_frame_buffer_v6(struct s5p_mfc_ctx *ctx)
...
+	const struct s5p_mfc_regs *mfc_regs = dev->mfc_regs;
 	...
 	int buf_size1;
 	int align_gap;
@@ -401,19 +403,19 @@ static int s5p_mfc_set_dec_frame_buffer_v6(struct s5p_mfc_ctx *ctx)
...
@@ -427,11 +429,11 @@ static int s5p_mfc_set_dec_frame_buffer_v6(struct s5p_mfc_ctx *ctx)
...
@@ -444,7 +446,7 @@ static int s5p_mfc_set_dec_frame_buffer_v6(struct s5p_mfc_ctx *ctx)
...
@@ -457,7 +459,7 @@ static int s5p_mfc_set_dec_frame_buffer_v6(struct s5p_mfc_ctx *ctx)
 		return -ENOMEM;
 	}
...
@@ -470,9 +472,10 @@ static int s5p_mfc_set_enc_stream_buffer_v6(struct s5p_mfc_ctx *ctx,
...
+	WRITEL(size, mfc_regs->e_stream_buffer_size);
 	...
 	mfc_debug(2, "stream buf addr: 0x%08lx, size: 0x%d\n",
 		  addr, size);
@@ -484,14 +487,10 @@ static void s5p_mfc_set_enc_frame_buffer_v6(struct s5p_mfc_ctx *ctx,
...
@@ -501,18 +500,14 @@ static void s5p_mfc_get_enc_frame_buffer_v6(struct s5p_mfc_ctx *ctx,
 		unsigned long *y_addr, unsigned long *c_addr)
 {
 	struct s5p_mfc_dev *dev = ctx->dev;
+	const struct s5p_mfc_regs *mfc_regs = dev->mfc_regs;
...
@@ -522,6 +517,7 @@ static void s5p_mfc_get_enc_frame_buffer_v6(struct s5p_mfc_ctx *ctx,
...
@@ -533,24 +529,24 @@ static int s5p_mfc_set_enc_ref_buffer_v6(struct s5p_mfc_ctx *ctx)
...
@@ -561,7 +557,7 @@ static int s5p_mfc_set_enc_ref_buffer_v6(struct s5p_mfc_ctx *ctx)
 		return -ENOMEM;
 	}
...
@@ -573,18 +569,19 @@ static int s5p_mfc_set_enc_ref_buffer_v6(struct s5p_mfc_ctx *ctx)
...
+		WRITEL(0x0, mfc_regs->e_mslice_size_bits);
 	...
 	return 0;
@@ -593,27 +590,28 @@ static int s5p_mfc_set_slice_mode(struct s5p_mfc_ctx *ctx)
...
+	WRITEL(reg, mfc_regs->e_gop_config);
 	...
 	/* multi-slice control */
 	/* multi-slice MB number or bit size */
@@ -621,65 +619,65 @@ static int s5p_mfc_set_enc_params(struct s5p_mfc_ctx *ctx)
 	reg = 0;
...
+	WRITEL(0x0, mfc_regs->e_padding_ctrl);
 	if (p->pad) {
 		reg = 0;
 		/** enable */
@@ -690,64 +688,64 @@ static int s5p_mfc_set_enc_params(struct s5p_mfc_ctx *ctx)
 		reg |= ((p->pad_cb & 0xFF) << 8);
 		/** y value */
 		reg |= p->pad_luma & 0xFF;
...
@@ -757,6 +755,7 @@ static int s5p_mfc_set_enc_params(struct s5p_mfc_ctx *ctx)
...
+	const struct s5p_mfc_regs *mfc_regs = dev->mfc_regs;
 	...
 	unsigned int reg = 0;
@@ -767,10 +766,10 @@ static int s5p_mfc_set_enc_params_h264(struct s5p_mfc_ctx *ctx)
...
+	WRITEL(reg, mfc_regs->e_gop_config);
 	...
 	/* profile & level */
 	reg = 0;
@@ -778,18 +777,19 @@ static int s5p_mfc_set_enc_params_h264(struct s5p_mfc_ctx *ctx)
...
+	WRITEL(reg, mfc_regs->e_rc_config);
 	...
 	/* max & min value of QP */
 	reg = 0;
@@ -797,16 +797,16 @@ static int s5p_mfc_set_enc_params_h264(struct s5p_mfc_ctx *ctx)
...
+		WRITEL(reg, mfc_regs->e_fixed_picture_qp);
 	...
 	/* frame rate */
@@ -814,38 +814,38 @@ static int s5p_mfc_set_enc_params_h264(struct s5p_mfc_ctx *ctx)
 		reg = 0;
...
@@ -855,7 +855,7 @@ static int s5p_mfc_set_enc_params_h264(struct s5p_mfc_ctx *ctx)
 		reg = 0x00;
...
@@ -865,28 +865,28 @@ static int s5p_mfc_set_enc_params_h264(struct s5p_mfc_ctx *ctx)
 		reg = 0x00;
...
+	WRITEL(0x0, mfc_regs->e_mb_rc_config);
 	if (p->rc_mb) {
 		reg = 0;
 		/** dark region */
@@ -897,92 +897,95 @@ static int s5p_mfc_set_enc_params_h264(struct s5p_mfc_ctx *ctx)
...
+		WRITEL(reg, mfc_regs->e_h264_frame_packing_sei_info);
 	...
 	if (p_h264->fmo) {
@@ -992,8 +995,8 @@ static int s5p_mfc_set_enc_params_h264(struct s5p_mfc_ctx *ctx)
...
@@ -1004,10 +1007,10 @@ static int s5p_mfc_set_enc_params_h264(struct s5p_mfc_ctx *ctx)
...
@@ -1018,11 +1021,11 @@ static int s5p_mfc_set_enc_params_h264(struct s5p_mfc_ctx *ctx)
 		}
...
+		WRITEL(0, mfc_regs->e_h264_fmo_num_slice_grp_minus1);
 	...
 	mfc_debug_leave();
@@ -1033,6 +1036,7 @@ static int s5p_mfc_set_enc_params_h264(struct s5p_mfc_ctx *ctx)
...
+	const struct s5p_mfc_regs *mfc_regs = dev->mfc_regs;
 	...
 	unsigned int reg = 0;
@@ -1042,10 +1046,10 @@ static int s5p_mfc_set_enc_params_mpeg4(struct s5p_mfc_ctx *ctx)
 	s5p_mfc_set_enc_params(ctx);
...
+	WRITEL(reg, mfc_regs->e_gop_config);
 	...
 	/* profile & level */
 	reg = 0;
@@ -1053,18 +1057,19 @@ static int s5p_mfc_set_enc_params_mpeg4(struct s5p_mfc_ctx *ctx)
...
+	WRITEL(reg, mfc_regs->e_rc_config);
 	...
 	/* max & min value of QP */
 	reg = 0;
@@ -1072,16 +1077,16 @@ static int s5p_mfc_set_enc_params_mpeg4(struct s5p_mfc_ctx *ctx)
...
+		WRITEL(reg, mfc_regs->e_fixed_picture_qp);
 	...
 	/* frame rate */
@@ -1089,21 +1094,21 @@ static int s5p_mfc_set_enc_params_mpeg4(struct s5p_mfc_ctx *ctx)
 		reg = 0;
 		reg |= ((p->rc_framerate_num & 0xFFFF) << 16);
 		reg |= p->rc_framerate_denom & 0xFFFF;
-		WRITEL(reg, S5P_FIMV_E_RC_FRAME_RATE_V6);
...
@@ -1113,6 +1118,7 @@ static int s5p_mfc_set_enc_params_mpeg4(struct s5p_mfc_ctx *ctx)
...
+	const struct s5p_mfc_regs *mfc_regs = dev->mfc_regs;
 	...
 	unsigned int reg = 0;
@@ -1125,18 +1131,19 @@ static int s5p_mfc_set_enc_params_h263(struct s5p_mfc_ctx *ctx)
 	reg = 0;
 	/** profile */
 	reg |= (0x1 << 4);
-	WRITEL(reg, S5P_FIMV_E_PICTURE_PROFILE_V6);
...
+	WRITEL(reg, mfc_regs->e_rc_config);
 	...
 	/* max & min value of QP */
 	reg = 0;
@@ -1144,16 +1151,16 @@ static int s5p_mfc_set_enc_params_h263(struct s5p_mfc_ctx *ctx)
...
+		WRITEL(reg, mfc_regs->e_fixed_picture_qp);
 	...
 	/* frame rate */
@@ -1161,16 +1168,16 @@ static int s5p_mfc_set_enc_params_h263(struct s5p_mfc_ctx *ctx)
 		reg = 0;
 		reg |= ((p->rc_framerate_num & 0xFFFF) << 16);
 		reg |= p->rc_framerate_denom & 0xFFFF;
-		WRITEL(reg, S5P_FIMV_E_RC_FRAME_RATE_V6);
...
+			WRITEL(p->vbv_delay, mfc_regs->e_vbv_init_delay);
 	...
 	mfc_debug_leave();
@@ -1181,6 +1188,7 @@ static int s5p_mfc_set_enc_params_h263(struct s5p_mfc_ctx *ctx)
...
+	const struct s5p_mfc_regs *mfc_regs = dev->mfc_regs;
 	...
 	unsigned int reg = 0;
@@ -1191,57 +1199,57 @@ static int s5p_mfc_set_enc_params_vp8(struct s5p_mfc_ctx *ctx)
 	s5p_mfc_set_enc_params(ctx);
...
+			WRITEL(p->vbv_delay, mfc_regs->e_vbv_init_delay);
 	...
 	/* VP8 specific params */
@@ -1265,7 +1273,7 @@ static int s5p_mfc_set_enc_params_vp8(struct s5p_mfc_ctx *ctx)
 	}
 	reg |= (val & 0xF) << 3;
...
@@ -1288,6 +1296,7 @@ static bool s5p_mfc_is_v6_fw_v2(struct s5p_mfc_dev *dev)
...
@@ -1295,9 +1304,9 @@ static int s5p_mfc_init_decode_v6(struct s5p_mfc_ctx *ctx)
...
@@ -1308,11 +1317,11 @@ static int s5p_mfc_init_decode_v6(struct s5p_mfc_ctx *ctx)
...
@@ -1327,21 +1336,21 @@ static int s5p_mfc_init_decode_v6(struct s5p_mfc_ctx *ctx)
...
@@ -1352,11 +1361,12 @@ static int s5p_mfc_init_decode_v6(struct s5p_mfc_ctx *ctx)
...
@@ -1367,11 +1377,12 @@ static int s5p_mfc_decode_one_frame_v6(struct s5p_mfc_ctx *ctx,
...
+	WRITEL(ctx->inst_no, mfc_regs->instance_id);
 	...
 	switch (last_frame) {
@@ -1395,6 +1406,7 @@ static int s5p_mfc_decode_one_frame_v6(struct s5p_mfc_ctx *ctx,
...
@@ -1410,13 +1422,13 @@ static int s5p_mfc_init_encode_v6(struct s5p_mfc_ctx *ctx)
 		return -EINVAL;
 	}
...
@@ -1426,14 +1438,16 @@ static int s5p_mfc_init_encode_v6(struct s5p_mfc_ctx *ctx)
...
@@ -1442,6 +1456,7 @@ static int s5p_mfc_h264_set_aso_slice_order_v6(struct s5p_mfc_ctx *ctx)
...
@@ -1452,7 +1467,7 @@ static int s5p_mfc_encode_one_frame_v6(struct s5p_mfc_ctx *ctx)
...
@@ -1827,28 +1842,26 @@ static void s5p_mfc_cleanup_queue_v6(struct list_head *lh, struct vb2_queue *vq)
...
+	ret = READL((void *)ofs);
 	...
 	return ret;
@@ -1856,50 +1869,51 @@ s5p_mfc_read_info_v6(struct s5p_mfc_ctx *ctx, unsigned int ofs)
...
@@ -1914,82 +1928,263 @@ static int s5p_mfc_err_dspl_v6(unsigned int err)
