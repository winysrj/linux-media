...
Received: from ispconfig2.arios.fr ([176.31.95.19]:60576 "EHLO
	ispconfig2.arios.fr" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1751690Ab3DQXJ6 (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Wed, 17 Apr 2013 19:09:58 -0400
Message-ID: <516F2BC0.60807@chauveau-central.net>
Date: Thu, 18 Apr 2013 01:09:52 +0200
...
Subject: Re: [PATCH] media:  ir-rc5-decoder: improve responsiveness by 100ms
 to 250ms
References: <516F0B33.7060500@chauveau-central.net> <516F1FC2.20208@chauveau-central.net>
In-Reply-To: <516F1FC2.20208@chauveau-central.net>
...
--- linux/drivers/media/rc/ir-rc5-decoder.c.orig    2013-04-17 
20:40:41.939326236 +0200
+++ linux/drivers/media/rc/ir-rc5-decoder.c    2013-04-18 
...
--- linux/drivers/media/rc/rc-main.c.orig    2013-04-18 
00:41:44.546364678 +0200
+++ linux/drivers/media/rc/rc-main.c    2013-04-18 00:42:49.122785516 +0200
...
>
> Finally, the KEYUP occurs 250ms after the KEYDOWN because of the 
> timeout in rc-main.c (see IR_KEYPRESS_TIMEOUT)
>
...
>
> The delay between the first KEYDOWN and final KEYUP is about 470ms for 
> a double message and 580ms for a triple message.
>
...
>
> First, the KEYDOWN is sent immediately after receiving the first half 
> of the last bit.
>
...
> the KEYUP event as soon as a long blank is received. This is 
> controlled by the macro RC5_UP_DELAY. An even better approach could be 
> to accumulate the duration of all pulses and space until 113ms is 
> reached but in practice waiting for a long blank seems to be good enough.
>
...
>
> For a double message, the delay between the first KEYDOWN and the 
> final KEYUP is around 310ms (vs 470ms).
...
>> using the ite-cir driver. Most of the information below should also 
>> be applicable to other decoders and ir drivers (e.g. rc6 for sure). 
>> The problem is that the current decoder is subject to a double timeout:
>>
...
>> events because STATE_FINISHED is not processed until a blank is 
>> received.
...
>> 20ms and 220ms after receiving the first pulse (versus 220ms and 
>> 470ms with the original decoder)
>>
>> For a double RC5 message the 2 KEYDOWNs and the KEYUP events 
>> respectively occur 21ms, 134ms and 334ms after the first pulse 
>> (versus 111ms, 335ms and 584ms)
>>
>> For a triple RC5 message the 3 KEYDOWNs and the KEYUP events 21ms, 
>> 134ms, 247 and 447ms after the first pulse (versus 111ms, 335ms, 
>> 448ms and 697ms)
>>
...
>> hard-coded key-repeat delay of 500ms and so is almost unusable 
>> without my patch.
>>
