...
Received: from arroyo.ext.ti.com ([192.94.94.40]:53618 "EHLO arroyo.ext.ti.com"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
	id S932753Ab0DGQgf convert rfc822-to-8bit (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Wed, 7 Apr 2010 12:36:35 -0400
...
Date: Wed, 7 Apr 2010 11:36:23 -0500
Subject: RE: [PATCH 1/2] OMAP2/3 V4L2: Add support for OMAP2/3 V4L2 driver
 on top of DSS2
Message-ID: <A69FA2915331DC488A831521EAE36FE4016A9D2A04@dlee06.ent.ti.com>
References: <hvaibhav@ti.com>
 <1270115880-21404-2-git-send-email-hvaibhav@ti.com>
 <201004062028.54365.hverkuil@xs4all.nl>
In-Reply-To: <201004062028.54365.hverkuil@xs4all.nl>
...
Thanks for reviewing this. Once Vaibhav addresses all of the outstanding comments against this patch, I will request a pull for this to 2.6.35.
...
>
>On Thursday 01 April 2010 11:57:59 hvaibhav@ti.com wrote:
...
>> index 933fbb0..aa88de8 100644
>> --- a/drivers/media/video/Kconfig
>> +++ b/drivers/media/video/Kconfig
>> @@ -521,6 +521,8 @@ config VIDEO_VIVI
>>
...
>> index c51c386..ed68dea 100644
>> --- a/drivers/media/video/Makefile
>> +++ b/drivers/media/video/Makefile
>> @@ -169,6 +169,8 @@ obj-$(CONFIG_VIDEO_IR_I2C)  += ir-kbd-i2c.o
>>
...
>> + * Allocate buffers
>> + */
>> +static unsigned long omap_vout_alloc_buffer(u32 buf_size, u32
>*phys_addr)
>> +{
>> +    u32 order, size;
...
>> +                    (u32)VID_MAX_HEIGHT);
>> +    pix->width = clamp(pix->width, (u32)VID_MIN_WIDTH,
>(u32)VID_MAX_WIDTH);
>> +
...
>
>Double semi-colon. In the category 'extreme nitpicking' :-)
>
>> +    info.pos_x = posx;
>> +    info.pos_y = posy;
>> +    info.out_width = outw;
...
>> +
>> +/*
>> + * Buffer setup function is called by videobuf layer when REQBUF ioctl
>is
...
>> +                    && *count > 4)
>
>Please don't break this line. It's very hard to read like this.
>
>> +            *count = 4;
>> +
...
>> + * converts user space virtual address into physical address if userptr
>memory
...
>> +
>> +/*
>> + * Buffer queue funtion will be called from the videobuf layer when
>_QBUF
...
>> +
>> +/*
>> + * Buffer release function is called from videobuf layer to release
>buffer
...
>> +            return -EBUSY;
>
>This *must* be fixed. try_fmt should *never* return -EBUSY. It is the only
>thing that I think really has to be fixed before this can go in.
>
>> +
>> +    ovid = &vout->vid_info;
>> +    ovl = ovid->overlays[0];
...
>> +}
>> +
>> +static int vidioc_s_crop(struct file *file, void *fh, struct v4l2_crop
>*crop)
>> +{
>> +    int ret = -EINVAL;
>> +    struct omap_vout_device *vout = fh;
...
>> +    .ioctl          = video_ioctl2,
>
>Can you use .unlocked_ioctl? That would avoid the BKL that's around ioctl
>in
>the kernel.
>
>> +    .mmap           = omap_vout_mmap,
...
>
>Typo: intitalization -> initialization
>
...
>
>You should set vfd->v4l2_dev to the address of the v4l2_device struct.
>This will become important in the future.
>
>> +
>> +    strlcpy(vfd->name, VOUT_NAME, sizeof(vfd->name));
>> +    vfd->vfl_type = VFL_TYPE_GRABBER;
>
>You don't set vfl_type. Instead you give this type as argument to
...
>> +            vid_dev->num_videos++;
>
>Is num_videos still used somewhere?
>
>> +
