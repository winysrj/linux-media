...
Received: from smtp.nokia.com ([192.100.122.233]:59592 "EHLO
	mgw-mx06.nokia.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1760559Ab0JGNRA (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Thu, 7 Oct 2010 09:17:00 -0400
From: "Matti J. Aaltonen" <matti.j.aaltonen@nokia.com>
To: linux-media@vger.kernel.org, mchehab@redhat.com,
	hverkuil@xs4all.nl, eduardo.valentin@nokia.com
Cc: "Matti J. Aaltonen" <matti.j.aaltonen@nokia.com>
Subject: [PATCH v12 2/3] V4L2: WL1273 FM Radio: Controls for the FM radio.
Date: Thu,  7 Oct 2010 16:16:12 +0300
Message-Id: <1286457373-1742-3-git-send-email-matti.j.aaltonen@nokia.com>
...
+		return -ETIMEDOUT;
+
+	dev_dbg(core->dev, "WL1273_CHANL_SET: %d\n", r);
+
...
+	dev_dbg(core->dev, "WL1273_POWER_ENB_SET: %d\n", r);
+
+	return	0;
+}
+
...
+	}
+
+	dev_dbg(core->dev, "%s: %dkHz\n", __func__, freq);
+
+	core->write(core, WL1273_INT_MASK_SET, core->irq_flags);
+
...
+	if (r) {
+		dev_err(core->dev, "FREQ_SET fails\n");
+		goto err;
+	}
+
+	INIT_COMPLETION(core->busy);
...
+	if (r) {
+		dev_err(core->dev, "TUNER_MODE_SET fails\n");
+		complete(&core->busy);
+		goto err;
+	}
+
...
+			return r;
+
+		dev_dbg(core->dev, "Freq get: 0x%04x\n", f);
+		if (core->band == WL1273_BAND_JAPAN)
...
+	__u8 *ptr;
+	int i, n, len, r;
+	struct wl1273_fw_packet *packets;
+
+	dev_dbg(dev, "%s:\n", __func__);
+
...
+
+	packets = kmalloc((packet_num + 1) * sizeof(*packets), GFP_KERNEL);
+	if (!packets) {
+		r = -ENOMEM;
+		goto out;
+	}
...
+	}
+
+	r = core->write_fw(core, packets, packet_num);
+	kfree(packets);
+
+	if (r != packet_num) {
...
+		if (r)
+			dev_err(core->dev, "%s: POWER_SET fails: %d\n",
+				__func__, r);
+	} else if (core->mode == WL1273_MODE_TX) {
+		int r = core->write(core, WL1273_PUPD_SET, WL1273_PUPD_SET_OFF);
+		if (r)
+			dev_err(core->dev,
+				"%s: PUPD_SET fails: %d\n", __func__, r);
+	}
...
+		core->pdata->disable();
+		dev_dbg(core->dev, "Back to reset\n");
+	}
+
+	return 0;
+}
...
+
+		core->mode = mode;
+		r = core->write(core, WL1273_INT_MASK_SET, core->irq_flags);
+		if (r) {
...
+
+	f = core->rx_frequency;
+	dev_dbg(core->dev, "rx_frequency: %d\n", f);
+
...
+
+	/* Wrap around */
+	dev_dbg(core->dev, "Wrap around in HW seek.\n");
+
+	if (seek_upward)
+		f = core->rangelow;
+	else
...
+	r = wl1273_fm_set_rx_freq(core, core->rx_frequency);
+	if (r)
+		dev_err(core->dev, "set freq fails: %d.\n", r);
+out:
+	return r;
+}
+
...
+		val = count;
+
+	radio->core->write(radio->core, WL1273_RDS_CONFIG_DATA_SET, val);
+
...
+	radio->core->write_data(radio->core, radio->write_buf, val + 1);
+
+	r = val;
+out:
...
+		if (copy_to_user(buf, &core->buffer[core->rd_index],
+				 RDS_BLOCK_SIZE))
+			break;
+
+		/* increment and wrap the read pointer */
+		core->rd_index += RDS_BLOCK_SIZE;
...
+	strlcpy(capability->bus_info, radio->core->bus_type,
...
+		r = core->write(core, WL1273_MOST_MODE_SET,
+					WL1273_RX_MONO);
+		if (r < 0) {
