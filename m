...
Received: from relaycp03.dominioabsoluto.net ([217.116.26.84]:34016 "EHLO
	relaycp03.dominioabsoluto.net" rhost-flags-OK-OK-OK-OK)
	by vger.kernel.org with ESMTP id S1755257AbbA2B1i (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Wed, 28 Jan 2015 20:27:38 -0500
From: "dCrypt" <dcrypt@telefonica.net>
To: <linux-media@vger.kernel.org>
Cc: <stoth@linuxtv.org>
References: <54472CB702988260@smtp.movistar.es> (added by	    postmaster@movistar.es) <02ee01d031ec$283a80f0$78af82d0$@net>
In-Reply-To: <02ee01d031ec$283a80f0$78af82d0$@net>
Subject: RE: [possible BUG, cx23885] Dual tuner TV card, works using one tuner only, doesn't work if both tuners are used 
Date: Thu, 29 Jan 2015 01:10:36 +0100
Message-ID: <006301d03b58$0181a9a0$0484fce0$@net>
MIME-Version: 1.0
Content-Type: multipart/mixed;
	boundary="----=_NextPart_000_0064_01D03B60.634611A0"
Content-Language: es
...
------=_NextPart_000_0064_01D03B60.634611A0
...
tuner, it locks and works
	- log excerpt extracted -> test1.1-adap0-ok.log=20

Test 1.2)=20
	- sudo tzap -a 1 -x -H -c channelsv3.conf "La 1 HD.", using second =
tuner after first tuner lock, it doesn't lock and doesn't work
...
Test 2.1)
	...
	- sudo tzap -a 1 -x -H -c channelsv3.conf "La 1 HD.", using second =
tuner, it locks and works
	- log excerpt extracted -> test2.1-adap1-ok.log=20

Test 2.2)=20
	- sudo tzap -a 0 -x -H -c channelsv3.conf "La 1 HD.", using first tuner =
after second tuner lock, it doesn't lock and doesn't work
...
cx23885_start_dma(), so the driver immediately cancels buffers and stops =
dma. However, I am not an expert and I can't follow the full workflow, =
so I could be wrong.
...
in VDR if I tuned one frontend, and VDR scanned EIT/EPG using the second =
tuner in the background; by disabling the second tuner it works. I =
managed to reproduce the problem by simply using dvbzap/dvbv5-zap in =
...
("basic" approach): =
http://linuxtv.org/wiki/index.php/How_to_Obtain,_Build_and_Install_V4L-DV=
B_Device_Drivers with the same result.
...
-----Mensaje original-----
De: linux-media-owner@vger.kernel.org =
[mailto:linux-media-owner@vger.kernel.org] En nombre de dCrypt Enviado =
el: viernes, 09 de enero de 2015 8:16
Para: blind Pete
...
> Hi dCrypt,
>
> I'm not a developer at all.  I'm not even sure why I read this list,=20
> but can you determine if the problem is associated with a particular=20
> kernel version?  i.e. if it works on x.y.z but fails on x.y.(z+1) you=20
> have a starting point.  If you use the word "regression" and a kernel=20
...
> > Hi again,
> >=20
> > I'm sorry if I sound quite rude, but I'm not sure if I am doing it=20
> > right or not. I subscribed to this mailing list in order to ask for=20
> > help, or to help with a bug that I've found (as instructed in the=20
> > wiki http://linuxtv.org/wiki/index.php/Bug_Report), but it seems to=20
> > me that the mailing list is filled up with developing messages. I=20
> > don't want to participate in the development, I am a developer but I =

> > don't have the skills nor the knowledge.
> >=20
> > If this is not the right place to direct my questions, I would=20
> > appreciate some advice.
> >=20
> > Thank you very much, and best regards.=20
> >=20
> > -----Mensaje original-----
> > De: linux-media-owner@vger.kernel.org=20
> > [mailto:linux-media-owner@vger.kernel.org] En nombre de dCrypt=20
> > Enviado el: jueves, 01 de enero de 2015 22:04
> > Para: linux-media@vger.kernel.org
> > Asunto: [BUG] Dual tuner TV card, works using one tuner only,=20
> > doesn't work if both tuners are used
> >=20
> > Hi,
> >=20
> > I just subscribed to the mailing list to submit information on the=20
> > bug which is driving me crazy since one month ago.
> >=20
> > I have a VDR based PVR at home, installed over an Ubuntu 14.04 LTS.=20
> > Everything was working perfectly, until beginning of December. It=20
> > seems to me that something changed that broke my PVR pretty bad.
> >=20
> > The problem is the following: tuning (zap) both tuners (it's not=20
> > needed that both are tuned simultaneously, only one after the other, =

> > in no particular order) makes the tuners to enter an state where=20
> > they can't lock the signal anymore.
> >=20
> > Facts:=20
> >=20
> > - My TV card is a Cinergy T PCIe Dual from Terratec=20
> > =
(http://www.linuxtv.org/wiki/index.php/TerraTec_Cinergy_T_PCIe_dual).
> > - The problem arose in the form of "frontend x/0 timed out while=20
> > tuning to channel ..." in /var/log/syslog. It happened when both=20
> > tuners are active, during EPG scan. The problem does not happen if=20
> > VDR is run with -D parameter to limit the number of frontends=20
> > enabled. Disabling the EPG scan with both frontends enabled=20
> > minimizes the problem, but doesn't solve it because tuning both=20
> > frontends without any EPG scan makes the error happen again. - I=20
> > initially thought about a problem in the DVB-T signal, because it=20
> > all started the 1st of December, during the transition to a new set =
of frequencies in Spain.
> > - Everything was working perfectly before the 1st, and the problems=20
> > started suddenly.
> > - I setup testing board for debugging, different board and=20
> > processor, less memory, lots of Linux distros tested, Windows tested =
as well.
> > - Both tuners works in windows without problems. Confirmed.=20
> > - I have completely discarded problems/errors in hardware (because=20
> > in Windows I can enable both tuners without problems) and VDR=20
> > (because I can reproduce the problems at OS level, without even =
having VDR installed).
> > - I have almost narrowed the problem at the cx23885 driver, because=20
> > when it happens, I can restart the TV card to working conditions by=20
> > executing "rmmod cx23885" and "modprobe cx23885"; however, as with=20
> > "rmmod" several dependencies are unloaded as well, I am stuck and I=20
> > am unable to go on with debugging to find out where the problem =
really is.
> > - Tools used to test and confirm the problem are: VDR, MythTV,=20
> > TVHeadend, dvbscan, dvbv5-scan, dvbv5-zap and others
> > - Linux distros tested: Ubuntu, Fedora, Suse, yaVDR (not sure if the =

> > card worked at all), MythBuntu ("dvb-fe-tool -a 1 -c DVBT" was=20
> > required to force DVB-T mode for the second tuner), and probably=20
> > others
> > - I have a Sony PlayTV also with dual tuners, which works without=20
> > any problem.
...
problem?=20
> > Is there something I can do?=20
> >=20
> > BR, and happy new year!=20
> >=20
> >=20
> > INFO & TEST:=20
> >=20
...
> > controller: Conexant Systems, Inc. CX23885 PCI Video and Audio=20
> >Decoder  (rev 04)
...
> >         Region 0: Memory at fba00000 (64-bit, non-prefetchable)=20
> >[size=3D2M]
> >         Capabilities: [40] Express (v1) Endpoint, MSI 00
> >                 DevCap: MaxPayload 128 bytes, PhantFunc 0, Latency=20
> >L0s  <64ns, L1 <1us
...
> >AuxPwr-
> > TransPend-
> >                 LnkCap: Port #0, Speed 2.5GT/s, Width x1, ASPM L0s=20
> >L1,
> >                 Exit
...
> >RejSnoopTrans-
> >                         Arb:    Fixed- WRR32- WRR64- WRR128-=20
> >TWRR128-
> > WRR256-
> >                         Ctrl:   Enable+ ID=3D0 ArbSelect=3DFixed=20
> >TC/VC=3Dff
...
> > [   12.812789] cx23885 driver version 0.0.3 loaded [   12.812997]=20
> > CORE cx23885[0]: subsystem: 153b:117e, board: TerraTec Cinergy T=20
> > PCIe Dual [card=3D34,autodetected] [   12.949340] cx25840 11-0044:=20
> > cx23885 A/V decoder found @ 0x88
> > (cx23885[0])
> > [   13.723953] cx25840 11-0044: loaded v4l-cx23885-avcore-01.fw=20
> > firmware
> > (16382 bytes)
> > [   13.739701] cx23885_dvb_register() allocating 1 frontend(s) [  =20
> > 13.739704] cx23885[0]: cx23885 based dvb card [   13.852565] DVB:=20
> > registering new adapter (cx23885[0]) [   13.852569] cx23885=20
> > 0000:03:00.0: DVB: registering adapter 0 frontend 0 (DRXK DVB-T)...
> > [   13.852749] cx23885_dvb_register() allocating 1 frontend(s) [  =20
> > 13.852750] cx23885[0]: cx23885 based dvb card [   13.958613] DVB:=20
> > registering new adapter (cx23885[0]) [   13.958618] cx23885=20
> > 0000:03:00.0: DVB: registering adapter 1 frontend 0 (DRXK DVB-C=20
> > DVB-T)...
...
-------------------------------------------------------------------->=20
> >=20
> > pvr@prueba:~$ sudo dvbv5-zap "La 1 HD." -c channelsv5.conf -a 0 -x=20
> >using  demux '/dev/dvb/adapter0/demux0'
> > reading channels from file 'channelsv5.conf'=20
> > service has pid type 05:  115
> > tuning to 770000000 Hz
> >        (0x00) Quality=3D Good Signal=3D 100,00% C/N=3D 11,30dB =
UCB=3D 2=20
> >postBER=3D 0  preBER=3D 57,9x10^-6 PER=3D 48,8x10^-6  Lock   (0x1f)=20
> >Quality=3D Good Signal=3D 100,00% C/N=3D 11,80dB UCB=3D 3 postBER=3D =
0  preBER=3D
> >55,1x10^-6 PER=3D 0  pvr@prueba:~$  pvr@prueba:~$ sudo dvbv5-zap "La =
1=20
> >HD." -c channelsv5.conf -a 0 -x using  demux=20
...
UCB=3D 3=20
> >postBER=3D 0  preBER=3D 63,6x10^-6 PER=3D 56,3x10^-6  Lock   (0x1f)=20
> >Quality=3D Good Signal=3D 100,00% C/N=3D 12,20dB UCB=3D 4 postBER=3D
...
UCB=3D 4=20
> >postBER=3D
> > 1,01x10^-6 preBER=3D 58,6x10^-6 PER=3D 61,0x10^-6  Lock   (0x1f)=20
> >Quality=3D Good Signal=3D 100,00% C/N=3D 12,10dB UCB=3D 4 postBER=3D =
0  preBER=3D
> >55,1x10^-6 PER=3D 0  pvr@prueba:~$  pvr@prueba:~$ sudo dvbv5-zap "La =
1=20
> >HD." -c channelsv5.conf -a 1 -x using  demux=20
...
> > pvr@prueba:~$ sudo modprobe cx23885  pvr@prueba:~$  pvr@prueba:~$=20
> >pvr@prueba:~$ sudo dvbv5-zap "La 1 HD." -c channelsv5.conf -a 1 -x=20
...
dvbv5-zap=20
> >"La 1 HD." -c channelsv5.conf -a 1 -x using  demux=20
> >'/dev/dvb/adapter1/demux0'
> > reading channels from file 'channelsv5.conf'=20
> > service has pid type 05:  115
> > tuning to 770000000 Hz
...
UCB=3D 0=20
> >postBER=3D 0  preBER=3D 36,7x10^-6 PER=3D 0  Lock   (0x1f) Quality=3D =
Good=20
> >Signal=3D 100,00% C/N=3D 11,90dB UCB=3D 1 postBER=3D 0  preBER=3D =
331x10^-6=20
> >PER=3D 0  pvr@prueba:~$ sudo dvbv5-zap "La 1 HD." -c channelsv5.conf =
-a
...
preBER=3D
> >55,1x10^-6 PER=3D 0  pvr@prueba:~$ sudo dvbv5-zap "La 1 HD." -c=20
...
> > linux-media" in the body of a message to majordomo@vger.kernel.org=20
...
=DD=A2j/   z =DE=96  2 =DE=99   & )=DF=A1 a  =7F  =1E G   h =0F j:+v   w =
=D9=A5
