...
Received: from lb3-smtp-cloud6.xs4all.net ([194.109.24.31]:36914 "EHLO
	lb3-smtp-cloud6.xs4all.net" rhost-flags-OK-OK-OK-OK)
	by vger.kernel.org with ESMTP id S1752347AbbHCOIH (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Mon, 3 Aug 2015 10:08:07 -0400
Received: from [127.0.0.1] (localhost [127.0.0.1])
	by tschai.lan (Postfix) with ESMTPSA id C90132A0095
	for <linux-media@vger.kernel.org>; Mon,  3 Aug 2015 16:07:48 +0200 (CEST)
Message-ID: <55BF75B4.2060301@xs4all.nl>
Date: Mon, 03 Aug 2015 16:07:48 +0200
From: Hans Verkuil <hverkuil@xs4all.nl>
MIME-Version: 1.0
To: Linux Media Mailing List <linux-media@vger.kernel.org>
Subject: [RFC] Media Controller, the next generation
...
entity IDs. Note that currently there are no unique pad IDs: adding this requires
a single change to struct media_pad_desc where one of the reserved fields is used
...
I'm going with mc_entity for now. An alternative name might be: mediav2_entity or
media_entity_v2 (I never liked the _desc suffix and media_entity can't be used due
...
media_entity for the external. As you can see, we didn't have time to discuss the
naming for these new structs. But for the purposes of this RFC I'm going with mc_
for now.

This struct is much smaller than the original: both name and type will be stored
as properties of the entity (properties will be the topic of a separate upcoming RFC).
Pretty much everything else has been removed except for num_pads: while even this
is not strictly necessary, it was considered to be useful to know for applications that
use this API.
...
interface. While not strictly necessary it is useful for applications to know.

There is currently only one flag: MEDIA_INTF_FL_DEFAULT (formerly
MEDIA_ENT_FL_DEFAULT). This flag tells applications whether this interface is
a default interface, i.e. the one apps should use by default. Useful when there
are e.g. multiple video interfaces.
...
The source/sink_id fields refer to pads (for data links) or interface (source_id)
and entity (sink_id) for the interface/entity relationships.

While not implemented initially this will also make it possible to model cases where
one entity controls another entity. Whether we actually will need this is uncertain,
but at least it can be supported should we require it.

Finally we will need a way to retrieve all this information. To do this a single struct
is created:
...
Applications can allocate arrays for each type that they want to retrieve (or NULL
if they are not interested) and fill in the num_ fields with the size of these
arrays. Existing DVB/V4L applications that are typically only interested in the
interfaces created by a device will only need the interfaces and can set the
others to NULL.

The driver will copy the data to the arrays and set the num_ fields to the total number
of elements.

So by zeroing the struct and calling the ioctl once the num_ fields will be filled in
with the number of elements each array requires. The application can allocate the
memory and call the ioctl again to get the full topology. The topology_version will
be increased whenever the topology changes. Static topologies that cannot change will
always return 0 here. Topologies that can change will start with version 1 and never
return 0 here.

As mentioned before, the property description is absent in this discussion, but the
idea is to allow properties for each object type except links (at least for now)
and to report all properties as part of the mc_topology ioctl.

A separate RFC will be written by Sakari for this.

Backwards compatibility notes:

- currently only entities are enumerated and that will be unchanged. So neither
  interfaces nor connectors will be seen by the existing API.
- the v4l-subdev interface will still be reported in the union of the old
  media_entity_desc struct.
- currently entities representing DMA engines are reported as type MEDIA_ENT_T_DEVNODE_V4L.
  This will change: such entities should be reported as having type DMA engine.
  I think we can make an alias for MEDIA_ENT_T_DEVNODE_V4L called MEDIA_ENT_T_DMA_ENGINE.
  The major/minor in the dev struct in the union will point to the video device.
- These are the only two cases where the dev struct is used, for all other cases it
  will be 0.
