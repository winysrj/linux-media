...
Received: from mail-qt0-f194.google.com ([209.85.216.194]:35975 "EHLO
        mail-qt0-f194.google.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1751423AbdJKXYZ (ORCPT
        <rfc822;linux-media@vger.kernel.org>);
        Wed, 11 Oct 2017 19:24:25 -0400
Received: by mail-qt0-f194.google.com with SMTP id 24so4423405qts.3
        for <linux-media@vger.kernel.org>; Wed, 11 Oct 2017 16:24:24 -0700 (PDT)
Date: Wed, 11 Oct 2017 19:24:19 -0400
In-Reply-To: <20171011210237.bpbfuhpf7om26ldi@gofer.mess.org>
References: <cover.1507618840.git.sean@mess.org> <176506027db4255239dc8ce192dc6652af75bd52.1507618840.git.sean@mess.org> <1507750996.2479.11.camel@gmail.com> <20171011210237.bpbfuhpf7om26ldi@gofer.mess.org>
MIME-Version: 1.0
...
Message-ID: <0D74D058-EE11-4BFF-974C-16DB6910D2CF@gmail.com>
Sender: linux-media-owner@vger.kernel.org
List-ID: <linux-media.vger.kernel.org>

On October 11, 2017 5:02:37 PM EDT, Sean Young <sean@mess=2Eorg> wrote:
>On Wed, Oct 11, 2017 at 03:43:16PM -0400, Andy Walls wrote:
>> On Tue, 2017-10-10 at 08:17 +0100, Sean Young wrote:
>> > The ir-kbd-i2c module already handles this very well=2E
>>=20
>> Hi Sean:
>>=20
...
>functionality
>> between two modules became problematic=2E
>>=20
>> Believe me, if i could have leveraged ir-kdb-i2c back when I ported
>> this, I would have=2E  I think it's a very bad idea to remove the
>> receiver from lirc_zilog=2E
>>=20
>> The cx18 and ivtv drivers both use lirc_zilog IIRC=2E  Have you looked
>at
...
>>=20
>> >=20
>> > Signed-off-by: Sean Young <sean@mess=2Eorg>
>> > ---
>> >  drivers/staging/media/lirc/lirc_zilog=2Ec | 901
>+++-------------------
>> > ----------
>> >  1 file changed, 76 insertions(+), 825 deletions(-)
>> >=20
>> > diff --git a/drivers/staging/media/lirc/lirc_zilog=2Ec
>> > b/drivers/staging/media/lirc/lirc_zilog=2Ec
>> > index 6bd0717bf76e=2E=2E757b3fc247ac 100644
>> > --- a/drivers/staging/media/lirc/lirc_zilog=2Ec
>> > +++ b/drivers/staging/media/lirc/lirc_zilog=2Ec
>> > @@ -64,28 +64,7 @@
...
>> > - * a=2E ir_devices_list access, insertions, deletions
>> > - * b=2E struct IR kref get()s and put()s
>> > - * c=2E serialization of ir_probe() for the two i2c_clients for a Z8
>> > - */
...
>> > -{
>> > -	struct IR *ir =3D container_of(ref, struct IR, ref);
>> > -
>> > -	/*
...
>> > >tx->ir put()
>> > -	 * ir->open_count =3D=3D  0 - happens on final close()
>> > -	 * ir_lock, tx_ref_lock, rx_ref_lock, all released
>> > -	 */
>> > -	if (ir->l)
>> > -		lirc_unregister_device(ir->l);
>> > -
>> > -	if (kfifo_initialized(&ir->rbuf=2Efifo))
...
>> > -
>> > -	mutex_lock(&ir_devices_lock);
>> > -	released =3D kref_put(&ir->ref, release_ir_device);
>> > -	mutex_unlock(&ir_devices_lock);
>> > -
>> > -	return released;
>> > -}
...
>> > -{
>> > -	struct IR_rx *rx =3D container_of(ref, struct IR_rx, ref);
>> > -	struct IR *ir =3D rx->ir;
>> > -
>> > -	/*
>> > -	 * This release function can't do all the work, as we want
>> > -	 * to keep the rx_ref_lock a spinlock, and killing the poll
>> > thread
>> > -	 * and releasing the ir reference can cause a sleep=2E  That
>> > work is
>> > -	 * performed by put_ir_rx()
>> > -	 */
>> > -	ir->l->features &=3D ~LIRC_CAN_REC_LIRCCODE;
...
>> > -
>> > -	spin_lock(&ir->rx_ref_lock);
>> > -	released =3D kref_put(&rx->ref, release_ir_rx);
>> > -	spin_unlock(&ir->rx_ref_lock);
...
>> > somewhere */
>> > -		wake_up_interruptible(&ir->rbuf=2Ewait_poll);
>> > -	}
...
>> > -{
>> > -	struct IR_tx *tx =3D container_of(ref, struct IR_tx, ref);
>> > -	struct IR *ir =3D tx->ir;
>> > -
>> > -	ir->l->features &=3D ~LIRC_CAN_SEND_LIRCCODE;
...
>> > -
>> > -	spin_lock(&ir->tx_ref_lock);
>> > -	released =3D kref_put(&tx->ref, release_ir_tx);
>> > -	spin_unlock(&ir->tx_ref_lock);
...
>> > -	int ret;
>> > -	int failures =3D 0;
>> > -	unsigned char sendbuf[1] =3D { 0 };
>> > -	struct lirc_buffer *rbuf =3D ir->l->buf;
>> > -	struct IR_rx *rx;
>> > -	struct IR_tx *tx;
>> > -
...
>> > -
>> > -		/*
>> > -		 * Lock i2c bus for the duration=2E  RX/TX chips
>> > interfere so
>> > -		 * this is worth it
>> > -		 */
...
>> > does this
>> > -		 * and it is a good point to detect chip failure=2E
>> > -		 */
>> > -		ret =3D i2c_master_send(rx->c, sendbuf, 1);
>> > -		if (ret !=3D 1) {
...
>> > -			break;
>> > -		}
>> > -		ret =3D i2c_master_recv(rx->c, keybuf,
>> > sizeof(keybuf));
>> > -		mutex_unlock(&ir->ir_lock);
>> > -		if (ret !=3D sizeof(keybuf)) {
>> > -			dev_err(ir->dev,
...
>> > -			if (got_data && (keybuf[0] =3D=3D 0x80)) {
>> > -				ret =3D 0;
>> > -				break;
>> > -			} else if (got_data && (keybuf[0] =3D=3D 0x00))
>> > {
>> > -				ret =3D -ENODATA;
>> > -				break;
>> > -			}
>> > -		} else if ((rx->b[0] & 0x80) =3D=3D 0) {
>> > -			ret =3D got_data ? 0 : -ENODATA;
>> > -			break;
>> > -		}
>> > -
>> > -		/* look what we have */
>> > -		code =3D (((__u16)rx->b[0] & 0x7f) << 6) | (rx->b[1]
>> > >> 2);
>> > -
>> > -		codes[0] =3D (code >> 8) & 0xff;
>> > -		codes[1] =3D code & 0xff;
>> > -
>> > -		/* return it */
...
>> > -
>> > -/*
>> > - * Main function of the polling thread -- from lirc_dev=2E
>> > - * We don't fit the LIRC model at all anymore=2E  This is horrible,
>> > but
>> > - * basically we have a single RX/TX device with a nasty failure
>mode
>> > - * that needs to be accounted for across the pair=2E  lirc lets us
>> > provide
>> > - * fops, but prevents us from using the internal polling, etc=2E if
>we
>> > do
>> > - * so=2E  Hence the replication=2E  Might be neater to extend the LI=
RC
>> > model
>> > - * to account for this but I'd think it's a very special case of
>> > seriously
>> > - * messed up hardware=2E
>> > - */
...
>> > */
>> > -		if (atomic_read(&ir->open_count) =3D=3D 0) {
...
>> > time you poll
>> > -		 * it (bad!)=2E  This is therefore just sufficient to
>> > catch all
>> > -		 * of the button presses=2E  It makes the remote much
>> > more
>> > -		 * responsive=2E  You can see the difference by
>> > running irw and
>> > -		 * holding down a button=2E  With 100ms, the old
>> > polling
>> > -		 * interval, you'll notice breaks in the repeat
>> > sequence
>> > -		 * corresponding to lost keypresses=2E
>> > -		 */
...
>> >  		buf[0] =3D (unsigned char)(i + 1);
>> >  		for (j =3D 0; j < tosend; ++j)
>> >  			buf[1 + j] =3D data_block[i + j];
>> > -		dev_dbg(tx->ir->dev, "%*ph", 5, buf);
>> > +		dev_dbg(&tx->l->dev, "%*ph", 5, buf);
>> >  		ret =3D i2c_master_send(tx->c, buf, tosend + 1);
>> >  		if (ret !=3D tosend + 1) {
>> > -			dev_err(tx->ir->dev,
>> > +			dev_err(&tx->l->dev,
...
>> >  	buf[1] =3D 0x20;
>> >  	ret =3D i2c_master_send(tx->c, buf, 2);
>> >  	if (ret !=3D 2) {
...
>> >  	}
>> > =20
>> >  	/* Here comes the firmware version=2E=2E=2E (hopefully) */
>> >  	ret =3D i2c_master_recv(tx->c, buf, 4);
>> >  	if (ret !=3D 4) {
...
>> >  		return 0;
>> >  	}
>> >  	if ((buf[0] !=3D 0x80) && (buf[0] !=3D 0xa0)) {
...
>> > -	ret =3D request_firmware(&fw_entry, "haup-ir-blaster=2Ebin", tx-
>> > >ir->dev);
>> > +	ret =3D request_firmware(&fw_entry, "haup-ir-blaster=2Ebin",
>> > &tx->l->dev);
>> >  	if (ret !=3D 0) {
>> > -		dev_err(tx->ir->dev,
>> > +		dev_err(&tx->l->dev,
>> >  			"firmware haup-ir-blaster=2Ebin not available
>> > (%d)\n",
>> >  			ret);
>> >  		ret =3D ret < 0 ? ret : -EFAULT;
>> >  		goto out;
>> >  	}
...
>> > =20
>> >  	/* Parse the file */
>> >  	tx_data =3D vmalloc(sizeof(*tx_data));
...
>> > -		    loff_t *ppos)
>> > -{
>> > -	struct IR *ir =3D lirc_get_pdata(filep);
>> > -	struct IR_rx *rx;
>> > -	struct lirc_buffer *rbuf =3D ir->l->buf;
>> > -	int ret =3D 0, written =3D 0, retries =3D 0;
>> > -	unsigned int m;
>> > -	DECLARE_WAITQUEUE(wait, current);
>> > -
>> > -	dev_dbg(ir->dev, "read called\n");
>> > -	if (n % rbuf->chunk_size) {
>> > -		dev_dbg(ir->dev, "read result =3D -EINVAL\n");
>> > -		return -EINVAL;
>> > -	}
>> > -
>> > -	rx =3D get_ir_rx(ir);
...
>> > in blocking
>> > -	 * mode and 'copy_to_user' is happy, wait for data=2E
>> > -	 */
>> > -	while (written < n && ret =3D=3D 0) {
...
>> > -				break;
>> > -			}
>> > -			m =3D lirc_buffer_read(rbuf, buf);
>> > -			if (m =3D=3D rbuf->chunk_size) {
>> > -				ret =3D copy_to_user(outbuf + written,
>> > buf,
>> > -						   rbuf-
>> > >chunk_size);
>> > -				written +=3D rbuf->chunk_size;
>> > -			} else {
>> > -				retries++;
>> > -			}
>> > -			if (retries >=3D 5) {
...
>> > -
>> > -	dev_dbg(ir->dev, "read result =3D %d (%s)\n", ret,
>> > -		ret ? "Error" : "OK");
>> > -
...
>> >  	ret =3D get_key_data(data_block, code, key);
>> > =20
>> >  	if (ret =3D=3D -EPROTO) {
>> > -		dev_err(tx->ir->dev,
>> > +		dev_err(&tx->l->dev,
>> >  			"failed to get data for code %u, key %u --
>> > check lircd=2Econf entries\n",
>> >  			code, key);
>> >  		return ret;
...
>> >  	ret =3D i2c_master_recv(tx->c, buf, 1);
>> >  	if (ret !=3D 1) {
>> > -		dev_err(tx->ir->dev, "i2c_master_recv failed with
>> > %d\n", ret);
>> > +		dev_err(&tx->l->dev, "i2c_master_recv failed with
...
>> > unsigned int code, unsigned int key)
>> >  		ret =3D i2c_master_send(tx->c, buf, 1);
>> >  		if (ret =3D=3D 1)
>> >  			break;
>> > -		dev_dbg(tx->ir->dev,
>> > +		dev_dbg(&tx->l->dev,
...
>> >  	i =3D i2c_master_recv(tx->c, buf, 1);
>> >  	if (i !=3D 1) {
>> > -		dev_err(tx->ir->dev, "i2c_master_recv failed with
>> > %d\n", ret);
>> > +		dev_err(&tx->l->dev, "i2c_master_recv failed with
...
>> > -	struct IR *ir =3D lirc_get_pdata(filep);
>> > -	struct IR_tx *tx;
>> > +	struct IR_tx *tx =3D lirc_get_pdata(filep);
>> >  	size_t i;
>> >  	int failures =3D 0;
>> > =20
...
>> > char __user *buf, size_t n,
>> >  			ret =3D send_code(tx, (unsigned int)command >>
>> > 16,
...
>> > -		 * status for Tx
>> > -		 */
>> > -		dev_dbg(ir->dev, "%s result =3D POLLERR\n", __func__);
>> > -		return POLLERR;
>> > -	}
>> > -
>> > -	/*
>> > -	 * Add our lirc_buffer's wait_queue to the poll_table=2E A
>> > wake up on
...
>> > blocking */
>> > -	ret =3D lirc_buffer_empty(rbuf) ? 0 : (POLLIN | POLLRDNORM);
>> > -
>> > -	dev_dbg(ir->dev, "%s result =3D %s\n", __func__,
...
>> > -	struct IR *ir =3D lirc_get_pdata(filep);
>> > +	struct IR_tx *tx =3D lirc_get_pdata(filep);
>> >  	unsigned long __user *uptr =3D (unsigned long __user *)arg;
>> >  	int result;
>> >  	unsigned long mode, features;
>> > =20
>> > -	features =3D ir->l->features;
>> > +	features =3D tx->l->features;
>> > =20
>> >  	switch (cmd) {
...
>> > =20
>> >  static const struct i2c_device_id ir_transceiver_id[] =3D {
...
>> > @@ -1329,16 +778,13 @@ static struct i2c_driver driver =3D {
>> >  		=2Ename	=3D "Zilog/Hauppauge i2c IR",
>> >  	},
>> >  	=2Eprobe		=3D ir_probe,
>> > -	=2Eremove		=3D ir_remove,
>> >  	=2Eid_table	=3D ir_transceiver_id,
>> >  };
>> > =20
>> >  static const struct file_operations lirc_fops =3D {
>> >  	=2Eowner		=3D THIS_MODULE,
>> >  	=2Ellseek		=3D no_llseek,
>> > -	=2Eread		=3D read,
>> >  	=2Ewrite		=3D write,
>> > -	=2Epoll		=3D poll,
>> >  	=2Eunlocked_ioctl	=3D ioctl,
>> >  #ifdef CONFIG_COMPAT
>> >  	=2Ecompat_ioctl	=3D ioctl,
...
>> > -static int ir_remove(struct i2c_client *client)
>> > -{
>> > -	if (strncmp("ir_tx_z8", client->name, 8) =3D=3D 0) {
>> > -		struct IR_tx *tx =3D i2c_get_clientdata(client);
>> > -
>> > -		if (tx) {
>> > -			mutex_lock(&tx->client_lock);
>> > -			tx->c =3D NULL;
...
>> > -			put_ir_tx(tx, false);
>> > -		}
>> > -	} else if (strncmp("ir_rx_z8", client->name, 8) =3D=3D 0) {
>> > -		struct IR_rx *rx =3D i2c_get_clientdata(client);
>> > -
>> > -		if (rx) {
>> > -			mutex_lock(&rx->client_lock);
>> > -			rx->c =3D NULL;
...
>> >  	 * The IR transmitter is at i2c address 0x70=2E
>> >  	 */
>> > =20
...
>> > functions */
>> > -	ir =3D get_ir_device_by_adapter(adap);
>> > -	if (!ir) {
...
>> > "lirc_zilog");
>> > -		ir->l->code_length =3D 13;
>> > -		ir->l->fops =3D &lirc_fops;
>> > -		ir->l->owner =3D THIS_MODULE;
>> > -		ir->l->dev=2Eparent =3D &adap->dev;
>> > -
>> > -		/*
...
>> > touch this
>> > -		 * buffer as we provide our own lirc_fops=2E
>> > -		 *
...
>> > -		ir->l->data =3D ir;
>> > -		ret =3D lirc_buffer_init(ir->l->buf, 2, BUFLEN / 2);
>> > -		if (ret) {
...
>> > -		tx =3D kzalloc(sizeof(*tx), GFP_KERNEL);
>> > -		if (!tx) {
>> > -			ret =3D -ENOMEM;
>> > -			goto out_put_xx;
>> > -		}
...
>> > -		rx->c =3D client;
>> > -		rx->hdpvr_data_fmt =3D
>> > -			       (id->driver_data & ID_FLAG_HDPVR) ?
>> > true : false;
>> > -
>> > -		/* An ir ref goes to the struct IR_rx instance */
>> > -		rx->ir =3D get_ir_device(ir, true);
>> > +	/* Set up a struct IR_tx instance */
>> > +	tx =3D devm_kzalloc(&client->dev, sizeof(*tx), GFP_KERNEL);
>> > +	if (!tx)
>> > +		return -ENOMEM;
>> > =20
...
>> > +	tx->c =3D client;
>> > +	tx->need_boot =3D 1;
>> > +	tx->post_tx_ready_poll =3D !(id->driver_data & ID_FLAG_HDPVR);
>> > =20
>> > -		/*
>> > -		 * Start the polling thread=2E
...
>> > -		rx->task =3D kthread_run(lirc_thread,
>> > get_ir_device(ir, true),
>> > -				       "zilog-rx-i2c-%d", adap->nr);
>> > -		if (IS_ERR(rx->task)) {
>> > -			ret =3D PTR_ERR(rx->task);
>> > -			dev_err(tx->ir->dev,
...
>> > -		}
>> > +	/* set lirc_dev stuff */
>> > +	tx->l =3D lirc_allocate_device();
>> > +	if (!tx->l)
>> > +		return -ENOMEM;
>> > =20
>> > -		/* Proceed only if the Tx client is also ready */
>> > -		if (!tx) {
>> > -			pr_info("probe of IR Rx on %s (i2c-%d) done=2E
>> > Waiting on IR Tx=2E\n",
>> > -				adap->name, adap->nr);
>> > -			goto out_ok;
>> > -		}
...
>> > +	tx->l->features |=3D LIRC_CAN_SEND_LIRCCODE;
>> > +	tx->l->code_length =3D 13;
>> > +	tx->l->fops =3D &lirc_fops;
>> > +	tx->l->owner =3D THIS_MODULE;
>> > +	tx->l->dev=2Eparent =3D &client->dev;
>> > =20
>> >  	/* register with lirc */
>> > -	ret =3D lirc_register_device(ir->l);
>> > +	ret =3D lirc_register_device(tx->l);
>> >  	if (ret < 0) {
>> > -		dev_err(tx->ir->dev,
...
>> > -	dev_info(ir->dev,
>> > +	/*
>> > +	 * Load the 'firmware'=2E  We do this before registering with
...
There's an ir_lock mutex in the driver to prevent simultaneous access to t=
he Rx and Tx functions of the z8=2E  Accessing Rx and Tx functions of the c=
hip together can cause it to do the wrong thing (sometimes hang?), IIRC=2E

I'll see if I can dig up my old disassembly of the z8's firmware to see if=
 that interlock is strictly necessary=2E

Yes I know ir-kbd-i2c is in mainline, but as I said, I had reasons for avo=
iding it when using Tx operation of the chip=2E  It's been 7 years, and I'm=
 getting too old to remember the reasons=2E :P
