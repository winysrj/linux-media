...
Received: from mx1.redhat.com ([209.132.183.28]:6124 "EHLO mx1.redhat.com"
	...
	id S1752765AbZKHJIo (ORCPT <rfc822;linux-media@vger.kernel.org>);
	Sun, 8 Nov 2009 04:08:44 -0500
Message-ID: <4AF68BC6.4040801@redhat.com>
Date: Sun, 08 Nov 2009 10:13:42 +0100
...
References: <4AF48F80.4000809@freemail.hu>
In-Reply-To: <4AF48F80.4000809@freemail.hu>
...
And the is nothing we can do here, as we cannot serialize mmap / ioctl calls at the
level needed (before mmap taking the mmap semaphore).


There are 2 surprising things in here, which together cause the circular problem:

1) A simple readdir on a sysfs dir takes the mmap semaphore
2) A simple usb_set_interface (only changing the alt setting!) takes the sysfs mutex

1 of these 2 is new in 2.6.32 I think otherwise we would have seen this before. So first of
all we should take this discussion to the general kernel mailing list and try to get one
of these 2 fixed.

Alternatively we could move the usb bandwidth negotiation (which does the usb_set_interface)
to device probe time, by pre-allocating the usb bandwidth, but we really don't want to do that,
as that means that in multiple isoc devices configurations only the first cam *probed* will
work (now usually only the first cam started works, which means atleast multiple cams can
...
Are you seeing this issue on uhci using machines, or is it more general ? Also keep in mind
the pac7302 is a very buggy device. I think we've had this issue (the need to sometimes plug
