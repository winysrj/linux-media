...
Received: from nblzone-211-213.nblnetworks.fi ([83.145.211.213]:50718 "EHLO
	hillosipuli.retiisi.org.uk" rhost-flags-OK-OK-OK-FAIL)
	by vger.kernel.org with ESMTP id S1750759AbaAPG3A (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Thu, 16 Jan 2014 01:29:00 -0500
Date: Thu, 16 Jan 2014 08:28:55 +0200
...
Message-ID: <20140116062815.GF9997@valkosipuli.retiisi.org.uk>
...
Do you need to serialise this on your own? Could you rely on the control
framework to do that? (See e.g. the smiapp driver how to do that.)

Same for lm3656_set_ctrl().

The same could have been done in the lm3560 driver and we wouldn't have even
seen the bugs related to failing to release the mutex when returning from
the control set/get functions. ;-)
...
"bval" is redundant. Some artistic indentation might be needed to keep under
80 chars / line, or changing some macro names. Entirely up to you if you
want to changes this.

> +	if (rval < 0)
> +		return rval;

The spec suggests that this should be in mA rather than µA. Do you think
that could be changeable?
...
No need to explicitly assign pm as NULL. Speaking of which --- how is your
chip powered? The lack of power control in the driver suggests it's always
powered and goes to a low power state automatically, or alternatively slight
extra power consumption is not a concern.
...
Do you actually need to include this from here?
...
Is there a difference between total flash brightness and flash brightness?
The controller only drives a single flash led, doesn't it? (And in that case
"LED1" below is redundant. But I understand that if you're using the same
names as the spec uses.)
...
What are these? Do you need more faults than what are defined currently?

NTC_TRIP sounds like the same as OVER_TEMPERATURE, for instance.
