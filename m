...
Received: from smtp-vbr5.xs4all.nl ([194.109.24.25]:3049 "EHLO
	smtp-vbr5.xs4all.nl" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1758981AbZKYVbU (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Wed, 25 Nov 2009 16:31:20 -0500
From: Hans Verkuil <hverkuil@xs4all.nl>
To: santiago.nunez@ridgerun.com
Subject: Re: [PATCH 3/4 v8] TVP7002 driver for DM365
Date: Wed, 25 Nov 2009 22:31:23 +0100
...
In-Reply-To: <1259177960-14913-1-git-send-email-santiago.nunez@ridgerun.com>
MIME-Version: 1.0
Content-Type: Text/Plain;
  charset="iso-8859-6"
Content-Transfer-Encoding: 7bit
Message-Id: <200911252231.23458.hverkuil@xs4all.nl>
Sender: linux-media-owner@vger.kernel.org
List-ID: <linux-media.vger.kernel.org>

On Wednesday 25 November 2009 20:39:20 santiago.nunez@ridgerun.com wrote:
...
Just put this on one line. The 80-column warning from checkpatch is just a
warning and readability of the code comes first.
...
This can be written more concisely:
	...
	for (i = 0; i < NUM_PRESETS; i++) {
		const struct v4l2_dv_enum_preset *preset =
					&tvp7002_presets[i].p_data;

		if (preset->preset == dv_preset->preset)
			return tvp7002_write_inittab(sd, preset->p_settings);
	}
...
> +
> +	pix->width = tvp7002_presets[current_preset].p_data.width;
> +	pix->height = tvp7002_presets[current_preset].p_data.height;

Recommend making a temp variable:

	const struct tvp7002_preset_definition *preset =
		&tvp7002_presets[device->current_preset];
...
> +
> +	pix->field = tvp7002_presets[current_preset].scanmode;
> +	pix->bytesperline = pix->width * 2;
> +	pix->sizeimage = pix->bytesperline * pix->height;
> +	pix->colorspace = tvp7002_presets[current_preset].color_space;
> +	pix->priv = 0;
> +
...
> +		if (lpfr  == tvp7002_presets[index].lines_per_frame &&
> +			progressive == tvp7002_presets[index].progressive) {
> +			if (tvp7002_presets[index].cpl_min == 0xffff)
> +				break;
> +			else if (cpln >= tvp7002_presets[index].cpl_min &&
> +				cpln  <= tvp7002_presets[index].cpl_max)
> +				break;
> +			else
> +				continue;
> +		}
...
This can be simplified as well:
	...
	const struct tvp7002_preset_definition *preset = tvp7002_presets;

	for (index = 0; index < NUM_PRESETS; index++, preset++) {
		if (lpfr == preset->lines_per_frame &&
			progressive == preset->progressive) {
			if (preset->cpl_min == 0xffff)
				break;
			if (cpln >= preset->cpl_min && cpln <= preset->cpl_max)
				break;
		}
	}


And in the rest of the function you can use the preset pointer as well.
...
> +	qpreset->preset = tvp7002_presets[index].p_data.preset;
> +	device->current_preset = index;
> +	error = tvp7002_s_dv_preset(sd, qpreset);

That's not right. This function shouldn't change the current preset, it should
just return what it detects. The application should call s_dv_preset to actually
choose this preset.

BTW: I think we need a g_dv_preset ops as well. That seems to be missing. Murali,
is there any reason why we do not have that?

> +
> +	/* Update lines per frame and clocks per line info */
> +	v4l2_dbg(1, debug, sd, "Current preset: %d %d",
> +					tvp7002_presets[index].p_data.width,
> +					tvp7002_presets[index].p_data.height);
> +
> +	return error;
> +}
> +
...
> +		return -EINVAL;
> +	memcpy(fmtdesc, tvp7002_fmt_list + fmtdesc->index, sizeof(*fmtdesc));
...
Did you check whether it is really needed to power the chip off and on here?

It still makes no sense to me that you have to do this.

> +		/* Reset device state with symbolic one */
...
> +		}
> +
> +		/* Set current preset information */
> +		preset.preset =
> +			tvp7002_presets[device->current_preset].p_data.preset;
...
Huh? Why do a query_dv_preset here? Your current preset is device->current_preset.

That said, if query_dv_preset no longer does a s_dv_preset, then it will be
interesting to show both the currently selected preset and the detected preset
in this status log.

> +
> +	/* Print standard related code values */
> +	for (i = 0; i < NUM_PRESETS; i++)
> +		if (tvp7002_presets[i].p_data.preset == preset.preset)

Using a preset pointer for &tvp7002_presets[i].p_data like we did above will
...
> +		return -EINVAL;
> +
> +	v4l2_info(sd, "DV Preset: %s\n", tvp7002_presets[i].p_data.name);
> +	v4l2_info(sd, "Pixels per line: %u\n",
> +					tvp7002_presets[i].p_data.width);
> +	v4l2_info(sd, "Lines per frame: %u\n",
> +					tvp7002_presets[i].p_data.height);
...
You can just print the value of device->gain here. And then the log_chk function
...
With a bit of luck it can be merged very soon after the timings API is merged.
That would be nice.
