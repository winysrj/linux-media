...
Received: from rv-out-0506.google.com ([209.85.198.227])
	...
	(envelope-from <chancleta@gmail.com>) id 1Jrh3a-00018Y-6n
	for linux-dvb@linuxtv.org; Fri, 02 May 2008 00:15:18 +0200
Received: by rv-out-0506.google.com with SMTP id b25so731947rvf.41
	for <linux-dvb@linuxtv.org>; Thu, 01 May 2008 15:15:08 -0700 (PDT)
Message-ID: <a4ac2da80805011515o2f8c1446of7eb084e4e9cbe7b@mail.gmail.com>
Date: Fri, 2 May 2008 00:15:08 +0200
From: "Daniel Guerrero" <chancleta@gmail.com>
To: "Ian Bonham" <ian.bonham@gmail.com>
In-Reply-To: <a4ac2da80804291215y46708712mcc7bfdad98518bcb@mail.gmail.com>
MIME-Version: 1.0
...
>  > It suggests you use dmesg to see what the full errors are, so try that=
. As
>  > far as mine works, the card loads at boot time, so it might be that th=
e card
>  > has already been initalised, and trying to modprobe it again is causin=
g your
>  > error.
>  >
>  > Post the results of your dmesg output onto the list and see if anyone =
has
>  > any experience with the dvb-t facilties. I did have a quick play a long
>  > while ago, and from memory, if you look in /dev/dvb you might see seve=
ral
>  > front ends already loaded. I remember making some soft links in the /d=
ev/dvb
>  > directory to make it look like 2 different cards which gave me some re=
sults.
>  > You need to make sure all listings data is NOT collected from the broa=
dcast
>  > signal in that case however, as the drivers will try to use both front=
ends
...
>  > > compiles fine, but because the default mode is dvb-s I tried to chan=
ge
...
>  > >
>  > (/lib/modules/2.6.24-16-generic/kernel/drivers/media/video/cx88/cx88-d=
vb.ko):
...
>  > > > Hi Daniel,
>  > > >
>  > > > I had this and found the answer on the v4l-dvb wiki. Growlizing pu=
t an
>  > edit
...
>  > > >
>  > > > What u need to do is delete your v4l-dvb checkout then run it agai=
n with
>  > the
>  > > > command :
>  > > >
...
>  > > >
>  > > > This will pull down the older release, then you can patch that wit=
h the
>  > > > mfe-7285 diff which you already have.
>  > > >
>  > > > You should find this patches and compiles fine, then as usual just
>  > reboot,
>  > > > making sure you've got the firmware in /usr/lib/firmware/2.6.24 et=
c etc
>  > > >
>  > > > HTH,
>  > > >
...
>  > > > >
>  > > > >
>  > > > > What do you men when you say "Then re-checked out the older v4l-=
dvb
>  > tree"
>  > > > ??
>  > > > >
...
>  > > > >
>  > > > >
>  > > > > > Many thanks for all your help everyone, following Hagen's tip =
I went
>  > > > into
>  > > > > > /lib/modules/2.6.24-16-generic/ubuntu/media and just "rm -rf"'=
d the
>  > > > whole
>  > > > > > cx88 directory. Then re-checked out the older v4l-dvb tree,
>  > repatched it
>  > > > > > with dev.kewl.org's stable mfe patch and everything seems to b=
e Ok
>  > now.
>  > > > > >
...
>  > > > > >
>  > > > > > >
>  > > > > > > Before you try the 'new' modules you have to remove the 'ori=
ginal'
>  > > > > > Ubuntu-Version of cx88*. These modules can found in
>  > > > > > /lib/modules/2.6.24-16-generic/ubuntu/media/cx88 (don't now wh=
y not
>  > in
...
>  > > > > > > > > Hi All.
>  > > > > > > > >
>  > > > > > > > > Ok, so just installed the shiny, spangly new Ubuntu 8.04=
LTS
>  > (Hardy
>  > > > > > Heron) on
>  > > > > > > > > my machine with the HVR4000 in, and now, no TV! It's gon=
e on
>  > with
>  > > > > > kernel
>  > > > > > > > > 2.6.24-16 on a P4 HyperThread, and everything worked jus=
t fine
>  > > > under
>  > > > > > Gutsy.
...
>  > > > > > > > >
>  > > > > > > > > Everything 'seems' to compile Ok, and installs fine. Whe=
n I
>  > reboot
>  > > > > > however I
>  > > > > > > > > get a huge chunk of borked stuff and no card. (Dmesg out=
put at
>  > end
>  > > > of
>  > > > > > > > > message)
