...
Received: from mailout4.w1.samsung.com ([210.118.77.14]:54693 "EHLO
	mailout4.w1.samsung.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1753916Ab1JXJGh (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Mon, 24 Oct 2011 05:06:37 -0400
MIME-version: 1.0
Content-transfer-encoding: 7BIT
Content-type: TEXT/PLAIN
Received: from euspt1 ([210.118.77.14]) by mailout4.w1.samsung.com
 (Sun Java(tm) System Messaging Server 6.3-8.04 (built Jul 29 2009; 32bit))
 with ESMTP id <0LTK0041VBYY0O40@mailout4.w1.samsung.com> for
 linux-media@vger.kernel.org; Mon, 24 Oct 2011 10:06:35 +0100 (BST)
Received: from linux.samsung.com ([106.116.38.10])
 by spt1.w1.samsung.com (iPlanet Messaging Server 5.2 Patch 2 (built Jul 14
 2004)) with ESMTPA id <0LTK00JOQBYXH1@spt1.w1.samsung.com> for
 linux-media@vger.kernel.org; Mon, 24 Oct 2011 10:06:34 +0100 (BST)
Date: Mon, 24 Oct 2011 11:06:18 +0200
From: Andrzej Pietrasiewicz <andrzej.p@samsung.com>
Subject: [PATCH v2] ARM: EXYNOS4: JPEG: driver initial release
To: linux-media@vger.kernel.org
Cc: Andrzej Pietrasiewicz <andrzej.p@samsung.com>,
	Kyungmin Park <kyungmin.park@samsung.com>,
	Marek Szyprowski <m.szyprowski@samsung.com>
Message-id: <1319447178-17603-1-git-send-email-andrzej.p@samsung.com>
...
This is the second version of the Exynos4 JPEG codec v4l2 driver. It includes
modifications resulting from Sakari's review - thanks Sakari. It also includes
...
Andrzej Pietrasiewicz (1):
 ...
 drivers/media/video/s5p-jpeg/jpeg-regs.h |  181 ++++
 7 files changed, 2195 insertions(+), 0 deletions(-)
...
+	reg &= ~S5P_JPG_BYTE_MASK;
+	reg |= (dri >> S5P_JPGDRI_U_SHIFT) & S5P_JPG_BYTE_MASK;
+	writel(reg, regs + S5P_JPGDRI_U);
+
+	reg = readl(regs + S5P_JPGDRI_L);
+	reg &= ~S5P_JPG_BYTE_MASK;
+	reg |= (dri >> S5P_JPGDRI_L_SHIFT) & S5P_JPG_BYTE_MASK;
+	writel(reg, regs + S5P_JPGDRI_L);
+}
+
+static inline void jpeg_qtbl(void __iomem *regs, unsigned short t,
+			     unsigned short n)
+{
+	unsigned long reg;
+
...
+}
+
+static inline void jpeg_htbl_ac(void __iomem *regs, unsigned short t)
+{
+	unsigned long reg;
+
...
+}
+
+static inline void jpeg_htbl_dc(void __iomem *regs, unsigned short t)
+{
+	unsigned long reg;
+
+	reg = readl(regs + S5P_JPG_HTBL);
...
+	reg &= ~S5P_JPG_BYTE_MASK;
+	reg |= (y >> S5P_VER_RES_U_SHIFT) & S5P_JPG_BYTE_MASK;
+	writel(reg, regs + S5P_JPGY_U);
+
+	reg = readl(regs + S5P_JPGY_L);
+	reg &= ~S5P_JPG_BYTE_MASK;
+	reg |= (y >> S5P_VER_RES_L_SHIFT) & S5P_JPG_BYTE_MASK;
...
+	reg &= ~S5P_JPG_BYTE_MASK;
+	reg |= (x >> S5P_HOR_RES_U_SHIFT) & S5P_JPG_BYTE_MASK;
+	writel(reg, regs + S5P_JPGX_U);
+
+	reg = readl(regs + S5P_JPGX_L);
+	reg &= ~S5P_JPG_BYTE_MASK;
+	reg |= (x >> S5P_HOR_RES_L_SHIFT) & S5P_JPG_BYTE_MASK;
...
+}
+
+static inline void jpeg_coef(void __iomem *regs, unsigned short i,
+			     unsigned short j, unsigned short coef)
+{
+	unsigned long reg;
+
...
+
+	jpeg_size |= (readl(regs + S5P_JPGCNT_U) & S5P_JPG_BYTE_MASK)
+		     << S5P_BYTE_CNT_U_SHIFT;
+	jpeg_size |= (readl(regs + S5P_JPGCNT_M) & S5P_JPG_BYTE_MASK)
+		     << S5P_BYTE_CNT_M_SHIFT;
+	jpeg_size |= (readl(regs + S5P_JPGCNT_L) & S5P_JPG_BYTE_MASK)
+		     << S5P_BYTE_CNT_L_SHIFT;
+
