...
Received: from smtp11.smtpout.orange.fr ([80.12.242.133]:19516 "EHLO
	smtp.smtpout.orange.fr" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1752941AbcHOTcV (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Mon, 15 Aug 2016 15:32:21 -0400
...
Subject: [PATCH v4 10/13] media: platform: pxa_camera: make a standalone v4l2 device
Date: Mon, 15 Aug 2016 21:02:00 +0200
Message-Id: <1471287723-25451-11-git-send-email-robert.jarzmik@free.fr>
...
@@ -1050,13 +1102,12 @@ static struct vb2_ops pxac_vb2_ops = {
...
+	memset(vq, 0, sizeof(*vq));
 	...
 	vq->drv_priv = pcdev;
@@ -1066,6 +1117,7 @@ static int pxa_camera_init_videobuf2(struct vb2_queue *vq,
...
+	vq->lock = &pcdev->mlock;
 	...
 	dev_dbg(pcdev_to_dev(pcdev),
@@ -1077,18 +1129,15 @@ static int pxa_camera_init_videobuf2(struct vb2_queue *vq,
 /*
...
+				  pcdev->current_fmt->host_fmt->bits_per_sample,
 	...
 	if (ret < 0)
 		return ret;
@@ -1139,24 +1188,20 @@ static int pxa_camera_set_bus_param(struct soc_camera_device *icd)
...
@@ -1202,13 +1247,12 @@ static bool pxa_camera_packing_supported(const struct soc_mbus_pixelfmt *fmt)
...
-	struct pxa_cam *cam;
 	struct v4l2_subdev_mbus_code_enum code = {
 		.which = V4L2_SUBDEV_FORMAT_ACTIVE,
 		.index = idx,
@@ -1222,25 +1266,16 @@ static int pxa_camera_get_formats(struct soc_camera_device *icd, unsigned int id
...
-
 	...
 	case MEDIA_BUS_FMT_UYVY8_2X8:
 		formats++;
@@ -1282,10 +1317,22 @@ static int pxa_camera_get_formats(struct soc_camera_device *icd, unsigned int id
 	return formats;
 }
...
@@ -1295,86 +1342,44 @@ static int pxa_camera_check_frame(u32 width, u32 height)
 		(width & 0x01);
 }
...
@@ -1385,7 +1390,7 @@ static int pxa_camera_try_fmt(struct soc_camera_device *icd,
...
+	xlate = soc_mbus_xlate_by_fourcc(pcdev->user_formats, pixfmt);
 	if (!xlate) {
 		...
 		return -EINVAL;
@@ -1401,26 +1406,18 @@ static int pxa_camera_try_fmt(struct soc_camera_device *icd,
...
@@ -1429,20 +1426,74 @@ static int pxa_camera_try_fmt(struct soc_camera_device *icd,
 		return -EINVAL;
 	}
...
@@ -1450,13 +1501,212 @@ static int pxa_camera_querycap(struct soc_camera_host *ici,
 	return 0;
 }
...
@@ -1465,8 +1715,7 @@ static int pxa_camera_suspend(struct device *dev)
...
+	if (pcdev->sensor) {
 		...
 		if (ret == -ENOIOCTLCMD)
 			ret = 0;
@@ -1477,8 +1726,7 @@ static int pxa_camera_suspend(struct device *dev)
...
@@ -1487,8 +1735,7 @@ static int pxa_camera_resume(struct device *dev)
...
+	if (pcdev->sensor) {
 		...
 		if (ret == -ENOIOCTLCMD)
 			ret = 0;
@@ -1501,27 +1748,12 @@ static int pxa_camera_resume(struct device *dev)
 	return ret;
 }
...
+	struct device_node *remote, *np = dev->of_node;
 	...
 	int err = of_property_read_u32(np, "clock-frequency",
 				       &mclk_rate);
@@ -1532,13 +1764,13 @@ static int pxa_camera_pdata_from_dt(struct device *dev,
...
@@ -1573,6 +1805,15 @@ static int pxa_camera_pdata_from_dt(struct device *dev,
...
@@ -1591,6 +1832,7 @@ static int pxa_camera_probe(struct platform_device *pdev)
 	};
 	dma_cap_mask_t mask;
...
@@ -1613,10 +1855,14 @@ static int pxa_camera_probe(struct platform_device *pdev)
...
+		pcdev->asd.match.i2c.address = pcdev->pdata->sensor_i2c_address;
 	...
 	if (err < 0)
 		return err;
@@ -1648,6 +1894,7 @@ static int pxa_camera_probe(struct platform_device *pdev)
...
+	mutex_init(&pcdev->mlock);
 	...
 	/*
 	 * Request the regions.
@@ -1710,19 +1957,48 @@ static int pxa_camera_probe(struct platform_device *pdev)
 		goto exit_free_dma;
 	}
...
+	v4l2_device_unregister(&pcdev->v4l2_dev);
 ...
 exit_free_dma_u:
@@ -1734,15 +2010,15 @@ exit_free_dma_y:
