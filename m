...
Received: from caramon.arm.linux.org.uk ([78.32.30.218]:34968 "EHLO
	caramon.arm.linux.org.uk" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S932266AbZJLQBN (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Mon, 12 Oct 2009 12:01:13 -0400
Date: Mon, 12 Oct 2009 17:00:15 +0100
...
Subject: Re: Global Video Buffers Pool - PMM and UPBuffer reference drivers
	[RFC]
Message-ID: <20091012160015.GC2480@n2100.arm.linux.org.uk>
...
mach/ include files shouldn't be exported to userspace, so in theory
...
default n is the default already.  It serves no purpose to add a default
that already is.
...
Legal values are 'n', 'm', 'y' not 'no' and 'yes'.
...
Does it really make sense to have this kind of stuff?
...
No.  DEBUG has a specific meaning in the kernel.  Use some other macro for
your own debugging, rather than trying to work around the kernel.
...
Is there no better error code?  Why impersonate -EPERM?
...
Pointers should not be initialized with integers - use NULL instead.
...
Hardware page table entries aren't exposed by the Linux page table
walking APIs.  Please don't do this.
...
pte_offset_map() needs a matching pte_unmap() call.  If you use highmem
with this, this will become very important.
...
Please use pfn_to_phys(pte_pfn(pte)).  Even better, please keep it as a
PFN so highmem can be used with this.
...
We have macros for converting from physical to pfns and vice versa.
Please use them.
