...
Received: from mailout4.w1.samsung.com ([210.118.77.14]:14089 "EHLO
	...
	with ESMTP id S1756066Ab0LPOY1 (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Thu, 16 Dec 2010 09:24:27 -0500
MIME-version: 1.0
...
Received: from linux.samsung.com ([106.116.38.10])
 ...
 2004)) with ESMTPA id <0LDI00A3BYOFJY@spt1.w1.samsung.com> for
 linux-media@vger.kernel.org; Thu, 16 Dec 2010 14:24:16 +0000 (GMT)
Date: Thu, 16 Dec 2010 15:24:03 +0100
From: Marek Szyprowski <m.szyprowski@samsung.com>
Subject: [PATCH 6/8] v4l: videobuf2: add read() and write() emulator
...
Message-id: <1292509445-15100-7-git-send-email-m.szyprowski@samsung.com>
...
+
 	if (b->type != q->type) {
 		...
 		return -EINVAL;
@@ -1030,6 +1040,11 @@ int vb2_dqbuf(struct vb2_queue *q, struct v4l2_buffer *b, bool nonblocking)
...
+
 	if (b->type != q->type) {
 		...
 		return -EINVAL;
@@ -1089,6 +1104,11 @@ int vb2_streamon(struct vb2_queue *q, enum v4l2_buf_type type)
 {
 	struct vb2_buffer *vb;
...
+
 	if (type != q->type) {
 		...
 		return -EINVAL;
@@ -1182,6 +1202,11 @@ static void __vb2_queue_cancel(struct vb2_queue *q)
  */
...
+
 	if (type != q->type) {
 		...
 		return -EINVAL;
@@ -1305,6 +1330,8 @@ int vb2_mmap(struct vb2_queue *q, struct vm_area_struct *vma)
 }
...
@@ -1325,9 +1352,30 @@ EXPORT_SYMBOL_GPL(vb2_mmap);
...
@@ -1397,11 +1445,362 @@ EXPORT_SYMBOL_GPL(vb2_queue_init);
  */
...
@@ -295,6 +300,10 @@ int vb2_streamoff(struct vb2_queue *q, enum v4l2_buf_type type);
