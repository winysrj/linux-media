...
Received: from mail-ea0-f171.google.com ([209.85.215.171]:57286 "EHLO
	mail-ea0-f171.google.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1756044Ab3AFRZo (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Sun, 6 Jan 2013 12:25:44 -0500
...
Subject: [PATCH V4 2/3] sta2x11_vip: convert to videobuf2 and control framework
Date: Sun,  6 Jan 2013 18:29:02 +0100
Message-Id: <1357493343-13090-2-git-send-email-federico.vaga@gmail.com>
...
@@ -840,304 +530,226 @@ static int vidioc_try_fmt_vid_cap(struct file *file, void *priv,
 	return 0;
 }
...
-		}
-		REG_WRITE(vip, DVP_CTL, REG_READ(vip, DVP_CTL) | DVP_CTL_ENA);
+	vip->alloc_ctx->mem_flags = GFP_DMA;
+	return 0;
+}
...
- */
 static int vip_gpio_reserve(struct device *dev, int pin, int dir,
 			    const char *name)
 {
@@ -1170,13 +782,6 @@ static int vip_gpio_reserve(struct device *dev, int pin, int dir,
 	return 0;
 }
...
- */
 ...
 {
 	if (pin != -1) {
@@ -1186,25 +791,6 @@ static void vip_gpio_release(struct device *dev, int pin, const char *name)
 	}
 }
...
- */
 ...
 {
@@ -1212,10 +798,17 @@ static int __devinit sta2x11_vip_init_one(struct pci_dev *pdev,
...
@@ -1223,6 +816,7 @@ static int __devinit sta2x11_vip_init_one(struct pci_dev *pdev,
 		goto disable;
 	}
...
+	/* Power configuration */
 	ret = vip_gpio_reserve(&pdev->dev, config->pwr_pin, 0,
 			       config->pwr_name);
 	if (ret)
@@ -1237,7 +831,6 @@ static int __devinit sta2x11_vip_init_one(struct pci_dev *pdev,
 			goto disable;
 		}
 	}
-
...
@@ -1251,17 +844,20 @@ static int __devinit sta2x11_vip_init_one(struct pci_dev *pdev,
 	}
 	usleep_range(5000, 25000);
...
@@ -1271,46 +867,52 @@ static int __devinit sta2x11_vip_init_one(struct pci_dev *pdev,
...
+	/* Get ADV7180 subdevice */
 	...
 	if (!vip->adapter) {
 		ret = -ENODEV;
@@ -1328,10 +930,11 @@ static int __devinit sta2x11_vip_init_one(struct pci_dev *pdev,
 	}
...
-	mutex_destroy(&vip->mutex);
 	...
 	vip_gpio_release(&pdev->dev, vip->config->pwr_pin,
 			 vip->config->pwr_name);
@@ -1407,18 +1002,12 @@ static void __devexit sta2x11_vip_remove_one(struct pci_dev *pdev)
...
- * power down mode.
 ...
 {
@@ -1429,15 +1018,15 @@ static int sta2x11_vip_suspend(struct pci_dev *pdev, pm_message_t state)
 	int i;
...
+		    reg_read(vip, registers_to_save[i]);
 	...
 	/* save pci state */
 	pci_save_state(pdev);
@@ -1453,16 +1042,9 @@ static int sta2x11_vip_suspend(struct pci_dev *pdev, pm_message_t state)
 	return 0;
 }
...
- * other, could not set device to power on state.
 ...
 {
@@ -1477,7 +1059,7 @@ static int sta2x11_vip_resume(struct pci_dev *pdev)
 	if (vip->disabled) {
 		ret = pci_enable_device(pdev);
 		if (ret) {
...
+			pr_warn("VIP: Can't enable device.\n");
 		...
 		vip->disabled = 0;
@@ -1488,7 +1070,7 @@ static int sta2x11_vip_resume(struct pci_dev *pdev)
...
@@ -1497,12 +1079,12 @@ static int sta2x11_vip_resume(struct pci_dev *pdev)
...
@@ -1515,7 +1097,7 @@ static DEFINE_PCI_DEVICE_TABLE(sta2x11_vip_pci_tbl) = {
 };
