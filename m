...
Received: from server.road.de ([85.10.209.111]:34072 "EHLO mail.road.de"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
	id S1751740AbZKPHNX (ORCPT <rfc822;linux-media@vger.kernel.org>);
	Mon, 16 Nov 2009 02:13:23 -0500
From: Uwe Taeubert <u.taeubert@road.de>
To: Guennadi Liakhovetski <g.liakhovetski@gmx.de>
Subject: Re: Problem on RJ54N1CB0C
Date: Mon, 16 Nov 2009 08:07:24 +0100
References: <200911130950.30581.u.taeubert@road.de> <Pine.LNX.4.64.0911131015260.4601@axis700.grange>
In-Reply-To: <Pine.LNX.4.64.0911131015260.4601@axis700.grange>
Cc: linux-media@vger.kernel.org
MIME-Version: 1.0
Content-Type: Multipart/Mixed;
  boundary="Boundary-00=_toPALFCkWulkp+8"
Message-Id: <200911160807.25160.u.taeubert@road.de>
Sender: linux-media-owner@vger.kernel.org
List-ID: <linux-media.vger.kernel.org>

--Boundary-00=_toPALFCkWulkp+8
...
Am Friday 13 November 2009 10:49:43 schrieben Sie:
> On Fri, 13 Nov 2009, Uwe Taeubert wrote:
> > Hello Guennadi,
>
> Hi Uwe
>
> > I'm working on the implementation on the RJ54N1CB0C in the ROAD HandyPC
> > in Berlin and our driver is derived from the mt9m001 driver. And I've the
> > problem of understanding the Sharp documentation. So I hope for a hint to
> > solve my problem.
>
> I suspect you're writing to me because you've seen my version of the
> RJ54N1CB0C driver. It is, however, always better to CC the respective
> mailing list to also let others benefit from this discussion. Please,
> add
>
...
>
> As for the driver, wouldn't it be possible for you to give my version a
> try? Can you also share your version of the driver?
>
> > 1. Because of our EXCLK of only 13MHz and our pixel reading interface up
> > to maximum 25MHz I had to adapt standard timing values to get high frame
> > rates. We're using different picture output formats: 320x240 mostly for
> > preview, 640x480, 800x600 and 1200x900 for a photo. If we do not need a
> > flash light, we take a picture for the photo out of the preview picture
> > stream switching the output resolution - that works.
> > I calculated the dividers to get 39.8MHz for RAMPCLK and TGCLK, 22.8MHz
> > on RCLK and 11.4MHz on OCLK. It works fine and I get 15FPS. Fine! But
> > only on low resolution. If I use these values to take a 1200x900 picture
> > I get eightfold pictures -  the timing seems to be wrong. One solution is
> > to change timing - it's nasty but works. But why I have to change the
> > timing? I can't find an explanation. Any ideas? The change of the timing
> > causes the 2nd problem.
>
> As you might have seen in my driver, I'm artificially disabling larger
> image formats because I was not able to obtain proper results in the
> preview mode. Instead I was getting a somewhat vertically squashed image
> padded at the bottom with a stripe of a colour, depending on one corner
> pixel. Are you at all able to get correct preview with those sizes - about
> VGA and larger? Would be interesting to see how you achieve that and try
> on my setup. As for why - sorry, no idea. I found chip's behaviour quite
> unlogical on many occasions too.
>
> > 2. It seems, the change of timing gives the camera more time for
> > exposure. So, the result picture is overexposed. It wouldn't be a problem
> > if I could find a register to limit exposure. Any help?
>
> I don't know what documentation you've got. The datasheet I've got is
> under NDA so, unfortunately, I won't be able to share information from it.
> But yes, there are registers to control Iris and Shutter, the only thing
> my driver is currently doing about this is controlling the auto-exposure
> area.
>
> > If I manipulate the luminance signal on
> > Register 0x04D8 and 0x04D9 I get a darker picture similar to the preview
> > but I shorten the range of the data output and the result is realy bad.
>
> 0x4d8 is just an analog gain, it doesn't change exposure.
>
> > I have to find a solution for at least one of the problems. It would be
> > nice to get help.
>
> Unfortunately I don't have ready answers to your questions. I didn't try
> the snapshot mode, I suspect, larger resolutions should work better in
> that mode. Even configuration examples that I have seen only configure
> smaller images for preview and larger for still shooting. So, it looks
> like this is not an unknown issue:-(
>
> Regards
> Guennadi
> ---
...
#if 0
(Cannot mark this comment with the ordinary comment sign, as there appears =
an
...
(where ADDRESS and VALUE are in hex format with upper case characters) to a=
 C
array:
...
while read line ; do
	...
	[ "${line:0:1}" =3D "#" ] && line=3D"- - $line"

	echo "$line" | tr -d \\r | (

		read address value comment

		while [ "${comment:0:1}" =3D "#" ] ; do
			comment=3D${comment:1}
		done

		[ ${#address} -gt 2 ] && address=3D${address:-2}
		[ -n "$comment" ] && comment=3D" /* $comment */"

		[ "$address" =3D FF ] && bank=3D$value

		[ "$address" !=3D "-" -a "$value" !=3D "-" ] && {
...
/* define USE_CENTRAL_EMPHASIS if the exposure controll with central emphas=
is=20
   is desired*/
...
static int led_mode =3D LEDMODE_FLASH;
...
static struct i2c_client *lz0p35du_i2c_client =3D 0;
static int lz0p35du_sysfs_installed =3D 0;
...
static const struct soc_camera_data_format lz0p35du_colour_formats[] =3D {
	{
		.name		=3D "RGB565",
		.depth		=3D 16,
...
static int dump_registers(void)
{
	...
	led_mode =3D LEDMODE_FLASH;

	down(&lz0p35du_lock);
	for (i =3D 0; i < ARRAY_SIZE(init_sequence); i++) {

		if (init_sequence[i].address =3D=3D 0xff &&
					init_sequence[i].value =3D=3D 0xff) {
			...
			if (init_sequence[i].address =3D=3D 0xff)
				bank =3D init_sequence[i].value;
		}

		if (bank =3D=3D 0x07 && init_sequence[i].address =3D=3D 0x04)
...
static int lz0p35du_read(const u16 reg)
{
	...
	down(&lz0p35du_lock);
	ret =3D i2c_smbus_write_byte_data(lz0p35du_i2c_client, 0xff, reg >> 8);
	...
	ret =3D i2c_smbus_read_byte_data(lz0p35du_i2c_client, reg & 0xff);
...
static int lz0p35du_write(const u16 reg, const u8 data)
{
	...
	ret =3D i2c_smbus_write_byte_data(lz0p35du_i2c_client, reg & 0xff, data);
...
static int lz0p35du_set_bus_param(struct soc_camera_device *icd,
				 unsigned long flags)
{
	if ((flags & SOCAM_DATAWIDTH_MASK) !=3D SOCAM_DATAWIDTH_8)
		return -EINVAL;
...
static void updateResize(struct lz0p35du *lz0p35du)
{
	...
	*/
	...
	hfac =3D 1600 * 1024 * 10 / (lz0p35du->width * lz0p35du->zoom);
	vfac =3D 1200 * 1024 * 10 / (lz0p35du->height * lz0p35du->zoom );
	fac =3D hfac < vfac ? hfac : vfac;
		...
		__u32 pixfmt, struct v4l2_rect *rect)
{
	int	tmp,
		...
		tmpwidth,
		tmphight;
	char	cEights,devider;

	struct lz0p35du *lz0p35du =3D container_of(icd, struct lz0p35du, icd);
...
ght);
	...
	lz0p35du->width =3D rect->width;
	lz0p35du->height =3D rect->height;
	updateResize(lz0p35du);

	/* Set output size */
	dev_dbg(&icd->dev, "Setting output still mode\n");

	tmp =3D (rect->width >> 8) & 0x07;
	tmp =3D tmp << 4;
	tmp =3D tmp | ((rect->height >> 8) & 0x07);
	...
	if (rect->width <=3D 256) {
		hmin =3D 0;
		hmax =3D rect->width - 1;
	} else {
		hmin =3D (rect->width - 256) / 2;
		hmax =3D hmin + 255;
	}

	if (rect->height <=3D 256) {
		vmin =3D 0;
		vmax =3D rect->height - 1;
	} else {
		vmin =3D (rect->height - 256) / 2;
		vmax =3D vmin + 255;
	}
	...
	tmp =3D (hmax >> 8) & 0x07;
	tmp =3D tmp << 4;
	tmp =3D tmp | ((vmax >> 8) & 0x07);
	lz0p35du_write(0x5cc, tmp);
	lz0p35du_write(0x5cd, hmax & 0xff);
	lz0p35du_write(0x5ce, vmax & 0xff);

	if((rect->width =3D=3D 240) &&  (rect->height =3D=3D 320)){
			printk(KERN_ERR "sh_out Switch to 240 x 320\n");
/*		lz0p35du_write(0x597, 0x06);
		lz0p35du_write(0x598, 0x40);
		lz0p35du_write(0x59b, 0x06);
		lz0p35du_write(0x59c, 0x40);
		lz0p35du_write(0x59e, 0x80);*/
/*		lz0p35du_write(0x4d8, 0x8d);
		lz0p35du_write(0x4d9, 0xf0);
		lz0p35du_write(0x570, 0x22);*/
		devider =3D 0x00;/*0*/
	}
	else if((rect->width =3D=3D 480) && (rect->height =3D=3D 640)){
 		devider =3D 0x01;/*1*/
 	}
 	else if((rect->width =3D=3D 600) && (rect->height =3D=3D 800)){
		devider =3D 0x02;
	}
	else if((rect->width =3D=3D 900) && (rect->height =3D=3D 1200)){
/*		lz0p35du_write(0x595, 0x03);
		lz0p35du_write(0x596, 0xec);*/
			printk(KERN_ERR "sh_out Switch to 900 x 1200\n");
/*		lz0p35du_write(0x597, 0x04);
		lz0p35du_write(0x598, 0x40);
		lz0p35du_write(0x59b, 0x04);
		lz0p35du_write(0x59c, 0x50);
		lz0p35du_write(0x59e, 0x58);*/
/*		lz0p35du_write(0x4d8, 0x40);
		lz0p35du_write(0x4d9, 0xfa);*/
/*		lz0p35du_write(0x570, 0x88);*/
		devider =3D 0x02;
	}
/*	else if((rect->width =3D=3D 1200) && (rect->height =3D=3D 1600)){
		lz0p35du_write(0x707, 0x03); Not tested yet
	}*/
	else {
...
t->width,rect->height);
	...
	lz0p35du_write(0x707, devider);

#ifdef USE_CENTRAL_EMPHASIS=09
	if((rect->width =3D=3D 240) &&  (rect->height =3D=3D 320)){
		lz0p35du_write(0x570, 0x22);
	}
	else if((rect->width =3D=3D 480) && (rect->height =3D=3D 640)){
	...
	tmpwidth =3D rect->width >> 2;
	tmphight =3D rect->height>> 2;
	dev_dbg(&icd->dev, "Setting mode\n");
	lz0p35du_write(0x580, 0);
	lz0p35du_write(0x585, 0);
	cEights =3D lz0p35du_read(0x058C);
	cEights &=3D 0x2A;/*set bit 7 to 0, bits 5, 3, 1 stay unchanged*/
	/*set central area (1/4 TO 3/4 FROM FROM UPPER LEFT)*/
	cEights |=3D (((tmpwidth >> 2) - 1) & 0x100) >> 2;/*8th bit of left upper =
X on bit 6*/
	cEights |=3D (((tmphight >> 2) - 1) & 0x100) >> 4;/*8th bit of left upper =
Y on bit 4*/
	cEights |=3D (((((tmpwidth * 3) - 3) >> 2) - 1) & 0x100) >> 6;/*8th bit of=
 right lower X on bit 2*/
	cEights |=3D (((((tmphight * 3) - 3) >> 2) - 1) & 0x100) >> 8;/*8th bit of=
 right lower Y on bit 0*/
	dev_dbg(&icd->dev, "Breite: %d, H=C3=B6he: %d\n",rect->width,rect->height);
	dev_dbg(&icd->dev, "Register 0x8C: %x\n",cEights);
	lz0p35du_write(0x58C, cEights);
	dev_dbg(&icd->dev, "Setting upper left; ULX: %x, ULY: %x\n",((tmpwidth >> =
2) - 1) & 0xFF,((tmphight >> 2) - 1) & 0xFF);
	lz0p35du_write(0x58D, ((tmpwidth >> 2) - 1) & 0xFF);/*left upper X*/
	lz0p35du_write(0x58E, ((tmphight >> 2) - 1) & 0xFF);/*left upper Y*/
	dev_dbg(&icd->dev, "Setting lower right; LRX: %x, LRY: %x\n",((((tmpwidth =
* 3) - 3) >> 2) - 1) & 0xFF,((((tmphight * 3) - 3) >> 2) - 1) & 0xFF);
	lz0p35du_write(0x58F, ((((tmpwidth * 3) - 3) >> 2) - 1) & 0xFF);/*right lo=
wer X*/
	lz0p35du_write(0x590, ((((tmphight * 3) - 3) >> 2) - 1) & 0xFF);/*right lo=
wer Y*/
#endif

	if (lz0p35du->flash_enabled) {
		tmp =3D lz0p35du_read(0x418);
		...
		if (tmp < 0) {
			...
			lz0p35du->flash_active =3D 1;
		}

		lz0p35du->flash_enabled =3D 0;
	}
			       ...
			       struct v4l2_format *f)
{
	...
	if (f->fmt.pix.width > 1600)
		f->fmt.pix.width =3D 1600;

	if (f->fmt.pix.height > 1200)
		f->fmt.pix.height =3D 1200;
			       ...
			       struct v4l2_chip_ident *id)
{
	...
	if (id->match_type !=3D V4L2_CHIP_MATCH_I2C_ADDR)
		return -EINVAL;

	if (id->match_chip !=3D lz0p35du->client->addr)
		return -ENODEV;

	id->ident	=3D lz0p35du->model;
	id->revision	=3D 0;
...
static const struct v4l2_queryctrl lz0p35du_controls[] =3D {
	{
	...
	{
		.id		=3D V4L2_CID_PRIVATE_BASE + 2,
		.type		=3D V4L2_CTRL_TYPE_BOOLEAN,
		.name		=3D "Permanent Light",
		.minimum	=3D 0,
		.maximum	=3D 1,
		.step		=3D 1,
		.default_value	=3D 0,
...
static struct soc_camera_ops lz0p35du_ops =3D {
	.owner			=3D THIS_MODULE,
	.probe			=3D lz0p35du_video_probe,
	.remove			=3D lz0p35du_video_remove,
	.init			=3D lz0p35du_init,
	.release		=3D lz0p35du_release,
	.start_capture		=3D lz0p35du_start_capture,
	.stop_capture		=3D lz0p35du_stop_capture,
	.set_fmt_cap		=3D lz0p35du_set_fmt_cap,
	.try_fmt_cap		=3D lz0p35du_try_fmt_cap,
	.set_bus_param		=3D lz0p35du_set_bus_param,
	.query_bus_param	=3D lz0p35du_query_bus_param,
	.controls		=3D lz0p35du_controls,
	.num_controls		=3D ARRAY_SIZE(lz0p35du_controls),
	.get_control		=3D lz0p35du_get_control,
	.set_control		=3D lz0p35du_set_control,
	.get_chip_id		=3D lz0p35du_get_chip_id,
	.frame_ready		=3D af_frame_ready,
	.flashlight		=3D lz0p35du_flashlight,
};
						...
						struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_FOCUS_AUTO:
		ctrl->value =3D af_is_auto() || af_is_scanning();
		break;
	...
	case V4L2_CID_PRIVATE_BASE:	/* Enable Flash */
		ctrl->value =3D lz0p35du->flash_enabled;
		break;

	case V4L2_CID_PRIVATE_BASE + 1:	/* Zoom factor in 1/10 */
		ctrl->value =3D lz0p35du->zoom;
		break;
						...
						struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_FOCUS_ABSOLUTE:	/* Focus value in % */
		...
		if (ctrl->value =3D=3D -1 && ! af_is_scanning()) {
			af_start(0);
			break;
		}
	...
	case V4L2_CID_PRIVATE_BASE:	/* Enable Flash */
		lz0p35du->flash_enabled =3D ctrl->value ? 1 : 0;
		break;
...
static int lz0p35du_video_probe(struct soc_camera_device *icd)
{
	...
	if (!icd->dev.parent || to_soc_camera_host(icd->dev.parent)->nr !=3D
								icd->iface)
		return -ENODEV;

	/* check the device id */

	id1 =3D lz0p35du_read(0x400);
	id2 =3D lz0p35du_read(0x401);

	if (id1 !=3D 0x51 || id2 !=3D 0x10) {
		printk(KERN_ERR "lz0p39ha_dsp: wrong id or read error\n");
		return -ENODEV;
	}

#warning create V4L identifier
//	lz0p35du->model =3D V4L2_IDENT_;
	lz0p35du->model =3D 0;

	/* set color formats */

	icd->formats =3D lz0p35du_colour_formats;
	icd->num_formats =3D ARRAY_SIZE(lz0p35du_colour_formats);
					 ...
					 const struct i2c_device_id *did)
{
	...
	struct soc_camera_device *icd;
	struct soc_camera_link *icl =3D client->dev.platform_data;
	int ret;

	lz0p35du_i2c_client =3D client;
	...
	lz0p35du =3D kzalloc(sizeof(struct lz0p35du), GFP_KERNEL);
	...
	lz0p35du->flash_enabled =3D 0;
	lz0p35du->flash_active =3D 0;
	lz0p35du->zoom =3D 10;		/* Zoom factor in 1/10 */
	lz0p35du->client =3D client;
	i2c_set_clientdata(client, lz0p35du);

	/* Second stage probe - when a capture adapter is there */

	icd =3D &lz0p35du->icd;
	icd->ops	=3D &lz0p35du_ops;
	icd->control	=3D &client->dev;
	icd->x_min	=3D 20;
	icd->y_min	=3D 12;
	icd->x_current	=3D 20;
	icd->y_current	=3D 12;
	icd->width_min	=3D 48;
	icd->width_max	=3D 1600;
	icd->height_min	=3D 32;
	icd->height_max	=3D 1200;
	icd->y_skip_top	=3D 1;
	icd->iface	=3D icl->bus_id;
...
#if 0
	ret =3D platform_device_register(&lz0p35du_flashlight_pd);
...
//	if (device_create_file(&icd->dev, &dev_attr_enable))
	if (device_create_file(&lz0p35du_flashlight_pd.dev, &dev_attr_enable))
		...
		lz0p35du_sysfs_installed =3D 1;
#endif

	af_workqueue =3D create_singlethread_workqueue("autofocus");
...
{
	struct lz0p35du *lz0p35du =3D i2c_get_clientdata(client);
...
static struct i2c_client *ad5821_i2c_client =3D 0;
static int ad5821_sysfs_installed =3D 0;
static int af_value =3D 0;
...
static int ad5821_write(u16 val)
{
	...
	val =3D val << 4;	/* AD5821 expects value in [13:4] */

	msgbuf[0] =3D (val & 0xff00) >> 8;
	msgbuf[1] =3D val & 0x00ff;

	if (i2c_master_send(ad5821_i2c_client, msgbuf, 2) !=3D 2)
		return -1;
	else
		return 0;
}
...
static int ad5821_read(void)
{
	...
	if (i2c_master_recv(ad5821_i2c_client, msgbuf, 2) =3D=3D 2) {
...
static ssize_t ad5821_show(struct device *dev, struct device_attribute *att=
r,
								char *buf)
{
...
static ssize_t ad5821_store(struct device *dev, struct device_attribute *at=
tr,
						const char *buf, size_t size)
{
	...
	if (buf[0] =3D=3D '0' && buf[1] =3D=3D 'x') {

		len =3D size > 5 ? 5 : size;
		len -=3D 2;
		buf +=3D 2;

		while (len--) {
			if (*buf >=3D '0' && *buf <=3D '9') {
				val =3D (val << 4) | (*buf - '0');
				valid =3D 1;
			} else if (*buf >=3D 'a' && *buf <=3D 'f') {
				val =3D (val << 4) | (*buf + 10 - 'a');
				valid =3D 1;
			} else if (*buf >=3D 'A' && *buf <=3D 'F') {
				val =3D (val << 4) | (*buf + 10 - 'A');
				valid =3D 1;
...
static int af_direction =3D 1;	/* always 1 or -1 */
static int af_last_value;
static int af_track =3D 0;
static int af_scanthrough =3D 0;
...
static void af_start(int track)
{
	...
	if (track) {
		af_track =3D 1;
		af_scanthrough =3D 0;
		af_stepsize =3D 0x40;	/* start value for stepsize with no
					/* particular reason for this value */
		af_setting =3D 0x400 / 2;	/* start in the middle */
		af_waitcnt =3D 3;
...
static void af_update(struct work_struct *ptr)
{
	...
	struct lz0p35du *lz0p35du =3D container_of(ptr, struct lz0p35du, work);

	if (lz0p35du->flash_active) {
		lz0p35du->flash_active =3D 0;
		val =3D lz0p35du_read(0x418);
...
#ifdef DEBUG
		sh_out15_8 =3D lz0p35du_read(0x0613);
		sh_out7_0 =3D lz0p35du_read(0x0614);
		sh_out =3D 0;
		sh_out =3D (sh_out15_8 << 8) + sh_out7_0;
		printk(KERN_ERR "sh_out (SH_Foto): %d\n",sh_out);
		sh_out15_8 =3D lz0p35du_read(0x0611);
		sh_out7_0 =3D lz0p35du_read(0x0612);
		sh_out =3D 0;
		sh_out =3D (sh_out15_8 << 8) + sh_out7_0;
	...
	for (addr =3D 0x0619; addr <=3D 0x061B; addr++) {

		val =3D lz0p35du_read(addr);

		if (val < 0)
			break;

		current_af_value =3D (current_af_value << 8) | val;
	}

	/* only if val >=3D 0, all three bytes of the automatic focus data could
	...
	if (af_scanthrough) {
		...
		af_waitcnt =3D 1;
#endif
		af_val[af_current_idx] =3D current_af_value;
#ifdef DEBUG
		printk(KERN_ERR "AF-Step: %d, AF-Value: %d\n",af_current_idx, current_af_=
value);
		sh_out15_8 =3D lz0p35du_read(0x0613);
		sh_out7_0 =3D lz0p35du_read(0x0614);
		sh_out =3D 0;
...
#endif
		if (af_current_idx =3D=3D AF_STEPS-1) {

			int i, max_val =3D 0, max_set =3D 0;

			for (i =3D 0; i < AF_STEPS; i++) {
				if (af_val[i] > max_val) {
					max_val =3D af_val[i];
					max_set =3D af_set[i];
				}
			...
			printk(KERN_ERR "max value =3D 0x%08x, setting =3D %d\n",
							max_val, max_set);
			ad5821_write(max_set);
			af_scanthrough =3D 0;
		} else {
			af_current_idx++;
			af_set[af_current_idx] =3D af_current_idx * (0x400 / AF_STEPS);
	...
	if (af_track) {
		...
		if (af_last_value =3D=3D -1) {
			...
			if (current_af_value - af_last_value > 5 &&
				...
				af_direction =3D -af_direction;
				af_stepsize =3D af_stepsize >> 1;

				if (af_stepsize =3D=3D 0)
					af_stepsize =3D 1;
			}
		} else if (af_last_value > current_af_value) {
		...
		if (restart) {
			...
			af_setting =3D af_setting + (af_stepsize * af_direction);
...
static const struct i2c_device_id lz0p39ha_id[] =3D {
	{ "lz0p39ha_dsp", 0 },
	{ }
};
...
static struct i2c_driver lz0p35du_i2c_driver =3D {
	.driver =3D {
		.name =3D "lz0p39ha_dsp",
	},
	.probe		=3D lz0p35du_probe,
	.remove		=3D lz0p35du_remove,
	.id_table	=3D lz0p39ha_id,
};

static const struct i2c_device_id ad5821_id[] =3D {
	{ "lz0p39ha_af", 0 },
	{ }
};
...
static struct i2c_driver ad5821_i2c_driver =3D {
	.driver =3D {
		.name =3D "lz0p39ha_af",
	},
	.probe		=3D ad5821_probe,
	.remove		=3D ad5821_remove,
	.id_table	=3D ad5821_id,
};
...
static int __init lz0p39ha_mod_init(void)
{
	...
	ret =3D i2c_add_driver(&lz0p35du_i2c_driver);
	...
	ret =3D i2c_add_driver(&ad5821_i2c_driver);
