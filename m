...
Received: from mga02.intel.com ([134.134.136.20]:11612 "EHLO mga02.intel.com"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
	id S1751065AbaBNKiG (ORCPT <rfc822;linux-media@vger.kernel.org>);
	Fri, 14 Feb 2014 05:38:06 -0500
Message-ID: <1392374281.23168.46.camel@smile.fi.intel.com>
Subject: Re: [RFC v3,3/3] media: i2c: add new dual LED Flash driver, lm3646
...
Date: Fri, 14 Feb 2014 12:38:01 +0200
In-Reply-To: <1392371190-32683-1-git-send-email-gshark.jeong@gmail.com>
References: <1392371190-32683-1-git-send-email-gshark.jeong@gmail.com>
...
> +#ifdef CONFIG_OF
> +
> +static int lm3646_parse_dt(struct i2c_client *client,
> +			   struct lm3646_platform_data *pdata)
> +{
...
I think you may remove above lines...

> +
> +	of_property_read_u32(node, "flash_timeout", &pdata->flash_timeout);

...and append return value check here and in other places where you do
something with devicetree api.


> +	if (pdata->flash_timeout > LM3646_FLASH_TOUT_MAX)
> +		pdata->flash_timeout = LM3646_FLASH_TOUT_MAX;

You may substitute those if's by
 var = min(var, CONST);

> +
> +	of_property_read_u32(node, "led1_torch_brt", &pdata->led1_torch_brt);
> +	if (pdata->led1_torch_brt > LM3646_LED1_TORCH_BRT_MAX)
> +		pdata->led1_torch_brt = LM3646_LED1_TORCH_BRT_MAX;
> +
> +	of_property_read_u32(node, "led1_flash_brt", &pdata->led1_flash_brt);
> +	if (pdata->led1_flash_brt > LM3646_LED1_FLASH_BRT_MAX)
...
> +		if (pdata == NULL)
> +			return -ENOMEM;
> +		if (lm3646_parse_dt(client, pdata) < 0) {

Just check for non-null?

> +			/* no data at device tree and platform data, use default */
> +			pdata->flash_timeout = LM3646_FLASH_TOUT_MAX;
> +			pdata->led1_torch_brt = LM3646_LED1_TORCH_BRT_MAX;
> +			pdata->led1_flash_brt = LM3646_LED1_FLASH_BRT_MAX;
> +		}
