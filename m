...
Received: from mailout3.samsung.com ([203.254.224.33]:11955 "EHLO
	mailout3.samsung.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1753347Ab2H0JPK (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Mon, 27 Aug 2012 05:15:10 -0400
Received: from epcpsbgm2.samsung.com (epcpsbgm2 [203.254.230.27])
 by mailout3.samsung.com
 (Oracle Communications Messaging Server 7u4-24.01(7.0.4.24.0) 64bit (built Nov
 17 2011)) with ESMTP id <0M9E00DSBPNMOHM0@mailout3.samsung.com> for
 linux-media@vger.kernel.org; Mon, 27 Aug 2012 18:15:08 +0900 (KST)
Received: from localhost.localdomain ([107.108.73.106])
 by mmp2.samsung.com (Oracle Communications Messaging Server 7u4-24.01
 (7.0.4.24.0) 64bit (built Nov 17 2011))
 with ESMTPA id <0M9E008UBPLLV3D0@mmp2.samsung.com> for
 linux-media@vger.kernel.org; Mon, 27 Aug 2012 18:15:07 +0900 (KST)
...
Subject: [PATCH v5 4/4] [media] s5p-mfc: Update MFC v4l2 driver to support
 MFC6.x
Date: Mon, 27 Aug 2012 17:28:03 +0530
Message-id: <1346068683-31610-5-git-send-email-arun.kk@samsung.com>
...
---
 ...
 drivers/media/video/s5p-mfc/Makefile         |    8 +-
 drivers/media/video/s5p-mfc/regs-mfc.h       |   32 +
 drivers/media/video/s5p-mfc/s5p_mfc.c        |   65 +-
 ...
 drivers/media/video/s5p-mfc/s5p_mfc_common.h |   61 +-
 drivers/media/video/s5p-mfc/s5p_mfc_ctrl.c   |  154 ++-
 drivers/media/video/s5p-mfc/s5p_mfc_dec.c    |  193 ++-
 drivers/media/video/s5p-mfc/s5p_mfc_enc.c    |  136 ++-
 drivers/media/video/s5p-mfc/s5p_mfc_opr.c    |   10 +-
 ...
 15 files changed, 2675 insertions(+), 167 deletions(-)
...
index c128fac..c047c86 100644
--- a/drivers/media/video/Kconfig
+++ b/drivers/media/video/Kconfig
@@ -1243,12 +1243,12 @@ config VIDEO_SAMSUNG_S5P_JPEG
...
@@ -278,12 +279,14 @@ static void s5p_mfc_handle_frame(struct s5p_mfc_ctx *ctx,
...
@@ -435,10 +438,28 @@ static void s5p_mfc_handle_seq_done(struct s5p_mfc_ctx *ctx,
...
+		}
 	...
 	s5p_mfc_clear_int_flags(dev);
 	clear_work_bit(ctx);
@@ -469,7 +490,7 @@ static void s5p_mfc_handle_init_buffers(struct s5p_mfc_ctx *ctx,
...
+		if (!ctx->dpb_flush_flag && ctx->head_processed) {
 			...
 			if (!list_empty(&ctx->src_queue)) {
 				src_buf = list_entry(ctx->src_queue.next,
@@ -965,6 +986,7 @@ static int s5p_mfc_probe(struct platform_device *pdev)
 		ret = -ENODEV;
 		goto err_res;
 	}
+
 	dev->mem_dev_r = device_find_child(&dev->plat_dev->dev, "s5p-mfc-r",
 					   match_child);
 	if (!dev->mem_dev_r) {
@@ -1199,12 +1221,47 @@ static struct s5p_mfc_variant mfc_drvdata_v5 = {
...
+	char	*fw_name;
 ...
 /**
@@ -272,6 +283,7 @@ struct s5p_mfc_priv_buf {
...
+ * @ctx_buf:		common context memory (MFCv6)
  ...
  *
  */
@@ -311,6 +323,7 @@ struct s5p_mfc_dev {
 	void *alloc_ctx[2];
...
@@ -345,6 +358,22 @@ struct s5p_mfc_h264_enc_params {
 	int level;
 	u16 cpb_size;
 	int interlace;
+	u8 hier_qp;
...
+	u32 aso_slice_order[8];
 ...
 /**
@@ -387,6 +416,7 @@ struct s5p_mfc_enc_params {
 	u32 rc_bitrate;
...
@@ -452,6 +482,8 @@ struct s5p_mfc_codec_ops {
  *			decoding buffer
...
@@ -476,9 +508,11 @@ struct s5p_mfc_codec_ops {
...
@@ -489,6 +523,10 @@ struct s5p_mfc_codec_ops {
...
@@ -537,6 +575,7 @@ struct s5p_mfc_ctx {
...
+	unsigned int head_processed;
 	...
 	/* Buffers */
 	void *bank1_buf;
@@ -566,10 +605,11 @@ struct s5p_mfc_ctx {
 	int display_delay;
...
@@ -578,16 +618,28 @@ struct s5p_mfc_ctx {
...
+	size_t scratch_buf_size;
 ...
 /*
@@ -628,4 +680,9 @@ struct mfc_control {
...
@@ -757,6 +825,7 @@ static int s5p_mfc_queue_setup(struct vb2_queue *vq,
 			void *allocators[])
 {
...
@@ -792,7 +861,13 @@ static int s5p_mfc_queue_setup(struct vb2_queue *vq,
...
@@ -643,13 +658,22 @@ static int enc_post_seq_start(struct s5p_mfc_ctx *ctx)
...
@@ -968,6 +992,17 @@ static int vidioc_s_fmt(struct file *file, void *priv, struct v4l2_format *f)
...
@@ -1020,12 +1056,15 @@ static int vidioc_reqbufs(struct file *file, void *priv,
 			return ret;
 		}
...
@@ -1272,6 +1311,13 @@ static int s5p_mfc_enc_s_ctrl(struct v4l2_ctrl *ctrl)
...
@@ -1491,6 +1537,7 @@ static int s5p_mfc_queue_setup(struct vb2_queue *vq,
...
@@ -1519,8 +1566,17 @@ static int s5p_mfc_queue_setup(struct vb2_queue *vq,
