...
Received: from service1.sh.cvut.cz ([147.32.127.214]:38648 "EHLO
	service1.sh.cvut.cz" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1753196AbZJ0Nrx (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Tue, 27 Oct 2009 09:47:53 -0400
...
Date: Tue, 27 Oct 2009 14:26:56 +0100
References: <200909101412.07415.lukas.karas@centrum.cz> <20091027100622.6de8899d@pedra.chehab.org>
In-Reply-To: <20091027100622.6de8899d@pedra.chehab.org>
Cc: linux-media@vger.kernel.org
MIME-Version: 1.0
Content-Type: Multipart/Mixed;
  boundary="Boundary-00=_gUv5KaHShGxaSRp"
Message-Id: <200910271426.56878.lukas.karas@centrum.cz>
Sender: linux-media-owner@vger.kernel.org
List-ID: <linux-media.vger.kernel.org>

--Boundary-00=_gUv5KaHShGxaSRp
...
Dne =C3=9At 27. =C5=99=C3=ADjna 2009 13:06:22 jste napsal(a):=20
> Hi Kuk=C3=A1=C5=A1,
>=20
...
>=20
> Luk=C3=A1=C5=A1 Karas <lukas.karas@centrum.cz> escreveu:
...
> >  		ir_codes    =3D &ir_codes_fusionhdtv_mce_table;
> >  		break;
> > +	case 0x0b:
> >  	case 0x7a:
> >  	case 0x47:
...
> >  		ir_codes    =3D &ir_codes_avermedia_cardbus_table;
> >  		break;
> >  	default:
...
> > +		u8 temp =3D 0;
> > +		int rc;
> >  		u8 data[] =3D { 0x3c, 0x33, 0x62};
> >  		struct i2c_msg msg =3D {.addr=3D0x09, .flags=3D0, .buf=3Ddata, .len =
=3D
...
> > +		msg.addr =3D 0x0b;
> > +		msg.len =3D 1;
> > +		if (1 !=3D i2c_transfer(&dev->i2c_adap,&msg,1)) {
...
> > +				   dev->name, msg.addr,(1 =3D=3D rc) ? "yes" : "no");
> > +				   if (rc =3D=3D 1)
...
> > +	struct saa7134_dev *dev =3D ir->c.adapter->algo_data;
> > +#else
> > +	struct saa7134_dev *dev =3D ir->c->adapter->algo_data;
> > +#endif
> > +
...
> > +
> > +	gpio =3D saa_readl(SAA7134_GPIO_GPSTATUS0 >> 2);
> > +
> > +	if (0x40000 &~ gpio)
...
> > +
> > +#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 30)
> > +	while(1 !=3D i2c_master_send(&ir->c, &b,1)) {
> > +#else
> > +	while(1 !=3D i2c_master_send(ir->c, &b,1)) {
> > +#endif
> > +	if ((attempt++) < 10){
> > +			msleep(10); /* wait a bit for next attempt - I don't know how make =
it
> > better */
> > +			continue;
> > +		}
...
> >  		dev->info.addr =3D 0x40;
> >  #endif
> >  		break;
> > +	case SAA7134_BOARD_FLYDVB_TRIO:
> > +#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 30)
> > +		snprintf(ir->c.name, sizeof(ir->c.name), "FlyDVB Trio");
> > +		ir->get_key   =3D get_key_flydvb_trio;
> > +		ir->ir_codes  =3D ir_codes_flydvb_table;
> > +#else
> > +		dev->init_data.name =3D "FlyDVB Trio";
> > +		dev->init_data.get_key =3D get_key_flydvb_trio;
> > +		dev->init_data.ir_codes =3D &ir_codes_flydvb_table;
> > +		dev->info.addr =3D 0x0b;
> > +#endif
> > +		break;
> >  	}
> >
> >  #if LINUX_VERSION_CODE >=3D KERNEL_VERSION(2, 6, 30)
> > --
...
Content-Type: text/x-patch;
  charset="UTF-8";
  name="FlyDVBTrio-IR.3.patch"
Content-Transfer-Encoding: 7bit
Content-Disposition: attachment;
	filename="FlyDVBTrio-IR.3.patch"
