...
Received: from mga02.intel.com ([134.134.136.20]:37218 "EHLO mga02.intel.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1754549AbeALITx (ORCPT <rfc822;linux-media@vger.kernel.org>);
        Fri, 12 Jan 2018 03:19:53 -0500
...
Date: Fri, 12 Jan 2018 08:18:50 +0000
Message-ID: <8E0971CCB6EA9D41AF58191A2D3978B61D4DEF46@PGSMSX111.gar.corp.intel.com>
...
            Sleep(5)
(could be less so it could be adjusted in coreboot then for shorter launch time. )
...
Sensor datasheet claimed before updating register and streaming, need do SW standby.
However Sony implements the function with single bit. (different from OVT)
So I will change the comment to "software standby" to replace "stream off" to clarify the purpose.
...
Will preciously adjust the delay here, and let me explain the delay based on sensor datasheet. (under Intel NDA) 
Also please help review if any comments.

Above delay (13~14ms) is the time from XCLR (reset) to streaming. So I was programming it as 13~14ms. 

(numbering from spec)
T6 = 0.4ms: MCLK start to reset rising till CCI Read version ID register wait time. 
T7 = 12ms: MCLK start and reset rising till Send streaming command wait time (to complete reading all parameters)

Since in coreboot, there are 5ms delay after reset, so assume T6" is 5ms. And from T6~T7, per the profiling data on DUT, the duration is always longer than 25ms (removed the manual delay) while loading all required register lists.
I would still prefer to keep a 1~2ms delay range before streaming on for safety purpose. 
...
If remove g_skip_frames callback, it implies always 0 when user queries. I would like to keep the flexibly if we will need change the skip_frames value. How do you think?
...
You are right. Will remove them first. We are almost done (DIGITAL_GAIN) and will submit separately.
...
> @@ -555,6 +555,17 @@ config VIDEO_APTINA_PLL  config VIDEO_SMIAPP_PLL
>          tristate
>  
> +config VIDEO_IMX258
...
> mode 100644 index 0000000..e4a44d6
> --- /dev/null
...
> +};
> +
> +/* 4208x3118 needs 1300Mbps/lane, 4 lanes */ static const struct 
> +imx258_reg mipi_data_rate_1300mbps[] = {
> +       {0x0301, 0x05},
> +       {0x0303, 0x02},
> +       {0x0305, 0x03},
> +       {0x0306, 0x00},
...
> +
> +/* Open sub-device */
> +static int imx258_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh 
> +*fh) {
