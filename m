...
Received: from bombadil.infradead.org ([198.137.202.133]:32772 "EHLO
        ...
        with ESMTP id S1728772AbeHMXeC (ORCPT
        <rfc822;linux-media@vger.kernel.org>);
        Mon, 13 Aug 2018 19:34:02 -0400
Date: Mon, 13 Aug 2018 17:50:00 -0300
From: Mauro Carvalho Chehab <mchehab+samsung@kernel.org>
To: Hans Verkuil <hverkuil@xs4all.nl>
Cc: Yong Zhi <yong.zhi@intel.com>, sakari.ailus@linux.intel.com,
...
Message-ID: <20180813174950.6fd3915f@coco.lan>
...
> >  create mode 100644 Documentation/media/uapi/v4l/pixfmt-meta-intel-ipu3=
.rst
> >  create mode 100644 include/uapi/linux/intel-ipu3.h
> >=20
> > diff --git a/Documentation/media/uapi/v4l/meta-formats.rst b/Documentat=
ion/media/uapi/v4l/meta-formats.rst
> > index 0c4e1ec..b887fca 100644
> > --- a/Documentation/media/uapi/v4l/meta-formats.rst
> > +++ b/Documentation/media/uapi/v4l/meta-formats.rst
> > @@ -12,6 +12,7 @@ These formats are used for the :ref:`metadata` interf=
ace only.
> >  .. toctree::
> >      :maxdepth: 1
> > =20
...
> > diff --git a/Documentation/media/uapi/v4l/pixfmt-meta-intel-ipu3.rst b/=
...
> > +
> > +For IPU3 ImgU, the 3A statistics accelerators collect different statis=
tics over
...
Just like you did with AWB, AF and AE, please place the full name in parent=
hesis
for RGBS and AWB.

> > +response, AF (Auto-focus) filter response, and AE (Auto-exposure) hist=
ogram.
> > +
...
> > +	IPU3_ALIGN struct ipu3_uapi_awb_raw_buffer awb_raw_buffer; =20
>=20
...
> > +
> > +IPU3 pipeline has a number of image processing stages, each of which t=
akes a
> > +set of parameters as input. The major stages of pipelines are shown he=
re:
> > +
...
=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=
=3D=3D=3D=3D=3D=3D=3D
> > +Optical Black Correction Optical Black Correction block subtracts a pr=
e-defined
...
> > +Color Correction	 Color Correction algo transforms sensor specific col=
or
...
> > +			 frames in time to remove anamolies / noise in pixel =20
>=20
> anamolies -> anomalies
>=20
> > +			 values. :c:type:`ipu3_uapi_isp_tnr3_vmem_params` and
> > +			 :c:type:`ipu3_uapi_isp_tnr3_params` are defined for ISP
> > +			 vector and data memory respectively. =20
>=20
...
=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=
=3D=3D=3D=3D=3D=3D=3D> +
> > +A few stages of the pipe will be executed by firmware running on the I=
SP =20
>=20
> pipe -> pipeline
>=20
> > +processor, while many others will use a set of fixed hardware blocks a=
lso
> > +called accelerator cluster (ACC) to crunch pixel data and produce stat=
istics.
> > +
> > +ACC parameters as defined by :c:type:`ipu3_uapi_acc_param`, can be sel=
ectively
> > +enabled / disabled by the user space through struct :c:type:`ipu3_uapi=
_flags`
...
> > +produced by dedicated user space libraries that comprise the important=
 tuning
> > +tools, thus freeing the developers from being bothered with the low le=
vel
> > +hardware and algorithm details.
> > +
> > +The meta data :c:type:`ipu3_uapi_params` will be sent to "ipu3-imgu pa=
rameters"
...
> > diff --git a/include/uapi/linux/intel-ipu3.h b/include/uapi/linux/intel=
-ipu3.h
> > new file mode 100644
...
> > + * create a grid-based output, and the data is then divided into "slic=
es".
> > + */
...
> > + * @grid: &ipu3_uapi_grid_config, the default grid resolution is 16x16=
 cells.
> > + *
> > + * The threshold is a saturation measure range [0..8191], 8191 is defa=
ult.
...
> > + *
> > + * @config: config for auto white balance as defined by &ipu3_uapi_awb=
_config_s
> > + */
...
> > + * @block_width_log2: Log2 of the width of the grid cell, 2^3 =3D 16.
> > + * @block_height_log2: Log2 of the height of the grid cell, 2^3 =3D 16.
...
> > + * @y_end: Y value of bottom right corner of ROI
> > + *
> > + * The AE block accumulates 4 global weighted histograms(R, G, B, Y) o=
ver
> > + * a defined ROI within the frame. The contribution of each pixel into=
 the
> > + * histogram, defined by &ipu3_uapi_ae_weight_elem LUT, is indexed by =
a grid.
> > + */
...
> > + *
> > + * Use weighted grid value to give a different contribution factor to =
each cell.
> > + */
...
> > + *	0, 0, 0, 128,
> > + *
> > + * As part of the raw frame pre-process stage, the WB and color conver=
sion need
...
> > + * @weights:	&IPU3_UAPI_AE_WEIGHTS is based on 32x24 blocks in the gri=
d.
...
> > +
> > +/**
> > + * struct ipu3_uapi_af_filter_config - AF 2D filter for contrast measu=
rements
> > + *
...
> > + * Rule:
> > + *		y_gen_rate_gr + y_gen_rate_r + y_gen_rate_b + y_gen_rate_gb =3D 32
...
> > + *		coeffs, default 7 (2^7 =3D 128).
> > + * @nf.__reserved1:	reserved
> > + * @nf.y2_nf:	Normalization factor for y2, should be log2 of the sum o=
f the
...
> > + *
> > + * @y_table:	Each color component will be convolved separately with fi=
lter1
> > + *		and filter2 and the result will be summed out and averaged for
> > + *		each cell.
> > + */
> > +struct ipu3_uapi_af_meta_data {
> > +	__u8 y_table[IPU3_UAPI_AF_Y_TABLE_MAX_SIZE] IPU3_ALIGN; =20
>=20
...
> > +	IPU3_ALIGN struct ipu3_uapi_af_meta_data meta_data; =20
>=20
...
> > + * @padding: paddings
> > + * @grid_cfg: See &ipu3_uapi_grid_config, default resolution 16x16. Us=
e large
...
> > +	 IPU3_UAPI_AWB_FR_SPARE_FOR_BUBBLES) * IPU3_UAPI_MAX_STRIPES) =20
>=20
> Add newline
>=20
> > +/**
...
> > + *
> > + * @bayer_table: Statistics output on the grid after convolving with 1=
D filter.
> > + */
...
> > + * @bayer_nf:	normalization factor for the convolution coeffs, to make=
 sure
...
> > + *
> > + * @num_of_stripes: A single frame is divided into several parts calle=
d stripes
...
> > + *
> > + * @awb_raw_buffer: auto white balance meta data &ipu3_uapi_awb_raw_bu=
ffer
> > + * @ae_raw_buffer: auto exposure raw data &ipu3_uapi_ae_raw_buffer_ali=
gned
> > + * @af_raw_buffer: &ipu3_uapi_af_raw_buffer for auto focus meta data
> > + * @awb_fr_raw_buffer: value as specified by &ipu3_uapi_awb_fr_raw_buf=
fer
> > + * @stats_4a_config: 4a statistics config as defined by &ipu3_uapi_4a_=
config.
> > + * @ae_join_buffers: 1 to use ae_raw_buffer.
> > + * @padding: padding config
> > + * @stats_3a_bubble_per_stripe: a &ipu3_uapi_stats_3a_bubble_info_per_=
stripe
...
> > +
> > +/* largest grid is 73x56, for grid_height_per_slice of 2, 73x2 =3D 146=
 */
...
> > + *
> > + * Precision u3.13, range [0,8). White balance correction is done by a=
pplying
...
> > + *
> > + * Defines the threshold that specifies how different a defect pixel c=
an be from
...
> > + *
> > + * @cf:	Free coefficient for threshold calculation, range [0..8191], d=
efault 0.
> > + * @__reserved0:	reserved
...
hmm... 7.75??? ci is an _u32 with 5 bits... it can't be a floating point va=
lue!
...
> > + *	default 6.
> > + * @__reserved1:	reserved
> > + * @r_nf:	Normalization shift value for r^2 calculation, range [12 .. =
20]
...
> > +
> > +/**
> > + * struct ipu3_uapi_bnr_static_config_thr_ctrl_shd_config - Shading co=
nfig
> > + *
> > + * @gr:	Coefficient defines lens shading gain approximation for gr cha=
nnel
> > + * @r:	Coefficient defines lens shading gain approximation for r chann=
el
> > + * @b:	Coefficient defines lens shading gain approximation for b chann=
el
> > + * @gb:	Coefficient defines lens shading gain approximation for gb cha=
nnel
> > + *
> > + * Parameters for noise model (NM) adaptation of BNR due to shading co=
rrection.
...
> > +
> > +/**
> > + * struct ipu3_uapi_bnr_static_config_opt_center_config - Optical cent=
er config
> > + *
...
> > + * @__reserved2:	reserved
> > + *
> > + * Distance from corner to optical center for NM adaptation due to sha=
ding
...
> > +
> > +/**
> > + * struct ipu3_uapi_bnr_static_config_lut_config - BNR square root loo=
kup table
> > + *
...
> > +
> > +/**
> > + * struct ipu3_uapi_bnr_static_config_bp_ctrl_config - Detect bad pixe=
ls (bp)
> > + *
...
> > + *			0 =E2=80=93 single defect pixel is expected,
> > + *			1 - 2 adjacent defect pixels are expected, default 1.
> > + * @bp_gain:	Defines how 2nd derivation that passes through a defect p=
ixel
...
> > + * @__reserved2:	reserved
> > + * @w1_coeff:	Enable influence of incorrect defect pixel correction to=
 be
...
> > +
> > +/**
> > + * struct ipu3_uapi_bnr_static_config_dn_detect_ctrl_config - Denoisin=
g config
> > + *
...
> > + *
> > + * beta and gamma parameter define the strength of the noise removal f=
ilter.
> > + * All above has precision u0.4, range [0..0xF].

again, what do you mean by u0.4? For hexadecimal values (here and everywher=
e)
please prefer lowecase.

> > + *
> > + * @__reserved0:	reserved
> > + * @max_inf:	Maximum increase of peripheral or diagonal element influe=
nce
...
enable.
> > + * @bnr_enable:	Bayer noise removal enable control, 0 - disable, 1 - e=
nable.
...
> > +
> > +/**
> > + * struct ipu3_uapi_bnr_static_config_opt_center_sqr_config - BNR opti=
cal square
> > + *
...
> > + *    #. Both structs are used in threshold formula to calculate r^2, =
where r
...
> > + *
> > + * @wb_gains:	white balance gains &ipu3_uapi_bnr_static_config_wb_gain=
s_config
...
> > + * @opt_center:	optical center &ipu3_uapi_bnr_static_config_opt_center=
_config
> > + *
...
> > +	struct ipu3_uapi_bnr_static_config_dn_detect_ctrl_config dn_detect_ct=
rl;
> > +	__u32 column_size;
> > +	struct ipu3_uapi_bnr_static_config_opt_center_sqr_config opt_center_s=
qr;
> > +} __packed;
> > +
> > +/**
...
disparity
> > + *
> > + * @gd_red:	Shading gain coeff for gr disparity level in bright red re=
gion.
...
> > + *		Precision u0.6, default 4(0.0625).
> > + * @__reserved2:	reserved
> > + * @gd_black:	Maximal green disparity level in dark region (stronger d=
isparity
> > + *		assumed to be image detail). Precision u14, default 80.
> > + * @__reserved3:	reserved
> > + * @gd_shading:	Change maximal green disparity level according to squa=
re
> > + *		distance from image center.
> > + * @__reserved4:	reserved
> > + * @gd_support:	Lower bound for the number of second green color pixel=
s in
> > + *		current pixel neighborhood with less than threshold difference
> > + *		from it.
> > + *
> > + * The shading gain coeff of red, green, blue and black are used to ca=
lculate
> > + * threshold given a pixel's color value and its coordinates in the im=
age.
> > + *
...
> > + * @__reserved1:	reserved
> > + * @lc_ctrl:	Parameter that controls weights of Chroma Homogeneity met=
ric
...
> > + * @__reserved5:	reserved
> > + *
> > + * The demosaic fixed function block is responsible to covert Bayer(mo=
saiced)
...
> > + *
> > + * Transform sensor specific color space to standard sRGB by applying =
3x3 matrix
> > + * and adding a bias vector O. The transformation is basically a rotat=
ion and
> > + * translation in the 3-dimensional color spaces. Here are the default=
s:
> > + *
> > + *	9775,	-2671,	1087,	0
> > + *	-1071,	8303,	815,	0
...
> > +
> > +/**
> > + * struct ipu3_uapi_gamma_corr_lut - Per-pixel tone mapping implemente=
d as LUT.
> > + *
...
> > + * that is implemented as a lookup table(LUT). The pixel component inp=
ut
...
> > +
> > +/**
> > + * struct ipu3_uapi_csc_mat_config - Color space conversion matrix con=
fig
> > + *
> > + * @coeff_c11:	Conversion matrix value, format s0.14, range (-1,1), de=
fault 1.
> > + * @coeff_c12:	Conversion matrix value, format s0.14, range (-1,1), de=
fault 0.
> > + * @coeff_c13:	Conversion matrix value, format s0.14, range (-1,1), de=
fault 0.
> > + * @coeff_b1:	Bias 3x1 coefficients, s13,0 range [-8191, 8181], defaul=
t 0.
> > + * @coeff_c21:	Conversion matrix value, format s0.14, range (-1,1), de=
fault 0.
> > + * @coeff_c22:	Conversion matrix value, format s0.14, range (-1,1), de=
fault 1.
> > + * @coeff_c23:	Conversion matrix value, format s0.14, range (-1,1), de=
fault 0.
> > + * @coeff_b2:	Bias 3x1 coefficients, s13,0 range [-8191, 8181], defaul=
t 0.
> > + * @coeff_c31:	Conversion matrix value, format s0.14, range (-1,1), de=
fault 0.
> > + * @coeff_c32:	Conversion matrix value, format s0.14, range (-1,1), de=
fault 0.
> > + * @coeff_c33:	Conversion matrix value, format s0.14, range (-1,1), de=
fault 1.
> > + * @coeff_b3:	Bias 3x1 coefficients, s13,0 range [-8191, 8181], defaul=
t 0.
...
> > + * UV -chroma) by applying the pixel's values by a 3x3 matrix and addi=
ng an
...
> > +
> > +/**
> > + * struct ipu3_uapi_shd_grid_config - Bayer shading(darkening) correct=
ion
> > + *
...
> > + * @__reserved0:	reserved
> > + * @block_height_log2:	Log2 of the height of the grid cell in pixel co=
unt
> > + *			u4, [0, 15], default value 6.
> > + * @__reserved1:	reserved
> > + * @grid_height_per_slice:	SHD_MAX_CELLS_PER_SET/width.
> > + *				(with SHD_MAX_CELLS_PER_SET =3D 146).
...
> > + * @gain_factor: Gain factor. Shift calculated anti shading value. Pre=
cision u2.
...
> > + * @__reserved: reserved
> > + *
> > + * Correction is performed by multiplying a gain factor for each of th=
e 4 Bayer
...
> > + * @black_level:	black level config for shading correction as defined =
by
...
> > + * @b01:	Always 0.
> > + *
> > + * Calculate weight for blending directed and non-directed denoise ele=
ments
> > + *
> > + * Note:
> > + * Each instance of Config Unit needs X coordinate of n points and
> > + * slope A factor between points calculated by driver based on calibra=
tion
> > + * parameters.
> > + */
...
> > +
> > +/**
> > + * struct ipu3_uapi_iefd_cux6_ed - Calculate power of non-directed sha=
rpening
...
> > +
> > +/**
> > + * struct ipu3_uapi_iefd_cux2_1 - Calculate power of non-directed deno=
ise
> > + *				  element apply.
> > + * @x0: X0 point of Config Unit, u9.0, default 0.
> > + * @x1: X1 point of Config Unit, u9.0, default 0.
...
> > +
> > +/**
> > + * struct ipu3_uapi_iefd_cux4 - Calculate power of non-directed sharpe=
ning
> > + *				element.
> > + *
> > + * @x0:	X0 point of Config Unit, u9.0, default 0.
...
> > + *
> > + * @horver_diag_coeff: Gradiant compensation, coefficient that compens=
ates for
> > + *		       different distance for vertical / horizontal and diagonal
> > + *		       * gradient calculation (~1/sqrt(2)).
> > + * @__reserved0: reserved
> > + * @clamp_stitch: Slope to stitch between clamped and unclamped edge v=
alues
> > + * @__reserved1: reserved
...
> > + * @__reserved1: reserved
> > + * @nega_lmt_dir: Sharpening limit for negative overshoots for directi=
on (edge).
> > + * @__reserved2: reserved
> > + * @posi_lmt_dir: Sharpening limit for positive overshoots for directi=
on (edge).
> > + * @__reserved3: reserved
> > + *
...
> > + *
> > + * @dir_shrp:	Weight of wide direct sharpening, u1.6, range 0-64, defa=
ult 64.
> > + * @__reserved0:	reserved
> > + * @dir_dns:	Weight of wide direct denoising, u1.6, range 0-64, defaul=
t 0.
> > + * @__reserved1:	reserved
...
> > + *		    0 =E2=80=93 disabled, 64 - use only unsharp.
> > + * @__reserved0: reserved
...
> > + *
> > + * @unsharp_coef0: unsharp coefficient 0 config. See &ipu3_uapi_unshar=
p_coef0
> > + * @unsharp_coef1: unsharp coefficient 1 config. See &ipu3_uapi_unshar=
p_coef1
> > + */
...
> > + *
> > + * @x:	Radial reset of x coordinate. Precision s12, [-4095 .. 4095], d=
efault 0.
> > + * @__reserved0:	reserved
> > + * @y:	Radial center y coordinate. Precision s12, [-4095 .. 4095], def=
ault 0.
> > + * @__reserved1:	reserved
> > + */
...
> > + * @rad_nf: Radial. R^2 normalization factor is scale down by 2^ - (15=
 + scale)
> > + * @__reserved0: reserved
> > + * @rad_inv_r2: Radial R^-2 normelized to (0.5..1), Prec' u7, range [0=
 .. 127].
> > + * @__reserved1: reserved
> > + */
...
64,
> > + *			 default 0.
> > + * @rad_ndir_far_dns_power: power of non-direct sharpening, u1.6, rang=
e 0-64,
> > + *			 default 0.
> > + * @__reserved: reserved
> > + */
...
> > + * @__reserved0: reserved
> > + * @rad_cu_unsharp_x1: X1 point for Config Unit unsharp for radial/cor=
ner point
> > + *			precision u9.0.
> > + * @__reserved1: reserved
> > + */
...
> > +
> > +/**
> > + * struct ipu3_uapi_yuvp1_iefd_rad_cfg - IEFd parameters changed radia=
lly over
> > + *					 the picture plain.
> > + *
> > + * @reset_xy: reset xy value in radial calculation. &ipu3_uapi_radial_=
reset_xy
...
> > + * @control: control setting, as defined by &ipu3_uapi_yuvp1_iefd_cont=
rol
> > + * @sharp: sharpness setting, as defined by &ipu3_uapi_yuvp1_iefd_shrp=
_cfg
> > + * @unsharp: unsharpness setting, as defined by &ipu3_uapi_yuvp1_iefd_=
unshrp_cfg
> > + * @rad: radial setting, as defined by &ipu3_uapi_yuvp1_iefd_rad_cfg
> > + * @vsslnm: vsslnm setting, as defined by &ipu3_uapi_yuvp1_iefd_vssnlm=
_cfg
> > + */
...
> > +
> > +/**
> > + * struct ipu3_uapi_yuvp1_chnr_sense_gain_config - Chroma noise reduct=
ion gains
> > + *
> > + * All sensitivity gain parameters have precision u13.0, range [0, 819=
1].
> > + *
...
> > +
> > +/**
> > + * struct ipu3_uapi_yuvp1_chnr_iir_fir_config - Chroma IIR/FIR filter =
config
> > + *
> > + * @fir_0h: Value of center tap in horizontal FIR, range [0..32], defa=
ult 8.
> > + * @__reserved0: reserved
> > + * @fir_1h: Value of distance 1 in horizontal FIR, range [0..32], defa=
ult 12.
> > + * @__reserved1: reserved
> > + * @fir_2h: Value of distance 2 tap in horizontal FIR, range [0..32], =
default 0.
> > + * @dalpha_clip_val: weight for previous row in IIR, range [1..256], d=
efault 0.
> > + * @__reserved2: reserved
> > + */
...
> > +
> > +/**
> > + * struct ipu3_uapi_yuvp1_y_ee_nr_lpf_config - Luma(Y) edge enhancemen=
t low-pass
> > + *					       filter coefficients
> > + *
...
> > + * @__reserved2: reserved
> > + * @enable: 0: Y_EE_NR disabled, output =3D input; 1: Y_EE_NR enabled.
> > + */
...
> > +
> > +/**
> > + * struct ipu3_uapi_yuvp1_y_ee_nr_sense_config - Luma(Y) edge enhancem=
ent
> > + *					noise reduction sensitivity gains
> > + *
> > + * @edge_sense_0: Sensitivity of edge in dark area. u13.0, default 819=
1.
> > + * @__reserved0: reserved
> > + * @delta_edge_sense: Difference in the sensitivity of edges between
> > + *		      the bright and dark areas. u13.0, default 0.
> > + * @__reserved1: reserved
> > + * @corner_sense_0: Sensitivity of corner in dark area. u13.0, default=
 0.
> > + * @__reserved2: reserved
> > + * @delta_corner_sense: Difference in the sensitivity of corners betwe=
en
...
> > +
> > +/**
> > + * struct ipu3_uapi_yuvp1_y_ee_nr_gain_config - Luma(Y) edge enhanceme=
nt
> > + *						noise reduction gain config
> > + *
> > + * @gain_pos_0: Gain for positive edge in dark area. u5.0, [0,16], def=
ault 2.
> > + * @__reserved0: reserved
> > + * @delta_gain_posi: Difference in the gain of edges between the brigh=
t and
> > + *		     dark areas for positive edges. u5.0, [0,16], default 0.
> > + * @__reserved1: reserved
> > + * @gain_neg_0: Gain for negative edge in dark area. u5.0, [0,16], def=
ault 8.
> > + * @__reserved2: reserved
...
> > +
> > +/**
> > + * struct ipu3_uapi_yuvp1_y_ee_nr_clip_config - Luma(Y) edge enhanceme=
nt
> > + *					noise reduction clipping config
> > + *
> > + * @clip_pos_0: Limit of positive edge in dark area
> > + * @__reserved0: reserved
> > + * @delta_clip_posi: Difference in the limit of edges between the brig=
ht
...
> > +
> > +/**
> > + * struct ipu3_uapi_yuvp1_y_ee_nr_frng_config - Luma(Y) edge enhanceme=
nt
...
> > + * @__reserved2: reserved
> > + * @t1: Parameter for enabling/disabling the edge enhancement, u1.0, [=
0,1],
> > + *	default 1.
...
> > +
> > +/**
> > + * struct ipu3_uapi_yuvp1_y_ee_nr_diag_config - Luma(Y) edge enhanceme=
nt
...
> > + *		 u4.0, [1,15], default 1.
> > + * @__reserved0: reserved
> > + * @hvw_hor: Weight of horizontal/vertical edge enhancement for hv edg=
e.
> > + *		u2.2, [1,15], default 4.
> > + * @dw_hor: Weight of diagonal edge enhancement for hv edge.
> > + *		u2.2, [1,15], default 1.
> > + * @hvw_diag: Weight of horizontal/vertical edge enhancement for diago=
nal edge.
> > + *		u2.2, [1,15], default 1.
...
> > +
> > +/**
> > + * struct ipu3_uapi_yuvp1_y_ee_nr_fc_coring_config - Luma(Y) edge enha=
ncement
> > + *		noise reduction false color correction (FCC) coring config
> > + *
> > + * @pos_0: Gain for positive edge in dark, u13.0, [0..16], default 0.
> > + * @__reserved0: reserved
> > + * @pos_delta: Gain for positive edge in bright, value: pos_0 + pos_de=
lta <=3D16
> > + *		u13.0, default 0.
> > + * @__reserved1: reserved
...
default 0.
> > + * @__reserved2: reserved
> > + * @neg_delta: Gain for negative edge in bright area. neg_0 + neg_delt=
a <=3D16
> > + *		u13.0, default 0.
> > + * @__reserved3: reserved
> > + *
...
reduction
> > + *
> > + * @lpf: low-pass filter config. See &ipu3_uapi_yuvp1_y_ee_nr_lpf_conf=
ig
> > + * @sense: sensitivity config. See &ipu3_uapi_yuvp1_y_ee_nr_sense_conf=
ig
> > + * @gain: gain config as defined in &ipu3_uapi_yuvp1_y_ee_nr_gain_conf=
ig
> > + * @clip: clip config as defined in &ipu3_uapi_yuvp1_y_ee_nr_clip_conf=
ig
> > + * @frng: fringe config as defined in &ipu3_uapi_yuvp1_y_ee_nr_frng_co=
nfig
> > + * @diag: diagonal edge config. See &ipu3_uapi_yuvp1_y_ee_nr_diag_conf=
ig
...
> > + *
> > + * @en:	0 =E2=80=93 TCC disabled. Output =3D input 1 =E2=80=93 TCC ena=
bled.
...
> > +
> > +/**
> > + * struct ipu3_uapi_yuvp2_tcc_macc_elem_static_config - Total color co=
rrection
...
> > +
> > +/**
> > + * struct ipu3_uapi_yuvp2_tcc_macc_table_static_config - Total color c=
orrection
...
> > +
> > +/**
> > + * struct ipu3_uapi_yuvp2_tcc_inv_y_lut_static_config - Total color co=
rrection
> > + *				inverse y lookup table
> > + *
> > + * @entries: lookup table for inverse y estimation, and use it to esti=
mate the
> > + *	     ratio between luma and chroma. Chroma by approximate the absol=
ute
> > + *	     value of the radius on the chroma plane (R =3D sqrt(u^2+v^2) )=
 and
...
> > + *
> > + * @entries: lookup table for gain piece wise linear transformation (P=
CWL)
> > + */
...
> > +
> > +/**
> > + * struct ipu3_uapi_yuvp2_tcc_r_sqr_lut_static_config - Total color co=
rrection
...
> > +
> > +/**
> > + * struct ipu3_uapi_yuvp2_tcc_static_config- Total color correction st=
atic
> > + *
...
> > +
> > +/*
> > + * struct ipu3_uapi_anr_plain_color - Advanced noise reduction plain c=
olor with
> > + *				      4x4 matrix
> > + *
> > + * Tunable parameters that are subject to modification according to the
...
> > +
> > +/**
> > + * struct ipu3_uapi_anr_transform_config - Advanced noise reduction tr=
ansform
> > + *
> > + * @enable: advanced noise reduction enabled.
> > + * @adaptive_treshhold_en: On IPU3, adaptive threshold is always enabl=
ed.
> > + * @__reserved1: reserved
> > + * @__reserved2: reserved
> > + * @alpha: using follow defaults: =20
>=20
> follow -> following
>=20
> > + *		13, 13, 13, 13, 0, 0, 0, 0
...
> > + * @sqrt_lut: 11 bits per element, values =3D
> > + *					[724 768 810 849 887
> > + *					923 958 991 1024 1056
> > + *					1116 1145 1173 1201 1086
> > + *					1228 1254 1280 1305 1330
> > + *					1355 1379 1402 1425 1448]
> > + * @xreset: Reset value of X for r^2 calculation Value: col_start-X_ce=
nter
> > + *	Constraint: Xreset + FrameWdith=E2=89=A44095 Xreset=E2=89=A5 -4095,=
 default -1632.
> > + * @__reserved3: reserved
> > + * @yreset: Reset value of Y for r^2 calculation Value: row_start-Y_ce=
nter
> > + *	 Constraint: Yreset + FrameHeight=E2=89=A44095 Yreset=E2=89=A5 -409=
5, default -1224.
> > + * @__reserved4: reserved
> > + * @x_sqr_reset: Reset value of X^2 for r^2 calculation Value =3D (Xre=
set)^2
> > + * @r_normfactor: Normalization factor for R. Default 14.
> > + * @__reserved5: reserved
> > + * @y_sqr_reset: Reset value of Y^2 for r^2 calculation Value =3D (Yre=
set)^2
> > + * @gain_scale: Parameter describing shading gain as a function of dis=
tance
...
> > +	struct ipu3_uapi_anr_stitch_pyramid pyramid[IPU3_UAPI_ANR_PYRAMID_SIZ=
E];
> > +} __packed;
> > +
> > +/**
...
> > + *
> > + * @__reserved1: reserved
> > + * @transform:	advanced noise reduction transform config as specified =
by
...
> > + *
> > + * ACC refers to the HW cluster containing all Fixed Functions(FFs). E=
ach FF
...
> > + * @chnr_c0:	chroma noise reduction config. &ipu3_uapi_yuvp1_chnr_conf=
ig
> > + * @y_ee_nr:	y edge enhancement and noise reduction config.
> > + *		&ipu3_uapi_yuvp1_y_ee_nr_config
> > + * @yds:	y down scaler config. See &ipu3_uapi_yuvp1_yds_config
> > + * @chnr:	chroma noise reduction config. See &ipu3_uapi_yuvp1_chnr_con=
fig
> > + * @__reserved1: reserved
...
> > + * @awb:	auto white balance config. As specified by &ipu3_uapi_awb_con=
fig
> > + */
...
> > + *
> > + * @lin_lutlow_gr: linearization look-up table for GR channel interpol=
ation.
> > + * @lin_lutlow_r: linearization look-up table for R channel interpolat=
ion.
> > + * @lin_lutlow_b: linearization look-up table for B channel interpolat=
ion.
> > + * @lin_lutlow_gb: linearization look-up table for GB channel interpol=
ation.
> > + *			lin_lutlow_gr / lin_lutlow_gr / lin_lutlow_gr /
> > + *			lin_lutlow_gr <=3D LIN_MAX_VALUE - 1.
...
> > +
> > +/**
> > + * struct ipu3_uapi_isp_tnr3_vmem_params - Temporal noise reduction ve=
ctor
> > + *					   memory parameters
> > + *
> > + * @slope: slope setting in interpolation curve for temporal noise red=
uction.
> > + * @__reserved1: reserved
...
> > +
> > +/**
> > + * struct ipu3_uapi_isp_tnr3_params - Temporal noise reduction v3 para=
meters
> > + *
...
> > + * @udiff: Threshold difference of U channel between bright and dark a=
rea.
> > + * @vdiff: Threshold difference of V channel between bright and dark a=
rea.
> > + */
...
> > +
> > +/**
> > + * struct ipu3_uapi_isp_xnr3_params - Extreme noise reduction v3 param=
eters
> > + *
> > + * @alpha: parameters for xnr3 alpha. See &ipu3_uapi_xnr3_alpha_params
> > + * @coring: parameters for xnr3 coring. See &ipu3_uapi_xnr3_coring_par=
ams
> > + * @blending: parameters for xnr3 blending. See &ipu3_uapi_xnr3_blendi=
ng_params
> > + */
...
> > +
> > +/**
> > + * struct ipu3_uapi_obgrid_param - Optical black level compensation pa=
rameters
> > + *
...
> > + *
> > + * Black level is different for red, green, and blue channels. So blac=
k level
...
> > +
> > +/**
> > + * struct ipu3_uapi_flags - bits to indicate which pipeline needs upda=
te
> > + *
> > + * @gdc: 0 =3D no update, 1 =3D update.
> > + * @obgrid: 0 =3D no update, 1 =3D update.
> > + * @__reserved1: Not used.
> > + * @acc_bnr: 0 =3D no update, 1 =3D update.
> > + * @acc_green_disparity: 0 =3D no update, 1 =3D update.
> > + * @acc_dm: 0 =3D no update, 1 =3D update.
> > + * @acc_ccm: 0 =3D no update, 1 =3D update.
> > + * @acc_gamma: 0 =3D no update, 1 =3D update.
> > + * @acc_csc: 0 =3D no update, 1 =3D update.
> > + * @acc_cds: 0 =3D no update, 1 =3D update.
> > + * @acc_shd: 0 =3D no update, 1 =3D update.
> > + * @__reserved2: Not used.
> > + * @acc_iefd: 0 =3D no update, 1 =3D update.
> > + * @acc_yds_c0: 0 =3D no update, 1 =3D update.
> > + * @acc_chnr_c0: 0 =3D no update, 1 =3D update.
> > + * @acc_y_ee_nr: 0 =3D no update, 1 =3D update.
> > + * @acc_yds: 0 =3D no update, 1 =3D update.
> > + * @acc_chnr: 0 =3D no update, 1 =3D update.
> > + * @acc_ytm: 0 =3D no update, 1 =3D update.
> > + * @acc_yds2: 0 =3D no update, 1 =3D update.
> > + * @acc_tcc: 0 =3D no update, 1 =3D update.
> > + * @acc_dpc: 0 =3D no update, 1 =3D update.
> > + * @acc_bds: 0 =3D no update, 1 =3D update.
> > + * @acc_anr: 0 =3D no update, 1 =3D update.
> > + * @acc_awb_fr: 0 =3D no update, 1 =3D update.
> > + * @acc_ae: 0 =3D no update, 1 =3D update.
> > + * @acc_af: 0 =3D no update, 1 =3D update.
> > + * @acc_awb: 0 =3D no update, 1 =3D update.
> > + * @__acc_osys: 0 =3D no update, 1 =3D update.
> > + * @__reserved3: Not used.
> > + * @lin_vmem_params: 0 =3D no update, 1 =3D update.
> > + * @tnr3_vmem_params: 0 =3D no update, 1 =3D update.
> > + * @xnr3_vmem_params: 0 =3D no update, 1 =3D update.
> > + * @tnr3_dmem_params: 0 =3D no update, 1 =3D update.
> > + * @xnr3_dmem_params: 0 =3D no update, 1 =3D update.
> > + * @__reserved4: Not used.
> > + * @obgrid_param: 0 =3D no update, 1 =3D update.
...
> > + * @tnr3_dmem_params:	tnr3 DMEM as specified by &ipu3_uapi_isp_tnr3_pa=
rams
> > + * @xnr3_dmem_params:	xnr3 DMEM as specified by &ipu3_uapi_isp_xnr3_pa=
rams
...
> > + * This is a "single plane" v4l2_meta_format using V4L2_BUF_TYPE_META_=
OUTPUT.
> > + *
> > + * struct ipu3_uapi_params as defined below contains a lot of paramete=
rs and
...
> seems a bit of a mix to me, it's not always clear why a certain approach =
is taken.
>=20
> Did you check if the layout of all these structs is the same between 32 a=
nd 64 bit code?
...
>=20
> I gather that with struct ipu3_uapi_flags you can indicate which blocks n=
eed to be updated,
> and that for those blocks that do not need updating the corresponding str=
ucts are ignored.
> If so, then that should be documented in pixfmt-meta-intel-ipu3.rst.
>=20
> And what about the initial values? E.g. after a reboot, do all the blocks=
 have sane default
> values? Or do you need to supply those yourself? And if it is the latter,=
 then we need
> some open source example code that programs it with sane values. Users ca=
n take that as a
> starting point and modify it.
>=20
> If userspace needs to provide sane initial values, then it would be best =
if code
> is added to v4l2-utils under contrib. Perhaps under a dual GPL/BSD licens=
e?
>=20
> Regards,
>=20
