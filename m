...
Received: from mailout1.w1.samsung.com ([210.118.77.11]:23213 "EHLO
	...
	with ESMTP id S1752867Ab1DEOJC (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Tue, 5 Apr 2011 10:09:02 -0400
Date: Tue, 05 Apr 2011 16:06:45 +0200
...
Message-id: <1302012410-17984-3-git-send-email-m.szyprowski@samsung.com>
MIME-version: 1.0
...
---
 ...
 8 files changed, 1314 insertions(+), 740 deletions(-)
...
+
+#define invalidate_slpt_ent(slpt_va) (deref_va(slpt_va) &= PAGE_FAULT)
+
...
+	}
+
+	s5p_domain->slpt_va = kzalloc(FLPT_ENTRIES * sizeof(void *), GFP_KERNEL);
...
+			s5p_domain->refcount[flpt_idx]++;
+			sysmmu_debug(3, "mapping %d@0x%lx\n", len, iova);
+		}
...
+}
+
+static void s5p_tlb_invalidate(void __iomem *regs)
+{
+	unsigned int reg;
+
+	/* TLB invalidate */
...
+				     const char *file, int line)
+{
+	sysmmu_debug(3, "%s:%d Attempting to unmap different size or non-existing\
+mapping %d@0x%lx\n", file, line, len, iova);
+	BUG();
+}
+
...
+		deref_va(flpt_va) = 0;
+		flush_cache(flpt_va, 4); /* one 4-byte entry */
+		s5p_tlb_invalidate(s5p_domain->sysmmu->regs);
+
+		return 0;
+	} else if (SZ_16M == len) {
+		int i;
