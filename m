...
Received: from mx3.redhat.com (mx3.redhat.com [172.16.48.32])
	by int-mx1.corp.redhat.com (8.13.1/8.13.1) with ESMTP id m9K19GIx013757
	for <video4linux-list@redhat.com>; Sun, 19 Oct 2008 21:09:16 -0400
Received: from mail03.idc.renesas.com (mail.renesas.com [202.234.163.13])
	by mx3.redhat.com (8.13.8/8.13.8) with ESMTP id m9K194gn014175
	for <video4linux-list@redhat.com>; Sun, 19 Oct 2008 21:09:05 -0400
Date: Mon, 20 Oct 2008 09:45:09 +0900
From: Kuninori Morimoto <morimoto.kuninori@renesas.com>
To: mchehab@infradead.org, khali@linux-fr.org
Message-id: <uskqsm67j.wl%morimoto.kuninori@renesas.com>
MIME-version: 1.0 (generated by SEMI 1.14.6 - "Maruoka")
Content-type: text/plain; charset=US-ASCII
Cc: V4L <video4linux-list@redhat.com>, i2c@lm-sensors.org
Subject: [PATCH v4] Add ov772x driver
...
>From 9f41139f16513c3e84d46b77acf2cac2e8c21ed9 Mon Sep 17 00: 00:00 2001
...
2) fix remove bits define and use include/linux/bitops.h
3) add i2c ML list

 drivers/media/video/Kconfig     |    6 +
 drivers/media/video/Makefile    |    1 +
 drivers/media/video/ov772x.c    |  966 +++++++++++++++++++++++++++++++++++++++
 include/media/ov772x.h          |   11 +
...
+
+/* COM2 */
+#define SOFT_SLEEP_MODE BIT(4)		/* Soft sleep mode */
+					/* Output drive capability */
+#define OCAP_1x         (ZERO)		/* 1x */
+#define OCAP_2x         BIT(0)		/* 2x */
+#define OCAP_3x         BIT(1)		/* 3x */
+#define OCAP_4x         (BIT(0)|BIT(1))	/* 4x */
+
+/* COM3 */
+#define SWAP_MASK       (BIT_MASK(3) << 3)
+
+#define VFIMG_ON_OFF    BIT(7)	/* Vertical flip image ON/OFF selection */
+#define HMIMG_ON_OFF    BIT(6)	/* Horizontal mirror image ON/OFF selection */
+#define SWAP_RGB        BIT(5)	/* Swap B/R  output sequence in RGB mode */
+#define SWAP_YUV        BIT(4)	/* Swap Y/UV output sequence in YUV mode */
+#define SWAP_ML         BIT(3)	/* Swap output MSB/LSB */
+				/* Tri-state option for output clock */
+#define NOTRI_CLOCK     BIT(2)	/*   0: Tri-state    at this period */
+				/*   1: No tri-state at this period */
+				/* Tri-state option for output data */
+#define NOTRI_DATA      BIT(1)	/*   0: Tri-state    at this period */
+				/*   1: No tri-state at this period */
+#define SCOLOR_TEST     BIT(0)	/* Sensor color bar test pattern */
+
+/* COM4 */
+					/* PLL frequency control */
+#define PLL_BYPASS      (ZERO)		/*  00: Bypass PLL */
+#define PLL_4x          BIT(6)		/*  01: PLL 4x */
+#define PLL_6x          BIT(7)		/*  10: PLL 6x */
+#define PLL_8x          (BIT(7)|BIT(6))	/*  11: PLL 8x */
+					/* AEC evaluate window */
+#define AEC_FULL        (ZERO)		/*  00: Full window */
+#define AEC_1p2         BIT(4)		/*  01: 1/2  window */
+#define AEC_1p4         BIT(5)		/*  10: 1/4  window */
+#define AEC_2p3         (BIT(5)|BIT(4))	/*  11: Low 2/3 window */
+
+/* COM5 */
+#define AFR_ON_OFF      BIT(7)		/* Auto frame rate control ON/OFF selection */
+#define AFR_SPPED       BIT(6)		/* Auto frame rate control speed slection */
+					/* Auto frame rate max rate control */
+#define AFR_NO_RATE     (ZERO)		/*   No  reduction of frame rate */
+#define AFR_1p2         BIT(4)		/*   Max reduction to 1/2 frame rate */
+#define AFR_1p4         BIT(5)		/*   Max reduction to 1/4 frame rate */
+#define AFR_1p8         (BIT(5)|BIT(4))	/*   Max reduction to 1/8 frame rate */
+					/* Auto frame rate active point control */
+#define AF_2x           (ZERO)		/*   Add frame when AGC reaches  2x gain */
+#define AF_4x           BIT(2)		/*   Add frame when AGC reaches  4x gain */
+#define AF_8x           BIT(3)		/*   Add frame when AGC reaches  8x gain */
+#define AF_16x          (BIT(2)|BIT(3))	/*   Add frame when AGC reaches 16x gain */
+					/* AEC max step control */
+#define AEC_NO_LIMIT    BIT(0)		/*   0 : AEC incease step has limit */
+					/*   1 : No limit to AEC increase step */
+
+/* COM7 */
+					/* SCCB Register Reset */
+#define SCCB_RESET      BIT(7)		/*   0 : No change */
+					/*   1 : Resets all registers to default */
+					/* Resolution selection */
+#define SLCT_MASK       BIT(6)		/*   Mask of VGA or QVGA */
+#define SLCT_VGA        (ZERO)		/*   0 : VGA */
+#define SLCT_QVGA       BIT(6)		/*   1 : QVGA */
+#define ITU656_ON_OFF   BIT(5)		/* ITU656 protocol ON/OFF selection */
+					/* RGB output format control */
+#define FMT_GBR422      (ZERO)		/*   00 : GBR 4:2:2 */
+#define FMT_RGB565      BIT(2)		/*   01 : RGB 565 */
+#define FMT_RGB555      BIT(3)		/*   10 : RGB 555 */
+#define FMT_RGB444      (BIT(3)|BIT(2))	/*   11 : RGB 444 */
+					/* Output format control */
+#define OFMT_YUV        (ZERO)		/*   00 : YUV */
+#define OFMT_P_BRAW     BIT(0)		/*   01 : Processed Bayer RAW */
+#define OFMT_RGB        BIT(1)		/*   10 : RGB */
+#define OFMT_BRAW       (BIT(1)|BIT(0))	/*   11 : Bayer RAW */
+
+/* COM8 */
+#define FAST_ALGO       BIT(7)	/* Enable fast AGC/AEC algorithm */
+				/* AEC Setp size limit */
+#define UNLMT_STEP      BIT(6)	/*   0 : Step size is limited */
+				/*   1 : Unlimited step size */
+#define BNDF_ON_OFF     BIT(5)	/* Banding filter ON/OFF */
+#define AEC_BND         BIT(4)	/* Enable AEC below banding value */
+#define AEC_ON_OFF      BIT(3)	/* Fine AEC ON/OFF control */
+#define AGC_ON          BIT(2)	/* AGC Enable */
+#define AWB_ON          BIT(1)	/* AWB Enable */
+#define AEC_ON          BIT(0)	/* AEC Enable */
+
+/* COM9 */
+#define BASE_AECAGC     BIT(7)		/* Histogram or average based AEC/AGC */
+					/* Automatic gain ceiling - maximum AGC value */
+#define GAIN_2x         (ZERO)		/*   000 :   2x */
+#define GAIN_4x         BIT(4)		/*   001 :   4x */
+#define GAIN_8x         BIT(5)		/*   010 :   8x */
+#define GAIN_16x        (BIT(4)|BIT(5))	/*   011 :  16x */
+#define GAIN_32x        BIT(6)		/*   100 :  32x */
+#define GAIN_64x        (BIT(6)|BIT(4))	/*   101 :  64x */
+#define GAIN_128x       (BIT(6)|BIT(5))	/*   110 : 128x */
+#define DROP_VSYNC      BIT(2)		/* Drop VSYNC output of corrupt frame */
+#define DROP_HREF       BIT(1)		/* Drop HREF  output of corrupt frame */
+
+/* COM11 */
+#define SGLF_ON_OFF     BIT(1)	/* Single frame ON/OFF selection */
+#define SGLF_TRIG       BIT(0)	/* Single frame transfer trigger */
+
+/* EXHCH */
+#define VSIZE_LSB       BIT(2)	/* Vertical data output size LSB */
+
+/* DSP_CTRL1 */
+#define FIFO_ON         BIT(7)	/* FIFO enable/disable selection */
+#define UV_ON_OFF       BIT(6)	/* UV adjust function ON/OFF selection */
+#define YUV444_2_422    BIT(5)	/* YUV444 to 422 UV channel option selection */
+#define CLR_MTRX_ON_OFF BIT(4)	/* Color matrix ON/OFF selection */
+#define INTPLT_ON_OFF   BIT(3)	/* Interpolation ON/OFF selection */
+#define GMM_ON_OFF      BIT(2)	/* Gamma function ON/OFF selection */
+#define AUTO_BLK_ON_OFF BIT(1)	/* Black defect auto correction ON/OFF */
+#define AUTO_WHT_ON_OFF BIT(0)	/* White define auto correction ON/OFF */
+
+/* DSP_CTRL3 */
+#define UV_MASK         BIT(7)	/* UV output sequence option */
+#define UV_ON           BIT(7)	/*   ON */
+#define UV_OFF          (ZERO)	/*   OFF */
+#define CBAR_MASK       BIT(5)	/* DSP Color bar mask */
+#define CBAR_ON         BIT(5)	/*   ON */
