...
Received: from mail-da0-f46.google.com ([209.85.210.46]:54350 "EHLO
	mail-da0-f46.google.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1750948Ab2LELnY convert rfc822-to-8bit (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Wed, 5 Dec 2012 06:43:24 -0500
Received: by mail-da0-f46.google.com with SMTP id p5so2252199dak.19
        for <linux-media@vger.kernel.org>; Wed, 05 Dec 2012 03:43:23 -0800 (PST)
MIME-Version: 1.0
In-Reply-To: <20121202214551.GL31879@valkosipuli.retiisi.org.uk>
References: <1349433287-28628-1-git-send-email-eballetbo@gmail.com>
	<20121202214551.GL31879@valkosipuli.retiisi.org.uk>
Date: Wed, 5 Dec 2012 12:43:23 +0100
Message-ID: <CAFqH_51PZ=SRNX5E6t5fG2X8eGhnOzrXz-Auni+q9BMTwnhLvw@mail.gmail.com>
...
2012/12/2 Sakari Ailus <sakari.ailus@iki.fi>:
> Hi Enric,
>
...
>
> Could you add a #define for 690, whatever that value signifies?
>
...
>> +             udelay(1);
>
> udelay(1)? I think we should expect set_clock() to sleep long enough for the
> clock to stabilise. Additional delay a sensor might require is typically
> longer than 1 盜. Well, I'm not saying it *can't* be so. :-)
>

Mmm, ok I need to check the Power-up, Reset, Clock and Standby Sequence ...

> On this board, is the regulator that supplies the sensor voltage always on?
> Or is it controllable? If so, it could make sense to access it using the
> regulator framework in the driver.
>

In my board is always on, so if I use the regulator framework I can't test.
...
>> +                                      unsigned int hratio)
>
> As this function can fail, how about returning the error code to the caller?
>
>> +{
>> +     struct i2c_client *client = v4l2_get_subdevdata(&mt9v034->subdev);
>> +     int ret;
...
>
> The test pattern control is standardised these days.
>
...
>> +};
>
> Please see v4l2_ctrl_new_std_menu_items() and "media:
> mt9p031/mt9t001/mt9v032: use V4L2_CID_TEST_PATTERN for test pattern
> control".
>

Ok, I'll do and resend version 2.
