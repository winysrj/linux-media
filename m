...
Received: from bear.ext.ti.com ([192.94.94.41]:44194 "EHLO bear.ext.ti.com"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
	id S1751298Ab0IOFhl convert rfc822-to-8bit (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Wed, 15 Sep 2010 01:37:41 -0400
From: "Hiremath, Vaibhav" <hvaibhav@ti.com>
To: Yanir Lubetkin <yanir.lubetkin@galilsoft.com>,
	"linux-media@vger.kernel.org" <linux-media@vger.kernel.org>
Date: Wed, 15 Sep 2010 11:07:33 +0530
Subject: RE: [PATCH] mt9p031 support for dm355-evm
Message-ID: <19F8576C6E063C45BE387C64729E739404689139E3@dbde02.ent.ti.com>
References: <4C900AAC.4050904@galilsoft.com>
In-Reply-To: <4C900AAC.4050904@galilsoft.com>
...
> owner@vger.kernel.org] On Behalf Of Yanir Lubetkin
> Sent: Wednesday, September 15, 2010 5:22 AM
...
> arago-project.org
>
[Hiremath, Vaibhav] Are you owner of all the changes below, if not I believe you should retain (or atleast CC) original author of the code-change.
...
>   {
[Hiremath, Vaibhav] There is not reference to the code have_sensor in the code, against which kernel repository this patch has been created?

If you have baselined out of Arago, then that is not expected, you should follow -

Changes touching to driver/media/ should be generated against linuxtv git repo
And
Changes touching arch specific files should be created against your platform git repo.

Else patches won't get applied cleanly.
...
> + * along with the linux kernel sources; if not, write to the Free
> Software
...
> +
> +#define MT9P031_NORMAL_OPERATION_MODE                (0x1F82)//(0x0002)//
> //write
...
> +}
> +
> +static int reg_set(struct i2c_client *client, const u8 reg, const u16
> data)
> +{
...
> +     {
> +             { 640, 480, 2575, 1951, 1470, 8, 496, 0x33, 0x33, 64, 0x8000,
> 32, 24,
> 32, 0x1F82, 0x12,  4, 2, },  // VGA_BIN_30FPS
> +             { 800, 600, 1599, 1199,    0, 8, 599, 0x11, 0x11, 64, 0x8000,
> 0,  0,
> 64, 0x1F82, 0xFF, 29, 4, },  // SVGA_30FPS
> +             {1024, 768, 2047, 1535,    0, 8, 767, 0x11, 0x11, 64, 0x8000,
> 0,  0,
> 64, 0x1F82, 0xFF, 29, 4, },  // XGA_30FPS
> +             {1280,1024, 2575, 1951,  776,10, 744, 0x11, 0x11, 64, 0x8000,
> 32, 24,
> 32, 0x1F8E, 0x1C,  4, 2, },  // SXGA_30FPS
> +             {1280, 720, 2583, 1491,  786, 0, 740, 0x11, 0x11, 64,
> 0x8000,272, 24,
> 32, 0x1F8E,   16,  3, 2, },
> +             {2592,1944, 2591, 1943,  450, 9,1942, 0x00, 0x00, 64, 0x8000,
> 66, 32,
...
> +     ret |= reg_clear(client, REG_MT9P031_ROW_ADDR_MODE,
> ADDRESS_MODE_MASK);
> +     ret |= reg_set(client, REG_MT9P031_ROW_ADDR_MODE,
> mt9p031_supported_formats[i].row_addr_mode);
> +     ret |= reg_clear(client, REG_MT9P031_COL_ADDR_MODE,
> ADDRESS_MODE_MASK);
...
> +//   status = dm355evm_msp_write(msp_has_tvp() ? 0 : MSP_VIDEO_IMAGER,
> +//                   DM355EVM_MSP_VIDEO_IN);
> +     status = dm355evm_msp_write(msp_use_soc_camera ?  MSP_VIDEO_IMAGER :
> 0,
>                       DM355EVM_MSP_VIDEO_IN);
>       if (status < 0)
