...
Received: from mx1.redhat.com ([209.132.183.28]:5177 "EHLO mx1.redhat.com"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
	id S1749667Ab0GFLr0 (ORCPT <rfc822;linux-media@vger.kernel.org>);
	Tue, 6 Jul 2010 07:47:26 -0400
Message-ID: <4C3317C6.8040104@redhat.com>
Date: Tue, 06 Jul 2010 08:47:18 -0300
From: Mauro Carvalho Chehab <mchehab@redhat.com>
MIME-Version: 1.0
To: Sedji Gaouaou <sedji.gaouaou@atmel.com>
CC: linux-media@vger.kernel.org
Subject: Re: [PATCH] Atmel IMAGE SENSOR INTERFACE (ISI) driver.
References: <1276094787-11214-1-git-send-email-sedji.gaouaou@atmel.com>
In-Reply-To: <1276094787-11214-1-git-send-email-sedji.gaouaou@atmel.com>
...
Sorry for taking a longer time to review... Too many patches on my queue.
It follows my comments.

Em 09-06-2010 11:46, Sedji Gaouaou escreveu:
> Hi,
> 
...
First of all, patches should have a Signed-off-by:, as described at the developers
section of our wiki page (linuxtv.org).

There's also a few CodingStyle non-compliances, according with scripts/checkpatch.pl,
as pointed at the end of this email.
> 
> Regards,
> Sedji
> 
...
Please use spaces to split the operator, like:
	(320 * 240 * 2)

This makes easier for review.
...
Hmm... why do you need so many static vars? Probably you should, instead
create an struct and put all dynamic data inside it. Also, if you have more than
one devices, this will be a problem, as one device will access data from the other
device.
...
Image size and frame rate should be controlled via V4L2 API, and not via static parameters.
...
Just declare widht/height as u32 and remove those typecasts.
...
Don't use { }  when there's just one clause for if and else.
...
You should also be filling fmt->description int his case.
...
That's wrong. You should, instead, check if the requested parameters 
are valid on the driver.
...
That's also wrong. You should be using this call to set capture parameters,
...
Huh? You already defined it at atmel_isi_capture_querycap()... Why are you
duplicating the code? The same applies to the callbacks bellow.
...
Seriously, please add a working logic, instead of a stub here. The proper way to
get the sensor information is via v4l subdev logic, breaking your driver into
sensors and the main driver.
...
Your comment is outdated, as your driver provides brightness control. 
...
There are some helper calls to handle control enumeration.
...
Please use videobuf stuff, instead of implementing your own logic. 
Videobuf allows other kind of memory maps, and a logic to allow reserving 
memory during boot time is being designed around it, in order to better
work on SoC designs.
...
Reseting the hardware at the first open is not a good idea, as it will discard
any changes/setups that might happen via some scripting before calling the video
application (for example, a call to v4l2-ctl, in order to adjust the brightness
to a desired level, before calling the driver).
...
You should really break the code into two separate modules, one for the subdev
and another for the main driver.
...
#222: FILE: drivers/media/video/atmel-isi.c:177:
+^Istruct frame_buffer ^I^Ivideo_buffer[ISI_VIDEO_BUFFERS];$

WARNING: please, no space before tabs
#256: FILE: drivers/media/video/atmel-isi.c:211:
+^Istruct v4l2_device ^I   ^Iv4l2_dev;$

WARNING: please, no space before tabs
#258: FILE: drivers/media/video/atmel-isi.c:213:
+^Istruct v4l2_subdev ^I^I*sd;$

WARNING: please, no space before tabs
#265: FILE: drivers/media/video/atmel-isi.c:220:
+^Istruct i2c_board_info ^I^Iboard_info;$

WARNING: unnecessary whitespace before a quoted newline
#1334: FILE: drivers/media/video/atmel-isi.c:1289:
+	pr_debug("atmel_isi: mmap called pgoff=%ld size=%ld \n",

WARNING: unnecessary whitespace before a quoted newline
#1360: FILE: drivers/media/video/atmel-isi.c:1315:
+	pr_debug("atmel_isi: vma start=0x%08lx, size=%ld phys=%ld \n",

ERROR: Missing Signed-off-by: line(s)

total: 1 errors, 6 warnings, 2249 lines checked
