...
Received: from mx1.redhat.com ([209.132.183.28]:3097 "EHLO mx1.redhat.com"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
	id S1751291Ab2LJSde (ORCPT <rfc822;linux-media@vger.kernel.org>);
	Mon, 10 Dec 2012 13:33:34 -0500
Date: Mon, 10 Dec 2012 16:33:13 -0200
...
Subject: Re: RFC: First draft of guidelines for submitting patches to
 linux-media
Message-ID: <20121210163313.424eb48d@redhat.com>
...
IMHO, the better is to give some examples for this specific warning (for example:
function calls and function declarations with more than 80 chars should likely be
broken, even if they have 81 columns; lines with an string at the end should likely
not be broken, even if it has more than 80 columns, etc).
...
> will be accepted, everything else will be postponed
...
> drawn-out process. This can be frustrating, but please stick with it.

Please add:
	"The reason for these measures is to warrant, in our best, that no
regressions will be added into the Kernel, keeping it with a high quality, and,
yet, allowing to release a new Kernel on every 7-9 weeks."
...
Europe or US), like the Linux Plumbers Conference, the Embedded Linux Conference
and/or the Embedded Linux Conference Europe.
...
Please add:
	Unfortunately, patchwork currently doesn't send you any email when a patch
successfully arrives there.
