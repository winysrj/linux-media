...
Received: from mx3.redhat.com (mx3.redhat.com [172.16.48.32])
	by int-mx1.corp.redhat.com (8.13.1/8.13.1) with ESMTP id m9O9xQ4W003575
	for <video4linux-list@redhat.com>; Fri, 24 Oct 2008 05:59:26 -0400
Received: from comal.ext.ti.com (comal.ext.ti.com [198.47.26.152])
	by mx3.redhat.com (8.13.8/8.13.8) with ESMTP id m9O9weoc024652
	for <video4linux-list@redhat.com>; Fri, 24 Oct 2008 05:58:41 -0400
From: Hardik Shah <hardik.shah@ti.com>
To: linux-omap@vger.kernel.org, linux-fbdev-devel@lists.sourceforge.net,
	video4linux-list@redhat.com
Date: Fri, 24 Oct 2008 15:23:28 +0530
Message-Id: <1224842008-24655-1-git-send-email-hardik.shah@ti.com>
Cc: 
Subject: [PATCHv2 4/4] OMAP 2/3 V4L2 Display Driver
...
Cleaned and restructured the OMAP 2/3 V4L2 Display Driver.

Converted the Driver to use the new_ioctl2 of the V4L2 frame work.
Moved the platform device registration to the board specific file.
Converted the custom IOCLTs in driver to the V4L2 standard ioctls.
Minor indentation cleanup done
...
---
 ...
 drivers/media/video/omap/omap_vout.c       | 3052 ++++++++++++++++++++++++++++
 drivers/media/video/omap/omap_voutdef.h    |  199 ++
 drivers/media/video/omap/omap_voutlib.c    |  283 +++
 drivers/media/video/omap/omap_voutlib.h    |   34 +
 include/linux/omap_vout.h                  |   45 +
 11 files changed, 3680 insertions(+), 2 deletions(-)
...
index 3f268d9..206a7d9 100644
--- a/arch/arm/mach-omap2/board-omap3evm.c
+++ b/arch/arm/mach-omap2/board-omap3evm.c
@@ -62,6 +62,34 @@ static struct platform_device omap3evm_smc911x_device = {
 	.resource	= &omap3evm_smc911x_resources [0],
 };
 
+static struct resource omap3evm_vout1_resource[] = {
+	[0] =	{
...
+	},
+};
+static struct resource omap3evm_vout2_resource[] = {
+	[0] =	{
...
+};
+
+static struct platform_device omap3evm_vout1_device = {
+	.name			= "omap_vout",
+	.num_resources	= ARRAY_SIZE(omap3evm_vout1_resource),
+	.resource 		= &omap3evm_vout1_resource[0],
+	.id		= 1,
+};
+static struct platform_device omap3evm_vout2_device = {
+	.name			= "omap_vout",
+	.num_resources	= ARRAY_SIZE(omap3evm_vout2_resource),
+	.resource 		= &omap3evm_vout2_resource[0],
+	.id		= 2,
+};
+
 ...
 {
 	int eth_cs;
@@ -236,6 +264,8 @@ static struct platform_device *omap3_evm_devices[] __initdata = {
 	&omap3_evm_lcd_device,
 	&omap3evm_smc911x_device,
 	&omap3evm_tv_device,
+	&omap3evm_vout1_device,
+	&omap3evm_vout2_device,
 };
 
 static void __init omap3_evm_init(void)
diff --git a/arch/arm/plat-omap/include/mach/omap34xx.h b/arch/arm/plat-omap/include/mach/omap34xx.h
index 4fb3556..da833f6 100644
...
+					(rotation_deg == 270) ?
+					((0x1) << DISPC_VID_ATTRIBUTES_VIDROT) :
+					(rotation_deg == 0) ?
...
+			}
+			vid_attributes |=
+				(rotation_deg == 90 || rotation_deg == 270) ?
...
+		vid_attributes |= DISPC_VID_ATTRIBUTES_VIDREPLICATIONENABLE;
+		break;
...
+			DISPC_VID_SIZE_VIDSIZEX) | ((((winheight - 1) / 2) <<
+			DISPC_VID_SIZE_VIDSIZEY_SHIFT) &
+			DISPC_VID_SIZE_VIDSIZEY);
+	} else {
+		vid_params.vid_size =
+			(((winwidth - 1) << DISPC_VID_SIZE_VIDSIZEX_SHIFT) &
+			DISPC_VID_SIZE_VIDSIZEX) | (((winheight - 1)
+			<< DISPC_VID_SIZE_VIDSIZEY_SHIFT) &
+			DISPC_VID_SIZE_VIDSIZEY);
+	}
+
...
+			DISPC_VID_PICTURE_SIZE_VIDORGSIZEY);
+	} else {
+		vid_params.vid_picture_size = (((cropwidth - 1)
+			<< DISPC_VID_PICTURE_SIZE_VIDORGSIZEX_SHIFT) &
+			DISPC_VID_PICTURE_SIZE_VIDORGSIZEX) |
+			(((cropheight - 1) <<
+			DISPC_VID_PICTURE_SIZE_VIDORGSIZEY_SHIFT) &
...
+	case 1:		/* Mirroring */
+		if (rotation_deg == 90 || rotation_deg == 270) {
+			row_inc_value = (-(MAX_PIXELS_PER_LINE + cropwidth) *
+					ps) + 1;
+			pixel_inc_value = 1;
+		} else if (rotation_deg == 180 || rotation_deg == 0) {
+			row_inc_value = (-(MAX_PIXELS_PER_LINE +
+					(cropwidth / vr_ps)) * ps) + 1;
+			pixel_inc_value = 1;
+		} else {
+			row_inc_value = 2 * ((cropwidth / vr_ps) - 1) *
+				ps + 1 + ((pix->width * ps) / vr_ps) -
+				(cropwidth / vr_ps) * ps;
...
+			mir_rot_deg = FIND_ROTATION_ANGLE(vout->rotation);
...
+	int err = -EINVAL;
+	omap_disp_get_dss();
+	err = omap_disp_enum_output(1, output->index, output->name);
+	if (!err)
...
+		if (*id == id_name[i].id) {
+			omap_disp_get_dss();
+			err = omap_disp_set_mode(1, id_name[i].name);
+			omap_disp_put_dss();
+		}
+	}
+	return err;
+
+}
+static int vidioc_g_std(struct file *file, void *fh, v4l2_std_id *id)
+{
+	int i;
+	char *s;
+	int err = -EINVAL;
...
+{
+	struct omap_vout_device *vout = ((struct omap_vout_fh *) fh)->vout;
+	unsigned int output_dev  = omap_disp_get_output_dev(vout->vid);
+
+	if (vout->streaming)
+		return -EBUSY;
...
+	if (vout->rotation == 90 ||
+		vout->rotation == 270) {
+		omap_disp_get_panel_size(output_dev, &(vout->fbuf.fmt.height),
+			&(vout->fbuf.fmt.width));
+	} else {
+		omap_disp_get_panel_size(output_dev, &(vout->fbuf.fmt.width),
...
+	struct omap_vout_device *vout = ((struct omap_vout_fh *) fh)->vout;
+	int bpp;
+	unsigned int output_device = omap_disp_get_output_dev(vout->vid);
+
+	if (vout->streaming)
+		return -EBUSY;
+
...
+	/* get the framebuffer parameters */
+	if (vout->rotation == 90 || vout->rotation == 270) {
+		omap_disp_get_panel_size(output_device,
+			&(vout->fbuf.fmt.height), &(vout->fbuf.fmt.width));
+	} else {
+		omap_disp_get_panel_size(output_device, &(vout->fbuf.fmt.width),
+			&(vout->fbuf.fmt.height));
+	}
+	omap_disp_put_dss();
+
...
+	}
+}
+static int vidioc_s_color_space_conv(struct file *file, void *fh,
...
+		v = 1;
+
+	current_colorconv_values[v][0][0] = a->coefficients[0][0];
+	current_colorconv_values[v][0][1] = a->coefficients[0][1];
+	current_colorconv_values[v][0][2] = a->coefficients[0][2];
+	current_colorconv_values[v][1][0] = a->coefficients[1][0];
+	current_colorconv_values[v][1][1] = a->coefficients[1][1];
+	current_colorconv_values[v][1][2] = a->coefficients[1][2];
+	current_colorconv_values[v][2][0] = a->coefficients[2][0];
+	current_colorconv_values[v][2][1] = a->coefficients[2][1];
+	current_colorconv_values[v][2][2] = a->coefficients[2][2];
+	omap_disp_get_dss();
+
...
+
+}
+static int vidioc_g_color_space_conv(struct file *file, void *fh,
+		struct v4l2_color_space_conversion *a)
+{
+	int v;
+	struct omap_vout_device *vout = ((struct omap_vout_fh *) fh)->vout;
...
+		v = 1;
+
+	memset(a, 0, sizeof(struct v4l2_color_space_conversion));
+	a->coefficients[0][0] = current_colorconv_values[v][0][0];
+	a->coefficients[0][1] = current_colorconv_values[v][0][1];
+	a->coefficients[0][2] = current_colorconv_values[v][0][2];
+	a->coefficients[1][0] = current_colorconv_values[v][1][0];
+	a->coefficients[1][1] = current_colorconv_values[v][1][1];
+	a->coefficients[1][2] = current_colorconv_values[v][1][2];
+	a->coefficients[2][0] = current_colorconv_values[v][2][0];
+	a->coefficients[2][1] = current_colorconv_values[v][2][1];
+	a->coefficients[2][2] = current_colorconv_values[v][2][2];
+
+	return 0;
+}
+
...
+				return 0;
+			}
+			vout->control[2].value = vout->mirror;
+			up(&vout->lock);
+			return -EINVAL;
+		}
...
+
+			vout->control[1].value = a->value;
+			output_dev = omap_disp_get_output_dev(vout->vid);
+
+			if (down_interruptible(&vout->lock))
+				return -EINVAL;
+
+			omap_disp_get_dss();
+
+			omap_disp_set_bg_color(output_dev,
+					vout->control[1].value);
+
+			omap_disp_put_dss();
+
+			up(&vout->lock);
...
+	rotation = (vout->rotation >= 0) ? vout->rotation : -1;
+
+	omap_vout_config_vlayer(vout->vid, &vout->pix, &vout->crop, &vout->win,
...
+		dest = (vout_linked == OMAP_VIDEO1) ? saved_v1out : saved_v2out;
+		rotation = (dest->rotation >= 0) ? dest->rotation : -1;
+		if (rotation > -1) {
+			if (!(vout->rotation > -1)) {
+				printk(KERN_WARNING "Rotation should be \
+					enabled in linking layer\n");
+				up(&vout->lock);
+				return -EINVAL;
+			}
+			if (rotation != vout->rotation) {
+				printk("Rotation must be same in \
+						both the layers\n");
+				up(&vout->lock);
+				return -EINVAL;
+			}
...
+.vidioc_s_color_space_conv 		= vidioc_s_color_space_conv,
+.vidioc_g_color_space_conv 		= vidioc_g_color_space_conv,
...
+#endif				/* PM */
+
+static int omap_vout_remove(struct platform_device *dev)
+{
+	omap_vout_cleanup_device(dev->id);
+	return 0;
+}
...
+	vfd->minor = -1;
+
+	sms_start_addr = dev->resource->start;
+	for (i = 0; i < 4; i++) {
+		for (j = 0; j < 4; j++) {
...
+	numbuffers = (vid == OMAP_VIDEO1) ?
+		video1_numbuffers : video2_numbuffers;
+	vout->buffer_size = (vid == OMAP_VIDEO1) ?
...
+				    dest->cropped_offset;
+			else
+				addr = dmabuf->bus_addr + dest->cropped_offset;
+			omap_disp_set_addr(dest->vid, addr, addr,
+					    addr + dest->tv_field1_offset);
+		}
+	} else {
...
+#define VIDIOC_S_OMAP_LINK		_IOW('V', 1, int)
+#define VIDIOC_G_OMAP_LINK		_IOR('V', 2, int)
+#define VIDIOC_S_OMAP_COLORKEY		_IOW('V', 3,\
+					struct omap_vout_colorkey)
+#define VIDIOC_G_OMAP_COLORKEY		_IOW('V', 4,\
+					struct omap_vout_colorkey)
+
