...
Received: from mailout1.w1.samsung.com ([210.118.77.11]:29488 "EHLO
	...
	with ESMTP id S1753465Ab0L2RdH (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Wed, 29 Dec 2010 12:33:07 -0500
Date: Wed, 29 Dec 2010 18:32:46 +0100
From: Sylwester Nawrocki <s.nawrocki@samsung.com>
Subject: [PATCH 04/15 v2] [media] s5p-fimc: Porting to videobuf 2
In-reply-to: <1293643975-4528-1-git-send-email-s.nawrocki@samsung.com>
To: linux-media@vger.kernel.org, linux-samsung-soc@vger.kernel.org
Cc: m.szyprowski@samsung.com, kyungmin.park@samsung.com,
	s.nawrocki@samsung.com
Message-id: <1293643975-4528-5-git-send-email-s.nawrocki@samsung.com>
MIME-version: 1.0
...
---
 ...
 drivers/media/video/s5p-fimc/fimc-capture.c |  264 ++++++++++++++++++++++++---
 drivers/media/video/s5p-fimc/fimc-core.c    |  216 ++++++++++------------
 drivers/media/video/s5p-fimc/fimc-core.h    |   48 ++---
 4 files changed, 363 insertions(+), 167 deletions(-)
...
+	ret = fimc_prepare_addr(ctx, &fimc_vb->vb, &ctx->d_frame,
 	...
 	if (ret)
 		return ret;
@@ -174,6 +174,7 @@ static int fimc_stop_capture(struct fimc_dev *fimc)
 {
 	unsigned long flags;
...
+	struct fimc_vid_buffer *buf;
 	...
 	cap = &fimc->vid_cap;
@@ -199,12 +200,224 @@ static int fimc_stop_capture(struct fimc_dev *fimc)
 			1 << ST_CAPT_STREAM);
...
@@ -244,11 +457,10 @@ static int fimc_capture_close(struct file *file)
...
@@ -262,13 +474,12 @@ static unsigned int fimc_capture_poll(struct file *file,
 {
...
+	ret = vb2_poll(&fimc->vid_cap.vbq, file, wait);
 	...
 	return ret;
@@ -278,13 +489,12 @@ static int fimc_capture_mmap(struct file *file, struct vm_area_struct *vma)
 {
 	struct fimc_ctx *ctx = file->private_data;
 	struct fimc_dev *fimc = ctx->fimc_dev;
-	struct fimc_vid_cap *cap = &fimc->vid_cap;
...
+	ret = vb2_mmap(&fimc->vid_cap.vbq, vma);
 	...
 	return ret;
@@ -470,7 +680,7 @@ static int fimc_cap_g_input(struct file *file, void *priv,
 }
...
@@ -525,7 +735,7 @@ static int fimc_cap_streamon(struct file *file, void *priv,
 	fimc->vid_cap.buf_index = fimc_hw_get_frame_index(fimc);
...
+			      enum v4l2_buf_type type)
 {
 	struct fimc_ctx *ctx = priv;
 	struct fimc_dev *fimc = ctx->fimc_dev;
@@ -553,7 +763,8 @@ static int fimc_cap_streamoff(struct file *file, void *priv,
 		return -ERESTARTSYS;
...
@@ -572,7 +783,7 @@ static int fimc_cap_reqbufs(struct file *file, void *priv,
 	if (mutex_lock_interruptible(&fimc->lock))
 		return -ERESTARTSYS;
...
@@ -589,7 +800,7 @@ static int fimc_cap_querybuf(struct file *file, void *priv,
...
@@ -603,7 +814,7 @@ static int fimc_cap_qbuf(struct file *file, void *priv,
 	if (mutex_lock_interruptible(&fimc->lock))
 		return -ERESTARTSYS;
...
+	ret = vb2_qbuf(&cap->vbq, buf);
 	...
 	mutex_unlock(&fimc->lock);
 	return ret;
@@ -618,7 +829,7 @@ static int fimc_cap_dqbuf(struct file *file, void *priv,
...
+	ret = vb2_dqbuf(&ctx->fimc_dev->vid_cap.vbq, buf,
 	...
 	mutex_unlock(&ctx->fimc_dev->lock);
@@ -777,6 +988,7 @@ int fimc_register_capture_device(struct fimc_dev *fimc)
...
@@ -827,10 +1039,16 @@ int fimc_register_capture_device(struct fimc_dev *fimc)
