...
Received: from smtp-68.nebula.fi ([83.145.220.68]:34754 "EHLO
	smtp-68.nebula.fi" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1755487Ab1LASJn (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Thu, 1 Dec 2011 13:09:43 -0500
Date: Thu, 1 Dec 2011 20:09:34 +0200
From: Sakari Ailus <sakari.ailus@iki.fi>
To: Doron Cohen <doronc@siano-ms.com>
Cc: linux-media@vger.kernel.org
Subject: Re: [PATCH v2] Add smsspi driver to support Siano SPI connected
 device
Message-ID: <20111201180933.GM29805@valkosipuli.localdomain>
References: <D945C405928A9949A0F33C69E64A1A3BD8D5D6@s-mail.siano-ms.ent>
MIME-Version: 1.0
Content-Type: text/plain; charset=us-ascii
Content-Disposition: inline
In-Reply-To: <D945C405928A9949A0F33C69E64A1A3BD8D5D6@s-mail.siano-ms.ent>
...
You should use dev_* macros instead of printk. If you have no access to
device, then use pr_* macros. See include/linux/device.h .

Dynamic printk also gives you a convenient way to choose debug verbosity at
runtime without using driver specific solutions.
...
ptr is unsigned char pointer; no need for bitwise and operation here nor
elsewhere.
...
The above case seems quite long. I'd put that into a separate function
instead.

Also, please do run scripts/checkpatch.pl to the patch.
...
You can combine the two if's.

Are you absolutely certain your buffer is large enough to do this?

I'd also prefer setting an unsigned int pointer first before performing the
check.

The 0xa5 byte is part of the first integer. Is this check really correct?
...
len may be initialised at declaration.
...
Isn't this a grave error in some cases? I.e. your function should tell the
...
Is there a need to copy the data instead of just using a pointer?
...
I advice leaving out the underscore prefix from the struct names. There's
really not reason to keep it.
...
These parameters must come from platform data (or device tree) instead of
module parameters.
...
I wonder if you could just use spi_sync_locked() instead. But I don't know
SPI well...
...
Please use a defined value instead of a magic number.
...
I suggest kzalloc instead.

You must also check whether the allocation succeeds.
...
If there is just one set of callbacks, why to use callbacks in the first
place? As far as I understand, the device does use SPI interface only.
...
It's not the same function here.
...
Do you know you really need to change the endianness, or transfer between
CPU and known endianness?

See e.g. cpu_to_be32() in include/linux/byteorder/generic.h.
...
You should use a known type for "context".
...
Shouldn't this come from platform data (or the device tree) instead?
...
I think you could just call the function directly without needing a
callback.
...
Shouldn't you free your resources here?
...
You're essentially defining a new interface to communicate with the SPI host
driver. Would it make sense to use the more generic interface to do that
instead? Perhaps someone else could comment that part --- I'm not the best
to comment it since I haven't worked on any SPI drivers in the past.
