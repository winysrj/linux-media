...
Received: from smtp-vbr13.xs4all.nl ([194.109.24.33]:3579 "EHLO
	smtp-vbr13.xs4all.nl" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1753907Ab0C1JBJ (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Sun, 28 Mar 2010 05:01:09 -0400
From: Hans Verkuil <hverkuil@xs4all.nl>
To: "Zhang, Xiaolin" <xiaolin.zhang@intel.com>
Subject: Re: [PATCH v2 4/10] V4L2 patches for Intel Moorestown Camera Imaging Drivers
Date: Sun, 28 Mar 2010 11:01:41 +0200
...
References: <33AB447FBD802F4E932063B962385B351D6D534E@shsmsx501.ccr.corp.intel.com>
In-Reply-To: <33AB447FBD802F4E932063B962385B351D6D534E@shsmsx501.ccr.corp.intel.com>
MIME-Version: 1.0
Content-Type: Text/Plain;
  charset="iso-8859-1"
Content-Transfer-Encoding: 7bit
Message-Id: <201003281101.41263.hverkuil@xs4all.nl>
...
> Subject: [PATCH 04/10] this patch is 2mp camera (ov2650) sensor subdev driver for intel moorestown camera imaging.
> 
...
As mentioned in the comments in patch 0/10 I think debug constructions like
this probably should just be removed.
...
Don't use Moorestown formats in a driver that can be used in other products
as well.

See also the comments made below on how to setup formats.
...
This is not right. The Moorestown driver should do this. In a generic subdev
driver you do not know how it is hooked up to the bridge driver and how it
should be woken up. One option is to use the v4l2_device notify callback.

See for example how that is used in bt819.c.
...
Remember, this ov2650 driver knows nothing about how it is connected to some
bridge driver. So you cannot use Moorestown specific structs and defines here,
otherwise this driver would be impossible to reuse in e.g. a USB webcam.

The bridge driver (or platform driver in the case of Moorestown) is the only
one that knows about which subdevs are hooked up and how they are hooked up.

So typically the platform code will have to tell the main Moorestown v4l driver
what is where and any other config data that is needed.

If a sensor driver needs specific setup config data, then a sensor-specific
header should be added in include/media. See for example include/media/mt9t112.h.
...
Don't use try/g/s_fmt in a subdev driver. Instead use the newer try/g/s_mbus_fmt
calls. The reason behind this is that v4l2_format really deals with what the
data looks like in memory, after it has passed through the DMA engine.

But between a sensor and e.g. Moorestown there is just a bus and no memory is
involved. So the new mbus_fmt ops should be used instead of the older _fmt ops
to negotiate bus formats.

I'm working on replacing _fmt with mbus_fmt in existing subdev drivers and
eventually the old _fmt ops will disappear.
...
Don't use this. Use v4l2_ctrl_query_fill() instead. See e.g. tvp514x.c for
an example of how to use this. Using this function ensures consistent control
names and will also make it easier to integrate later into a new control
framework that I am working on.
...
static const. Ditto for the similar arrays below.
