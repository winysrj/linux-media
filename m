...
Received: from mo4-p00-ob.smtp.rzone.de ([81.169.146.220]:25519 "EHLO
        mo4-p00-ob.smtp.rzone.de" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1751190AbdFZGDn (ORCPT
        <rfc822;linux-media@vger.kernel.org>);
        Mon, 26 Jun 2017 02:03:43 -0400
...
Date: Mon, 26 Jun 2017 08:03:07 +0200
...
Message-Id: <BE150A6E-5385-49E8-BA9C-261483C29A44@goldelico.com>
References: <1498143942-12682-1-git-send-email-hugues.fruchet@st.com> <1498143942-12682-7-git-send-email-hugues.fruchet@st.com>
...
> + * Author: Hugues Fruchet <hugues.fruchet@st.com> for =
STMicroelectronics.
> + *
> + * OV9655 initial support based on a driver written by H. Nikolaus =
Schaller:
> + *   =
...
> +#define  OV9655_TRISTATE		/* seems to have a different =
function */
...
> #define REG_COM10		0x15	/* PCLK, HREF, HSYNC signals =
polarity */
> +#define  OV9655_SLAVE_PIN	0x80	/* SLHS/SLVS instead of =
RESETB/PWDN */
> #define  COM10_HSYNC		0x40	/* HSYNC instead of HREF */
> #define  COM10_PCLK_HB		0x20	/* Suppress PCLK on =
horiz blank */
> -#define  COM10_HREF_REV		0x08	/* Reverse HREF */
> +#define  OV9655_COM10_PCLK_REV		0x10	/* PCLK reverse =
*/
> +#define  COM10_HREF_REV	0x08	/* Reverse HREF */
> #define  COM10_VS_LEAD		0x04	/* VSYNC on clock =
leading edge */
> +#define  OV9655_COM10_RESET_OPTION	0x04	/* Reset signal end =
point */
> #define  COM10_VS_NEG		0x02	/* VSYNC negative */
> #define  COM10_HS_NEG		0x01	/* HSYNC negative */
> +#define OV9655_REG16		0x16	/* dummy frame and blanking */
> +#define   OV9655_REG16_DUMMY_8	0x20	/* dummy frame when gain =
> 8 */
...
> +#define OV9655_ADC		0x37	/* ADC Control 1 (Range =
adjustment) */
> #define REG_ACOM		0x38	/* reserved */
> -#define REG_OFON		0x39	/* Power down register */
> +#define OV9655_ADC2		0x38	/* ADC Control 2 (Range =
adjustment) */
> +#define REG_OFON		0x39	/* Power down register (ov9650 =
only) */
...
> #define  TSLB_YUYV_MASK		0x0c	/* UYVY or VYUY - see =
com13 */
...
> +
> #define REG_COM11		0x3b	/* Night mode, banding filter =
enable */
> #define  COM11_NIGHT		0x80	/* Night mode enable */
> #define  COM11_NMFR		0x60	/* Two bit NM frame rate */
> @@ -142,25 +191,38 @@
> #define  COM12_HREF		0x80	/* HREF always */
> #define REG_COM13		0x3d	/* Gamma selection, Color matrix =
en. */
...
> +#define OV9655_REG_COM14	0x3e	/* pixel correction/zoom ON/OFF =
sel. */
> +#define  OV9655_COM14_BLACK_PIX	0x08	/* Black pixel =
correction */
> +#define  OV9655_COM14_WHITE_PIX	0x04	/* White pixel =
correction */
...
> #define REG_COM17		0x42	/* Single frame out, banding =
filter */
...
> +#define REG_AWBOP(__n)		(0x59 + (__n) - 1) /* AWB =
control options */
> +#define REG_BLMT		0x5F	/* AWB Blue Component Gain Limit =
*/
> +#define REG_RLMT		0x60	/* AWB Red Component Gain Limit =
*/
> +#define REG_GLMT		0x61	/* AWB Green Component Gain =
Limit */
> /* Lens Correction Option 1...5, __n =3D 0...5 */
> #define REG_LCC(__n)		(0x62 + (__n) - 1)
> #define  LCC5_LCC_ENABLE	0x01	/* LCC5, enable lens correction =
*/
> @@ -170,10 +232,26 @@
> #define REG_HV			0x69	/* Manual banding filter =
MSB */
> #define REG_MBD			0x6a	/* Manual banding filter =
value */
> #define REG_DBLV		0x6b	/* reserved */
> +#define OV9655_REG_DBLV		0x6b	/* PLL, DVDD regu =
bypass, bandgap */
...
*/
> #define  COM22_WHTPCOR		0x02	/* White pixel =
correction enable */
> @@ -181,6 +259,8 @@
> #define  COM22_DENOISE		0x10	/* White pixel =
correction option */
...
*/
> @@ -193,6 +273,17 @@
> #define REG_AECHM		0xa1	/* Exposure value - bits =
AEC[15:10] */
...
> +#define OV9655_REG_BLC(__n)	(0xac + (__n) - 1) /* Black Level =
Control */
...
*/
> +#define OV9655_REG_COM24	0xc7	/* Pixel clock frequency =
selection */
...
>=20
> +/* OV9655 */
> +static const struct i2c_rv ov9655_init_regs[] =3D {
> +	{ REG_GAIN, 0x00 },
> +	{ REG_BLUE, 0x80 },
> +	{ REG_RED, 0x80 },
> +	{ REG_VREF, 0x02 },
...
> +	{ REG_CLKRC, 0x01 },/* F(internal clk) =3D F(input clk) / 2 */
...
> +};
> +
> +static const struct i2c_rv ov9655_qvga_regs[] =3D {
> +	{ REG_HREF, 0x12 },
...
> +};
> +
> +static const struct i2c_rv ov9655_qqvga_regs[] =3D {
> +	{ REG_HREF, 0xa4 },
...
> +};
> +
> +static const struct i2c_rv ov9655_vga_regs[] =3D {
> +	{ REG_GAIN, 0x11 },
> +	{ REG_VREF, 0x12 },
> +	{ REG_B_AVE, 0x2e },
> +	{ REG_GB_AVE, 0x2e },
...
> +};
> +
> +static const struct ov965x_framesize ov9655_framesizes[] =3D {
> +	{
> +		.width		=3D VGA_WIDTH,
> +		.height		=3D VGA_HEIGHT,
> +		.regs		=3D ov9655_vga_regs,
> +		.max_exp_lines	=3D 498,
> +	}, {
> +		.width		=3D QVGA_WIDTH,
> +		.height		=3D QVGA_HEIGHT,
> +		.regs		=3D ov9655_qvga_regs,
> +		.max_exp_lines	=3D 248,
> +	},
> +	{
> +		.width		=3D QQVGA_WIDTH,
> +		.height		=3D QQVGA_HEIGHT,
> +		.regs		=3D ov9655_qqvga_regs,
> +		.max_exp_lines	=3D 124,
> +	},
> +};
> +
> +static const struct ov965x_pixfmt ov9655_formats[] =3D {
> +	{ MEDIA_BUS_FMT_RGB565_2X8_LE, V4L2_COLORSPACE_SRGB, 0x08},
> +};
> +
> static inline struct v4l2_subdev *ctrl_to_sd(struct v4l2_ctrl *ctrl)
> {
> 	return &container_of(ctrl->handler, struct ov965x, =
ctrls.handler)->sd;
> @@ -894,12 +1272,16 @@ static int ov965x_set_test_pattern(struct =
ov965x *ov965x, int value)
> {
> 	int ret;
> 	u8 reg;
> +	u8 addr =3D (ov965x->id =3D=3D OV9655V5_ID) ?
> +			REG_COM3 : REG_COM23;
> +	u8 mask =3D (ov965x->id =3D=3D OV9655V5_ID) ?
> +			COM3_COLORBAR : COM23_TEST_MODE;
>=20
> -	ret =3D ov965x_read(ov965x->client, REG_COM23, &reg);
> +	ret =3D ov965x_read(ov965x->client, addr, &reg);
> 	if (ret < 0)
> 		return ret;
> -	reg =3D value ? reg | COM23_TEST_MODE : reg & ~COM23_TEST_MODE;
> -	return ov965x_write(ov965x->client, REG_COM23, reg);
> +	reg =3D value ? reg | mask : reg & ~mask;
> +	return ov965x_write(ov965x->client, addr, reg);
> }
>=20
> static int __g_volatile_ctrl(struct ov965x *ov965x, struct v4l2_ctrl =
*ctrl)
> @@ -1102,6 +1484,30 @@ static int ov965x_initialize_controls(struct =
ov965x *ov965x)
> 	return 0;
> }
>=20
...
> +{
> +	const struct v4l2_ctrl_ops *ops =3D &ov965x_ctrl_ops;
> +	struct ov965x_ctrls *ctrls =3D &ov965x->ctrls;
...
>  */
> @@ -1518,9 +1924,15 @@ static int ov965x_detect_sensor(struct =
v4l2_subdev *sd)
>=20
> 	if (!ret) {
> 		ov965x->id =3D OV965X_ID(pid, ver);
> -		if (ov965x->id =3D=3D OV9650_ID || ov965x->id =3D=3D =
OV9652_ID) {
> +		switch (ov965x->id) {
> +		case OV9650_ID:
> +		case OV9652_ID:
...
> 				 ov965x->id, ret);
> 			ret =3D -ENODEV;
> @@ -1598,18 +2010,28 @@ static int ov965x_probe(struct i2c_client =
*client,
> 	if (ret < 0)
> 		goto err_me;
>=20
> -	ov965x->init_regs =3D ov965x_init_regs;
> -	ov965x->initialize_controls =3D ov965x_initialize_controls;
> -	ov965x->framesizes =3D ov965x_framesizes;
> -	ov965x->nb_of_framesizes =3D ARRAY_SIZE(ov965x_framesizes);
> -	ov965x->formats =3D ov965x_formats;
> -	ov965x->nb_of_formats =3D ARRAY_SIZE(ov965x_formats);
> -	ov965x->intervals =3D ov965x_intervals;
> -	ov965x->nb_of_intervals =3D ARRAY_SIZE(ov965x_intervals);
> -	ov965x->fiv =3D &ov965x_intervals[0];
> -	ov965x->set_frame_interval =3D __ov965x_set_frame_interval;
> -	ov965x->update_exposure_ctrl =3D ov965x_update_exposure_ctrl;
> -	ov965x->set_params =3D __ov965x_set_params;
> +	if (ov965x->id !=3D OV9655V5_ID) {
> +		ov965x->init_regs =3D ov965x_init_regs;
> +		ov965x->initialize_controls =3D =
ov965x_initialize_controls;
> +		ov965x->framesizes =3D ov965x_framesizes;
> +		ov965x->nb_of_framesizes =3D =
ARRAY_SIZE(ov965x_framesizes);
> +		ov965x->formats =3D ov965x_formats;
> +		ov965x->nb_of_formats =3D ARRAY_SIZE(ov965x_formats);
> +		ov965x->intervals =3D ov965x_intervals;
> +		ov965x->nb_of_intervals =3D =
ARRAY_SIZE(ov965x_intervals);
> +		ov965x->fiv =3D &ov965x_intervals[0];
> +		ov965x->set_frame_interval =3D =
__ov965x_set_frame_interval;
> +		ov965x->update_exposure_ctrl =3D =
ov965x_update_exposure_ctrl;
> +		ov965x->set_params =3D __ov965x_set_params;
> +	} else {
> +		ov965x->init_regs =3D ov9655_init_regs;
> +		ov965x->initialize_controls =3D =
ov9655_initialize_controls;
> +		ov965x->framesizes =3D ov9655_framesizes;
> +		ov965x->nb_of_framesizes =3D =
ARRAY_SIZE(ov9655_framesizes);
> +		ov965x->formats =3D ov9655_formats;
> +		ov965x->nb_of_formats =3D ARRAY_SIZE(ov9655_formats);
> +		ov965x->set_params =3D ov965x_set_frame_size;
> +	};
>=20
> 	ov965x->frame_size =3D &ov965x->framesizes[0];
> 	ov965x_get_default_format(ov965x, &ov965x->format);
> @@ -1652,6 +2074,7 @@ static int ov965x_remove(struct i2c_client =
*client)
...
i2c device ids should be lower case to match compatible-strings in DT =
for
...
edia/i2c/ov9650.c;h=3Dc310cbee131665893d2d1df0ab1246bd9b1d41fe;hp=3Ded5d0a=
53a9c72036d6e017094b68111b5eb7f00d;hb=3D115b9c59202aa2fb1fecb691ebeef51220=
d363b8;hpb=3Dda8ae2b038a448c8f822b3a4f20ed378db6d2934>
...
> @@ -1659,6 +2082,7 @@ static int ov965x_remove(struct i2c_client =
*client)
> static const struct of_device_id ov965x_of_match[] =3D {
> 	{ .compatible =3D "ovti,ov9650", .data =3D (void *)0x9650 },
> 	{ .compatible =3D "ovti,ov9652", .data =3D (void *)0x9652 },
> +	{ .compatible =3D "ovti,ov9655", .data =3D (void *)0x9655 },
> 	{ /* sentinel */ }
> };
