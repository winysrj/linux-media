...
Received: from mailout4.w1.samsung.com ([210.118.77.14]:60709 "EHLO
	mailout4.w1.samsung.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1752521Ab2KWPXD (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Fri, 23 Nov 2012 10:23:03 -0500
Received: from eusync4.samsung.com (mailout4.w1.samsung.com [210.118.77.14])
 by mailout4.w1.samsung.com
 (Oracle Communications Messaging Server 7u4-24.01(7.0.4.24.0) 64bit (built Nov
 17 2011)) with ESMTP id <0MDY00M4H5F45780@mailout4.w1.samsung.com> for
 linux-media@vger.kernel.org; Fri, 23 Nov 2012 15:23:28 +0000 (GMT)
...
Subject: [PATCH RFC 1/3] [media] s5c73m3: Add driver
Date: Fri, 23 Nov 2012 16:22:28 +0100
Message-id: <1353684150-24581-2-git-send-email-a.hajda@samsung.com>
...
+	if (ret < 0) {
+		pr_info("AF soft-landing failed\n");
+		return ret;
+	}
+
+	for (;;) {
...
+		msleep(25);
+	}
+	pr_info("AF soft-landing %s after %dms\n", msg,
+					jiffies_to_msecs(jiffies - start));
+
+	return ret;
+}
...
+	}
+
+	pr_cont("Loading firmware (%s, %d B)...", fw_name, fw->size);
+
...
+
+
+	ret = s5c73m3_isp_command(state, COMM_CHG_MODE, chg_mode);
+
+	return ret;
+}
+
...
+		if (ret < 0)
+			pr_err("Error setting frame rate(%d)", ret);
+	}
+
+	ret = s5c73m3_check_status(state, REG_STATUS_ISP_COMMAND_COMPLETED);
+	if (ret)
...
+	pr_info("set pad=%d,which=%d,code=%08x,size=%dx%d\n", fmt->pad,
+		fmt->which, fmt->format.code, fmt->format.width,
+		fmt->format.height);
+
+	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
