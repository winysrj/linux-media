...
Received: from wa-out-1112.google.com ([209.85.146.176]:40202 "EHLO
	wa-out-1112.google.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1751775AbZGFLaX (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Mon, 6 Jul 2009 07:30:23 -0400
Received: by wa-out-1112.google.com with SMTP id j5so766101wah.21
        for <linux-media@vger.kernel.org>; Mon, 06 Jul 2009 04:30:27 -0700 (PDT)
From: Brian Johnson <brijohn@gmail.com>
To: linux-media@vger.kernel.org
Cc: hdegoede@redhat.com, Brian Johnson <brijohn@gmail.com>
Subject: [PATCH 1/1] gspca: Add sn9c20x subdriver
Date: Mon,  6 Jul 2009 07:30:22 -0400
Message-Id: <1246879822-21348-2-git-send-email-brijohn@gmail.com>
In-Reply-To: <1246879822-21348-1-git-send-email-brijohn@gmail.com>
...
+	sd->quality = 95;
+
+	sn9c20x_create_debugfs_files(gspca_dev);
+
+#ifdef CONFIG_USB_GSPCA_SN9C20X_EVDEV
...
+	sn9c20x_remove_debugfs_files(gspca_dev);
+
+#ifdef CONFIG_USB_GSPCA_SN9C20X_EVDEV
...
diff --git a/drivers/media/video/gspca/sn9c20x/sn9c20x.h b/drivers/media/video/gspca/sn9c20x/sn9c20x.h
new file mode 100644
index 0000000..5683452
--- /dev/null
+++ b/drivers/media/video/gspca/sn9c20x/sn9c20x.h
@@ -0,0 +1,104 @@
+/*
+ *	Sonix sn9c201 sn9c202 library
+ *	Copyright (C) 2008-2009 microdia project <microdia@googlegroups.com>
...
+#endif
+
+#ifdef CONFIG_USB_GSPCA_SN9C20X_DEBUGFS
...
+void sn9c20x_create_debugfs_files(struct gspca_dev *gspca_dev);
+void sn9c20x_remove_debugfs_files(struct gspca_dev *gspca_dev);
+#else
+static inline void sn9c20x_init_debugfs(void) {}
+static inline void sn9c20x_uninit_debugfs(void) {}
+static inline int sn9c20x_create_debugfs_files(struct gspca_dev *gspca_dev) {}
+static inline int sn9c20x_remove_debugfs_files(struct gspca_dev *gspca_dev) {}
+#endif
+
+#endif
diff --git a/drivers/media/video/gspca/sn9c20x/sn9c20x_debugfs.c b/drivers/media/video/gspca/sn9c20x/sn9c20x_debugfs.c
new file mode 100644
index 0000000..a9e6136
--- /dev/null
+++ b/drivers/media/video/gspca/sn9c20x/sn9c20x_debugfs.c
@@ -0,0 +1,320 @@
+/*
+ *	Sonix sn9c201 sn9c202 library
+ *	Copyright (C) 2008-2009 microdia project <microdia@googlegroups.com>
...
+	u8 values[16];
+	loff_t *spos = (loff_t *)v;
+	u16 reg = 0x1000 + (*spos * 16);
+	struct gspca_dev *gspca_dev = m->private;
+
+	if (reg_r(gspca_dev, reg, 16) < 0)
+		return -EACCES;
+
+	memcpy(values, gspca_dev->usb_buf, 16);
+
...
+}
+
+static const struct seq_operations bridge_dump_seq_ops = {
...
+}
+
+static const struct file_operations bridge_dump_ops = {
+	.owner		= THIS_MODULE,
...
+};
+
+int bridge_value_set(void *data, u64 val)
+{
+	struct gspca_dev *gspca_dev = data;
+	struct sd *sd = (struct sd *) gspca_dev;
+	u8 value = (u64)val;
+	if (gspca_dev != NULL) {
+		if (sd->bridge_addr >= 0x1000 &&
+		    sd->bridge_addr <= 0x11ff) {
+			reg_w1(gspca_dev, sd->bridge_addr, value);
+		}
+	}
+	return 0;
+}
+
+int bridge_value_get(void *data, u64 *val)
+{
+	struct gspca_dev *gspca_dev = data;
+	struct sd *sd = (struct sd *) gspca_dev;
+	if (gspca_dev != NULL) {
+		if (sd->bridge_addr >= 0x1000 &&
+		    sd->bridge_addr <= 0x11ff) {
+			if (reg_r(gspca_dev, sd->bridge_addr, 1) >= 0)
+				*val = gspca_dev->usb_buf[0];
+		}
+	}
+	return 0;
+}
+
+int sensor_value8_set(void *data, u64 val)
+{
+	struct gspca_dev *gspca_dev = data;
+	struct sd *sd = (struct sd *) gspca_dev;
+	u8 value = (u8)val;
+	if (gspca_dev != NULL)
+		i2c_w1(gspca_dev, sd->sensor_addr, value);
+	return 0;
+}
+
+int sensor_value8_get(void *data, u64 *val)
+{
+	struct gspca_dev *gspca_dev = data;
+	struct sd *sd = (struct sd *) gspca_dev;
+	u8 value;
+	if (gspca_dev != NULL) {
+		if (i2c_r1(gspca_dev, sd->sensor_addr, &value) >= 0)
+			*val = value & 0xFF;
+	}
+	return 0;
+}
+
+int sensor_value16_set(void *data, u64 val)
+{
+	struct gspca_dev *gspca_dev = data;
+	struct sd *sd = (struct sd *) gspca_dev;
+	u16 value = (u16)val;
+	if (gspca_dev != NULL)
+		i2c_w2(gspca_dev, sd->sensor_addr, value);
+	return 0;
+}
+
+int sensor_value16_get(void *data, u64 *val)
+{
+	struct gspca_dev *gspca_dev = data;
+	struct sd *sd = (struct sd *) gspca_dev;
+	u16 value;
+	if (gspca_dev != NULL) {
+		if (i2c_r2(gspca_dev, sd->sensor_addr, &value) >= 0)
+			*val = value & 0xFFFF;
+	}
+	return 0;
+}
...
+	char device_name[9];
+	if (debug_dir) {
+		snprintf(device_name, 9, "video%d", gspca_dev->vdev.minor);
+		sd->dent_device =
+			debugfs_create_dir(device_name, debug_dir);
+		if (sd->dent_device) {
+			sd->bridge_addr = 0x1000;
+			sd->sensor_addr = 0x00;
+			sd->dent_bridge_addr =
...
+{
+	struct sd *sd = (struct sd *) gspca_dev;
+	if (sd->dent_bridge_val)
+		debugfs_remove(sd->dent_bridge_val);
+	if (sd->dent_bridge_addr)
+		debugfs_remove(sd->dent_bridge_addr);
+	if (sd->dent_bridge_dump)
+		debugfs_remove(sd->dent_bridge_dump);
+	if (sd->dent_sensor_addr)
+		debugfs_remove(sd->dent_sensor_addr);
+	if (sd->dent_sensor_val8)
+		debugfs_remove(sd->dent_sensor_val8);
+	if (sd->dent_sensor_val16)
+		debugfs_remove(sd->dent_sensor_val16);
+	if (sd->dent_device)
+		debugfs_remove(sd->dent_device);
+	kref_put(&debug_ref, debugfs_delete);
+}
+
diff --git a/include/linux/videodev2.h b/include/linux/videodev2.h
index 8a025d5..ba74b88 100644
