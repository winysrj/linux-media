...
Received: from mga03.intel.com ([134.134.136.65]:29779 "EHLO mga03.intel.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1751484AbdJCKyd (ORCPT <rfc822;linux-media@vger.kernel.org>);
        Tue, 3 Oct 2017 06:54:33 -0400
Message-ID: <1507028067.16112.244.camel@linux.intel.com>
Subject: Re: [PATCH v3] staging: atomisp: add a driver for ov5648 camera
 sensor
...
Date: Tue, 03 Oct 2017 13:54:27 +0300
In-Reply-To: <1506972665-4647-1-git-send-email-d.filoni@ubuntu.com>
References: <1506972665-4647-1-git-send-email-d.filoni@ubuntu.com>
...
> +++ b/drivers/staging/media/atomisp/i2c/Makefile
> @@ -8,6 +8,7 @@ obj-$(CONFIG_VIDEO_ATOMISP_MT9M114)    += atomisp-
> mt9m114.o
...
>  
>  obj-$(CONFIG_VIDEO_ATOMISP_MSRLIST_HELPER) += atomisp-
> libmsrlisthelper.o
...
> +		dev_err(&client->dev, "read_reg error, invalid data
> length %x\n",
> +			data_length);
> +		return -EINVAL;
> +	}
...
> +}
> +
> +static int ov5648_i2c_write(struct i2c_client *client, u16 len, u8
> *data)
> +{
> +	const int num_msg = 1;
> +	struct i2c_msg msg;
...
> +	if (ret)
> +		dev_err(&client->dev,
> +			"write error: wrote 0x%x to offset 0x%x,
> error: %d\n",
> +			val, reg, ret);
> +
> +	return ret;
...
> + *
> + * This function initializes a list of registers. When consecutive
> addresses
> + * are found in a row on the list, this function creates a buffer and
> sends
> + * consecutive data in a single i2c_transfer().
> + *
> + * __ov5648_flush_reg_array, __ov5648_buf_reg_array() and
> + * __ov5648_write_reg_is_consecutive() are internal functions to
> + * ov5648_write_reg_array_fast() and should be not used anywhere
> else.
> + *
> + */
...
> +	}
> +
> +	/* When first item is added, we need to store its starting
> address */
> +	if (ctrl->index == 0)
...
> +
> +	/*
> +	 * Buffer cannot guarantee free space for u32? Better flush
> it to avoid
...
> +}
> +
> +static int __ov5648_write_reg_is_consecutive(struct i2c_client
> *client,
> +					     struct ov5648_write_ctrl
> *ctrl,
> +					     const struct ov5648_reg
> *next)
> +{
> +	if (ctrl->index == 0)
> +		return 1;
...
> +				  const struct ov5648_resolution
> *res)
> +{
...
> +
> +	ret = ov5648_read_reg(client, OV5648_16BIT,
> +			      OV5648_HORIZONTAL_OUTPUT_SIZE_H,
> &reg_val);
> +	if (ret)
> +		return ret;
...
> +}
> +
> +static long __ov5648_set_exposure(struct v4l2_subdev *sd, int
> coarse_itg,
...
> +
> +	ret = ov5648_write_reg(client, OV5648_8BIT,
> +			       OV5648_EXPOSURE_M, (exp_val >> 8) &
> 0xFF);
> +	if (ret)
> +		return ret;
> +
> +	ret = ov5648_write_reg(client, OV5648_8BIT,
> +			       OV5648_EXPOSURE_H, (exp_val >> 16) &
> 0x0F);
> +	if (ret)
> +		return ret;
> +
...
> +}
> +
> +static long ov5648_ioctl(struct v4l2_subdev *sd, unsigned int cmd,
> void *arg)
> +{
> +	switch (cmd) {
...
> +
> +/*
> + * This returns the exposure time being used. This should only be
> used
...
> +{
> +	struct ov5648_device *dev = to_ov5648_sensor(sd);
> +	struct camera_sensor_platform_data *pdata = dev-
> >platform_data;
...
> +	u16 val;
> +
> +	ret = ov5648_read_reg(client, OV5648_8BIT, OV5648_VFLIP_REG,
> &val);
> +	if (ret)
> +		return ret;
> +
...
> +	u16 val;
> +
> +	ret = ov5648_read_reg(client, OV5648_8BIT, OV5648_HFLIP_REG,
> &val);
> +	if (ret)
> +		return ret;
> +
...
> +	if (ret) {
> +		dev_err(&client->dev, "write global settings error:
> %d\n", ret);
> +		mutex_unlock(&dev->input_lock);
> +		return ret;
> +	}
...
> +
> +	if (R_gain > 0x400) {
> +		ov5648_write_reg(client, OV5648_8BIT, 0x5186, R_gain
> >> 8);
> +		ov5648_write_reg(client, OV5648_8BIT, 0x5187, R_gain
> & 0x00ff);
> +	}
> +	if (G_gain > 0x400) {
> +		ov5648_write_reg(client, OV5648_8BIT, 0x5188, G_gain
> >> 8);
> +		ov5648_write_reg(client, OV5648_8BIT, 0x5189, G_gain
> & 0x00ff);
> +	}
> +	if (B_gain > 0x400) {
> +		ov5648_write_reg(client, OV5648_8BIT, 0x518a, B_gain
> >> 8);
> +		ov5648_write_reg(client, OV5648_8BIT, 0x518b, B_gain
> & 0x00ff);
> +	}
> +	#ifdef OV5648_DEBUG_EN
> +	dev_dbg(&client->dev, "AWB: rgain=[%x], ggain=[%x],
> bgain=[%x]\n",
...
> +				B_gain = 0x400;
> +				G_gain = G_gain_B;
> +				R_gain = G_gain * RG_RADIO_TYPICAL /
> rg;
> +			} else {
> +				R_gain = 0x400;
> +				G_gain = G_gain_R;
> +				B_gain = G_gain * BG_RADIO_TYPICAL /
> bg;
> +			}
> +		}
> +	}
...
> +	}
> +
> +	ret = ov5648_write_reg_array(client, ov5648_res[dev-
> >fmt_idx].regs);
> +	if (ret)
...
> +	if (ret) {
> +		dev_err(&client->dev,
> +			"failed to get integration_factor, error:
> %d\n", ret);
> +		goto err;
> +	}
> +
...
> +	interval->interval.denominator = ov5648_res[dev-
> >fmt_idx].fps;
> +
> +	return 0;
> +}
...
> +		return -ENOMEM;
> +
> +	dev->camera_module = devm_kzalloc(&client->dev, len,
> GFP_KERNEL);
...
> +		dev_info(&client->dev, "CameraModule gmin variable is
> missing\n");
> +	}
> +
...
> +MODULE_LICENSE("GPL");
> diff --git a/drivers/staging/media/atomisp/i2c/ov5648.h
...
> +					* Bit[3:0] Bit[19:16] of
> exposure,
> +					* remaining 16 bits lies in
> +					* Reg0x3501&Reg0x3502
> +					*/
> +#define OV5648_EXPOSURE_M	0x3501	/* Bit[15:8] of
> exposure*/
> +#define OV5648_EXPOSURE_L	0x3502	/* Bit[7:0] of
> exposure*/
> +#define OV5648_AGC_H		0x350A	/* Bit[1:0] means
> Bit[9:8] of gain */
...
> +#define OV5648_VERTICAL_START_H		0x3802	/*
> Bit[11:8] */
> +#define OV5648_VERTICAL_START_L		0x3803	/*
> Bit[7:0] */
> +#define OV5648_HORIZONTAL_END_H		0x3804	/*
> Bit[11:8] */
> +#define OV5648_HORIZONTAL_END_L		0x3805	/*
> Bit[7:0] */
> +#define OV5648_VERTICAL_END_H		0x3806	/*
> Bit[11:8] */
> +#define OV5648_VERTICAL_END_L		0x3807	/*
> Bit[7:0] */
> +#define OV5648_HORIZONTAL_OUTPUT_SIZE_H	0x3808	/*
> Bit[3:0] */
> +#define OV5648_HORIZONTAL_OUTPUT_SIZE_L	0x3809	/*
> Bit[7:0] */
> +#define OV5648_VERTICAL_OUTPUT_SIZE_H	0x380a	/*
> Bit[3:0] */
> +#define OV5648_VERTICAL_OUTPUT_SIZE_L	0x380b	/*
> Bit[7:0] */
> +#define OV5648_TIMING_HTS_H		0x380C	/* HTS High
> 8-bit */
> +#define OV5648_TIMING_HTS_L		0x380D	/* HTS Low
> 8-bit */
> +#define OV5648_TIMING_VTS_H		0x380e	/* VTS High
> 8-bit */
> +#define OV5648_TIMING_VTS_L		0x380f	/* VTS Low
> 8-bit */
> +
...
> +	{OV5648_8BIT, 0x303d, 0x20},
> +
> +	{OV5648_8BIT, 0x350a, 0x00}, /* Def. analog gain = 0x040/16.0
> = 4x */
> +	{OV5648_8BIT, 0x350b, 0x40},
> +	{OV5648_8BIT, 0x3501, 0x45},/* Def. exposure = 0x45e0 (1118
> lines) */
...
> +	{OV5648_8BIT, 0x3814, 0x31},/* X subsample step: odd = 3,
> even = 1 */
> +	{OV5648_8BIT, 0x3815, 0x31},/* Y subsample step: odd = 3,
> even = 1 */
