...
Received: from mailout3.samsung.com ([203.254.224.33]:55678 "EHLO
	...
	with ESMTP id S1752780AbbIILUR (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Wed, 9 Sep 2015 07:20:17 -0400
Received: from epcpsbgr2.samsung.com
 (u142.gpu120.samsung.co.kr [203.254.230.142])
 by mailout3.samsung.com (Oracle Communications Messaging Server 7.0.5.31.0
 64bit (built May  5 2014))
 with ESMTP id <0NUE00D2OQ5DUG60@mailout3.samsung.com> for
...
Subject: [RFC PATCH v4 4/8] [media] videobuf2: Restructure vb2_buffer (3/3)
Date: Wed, 09 Sep 2015 20:19:53 +0900
Message-id: <1441797597-17389-5-git-send-email-jh1009.sung@samsung.com>
...
---
 ...
 drivers/media/platform/ti-vpe/vpe.c                |   83 ++++++------
 ...
 66 files changed, 880 insertions(+), 729 deletions(-)
...
index d82c2f2..de22557 100644
--- a/drivers/media/platform/ti-vpe/vpe.c
+++ b/drivers/media/platform/ti-vpe/vpe.c
@@ -376,27 +376,32 @@ struct vpe_ctx {
 	struct vpe_dev		*dev;
...
+
+	dma_addr_t		mv_buf_dma[2];	/* dma addrs of motion vector
+							in/out bufs */
+	void			*mv_buf[2];	/* virtual addrs of motion
+							vector bufs */
+	size_t			mv_buf_size;	/* current motion vector
+							buffer size */
+	struct vpdma_buf	mmr_adb;	/* shadow reg addr/data
+							block */
+	struct vpdma_buf	sc_coeff_h;	/* h coeff buffer */
+	struct vpdma_buf	sc_coeff_v;	/* v coeff buffer */
+	struct vpdma_desc_list	desc_list;	/* DMA descriptor list */
+
+	bool			deinterlacing;	/* using de-interlacer */
+	bool			load_mmrs;	/* have new shadow reg
+							values */
 
 	unsigned int		src_mv_buf_selector;
 };
@@ -988,7 +993,7 @@ static void add_out_dtd(struct vpe_ctx *ctx, int port)
 {
...
@@ -1025,11 +1030,12 @@ static void add_in_dtd(struct vpe_ctx *ctx, int port)
 {
...
+	int field = vbuf->field == V4L2_FIELD_BOTTOM;
 	...
 	dma_addr_t dma_addr;
 	u32 flags = 0;
@@ -1222,8 +1228,7 @@ static irqreturn_t vpe_irq(int irq_vpe, void *data)
...
@@ -1286,20 +1291,18 @@ static irqreturn_t vpe_irq(int irq_vpe, void *data)
...
@@ -1308,7 +1311,7 @@ static irqreturn_t vpe_irq(int irq_vpe, void *data)
...
@@ -1825,6 +1828,7 @@ static int vpe_queue_setup(struct vb2_queue *vq,
...
+	struct vb2_v4l2_buffer *vbuf = to_vb2_v4l2_buffer(vb);
 	...
 	int i, num_planes;
@@ -1836,10 +1840,10 @@ static int vpe_buf_prepare(struct vb2_buffer *vb)
...
@@ -1862,9 +1866,10 @@ static int vpe_buf_prepare(struct vb2_buffer *vb)
