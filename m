...
Received: from lb2-smtp-cloud2.xs4all.net ([194.109.24.25]:55845 "EHLO
	...
	by vger.kernel.org with ESMTP id S1754521AbcBEP2Z (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Fri, 5 Feb 2016 10:28:25 -0500
...
Subject: [PATCHv11 07/17] cec: add HDMI CEC framework
Date: Fri,  5 Feb 2016 16:27:50 +0100
Message-Id: <1454686080-39018-8-git-send-email-hverkuil@xs4all.nl>
...
---
 ...
 8 files changed, 5149 insertions(+)
...
index 30aca4a..df1f4bf 100644
--- a/MAINTAINERS
+++ b/MAINTAINERS
@@ -2699,6 +2699,20 @@ F:	drivers/net/ieee802154/cc2520.c
...
+		retries = data->msg.len == 1 ? 1 : 3;
+		/* Set the signal free time */
+		if (data->retries) {
+			/* should be >= 3 * 2.4ms for a retry */
+			signal_free_time_ms = 8;
+		} else if (data->new_initiator) {
+			/* should be >= 5 * 2.4ms for new initiator */
+			signal_free_time_ms = 12;
+		} else {
+			/*
+			 * should be >= 7 * 2.4ms for sending another frame
+			 * immediately after another.
+			 */
+			signal_free_time_ms = 17;
+		}
+		adap->ops->adap_transmit(adap, &retries, signal_free_time_ms,
+					 &data->msg);
+		/* If the hw can't retry, then we do it ourselves. */
+		if (retries == 0 && data->retries == 0)
+			data->retries = data->msg.len == 1 ? 1 : 3;
+unlock:
...
+		if (data->retries && (status & CEC_TX_STATUS_ARB_LOST)) {
+			data->retries--;
...
+	int res = 0;
+
+	/* Note: adap->lock is taken when this function is called */
...
+	msg->tx_arb_lost_cnt = msg->tx_nack_cnt = msg->tx_error_cnt = 0;
+	data->msg = *msg;
+	data->fh = fh;
+	data->adap = adap;
+	data->blocking = block;
...
+	 * another 100 ms as a margin. So if we don't get a completion
+	 * before that time something is really wrong and we time out.
+	 */
+	timeout = 5 * 400 + 100;
+	/* Add the requested timeout if we have to wait for a reply */
+	if (msg->reply)
+		timeout += msg->timeout;
...
+			if (msg->reply && (msg->tx_status & CEC_TX_STATUS_OK))
...
+		} else if (fh->monitor) {
+			err = -EPERM;
+		} else if (adap->cec_owner && adap->cec_owner != fh) {
+			err = -EBUSY;
+		} else {
+			if (block || !msg.reply)
+				fh = NULL;
...
+	adap->name = kstrdup_const(name, GFP_KERNEL);
+	if (!adap->name) {
+		kfree(adap);
+		return ERR_PTR(-ENOMEM);
+	}
...
+			struct cec_log_addrs *log_addrs, bool block);
+int _cec_enable(struct cec_adapter *adap, bool enable);
+/* cec_enable takes the cec_adapter lock and calls _cec_enable */
+int cec_enable(struct cec_adapter *adap, bool enable);
+u8 cec_sink_cdc_hpd(struct cec_adapter *adap, u8 input_port, u8 cdc_hpd_state);
+void cec_log_status(struct cec_adapter *adap);
+
...
+		       u8 arb_lost_cnt, u8 nack_cnt, u8 error_cnt);
+void cec_received_msg(struct cec_adapter *adap, struct cec_msg *msg);
+void cec_connected_inputs(struct cec_adapter *adap, u16 connected_inputs);
+
...
+#define CEC_TX_STATUS_ERROR		(1 << 3)
+#define CEC_TX_STATUS_MAX_RETRIES	(1 << 4)
+
