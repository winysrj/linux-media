...
Received: from aer-iport-2.cisco.com ([173.38.203.52]:8541 "EHLO
	...
	with ESMTP id S932306AbcCROQp (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Fri, 18 Mar 2016 10:16:45 -0400
...
Subject: [PATCHv13 10/17] DocBook/media: add CEC documentation
Date: Fri, 18 Mar 2016 15:07:09 +0100
Message-Id: <1458310036-19252-11-git-send-email-hans.verkuil@cisco.com>
...
+returns <constant>CEC_ADAP_DISABLED</constant>). <constant>CEC_ADAP_S_PHYS_ADDR</constant>
+can only be called by a file handle in initiator mode (see &CEC-S-MODE;).</para>
+
...
+available if <constant>CEC_CAP_STATE</constant> is set. <constant>CEC_ADAP_S_STATE</constant>
+can only be called by a file handle in initiator mode (see &CEC-S-MODE;).</para>
+
...
+available if <constant>CEC_CAP_VENDOR_ID</constant> is set. <constant>CEC_ADAP_S_VENDOR_ID</constant>
+can only be called by a file handle in initiator mode (see &CEC-S-MODE;).</para>
+
...
+  </refsect1>
+</refentry>
diff --git a/Documentation/DocBook/media/v4l/cec-ioc-adap-log-status.xml b/Documentation/DocBook/media/v4l/cec-ioc-adap-log-status.xml
new file mode 100644
index 0000000..c059531
--- /dev/null
+++ b/Documentation/DocBook/media/v4l/cec-ioc-adap-log-status.xml
@@ -0,0 +1,51 @@
...
+	    <entry>When in passthrough mode this message has to be handled by userspace,
+	    otherwise the core will return the vendor ID that was set with &CEC-ADAP-S-VENDOR-ID;.</entry>
+	  </row>
+	  <row>
...
+	  </row>
+	  <row>
+	    <entry><constant>CEC_MSG_GIVE_DEVICE_POWER_STATUS</constant></entry>
+	    <entry>When in passthrough mode this message has to be handled by userspace,
+	    otherwise the core will return the current power status.</entry>
+	  </row>
+	  <row>
