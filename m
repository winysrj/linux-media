...
Received: from smtp06.smtpout.orange.fr ([80.12.242.128]:45621 "EHLO
	smtp.smtpout.orange.fr" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1752835AbcHHTi7 (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Mon, 8 Aug 2016 15:38:59 -0400
...
Subject: [PATCH v3 11/14] media: platform: pxa_camera: make a standalone v4l2 device
Date: Mon,  8 Aug 2016 21:30:49 +0200
Message-Id: <1470684652-16295-12-git-send-email-robert.jarzmik@free.fr>
...
---
 ...
 drivers/media/platform/soc_camera/pxa_camera.c | 755 +++++++++++++++++--------
 include/linux/platform_data/media/camera-pxa.h |   2 +
 3 files changed, 520 insertions(+), 239 deletions(-)
...
index 7a810eb32cc0..1e15734ae287 100644
...
+	return ERR_PTR(ret);
 ...
 /*
@@ -259,7 +356,12 @@ static struct pxa_buffer *vb2_to_pxa_buffer(struct vb2_buffer *vb)
...
@@ -340,7 +442,7 @@ static void pxa_videobuf_set_actdma(struct pxa_camera_dev *pcdev,
...
@@ -674,51 +776,6 @@ static irqreturn_t pxa_camera_irq(int irq, void *data)
 	return IRQ_HANDLED;
 }
...
-
 ...
 {
@@ -744,12 +801,9 @@ static int test_platform_param(struct pxa_camera_dev *pcdev,
 	return -EINVAL;
 }
...
@@ -761,7 +815,7 @@ static void pxa_camera_setup_cicr(struct soc_camera_device *icd,
...
+	switch (pcdev->current_fmt->host_fmt->bits_per_sample) {
 	case 10:
 		dw = 4;
 		bpp = 0x40;
@@ -795,7 +849,7 @@ static void pxa_camera_setup_cicr(struct soc_camera_device *icd,
 	if (cicr0 & CICR0_ENB)
...
@@ -824,7 +878,7 @@ static void pxa_camera_setup_cicr(struct soc_camera_device *icd,
 	}
...
@@ -936,13 +990,12 @@ static int pxac_vb2_prepare(struct vb2_buffer *vb)
 {
...
+		vb2_set_plane_payload(vb, 0, pcdev->current_pix.sizeimage);
 	...
 	default:
 		return -EINVAL;
@@ -952,7 +1005,7 @@ static int pxac_vb2_prepare(struct vb2_buffer *vb)
...
+	WARN_ON(!pcdev->current_fmt);
 ...
 #ifdef DEBUG
 	/*
@@ -992,8 +1045,7 @@ static int pxac_vb2_queue_setup(struct vb2_queue *vq,
 				void *alloc_ctxs[])
 {
...
@@ -1053,13 +1105,12 @@ static struct vb2_ops pxac_vb2_ops = {
...
+	memset(vq, 0, sizeof(*vq));
 	...
 	vq->drv_priv = pcdev;
@@ -1068,6 +1119,7 @@ static int pxa_camera_init_videobuf2(struct vb2_queue *vq,
...
+	vq->lock = &pcdev->mlock;
 	...
 	dev_dbg(pcdev_to_dev(pcdev),
@@ -1079,18 +1131,15 @@ static int pxa_camera_init_videobuf2(struct vb2_queue *vq,
 /*
...
+				  pcdev->current_fmt->host_fmt->bits_per_sample,
 	...
 	if (ret < 0)
 		return ret;
@@ -1141,24 +1190,20 @@ static int pxa_camera_set_bus_param(struct soc_camera_device *icd)
...
@@ -1204,13 +1249,12 @@ static bool pxa_camera_packing_supported(const struct soc_mbus_pixelfmt *fmt)
...
-	struct pxa_cam *cam;
 	struct v4l2_subdev_mbus_code_enum code = {
 		.which = V4L2_SUBDEV_FORMAT_ACTIVE,
 		.index = idx,
@@ -1224,25 +1268,16 @@ static int pxa_camera_get_formats(struct soc_camera_device *icd, unsigned int id
...
-
 	...
 	case MEDIA_BUS_FMT_UYVY8_2X8:
 		formats++;
@@ -1284,10 +1319,22 @@ static int pxa_camera_get_formats(struct soc_camera_device *icd, unsigned int id
 	return formats;
 }
...
@@ -1297,86 +1344,44 @@ static int pxa_camera_check_frame(u32 width, u32 height)
 		(width & 0x01);
 }
...
@@ -1387,7 +1392,7 @@ static int pxa_camera_try_fmt(struct soc_camera_device *icd,
...
+	xlate = soc_mbus_xlate_by_fourcc(pcdev->user_formats, pixfmt);
 	if (!xlate) {
 		...
 		return -EINVAL;
@@ -1403,26 +1408,18 @@ static int pxa_camera_try_fmt(struct soc_camera_device *icd,
...
@@ -1431,20 +1428,74 @@ static int pxa_camera_try_fmt(struct soc_camera_device *icd,
 		return -EINVAL;
 	}
...
@@ -1452,13 +1503,215 @@ static int pxa_camera_querycap(struct soc_camera_host *ici,
 	return 0;
 }
...
@@ -1467,8 +1720,7 @@ static int pxa_camera_suspend(struct device *dev)
...
+	if (pcdev->sensor) {
 		...
 		if (ret == -ENOIOCTLCMD)
 			ret = 0;
@@ -1479,8 +1731,7 @@ static int pxa_camera_suspend(struct device *dev)
...
@@ -1489,8 +1740,7 @@ static int pxa_camera_resume(struct device *dev)
...
+	if (pcdev->sensor) {
 		...
 		if (ret == -ENOIOCTLCMD)
 			ret = 0;
@@ -1503,27 +1753,12 @@ static int pxa_camera_resume(struct device *dev)
 	return ret;
 }
...
+	struct device_node *remote, *np = dev->of_node;
 	...
 	int err = of_property_read_u32(np, "clock-frequency",
 				       &mclk_rate);
@@ -1534,13 +1769,13 @@ static int pxa_camera_pdata_from_dt(struct device *dev,
...
@@ -1575,6 +1810,15 @@ static int pxa_camera_pdata_from_dt(struct device *dev,
...
@@ -1593,6 +1837,7 @@ static int pxa_camera_probe(struct platform_device *pdev)
 	};
 	dma_cap_mask_t mask;
...
@@ -1619,10 +1864,14 @@ static int pxa_camera_probe(struct platform_device *pdev)
...
+		pcdev->asd.match.i2c.address = pcdev->pdata->sensor_i2c_address;
 	...
 	if (err < 0)
 		return err;
@@ -1654,6 +1903,7 @@ static int pxa_camera_probe(struct platform_device *pdev)
...
+	mutex_init(&pcdev->mlock);
 	...
 	/*
 	 * Request the regions.
@@ -1716,19 +1966,48 @@ static int pxa_camera_probe(struct platform_device *pdev)
 		goto exit_free_dma;
 	}
...
+	v4l2_device_unregister(&pcdev->v4l2_dev);
 ...
 exit_free_dma_u:
@@ -1740,16 +2019,16 @@ exit_free_dma_y:
