...
Received: from perceval.ideasonboard.com ([213.167.242.64]:60198 "EHLO
        perceval.ideasonboard.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1752056AbeERIPT (ORCPT
        <rfc822;linux-media@vger.kernel.org>);
        Fri, 18 May 2018 04:15:19 -0400
...
Date: Fri, 18 May 2018 11:15:39 +0300
Message-ID: <3216261.G88TfqiCiH@avalon>
In-Reply-To: <644920d91d1f69d659f233c6a52382d3f919babc.camel@ndufresne.ca>
References: <20180517160708.74811cfb@vento.lan> <644920d91d1f69d659f233c6a52382d3f919babc.camel@ndufresne.ca>
MIME-Version: 1.0
...
=20
ensure that we will continue having open-source camera support in the futur=
e.=20
...
So far, by pushing the V4L2 API as the proper way to support cameras, we ha=
ve=20
...
=20
consider. Of course, the drawback is that some vendors have simply decided =
not=20
to care about upstream camera support.

If we move the camera API one level up to userspace (and whether the API wi=
ll=20
be defined by Pipewire, by libv4l or by something else), we'll make it easi=
er=20
for vendors not to play along. My big question is how to prevent that. I th=
ink=20
there's still value there in mandating V4L2 as the only API for cameras, an=
d=20
...
minimum with proper documentation of custom ioctls used to configure the=20
hardware), then the community will have the opportunity to implement an ope=
n-
source 3A library. My main concern is thus about all component other than t=
he=20
3A library.
...
> > application in order to prepare the device for its usage (setup pipelin=
es,
...
> > used by the application to be a random number (on OMAP3 driver, typical=
ly,
...
> > cameras was added (with has OMAP3 SoC). It is required because the came=
ra
> > hardware on SoC comes with a media processor (ISP), with does a lot more
> > than just capturing, allowing complex algorithms to enhance image quali=
ty
> > in runtime.
> >=20
...
> > format, but they usually have their own proprietary ways to pack the da=
ta,
