...
Received: from mail-ew0-f220.google.com ([209.85.219.220]:55868 "EHLO
	mail-ew0-f220.google.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1032020Ab0B1UVT (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Sun, 28 Feb 2010 15:21:19 -0500
Received: by ewy20 with SMTP id 20so917483ewy.21
        for <linux-media@vger.kernel.org>; Sun, 28 Feb 2010 12:21:17 -0800 (PST)
Subject: [PATCH 2/2] New driver for MI2020 sensor with GL860 bridge
From: Olivier Lorin <olorin75@gmail.com>
To: V4L Mailing List <linux-media@vger.kernel.org>
Cc: Jean-Francois Moine <moinejf@free.fr>
Content-Type: text/plain
Date: Sun, 28 Feb 2010 21:21:14 +0100
Message-Id: <1267388474.1854.32.camel@miniol>
Mime-Version: 1.0
...
diff
-rupN ../i/linux/drivers/media/video/gspca/gl860/gl860.c ./linux/drivers/media/video/gspca/gl860/gl860.c
--- ../i/linux/drivers/media/video/gspca/gl860/gl860.c	2010-02-28
15:04:52.000000000 +0100
+++ ./linux/drivers/media/video/gspca/gl860/gl860.c	2010-02-28
14:55:23.000000000 +0100
@@ -93,7 +93,6 @@ SD_SETGET(lightsource)
 /* control table */
...
@@ -123,8 +122,6 @@ static int gl860_build_control_table(str
...
@@ -191,19 +188,6 @@ static const struct sd_desc sd_desc_mi20
...
@@ -348,8 +332,6 @@ static int sd_config(struct gspca_dev *g
 		sd->sensor = ID_OV9655;
...
@@ -373,13 +355,6 @@ static int sd_config(struct gspca_dev *g
...
@@ -774,8 +749,6 @@ static int gl860_guess_sensor(struct gsp
...
diff
-rupN ../i/linux/drivers/media/video/gspca/gl860/gl860.h ./linux/drivers/media/video/gspca/gl860/gl860.h
--- ../i/linux/drivers/media/video/gspca/gl860/gl860.h	2010-02-28
...
diff
-rupN ../i/linux/drivers/media/video/gspca/gl860/gl860-mi2020.c ./linux/drivers/media/video/gspca/gl860/gl860-mi2020.c
--- ../i/linux/drivers/media/video/gspca/gl860/gl860-mi2020.c	2010-02-28
...
+static u8 dat_lightsrc1[] = {0x8c, 0xa2, 0x0c};
+
...
\x01";
@@ -352,6 +300,7 @@ void mi2020_init_settings(struct gspca_d
 	sd->vcur.hue        =  0;
 	sd->vcur.saturation = 60;
 	sd->vcur.whitebal   = 50;
+	sd->vcur.lightsource = 0;
 	sd->vcur.mirror = 0;
 	sd->vcur.flip   = 0;
 	sd->vcur.AC50Hz = 1;
@@ -364,15 +313,10 @@ void mi2020_init_settings(struct gspca_d
...
@@ -385,51 +329,9 @@ void mi2020_init_settings(struct gspca_d
...
@@ -442,8 +344,16 @@ static int mi2020_init_at_startup(struct
...
@@ -451,17 +361,17 @@ static int mi2020_init_pre_alt(struct gs
 {
...
@@ -473,10 +383,10 @@ static int mi2020_init_post_alt(struct g
 	struct sd *sd = (struct sd *) gspca_dev;
 	s32 reso = gspca_dev->cam.cam_mode[(s32) gspca_dev->curr_mode].priv;
...
+	s32 lightsrc = sd->vcur.lightsource;
 
 	u8 dat_freq2[] = {0x90, 0x00, 0x80};
 	u8 dat_multi1[] = {0x8c, 0xa7, 0x00};
@@ -485,6 +395,7 @@ static int mi2020_init_post_alt(struct g
 	u8 dat_multi4[] = {0x90, 0x00, 0x00};
 	u8 dat_hvflip2[] = {0x90, 0x04, 0x6c};
 	u8 dat_hvflip4[] = {0x90, 0x00, 0x24};
+	u8 dat_lightsrc2[] = {0x90, 0x00, 0x00};
 	u8 c;
 
 	sd->nbIm = -1;
@@ -492,23 +403,26 @@ static int mi2020_init_post_alt(struct g
...
+		dat_lightsrc2[2] = 0x17;
+	} else if (lightsrc == 1) {
+		dat_multi4[2] = dat_multi2[2] = 0;
+		dat_lightsrc2[2] = 0x35;
+	} else if (lightsrc == 2) {
...
@@ -524,8 +438,7 @@ static int mi2020_init_post_alt(struct g
...
+		fetch_idxdata(gspca_dev, tbl_init_post_alt_low1,
 		...
 		if (reso == IMAGE_800)
@@ -535,87 +448,10 @@ static int mi2020_init_post_alt(struct g
...
+		msleep(120);
 	...
 	case IMAGE_1280:
@@ -644,108 +480,62 @@ static int mi2020_init_post_alt(struct g
 					3, "\x90\x04\xb0");
 		}
...
+	ctrl_out(gspca_dev, 0x40, 3, 0x7a00, 0x0033, 3, dat_multi3);
+	ctrl_out(gspca_dev, 0x40, 3, 0x7a00, 0x0033, 3, dat_multi4);
+	ctrl_out(gspca_dev, 0x40, 3, 0x7a00, 0x0033, 3, dat_lightsrc1);
+	ctrl_out(gspca_dev, 0x40, 3, 0x7a00, 0x0033, 3, dat_lightsrc2);
+	ctrl_out(gspca_dev, 0x40, 3, 0x7a00, 0x0033, 3, dat_multi5);
+	ctrl_out(gspca_dev, 0x40, 3, 0x7a00, 0x0033, 3, dat_multi6);
+	msleep(7);
+	ctrl_in(gspca_dev, 0xc0, 2, 0x0000, 0x0000, 1, &c);
...
+	sd->vold.lightsource = lightsrc;
 
 	mi2020_camera_settings(gspca_dev);
 
@@ -770,9 +560,10 @@ static int mi2020_configure_alt(struct g
 	return 0;
 }
...
@@ -783,6 +574,7 @@ static int mi2020_camera_settings(struct
 	s32 mirror = (((sd->vcur.mirror > 0) ^ sd->mirrorMask) > 0);
 	s32 flip   = (((sd->vcur.flip   > 0) ^ sd->mirrorMask) > 0);
 	s32 freq   = (sd->vcur.AC50Hz > 0);
+	s32 lightsrc = sd->vcur.lightsource;
...
@@ -793,6 +585,7 @@ static int mi2020_camera_settings(struct
 	u8 dat_multi4[] = {0x90, 0x00, 0x00};
 	u8 dat_hvflip2[] = {0x90, 0x04, 0x6c};
 	u8 dat_hvflip4[] = {0x90, 0x00, 0x24};
+	u8 dat_lightsrc2[] = {0x90, 0x00, 0x00};
 	...
 	if (sd->nbIm < 4) {
@@ -810,67 +603,89 @@ static int mi2020_camera_settings(struct
 		msleep(20);
 	}
 
+	if (lightsrc != sd->vold.lightsource) {
+		sd->vold.lightsource = lightsrc;
+		if (lightsrc < 0 || lightsrc > sd->vmax.lightsource)
+			lightsrc = 0;
+
+		dat_multi1[2] = 0x9d;
+		dat_multi3[2] = dat_multi1[2] + 1;
...
+		dat_multi4[2] = dat_multi2[2] = cntr * 0x10 + 2 - gam;
 		...
 		ctrl_out(gspca_dev, 0x40, 3, 0x7a00, 0x0033, 3, dat_multi2);
 		ctrl_out(gspca_dev, 0x40, 3, 0x7a00, 0x0033, 3, dat_multi3);
@@ -879,14 +694,14 @@ static int mi2020_camera_settings(struct
 		ctrl_out(gspca_dev, 0x40, 3, 0x7a00, 0x0033, 3, dat_multi6);
 	}
...
+		dat_multi4[2] = dat_multi2[2] = backlight;
 		...
 		ctrl_out(gspca_dev, 0x40, 3, 0x7a00, 0x0033, 3, dat_multi2);
 		ctrl_out(gspca_dev, 0x40, 3, 0x7a00, 0x0033, 3, dat_multi3);
@@ -895,20 +710,6 @@ static int mi2020_camera_settings(struct
 		ctrl_out(gspca_dev, 0x40, 3, 0x7a00, 0x0033, 3, dat_multi6);
 	}
...
@@ -929,9 +730,6 @@ static int mi2020_camera_settings(struct
