...
Received: from nblzone-211-213.nblnetworks.fi ([83.145.211.213]:39558 "EHLO
        hillosipuli.retiisi.org.uk" rhost-flags-OK-OK-OK-FAIL)
        by vger.kernel.org with ESMTP id S940166AbdEXMvv (ORCPT
        <rfc822;linux-media@vger.kernel.org>);
        Wed, 24 May 2017 08:51:51 -0400
Date: Wed, 24 May 2017 15:51:12 +0300
...
Message-ID: <20170524125111.GJ29527@valkosipuli.retiisi.org.uk>
...
> +};
> +
> +#define OV13858_GET_PIXEL_RATE(index) (link_freq_configs[index].pixel_rate)

Please refer to link_freq_configs directly. No really reason to have a
macro.
...
> +
> +/*
> + * Get the index of matched bayer format
> + */
> +static int ov13858_get_bayer_format(u32 format)
> +{
> +	int i;
> +
...
> +
> +/*
> + * This function is called only after powerup to change the default bayer order
...
> +	val++;
> +
> +	return ov13858_write_reg(ov13858, reg, OV13858_REG_VALUE_16BIT, val);
> +}
> +
> +/*
...
Instead of changing the vertical blanking interval implicitly, could you do it
explicitly though the VBLANK control instead?

As you do already control the vertical sync and provide the pixel rate
control, how about adding a HBLANK control as well? I suppose it could be
added later on as well. And presumably will be read only.
...
If you want group hold for that, too, we need a new callback (or two) for
the control handler I believe.
...
usage_count directly does not look like something a driver should be doing.
...
It'd be nice to have ov13858_do_get_pad_format() defined right above this
function.
