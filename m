...
Received: from bear.ext.ti.com ([192.94.94.41]:37883 "EHLO bear.ext.ti.com"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
	id S1755930Ab0KJNHW convert rfc822-to-8bit (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Wed, 10 Nov 2010 08:07:22 -0500
...
Date: Wed, 10 Nov 2010 18:37:00 +0530
Subject: RE: [PATCH 1/6] davinci vpbe: V4L2 display driver for DM644X SoC
Message-ID: <E0D41E29EB0DAC4E9F3FF173962E9E9402DC1A7C77@dbde02.ent.ti.com>
In-Reply-To: <201011090849.40940.hverkuil@xs4all.nl>
...
Hans,
 Thank you for the review. I have taken care of the points you mentioned. The name comparison is unnecessary. I have also replaced the native struct definitions with those of v4l2. Request you to go through the rest of the patches so I can send the set once again.

Thanks and Regards,
-Manju

On Tue, Nov 09, 2010 at 13:19:40, Hans Verkuil wrote:
> On Monday, November 08, 2010 15:54:16 Manjunath Hadli wrote:
...
> > +                   } else {
>
> 'else' is not needed since the 'if' part continues to the 'for' loop.
>
> > +                           /*
...
>
> camelCase! Use cur_frm and next_frm instead (same as the dm6467 driver).
>
...
>
> Just a general remark: a lot of these TI drivers are all very similar, probably
> because they all have similar IP in them. Would it be possible to refactor those
> identical parts into some common code? This is not a requirement for this patch
> series, but some work in that direction would be helpful I think.
>
> > +           } else {
> > +                   /*
> > +                    * Interlaced mode
...
> > +                                                       next, struct
>
> Please, don't split up a line like this. Use something like this instead:
>
>                       layer->nextFrm = list_entry(layer->dma_queue.next,
>                                               struct videobuf_buffer,
>                                               queue);
>
> The 80 char limit is a warning only, don't blindly follow it.
...
> > +                   if (!strcmp(vpbe_dev->current_timings.name,
> > +                   VID_ENC_STD_NTSC) ||
> > +                   !strcmp(vpbe_dev->current_timings.name,
> > +                   VID_ENC_STD_PAL)) {
...
> > +};
> > +
> > +/* vpbe output interface types */
> > +enum vpbe_if_types {
> > +   VPBE_ANALOG_TV_IF,
> > +   VPBE_DIGITAL_IF_YCC8,
> > +   VPBE_DIGITAL_IF_YCC16,
> > +   VPBE_DIGITAL_IF_SRGB,
> > +   VPBE_DIGITAL_IF_PRGB,
> > +};
> > +
> > +/* RGB modes */
> > +enum vpbe_rgb_modes {
...
> > +};
> > +
> > +struct vpbe_if_params {
> > +   enum vpbe_if_types if_type;
> > +   /*
> > +    * Bool to indicate if Y and C outputs to be swapped. This is applicable
> > +    * for YCC8 and YCC16
> > +    */
> > +   unsigned int swap_yc:1;
> > +   /* Bool to indicate if BT.656 is enabled. Applicable for YCC8/YCC16 */
> > +   unsigned int bt656:1;
> > +   /* Enable RGB666. Applicable for PRGB if type */
> > +   enum vpbe_rgb_modes rgb_mode;
> > +};
> > +
> > +/*
> > + * struct vpbe_enc_fract
...
> > +};
>
> What's wrong with struct v4l2_fract?
>
> > +
> > +/*
...
>
> Hmm. Isn't this the same format, just with a different colorspace?
>
...
>
> Not sure if this is such a great idea. Why not use an enum instead rather than
> having to strcmp these strings every time? Or use the DV preset values (extending
> them as needed).
>
> Regards,
>
