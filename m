...
Received: from mail.gmx.net ([213.165.64.20]:38089 "HELO mail.gmx.net"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with SMTP
	id S1752960AbZG2TE4 (ORCPT <rfc822;linux-media@vger.kernel.org>);
	Wed, 29 Jul 2009 15:04:56 -0400
Date: Wed, 29 Jul 2009 21:05:04 +0200 (CEST)
...
Subject: [PATCH 4/4 v2] soc-camera: V4L2 API compliant scaling (S_FMT) and
 cropping (S_CROP)
In-Reply-To: <Pine.LNX.4.64.0907291713590.4983@axis700.grange>
Message-ID: <Pine.LNX.4.64.0907292059010.4983@axis700.grange>
References: <Pine.LNX.4.64.0907291640010.4983@axis700.grange>
 <Pine.LNX.4.64.0907291713590.4983@axis700.grange>
MIME-Version: 1.0
...
Changes since v1: fix a couple of wrong pointers in dev_dbg().
 ...
 15 files changed, 1719 insertions(+), 660 deletions(-)
...
@@ -562,10 +650,9 @@ static int mt9m001_video_probe(struct soc_camera_device *icd,
...
+	dev_dbg(&icd->dev, "Video removed: %p, %p\n",
 	...
 	if (icl->free_bus)
 		icl->free_bus(icl);
@@ -584,8 +671,11 @@ static struct v4l2_subdev_core_ops mt9m001_subdev_core_ops = {
...
@@ -627,15 +717,13 @@ static int mt9m001_probe(struct i2c_client *client,
...
+	dev_dbg(&icd->dev, "Video removed: %p, %p\n",
 	...
 	if (icl->free_bus)
 		icl->free_bus(icl);
@@ -707,8 +788,11 @@ static struct v4l2_subdev_core_ops mt9v022_subdev_core_ops = {
...
@@ -751,16 +835,13 @@ static int mt9v022_probe(struct i2c_client *client,
...
@@ -850,7 +854,7 @@ static u32 mclk_get_divisor(struct platform_device *pdev,
...
+	*size = PAGE_ALIGN(icd->user_width * icd->user_height *
 	...
 	if (0 == *count)
@@ -177,8 +185,9 @@ static void free_buffer(struct videobuf_queue *vq,
...
+	dev_dbg(dev, "%s (vb=0x%p) 0x%08lx %zd\n", __func__,
 	...
 	if (in_interrupt())
@@ -186,7 +195,7 @@ static void free_buffer(struct videobuf_queue *vq,
...
@@ -217,7 +226,7 @@ static void sh_mobile_ceu_capture(struct sh_mobile_ceu_dev *pcdev)
...
@@ -226,12 +235,12 @@ static void sh_mobile_ceu_capture(struct sh_mobile_ceu_dev *pcdev)
...
@@ -265,12 +274,12 @@ static int sh_mobile_ceu_videobuf_prepare(struct videobuf_queue *vq,
...
@@ -455,18 +464,6 @@ static unsigned int size_dst(unsigned int src, unsigned int scale)
...
-
 ...
 {
 	u16 scale;
@@ -486,65 +483,46 @@ static u16 calc_scale(unsigned int src, unsigned int *dst)
...
+	in_height = rect->height;
 	if (pcdev->is_interlaced) {
 		height /= 2;
 		in_height /= 2;
@@ -552,10 +530,17 @@ static void sh_mobile_ceu_set_rect(struct soc_camera_device *icd,
 		cdwdr_width *= 2;
 	}
...
@@ -667,8 +652,8 @@ static int sh_mobile_ceu_set_bus_param(struct soc_camera_device *icd,
...
@@ -691,11 +676,10 @@ static int sh_mobile_ceu_set_bus_param(struct soc_camera_device *icd,
...
@@ -748,6 +732,7 @@ static const struct soc_camera_data_format sh_mobile_ceu_formats[] = {
...
@@ -762,7 +747,6 @@ static int sh_mobile_ceu_get_formats(struct soc_camera_device *icd, int idx,
 			return -ENOMEM;
...
@@ -797,8 +781,7 @@ static int sh_mobile_ceu_get_formats(struct soc_camera_device *icd, int idx,
...
@@ -840,176 +823,487 @@ static bool is_inside(struct v4l2_rect *r1, struct v4l2_rect *r2)
...
@@ -1022,121 +1316,137 @@ static int sh_mobile_ceu_set_fmt(struct soc_camera_device *icd,
 	struct sh_mobile_ceu_dev *pcdev = ici->priv;
 	struct sh_mobile_ceu_cam *cam = icd->host_priv;
 	struct v4l2_pix_format *pix = &f->fmt.pix;
