...
Received: from mx07-00178001.pphosted.com ([62.209.51.94]:58163 "EHLO
	mx07-00178001.pphosted.com" rhost-flags-OK-OK-OK-OK)
	by vger.kernel.org with ESMTP id S1752569AbbFHH16 convert rfc822-to-8bit
	(ORCPT <rfc822;linux-media@vger.kernel.org>);
	Mon, 8 Jun 2015 03:27:58 -0400
From: Fabien DESSENNE <fabien.dessenne@st.com>
To: Mauro Carvalho Chehab <mchehab@osg.samsung.com>
CC: "linux-media@vger.kernel.org" <linux-media@vger.kernel.org>,
	Benjamin Gaignard <benjamin.gaignard@linaro.org>,
	Hugues FRUCHET <hugues.fruchet@st.com>
Date: Mon, 8 Jun 2015 09:26:46 +0200
Subject: RE: [PATCH v3 2/3] [media] bdisp: 2D blitter driver using v4l2
 mem2mem framework
Message-ID: <15ED7CB7B68B4D4C96C7D27A1A23941201B9EFDE63@SAFEX1MAIL2.st.com>
References: <1431446531-11492-1-git-send-email-fabien.dessenne@st.com>
	<1431446531-11492-3-git-send-email-fabien.dessenne@st.com>
 <20150606074302.17e3696f@recife.lan>
In-Reply-To: <20150606074302.17e3696f@recife.lan>
...
I will take care of it [fabien.dessenne@st.com]
As I am not comfortable with the maintainer list update procedure, may I ask you to update the MAINTAINERS file?
...
> > + * Authors: Fabien Dessenne <fabien.dessenne@st.com> for
> STMicroelectronics.
...
> > +{
> > +   int size = (BDISP_HF_NB * NB_H_FILTER) + (BDISP_VF_NB *
> NB_V_FILTER);
> > +
...
> > +   size = (BDISP_HF_NB * NB_H_FILTER) + (BDISP_VF_NB *
> NB_V_FILTER);
> > +   dma_set_attr(DMA_ATTR_WRITE_COMBINE, &attrs);
...
> > + * 0 on success
> > + */
> > +static int bdisp_hw_get_hv_inc(struct bdisp_ctx *ctx, u16 *h_inc, u16
> *v_inc)
> > +{
...
> > + *
> > + * Check which blitter operations are expected and sets the scaling
> increments
> > + *
> > + * RETURNS:
> > + * 0 on success
> > + */
> > +static int bdisp_hw_get_op_cfg(struct bdisp_ctx *ctx, struct bdisp_op_cfg
> *c)
> > +{
> > +   struct device *dev = ctx->bdisp_dev->dev;
...
> > +           ret = (BDISP_NV12 << BLT_TTY_COL_SHIFT) |
> BLT_TTY_BIG_END;
> > +           break;
> > +   case V4L2_PIX_FMT_RGB565:
> > +           ret = (BDISP_RGB565 << BLT_TTY_COL_SHIFT);
> > +           break;
> > +   case V4L2_PIX_FMT_XBGR32: /* This V4L format actually refers to
> xRGB */
> > +           ret = (BDISP_XRGB8888 << BLT_TTY_COL_SHIFT);
> > +           break;
> > +   case V4L2_PIX_FMT_RGB24:  /* RGB888 format */
> > +           ret = (BDISP_RGB888 << BLT_TTY_COL_SHIFT) |
> BLT_TTY_BIG_END;
> > +           break;
> > +   case V4L2_PIX_FMT_ABGR32: /* This V4L format actually refers to
> ARGB */
> > +
> > +   default:
> > +           ret = (BDISP_ARGB8888 << BLT_TTY_COL_SHIFT) |
> BLT_TTY_ALPHA_R;
> > +           break;
> > +   }
> > +
...
> > +                           enum bdisp_target_plan t_plan, int
> src_x_offset)
> > +{
> > +   struct bdisp_frame *src = &ctx->src;
> > +   struct bdisp_frame *dst = &ctx->dst;
...
> > +           node->ins = BLT_INS_S1_OFF | BLT_INS_S2_MEM |
> BLT_INS_S3_OFF;
> > +           break;
> > +   case 2:
> > +           /* Src3 = Y
...
> > +           h_inc = cfg->h_inc;
> > +           v_inc = cfg->v_inc;
> > +           if (!cfg->src_420 && cfg->dst_420 && (t_plan ==
> BDISP_CBCR)) {
...
> > +#define BLT_INS_S1_MEM          0x00000001      /* src1 fetched from
> memory */
...
> > +#define BLT_INS_S2_MEM          0x00000008      /* src2 fetched from
> memory */
...
> > +#define BLT_INS_S3_MEM          0x00000020      /* src3 fetched from
> memory */
...
> > +};
> > +
> > +static inline void bdisp_ctx_state_lock_set(u32 state, struct bdisp_ctx
> *ctx)
> > +{
> > +   unsigned long flags;
> > +
...
> > +}
> > +
> > +static inline void bdisp_ctx_state_lock_clear(u32 state, struct bdisp_ctx
> *ctx)
> > +{
> > +   unsigned long flags;
> > +
...
> > +           bdisp_ctx_state_lock_clear(BDISP_CTX_STOP_REQ |
> BDISP_CTX_ABORT,
> > +                                      ctx);
...
> > +   struct bdisp_frame *frame = ctx_get_frame(ctx, vb->vb2_queue-
> >type);
> > +
> > +   if (IS_ERR(frame)) {
> > +           dev_err(ctx->bdisp_dev->dev, "Invalid frame (%p)\n",
...
> > +}
> > +
> > +static int bdisp_enum_fmt(struct file *file, void *fh, struct v4l2_fmtdesc
> *f)
> > +{
> > +   struct bdisp_ctx *ctx = fh_to_ctx(fh);
> > +   const struct bdisp_fmt *fmt;
...
> > +           dev_dbg(ctx->bdisp_dev->dev, "Not supported for
> capture\n");
> > +           return -EINVAL;
> > +   }
> > +
...
> > +}
> > +
> > +static int bdisp_streamon(struct file *file, void *fh, enum v4l2_buf_type
> type)
> > +{
> > +   struct bdisp_ctx *ctx = fh_to_ctx(fh);
> > +
...
> > +                                test_bit(ST_M2M_SUSPENDED, &bdisp-
> >state),
> > +                                BDISP_WORK_TIMEOUT);
> > +
...
> > +
> > +   if (dev->of_node)
> > +           bdisp->id = of_alias_get_id(pdev->dev.of_node,
> BDISP_NAME);
> > +   else
> > +           bdisp->id = pdev->id;
> > +
...
> > +   }
> > +
> > +   ret = devm_request_threaded_irq(dev, res->start,
> bdisp_irq_handler,
...
> > +#define MAX_NB_NODE             (MAX_OUTPUT_PLANES *
