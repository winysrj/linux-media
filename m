...
Received: from mailout4.w1.samsung.com ([210.118.77.14]:29713 "EHLO
	mailout4.w1.samsung.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1752046Ab1IPRFh (ORCPT
...
Received: from euspt2 ([210.118.77.14]) by mailout4.w1.samsung.com
 ...
 with ESMTP id <0LRM00JUPKTBUD50@mailout4.w1.samsung.com> for
 linux-media@vger.kernel.org; Fri, 16 Sep 2011 18:05:35 +0100 (BST)
Received: from linux.samsung.com ([106.116.38.10])
 by spt2.w1.samsung.com (iPlanet Messaging Server 5.2 Patch 2 (built Jul 14
 2004)) with ESMTPA id <0LRM00K6VKTAOD@spt2.w1.samsung.com> for
 linux-media@vger.kernel.org; Fri, 16 Sep 2011 18:05:35 +0100 (BST)
Date: Fri, 16 Sep 2011 19:05:30 +0200
From: Sylwester Nawrocki <s.nawrocki@samsung.com>
Subject: [PATCH 3/3] v4l: Add v4l2 sub-device driver for S5K6AAFX sensor
In-reply-to: <1316192730-18099-1-git-send-email-s.nawrocki@samsung.com>
To: linux-media@vger.kernel.org
Cc: m.szyprowski@samsung.com, kyungmin.park@samsung.com,
	laurent.pinchart@ideasonboard.com, s.nawrocki@samsung.com,
	sw0312.kim@samsung.com, riverful.kim@samsung.com
Message-id: <1316192730-18099-4-git-send-email-s.nawrocki@samsung.com>
...
+#define REG_USER_GLAMOUR		0x01ec
+#define REG_G_SPEC_EFFECTS		0x01ee
+#define REG_G_ENABLE_PREV		0x01f0
+#define REG_G_ENABLE_PREV_CHG		0x01f2
+#define REG_G_ENABLE_CAPT		0x01f4
+#define REG_G_ENABLE_CAPT_CHG		0x01f6
+#define REG_G_NEW_CFG_SYNC		0x01f8
+
+#define REG_G_PREV_IN_WIDTH		0x01fa
+#define REG_G_PREV_IN_HEIGHT		0x01fc
+#define REG_G_PREV_IN_XOFFS		0x01fe
+#define REG_G_PREV_IN_YOFFS		0x0200
+#define REG_G_CAPT_IN_WIDTH		0x0202
+#define REG_G_CAPT_IN_HEIGHT		0x0204
+#define REG_G_CAPT_IN_XOFFS		0x0206
+#define REG_G_CAPT_IN_YOFFS		0x0208
...
+#define REG_G_ACTIVE_CAPT_CFG		0x0224
+#define REG_G_CAPT_CFG_CHG		0x0226
+#define REG_G_CAPT_CFG_ERROR		0x0228
+
+#define REG_SLEEP_MODE			0x022e
+#define REG_SLEEP_MODE_CHG		0x0230
+
...
+#define REG_P_SHARPBLUR(n)		PREG(n, 0x025a)
+#define REG_P_GLAMOUR(n)		PREG(n, 0x025c)
+#define REG_P_COLORTEMP(n)		PREG(n, 0x025e)
+#define REG_P_DEVGAMMAINDEX(n)		PREG(n, 0x0260)
+#define REG_P_PREV_MIRROR(n)		PREG(n, 0x0262)
+#define REG_P_CAPT_MIRROR(n)		PREG(n, 0x0264)
+
+/* Extended image property controls */
+#define REG_SF_USR_LEI_L		0x03c0
+#define REG_SF_USR_LEI_H		0x03c2
+#define REG_SF_USR_LEI_CHG		0x03c4
...
+
+#define REG_SF_ANALOG_GAIN		0x03f2
+#define REG_SF_ANALOG_CHG		0x03f4
+#define REG_SF_DIG_GAIN			0x03f6
+#define REG_SF_DIG_CHG			0x04f8
+
...
+	/* TODO: revisit the colorspace (range is 16-240) */
...
+}
+
+/* Write the command register. Assume Command_Wr_addH = 0x7000. */
...
+}
+
+/* Read the command register. Assume Command_Rd_addH = 0x7000. */
...
+}
+
+static int s5k6aa_bulk_write(struct v4l2_subdev *sd,
...
+}
+
+static int s5k6aa_preview_cfg_status(struct i2c_client *client)
+{
+	u16 error = 0;
+
+	/* Null 'error' means the configuration is applied successfully */
...
+static int s5k6aa_set_mirror(struct s5k6aa *s5k6aa, int value)
+{
+	struct i2c_client *client = v4l2_get_subdevdata(&s5k6aa->sd);
+	struct s5k6aa_preset *preset = s5k6aa->preset;
+
+	u32 hflip = value ^ preset->inv_hflip;
+	u32 vflip = s5k6aa->ctrls.vflip->val ^ preset->inv_vflip;
+
...
+				      struct s5k6aa_preset *p, bool apply)
+{
+	struct i2c_client *client = v4l2_get_subdevdata(&s5k6aa->sd);
+	int ret;
+
+	if (WARN_ON(p->pixfmt == NULL))
+		return -EINVAL;
+
+	ret = s5k6aa_write(client, REG_P_OUT_WIDTH(p->index),
+			   p->out_size.width);
+	if (!ret)
+		ret = s5k6aa_write(client, REG_P_OUT_HEIGHT(p->index),
+				   p->out_size.height);
+	if (!ret)
+		ret = s5k6aa_write(client, REG_P_FMT(p->index),
+				   p->pixfmt->reg_p_fmt);
+	if (!ret && apply) {
+		ret = s5k6aa_write(client, REG_G_PREV_CFG_CHG, 1);
+		if (!ret)
+			ret = s5k6aa_preview_cfg_status(client);
+	}
+
+	return ret;
+}
...
+{
+	/* TODO: read and check REG_G_NEW_CFG_SYNC with timeout */
+	int ret = s5k6aa_write(client, REG_G_PREV_CFG_CHG, 1);
+	return ret ? ret : s5k6aa_write(client, REG_G_NEW_CFG_SYNC, 1);
+}
+
+/**
...
+
+	ret = s5k6aa_set_output_framefmt(s5k6aa, p, false);
+	if (!ret)
+		ret = s5k6aa_write(client, REG_P_MAX_OUT_RATE(p->index),
+				   S5K6AA_MAX_PCLK_FREQ);
+	if (!ret)
+		ret = s5k6aa_write(client, REG_P_MIN_OUT_RATE(p->index),
+				   S5K6AA_MIN_PCLK_FREQ);
+	if (!ret)
+		ret = s5k6aa_write(client, REG_P_FR_RATE_TYPE(p->index),
+				   p->frame_rate_type);
+	if (!ret)
+		ret = s5k6aa_write(client, REG_P_FR_RATE_Q_TYPE(p->index),
+				   frame_rate_q);
+	if (!ret)
+		ret = s5k6aa_write(client, REG_P_MAX_FR_TIME(p->index),
+				   s5k6aa->fiv->reg_fr_time + 15);
+	if (!ret)
+		ret = s5k6aa_write(client, REG_P_MIN_FR_TIME(p->index),
+				   s5k6aa->fiv->reg_fr_time - 15);
+	if (!ret)
+		s5k6aa_sync_preview_preset(client, 0);
+
+	if (!ret) {
+		ret = s5k6aa_preview_cfg_status(client);
...
+	if (ret)
+		return ret;
+	ret = s5k6aa_bulk_write(sd, s5k6aa_analog_config);
+	if (ret)
+		return ret;
+	msleep(100);
+
...
+	s5k6aa->power += on ? 1 : -1;
+
+	if (WARN_ON(s5k6aa->power < 0) || !on != !s5k6aa->power)
+		goto done;
+
+	if (on) {
...
+		return ret;
+
+	return v4l2_ctrl_handler_setup(&s5k6aa->ctrls.handler);
+}
+
...
+	int ret;
+
+	ret = s5k6aa_write(c, REG_G_ENABLE_PREV, !!enable);
+	if (!ret)
+		ret = s5k6aa_write(c, REG_G_ENABLE_PREV_CHG, 1);
+	if (!ret)
...
+	fse->min_height = S5K6AA_WIN_WIDTH_MAX;
+
+	return 0;
+}
+
+static const struct s5k6aa_pixfmt *
+s5k6aa_try_format(struct s5k6aa *s5k6aa, struct v4l2_mbus_framefmt *mf)
+{
+	int i = ARRAY_SIZE(s5k6aa_formats);
+
...
+			      S5K6AA_WIN_HEIGHT_MAX, 1, 0);
+	while (--i)
+		if (mf->code == s5k6aa_formats[i].code)
+			break;
+
+	mf->colorspace	= s5k6aa_formats[i].colorspace;
...
+		err = s5k6aa_write(c, REG_P_SATURATION(pid), ctrl->val);
+		break;
+
+	case V4L2_CID_SHARPNESS:
+		err = s5k6aa_write(c, REG_P_SHARPBLUR(pid), ctrl->val);
+		break;
+
...
+
+	default:
+		v4l2_err(sd, "Unsupported control id: %d\n", ctrl->id);
+		err = -EINVAL;
+	}
...
+{
+	struct s5k6aa_ctrls *ctrls = &to_s5k6aa(sd)->ctrls;
+
+	v4l2_ctrl_handler_log_status(&ctrls->handler, sd->name);
+	return 0;
+}
+
...
+	/* FIXME: there is no way to use this op on standalone subdevs */
...
+};
+
+#define s5k6aa_new_ctrl(__t, priv, args...) \
+	v4l2_ctrl_new_##__t(&(priv)->ctrls.handler, &s5k6aa_ctrl_ops, ##args)
+
+static int s5k6aa_initialize_ctrls(struct s5k6aa *s5k6aa)
+{
+	int ret;
+
+	ret = v4l2_ctrl_handler_init(&s5k6aa->ctrls.handler, 17);
+	if (ret)
+		return ret;
+	/* Auto white balance cluster */
+	s5k6aa->ctrls.awb = s5k6aa_new_ctrl(std, s5k6aa,
+				V4L2_CID_AUTO_WHITE_BALANCE, 0, 1, 1, 1);
+	s5k6aa->ctrls.gain_red = s5k6aa_new_ctrl(std, s5k6aa,
+				V4L2_CID_RED_BALANCE, 0, 256, 1, 127);
+	s5k6aa->ctrls.gain_blue = s5k6aa_new_ctrl(std, s5k6aa,
+				V4L2_CID_BLUE_BALANCE, 0, 256, 1, 127);
+	/* s5k6aa->ctrls.gain_green = s5k6aa_new_ctrl(std, s5k6aa, */
+	/* 			V4L2_CID_GREEN1_GAIN, 0, 256, 1, 127); */
+	v4l2_ctrl_auto_cluster(3, &s5k6aa->ctrls.awb, 0, false);
+
+	s5k6aa->ctrls.hflip = s5k6aa_new_ctrl(std, s5k6aa,
+				V4L2_CID_HFLIP, 0, 1, 1, 0);
+	s5k6aa->ctrls.vflip = s5k6aa_new_ctrl(std, s5k6aa,
+				V4L2_CID_VFLIP, 0, 1, 1, 0);
+	v4l2_ctrl_cluster(2, &s5k6aa->ctrls.hflip);
+
+	s5k6aa->ctrls.auto_exp = s5k6aa_new_ctrl(std_menu, s5k6aa,
+				V4L2_CID_EXPOSURE_AUTO,
+				V4L2_EXPOSURE_MANUAL, 0, V4L2_EXPOSURE_AUTO);
+	/* Exposure time: x 1 us */
+	s5k6aa->ctrls.exposure = s5k6aa_new_ctrl(std, s5k6aa,
+				V4L2_CID_EXPOSURE, 0, 6000000U, 1, 100000U);
+	/* Total gain: 256 <=> 1 x */
+	s5k6aa->ctrls.gain = s5k6aa_new_ctrl(std, s5k6aa,
+				V4L2_CID_GAIN, 0, 256, 1, 256);
+	v4l2_ctrl_auto_cluster(3, &s5k6aa->ctrls.auto_exp, 0, false);
+
+	s5k6aa_new_ctrl(std_menu, s5k6aa, V4L2_CID_POWER_LINE_FREQUENCY,
+		       V4L2_CID_POWER_LINE_FREQUENCY_AUTO, 0,
+		       V4L2_CID_POWER_LINE_FREQUENCY_AUTO);
+
+	s5k6aa_new_ctrl(std_menu, s5k6aa, V4L2_CID_COLORFX,
+			V4L2_COLORFX_SKETCH, 0x3D0, V4L2_COLORFX_NONE);
+
+	/* TODO: normalize to standard value range */
+	s5k6aa_new_ctrl(std, s5k6aa, V4L2_CID_WHITE_BALANCE_TEMPERATURE,
+			0, 256, 1, 0);
+	s5k6aa_new_ctrl(std, s5k6aa, V4L2_CID_SATURATION, 0, 256, 1, 0);
+	s5k6aa_new_ctrl(std, s5k6aa, V4L2_CID_SHARPNESS, 0, 256, 1, 0);
+
+	/* TODO: */
+	s5k6aa_new_ctrl(std, s5k6aa, V4L2_CID_BRIGHTNESS, -127, 127, 1, 10);
+
+	s5k6aa->sd.ctrl_handler = &s5k6aa->ctrls.handler;
+
+	if (s5k6aa->ctrls.handler.error)
+		v4l2_ctrl_handler_free(&s5k6aa->ctrls.handler);
+
+	return s5k6aa->ctrls.handler.error;
+}
+
+/*
+ * GPIO setup
...
new file mode 100644
index 0000000..b3b0f81
--- /dev/null
+++ b/include/media/s5k6aa.h
@@ -0,0 +1,51 @@
+/*
+ * Driver header for S5K6AAFX camera sensor
+ *
+ * Copyright (C) 2011 Samsung Electronics Co., Ltd.
+ *
+ * This program is free software; you can redistribute it and/or modify
