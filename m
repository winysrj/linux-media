...
Received: from arroyo.ext.ti.com ([192.94.94.40]:35094 "EHLO arroyo.ext.ti.com"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
	id S1753849Ab1AJMlN (ORCPT <rfc822;linux-media@vger.kernel.org>);
	Mon, 10 Jan 2011 07:41:13 -0500
From: manjunatha_halli@ti.com
To: mchehab@infradead.org, hverkuil@xs4all.nl
Cc: linux-kernel@vger.kernel.org, linux-media@vger.kernel.org,
	Manjunatha Halli <manjunatha_halli@ti.com>
Subject: [RFC V9 2/7] drivers:media:radio: wl128x: FM Driver V4L2 sources
Date: Mon, 10 Jan 2011 08:04:55 -0500
Message-Id: <1294664700-26949-3-git-send-email-manjunatha_halli@ti.com>
In-Reply-To: <1294664700-26949-2-git-send-email-manjunatha_halli@ti.com>
...
+		return fm_rx_set_volume(fmdev,
+				(u16)ctrl->val);
+		break;
+	case V4L2_CID_AUDIO_MUTE:	/* set mute */
+		return fmc_set_mute_mode(fmdev,
+				(u8)ctrl->val);
+		break;
+	case V4L2_CID_TUNE_POWER_LEVEL:
+		/* set TX power level - ext control */
+		return fm_tx_set_pwr_lvl(fmdev,
+				(u8)ctrl->val);
+		break;
+	case V4L2_CID_TUNE_PREEMPHASIS:
+		return fm_tx_set_preemph_filter(fmdev,
+				(u8) ctrl->val);
+		break;
+	default:
+		return -EINVAL;
...
+		return -EPERM;
+
+	ret = fm_rx_get_currband_freq_range(fmdev, &bottom_freq,
+						 &top_freq);
+	if (ret != 0)
+		return ret;
+
...
+		ret = fmc_set_mode(fmdev, FM_MODE_RX);
+		if (ret < 0) {
+			fmerr("Failed to set RX mode; unable to "
...
+	freq->frequency = (u32)(freq->frequency / 16);
+
+	ret = fmc_set_freq(fmdev, freq->frequency);
+
+	return ret;
+}
+
...
+		if (ret != 0) {
+			fmerr("Failed to set TX mode; unable to "
