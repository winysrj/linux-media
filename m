...
Received: from comal.ext.ti.com ([198.47.26.152]:42715 "EHLO comal.ext.ti.com"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
	id S1752965AbZDULZK convert rfc822-to-8bit (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Tue, 21 Apr 2009 07:25:10 -0400
...
Date: Tue, 21 Apr 2009 16:54:56 +0530
Subject: RE: [Review PATCH 3/3] OMAP2/3 V4L2 Display Driver
Message-ID: <5A47E75E594F054BAF48C5E4FC4B92AB02FB2F2BB4@dbde02.ent.ti.com>
In-Reply-To: <200904181559.24471.hverkuil@xs4all.nl>
...
> Subject: Re: [Review PATCH 3/3] OMAP2/3 V4L2 Display Driver
>
> On Monday 06 April 2009 14:40:54 Hardik Shah wrote:
> > This is the version 4th of the driver. Most of the community comments
> > are addressed.
> >
...
> >
> > TODOS
> > 1. Flipping support to be added.
> > 2. Rotation needs to add for YUV and UYVY formats.
> > 3. Ioctls needs to added for color space conversion matrix
> > coefficient programming.
> > 4. To be tested on DVI resolutions.
> > 5. Add S_OUTPUT and S_STD ioctls and corresponding G_ versions.
> > Supported through compile time option on current post.
> >
> > Community comments fixed.
...
> >  drivers/media/video/omap/omap_vout.c    | 2532
> +++++++++++++++++++++++++++++++
> >  drivers/media/video/omap/omap_voutdef.h |  137 ++
> >  drivers/media/video/omap/omap_voutlib.c |  265 ++++
> >  drivers/media/video/omap/omap_voutlib.h |   34 +
> >  9 files changed, 3064 insertions(+), 2 deletions(-)
...
> >  {
> >     int eth_cs;
> > @@ -307,7 +320,6 @@ static struct omap_dss_display_config
...
> >  {
> >     if (lcd_enabled) {
> > @@ -433,6 +445,8 @@ static struct omap_board_config_kernel
...
> > index 19cf3b8..cf041a6 100644
> > --- a/drivers/media/video/Kconfig
> > +++ b/drivers/media/video/Kconfig
> > @@ -711,6 +711,19 @@ config VIDEO_CAFE_CCIC
> >       CMOS camera controller.  This is the controller found on first-
> >       generation OLPC systems.
> >
> > +config VIDEO_OMAP3
> > +#Right now camera is not merged but in future will be merged with camera
>
...
> there is only the 50 and 60 Hz SDTV standards. For output over a Composite
...
> NTSC/PAL.
>
[Shah, Hardik] OMAP DSS is having the internal video encoder for converting the digital to analog standards like NTSC and PAL with a s-video and composite output.  Currently DSS does not support changing of the TV standards dynamically so I have made it as a compile time option.  Once DSS will support that I will add the S_STD and G_STD for standards.  Internally it will call the DSS2 library APIs to change the standard.
> > +
> > +endchoice
...
> > +#endif
>
> This might be better as a debug module option.
[Shah, Hardik] Done.
>
> > +
...
>
> I don't understand this. What are you attempting to do? Since MIN_WIDTH is 0,
> I'd say that what you really are doing here is to replace the special value 0
> by the framebuffer size. Other values are apparently still allowed, including
> sizes that are larger than the framebuffer. Is there a reason for the special
> handling of 0? And is a size of e.g. 1x1 really allowed?
>
> The name MIN_WIDTH was very misleading to me, since that normally implies a
> true minimum size and not a 0 value.
[Shah, Hardik] Minimum width supported by the DSS is 2X2 so changed minimum values changed to 2X2.
>
...
>
> Add a small inline function like this:
>
...
> }
>
> and use it where applicable. It's shorter and clearer than checking against
> these magic numbers.
[Shah, Hardik] Done.
>
...
> > +           *count = 4;
>
> As I mentioned earlier: wouldn't it be better to write a small inline function
> like this:
>
...
[Shah, Hardik] Rotation needs to be enabled if we want mirroring.  So just checking of whether rotation is enabled or not will do the job. Done that way.
 >
> > +
...
> > +           int mirror)
> > +{
> > +   if ((v4l2_rotation == 0) || (v4l2_rotation == 90) ||
> > +               (v4l2_rotation == 180) || (v4l2_rotation == 270)) {
> > +           switch (v4l2_rotation) {
>
> Both an 'if' and a 'switch' is overkill. Just the switch is sufficient!
>
> > +           case 90:
> > +                   *dss_rotation = 1;
> > +                   return 0;
...
> > +   int rotation_deg;
>
> Just 'rotation' since it is not in degrees.
[Shah, Hardik] changed.
>
...
[Shah, Hardik] It will not work when rotation will be 0 and I want 2 because of mirroring. (4-0) % 2 is 0 I want 2 here.  So not changing it.
>
> > +
...
> > +   }
>
> Yuck. It's much better to use video_set_drvdata when registering the
> video_devices and let the drvdata point to struct omap_vout_device.
> No need to do any comparisons against minors, just call video_drvdata(file).
[Shah, Hardik] I did not know this.  Implemented.
>
> > +
> > +   if (vout == NULL) {
...
> > +   q = &fh->vbq;
>
> videobuf_queue should be part of omap_vout_device, not of the omap_vout_fh.
> Sadly many v4l2 drivers do it like this, but it is wrong.
[Shah, Hardik] Done.
>
...
> > +    */
>
> Why would this be a problem? This comment seems to be out-of-place.
[Shah, Hardik] Done
>
> > +   win->w = vout->win.w;
...
> > +                   return -EINVAL;
> > +
> > +           ovl->manager->get_default_color(ovl->manager, &color);
> > +           ctrl->value = color;
> > +   }
> > +
> > +   default:
...
[Shah, Hardik] Rotation 0 means the rotation using Virtual Frame Buffer Rotation (VRFB) engine.  It does not support rotation with packed RGB24 format.  -1 means VRFB is not used.  So it should be -1;
>
> > +                   return -EINVAL;
> > +           mutex_lock(&vout->lock);
...
> > +}
>
> Please add empty lines between functions. This happens multiple times in this
> source.
[Shah, Hardik] Done
>
...
> > +           struct omap_overlay *ovl = ovid->overlays[t];
>
> Also add empty lines after a declaring local variables. It makes it easier
> to read. This is done multiple times in this source.
>
> > +           if (ovl->manager && ovl->manager->display) {
> > +                   struct omap_overlay_info info;
> > +                   ovl->get_overlay_info(ovl, &info);
...
> > +   struct omap_vout_fh *ofh = (struct omap_vout_fh *)fh;
>
> No need to cast void pointers. This too happens multiple times.
[Shah, Hardik] Done.
>
> > +   struct omap_vout_device *vout = ofh->vout;
> > +   int t, r = 0;
...
> init)
>
> I think that it is better to make two versions of this function:
>
> int omapvid_init(struct omap_vout_device *vout, u32 addr)
>
> and
>
> int omapvid_apply_changes(struct omap_vout_device *vout, u32 addr)
>
> There is very little overlap between the two and that makes this function
> hard to read.
[Shah, Hardik] Done.
>
> > +{
...
> > +   {
>
> Why this curly bracket?
>
> > +           if (vout->vid == OMAP_VIDEO1)
> > +                   return OMAP_DSS_COLOR_RGB24U;
> > +           else
>
> 'else' is not needed. Actually, using ? : here is probably better.
[Shah, Hardik] Done
>
