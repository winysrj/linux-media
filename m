...
Received: from moutng.kundenserver.de ([212.227.17.9]:52063 "EHLO
	moutng.kundenserver.de" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1754284Ab1BPMpp (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Wed, 16 Feb 2011 07:45:45 -0500
Date: Wed, 16 Feb 2011 13:45:30 +0100 (CET)
...
Message-ID: <Pine.LNX.4.64.1102161012490.20711@axis700.grange>
...
On Mon, 14 Feb 2011, achew@nvidia.com wrote:
...
A nitpick - if you don't have to resubmit the patch, you can leave it as 
is or modify in an incremental patch - I think, a name like "to_ov9740()" 
would suffice and make lines a bit shorter, several other drivers use this 
naming too. We might patch ov9640 to bring it in line with others and 
...
Please, put spaces in these comments too, and in a few more below.
...
Mixed cases... Maybe RES_720P_* and RES_VGA_*? In fact, I would make of 
these an enum and an array:
...
Esthetically I find it a bit disturbing - a space after '{' and no space 
before '}'. Please change.
...
Personally I dislike these register - value pair arrays. I _might_ 
understand and accept them in complicated cases, especially with 
undocumented registers, but - I really don't think it is a good idea to 
use these for one / two register set up sequences like the two arrays 
above.
...
No, you don't need IS_ERR_VALUE() here - "ret" is not a pointer. Applies 
to all occurrances, of course.
...
calling 2 memcpy()s - once for 2 and once for 1 byte seems a bit 
suboptimal to me. How about
...
maybe using "array" or "list" would look nicer than "ary."
...
A cleaner approach would be to get the bus-width from the platform data.
...
You would use the global array here.
...
the "code" parameter is unused, please, remove, also from the name.
...
and here like ov9740_resolutions[OV9740_VGA].width
...
nnnnnooo... try:fmt shouldn't fail if the format is unsupported. Just fill 
in whatever you support.
...
Someone might correct me if I am wrong, but I seem to remember, that with 
i2c devices you shouldn't use __devinit and __devexit. Try to look at 
other drivers, also not only soc-camera.
...
This is not BE-safe. Better something like
...
This test will fail on BE - see above
...
this variable doesn't seem to be very intensively used;) Please, remove.
...
Superfluous empty line. Also, please implement at least g_crop and 
cropcap.
