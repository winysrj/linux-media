...
Received: from mail.gmx.net ([213.165.64.20]:40320 "HELO mail.gmx.net"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with SMTP
	id S1751262AbZDBPrH (ORCPT <rfc822;linux-media@vger.kernel.org>);
	Thu, 2 Apr 2009 11:47:07 -0400
Date: Thu, 2 Apr 2009 17:47:15 +0200 (CEST)
...
In-Reply-To: <20090330145310.20826.77060.stgit@localhost.localdomain>
Message-ID: <Pine.LNX.4.64.0904010034300.12031@axis700.grange>
...
And if it is a module? Perhaps, would be better to define a boolean config 
variable, select it from your main Kconfig entry, and use it like

obj-$(CONFIG_MX1_VIDEO)	+= ksym_mx1.o mx1_camera_fiq.o

Then you can also use this variable everywhere instead of
...
You said you didn't know how to properly fix this. Just look at other 
examples: pxa, sh_mobile_ceu, mx3. They all do the same thing - compare 
against some fixed limit, and if the limit is exceeded, they reduce the 
number of buffers to fit within the limit.
...
better, this doesn't look like an improvement to me.
...
Are you sure, that if ->actuve == NULL, i.e., if DMA is currently 
inactive, the configuration of the controller will be preserved across a 
suspend / resume? If not, better don't implement any PM at all.
...
Missing clk_put() in the error path and in mx1_camera_remove()
