...
Received: from bombadil.infradead.org ([198.137.202.9]:43389 "EHLO
	bombadil.infradead.org" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1750892AbaKCJSa (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Mon, 3 Nov 2014 04:18:30 -0500
...
Subject: [PATCH] cx231xx: Improve the log message
Date: Mon,  3 Nov 2014 07:18:05 -0200
Message-Id: <58ce9c7298818617bb1f83efd0443384bad816f4.1415005965.git.mchehab@osg.samsung.com>
...
Full log with this patch:
 ...
 cx231xx 1-2:1.1: New device Conexant Corporation Polaris AV Capturb @ 480 Mbps (1554:5010) with 7 interfaces
 cx231xx 1-2:1.1: Identified as Pixelview PlayTV USB Hybrid (card=10)
 i2c i2c-8: Added multiplexed i2c bus 10
 i2c i2c-8: Added multiplexed i2c bus 11
 cx25840 7-0044: cx23102 A/V decoder found @ 0x88 (cx231xx #0-0)
 cx25840 7-0044: loaded v4l-cx231xx-avcore-01.fw firmware (16382 bytes)
 Chip ID is not zero. It is not a TEA5767
 tuner 9-0060: Tuner -1 found with type(s) Radio TV.
 tda18271 9-0060: creating new instance
 TDA18271HD/C2 detected @ 9-0060
 tda18271: performing RF tracking filter calibration
 tda18271: RF tracking filter calibration complete
 cx231xx 1-2:1.1: v4l2 driver version 0.0.3
 cx231xx 1-2:1.1: Registered video device video0 [v4l2
 cx231xx 1-2:1.1: Registered VBI device vbi0
 Registered IR keymap rc-pixelview-002t
 input: i2c IR (Pixelview PlayTV USB Hy as /devices/virtual/rc/rc0/input17
 rc0: i2c IR (Pixelview PlayTV USB Hy as /devices/virtual/rc/rc0
 ir-kbd-i2c: i2c IR (Pixelview PlayTV USB Hy detected at i2c-9/9-0030/ir0 [cx231xx #0-2
 cx231xx 1-2:1.1: video EndPoint Addr 0x84, Alternate settings: 5
 cx231xx 1-2:1.1: VBI EndPoint Addr 0x85, Alternate settings: 2
 cx231xx 1-2:1.1: sliced CC EndPoint Addr 0x86, Alternate settings: 2
 cx231xx 1-2:1.1: TS EndPoint Addr 0x81, Alternate settings: 6
 usbcore: registered new interface driver cx231xx
 cx231xx 1-2:1.1: audio EndPoint Addr 0x83, Alternate settings: 3
 cx231xx 1-2:1.1: Cx231xx Audio Extension initialized
 cx231xx 1-2:1.1: dvb_init: looking for demod on i2c bus: 9
 i2c i2c-11: Detected a Fujitsu mb86a20s frontend
 tda18271 9-0060: attaching existing instance
 DVB: registering new adapter (cx231xx #0)
 cx231xx 1-2:1.1: DVB: registering adapter 0 frontend 0 (Fujitsu mb86A20s)...
 cx231xx 1-2:1.1: Successfully loaded cx231xx-dvb
 cx231xx 1-2:1.1: Cx231xx dvb Extension initialized

diff --git a/drivers/media/usb/cx231xx/cx231xx-417.c b/drivers/media/usb/cx231xx/cx231xx-417.c
index a87cddedbe29..0fd098b29b93 100644
...
index bdaef2cc5470..1f3b0a3baa0d 100644
...
+				dev_dbg(dev->dev,
 				...
 				value &= 0xFFFFFFFC;
 				value |= 0x3;
@@ -2583,14 +2583,14 @@ int cx231xx_initialize_stream_xfer(struct cx231xx *dev, u32 media_type)
...
+			dev_dbg(dev->dev,
 				...
 				__func__);
 			status = cx231xx_mode_register(dev, TS_MODE_REG, 0x100);
@@ -2945,7 +2945,7 @@ int cx231xx_gpio_i2c_read_ack(struct cx231xx *dev)
 			 (nCnt > 0));
...
index a2fc7e8f7327..d4ec1029c809 100644
...
+	struct device *d = &interface->dev;
 	...
 	int retval = -ENODEV;
@@ -1401,8 +1402,8 @@ static int cx231xx_usb_probe(struct usb_interface *interface,
 	u8 idx;
...
@@ -1519,8 +1521,7 @@ static int cx231xx_usb_probe(struct usb_interface *interface,
...
@@ -1531,7 +1532,7 @@ static int cx231xx_usb_probe(struct usb_interface *interface,
...
@@ -1548,9 +1549,8 @@ static int cx231xx_usb_probe(struct usb_interface *interface,
 						wMaxPacketSize);
...
@@ -1614,7 +1614,7 @@ static void cx231xx_usb_disconnect(struct usb_interface *interface)
...
index 4c5e25c0b3a1..156fa760ab97 100644
...
index c238d860a06c..e06afb7a023f 100644
...
+		dev_dbg(dev->dev, "DVB transfer mode is BULK.\n");
 		...
 		rc = cx231xx_set_mode(dev, CX231XX_DIGITAL_MODE);
 		if (rc < 0)
@@ -374,20 +374,20 @@ static int attach_xc5000(u8 addr, struct cx231xx *dev)
 	cfg.i2c_addr = addr;
...
+		dev_err(dev->dev, "%s/2: dvb frontend not attached. "
 	...
 	if (!fe) {
-		dev_err(&dev->udev->dev, "%s/2: xc5000 attach failed\n", dev->name);
...
@@ -426,16 +426,16 @@ int cx231xx_reset_analog_tuner(struct cx231xx *dev)
...
@@ -459,7 +459,7 @@ static int register_dvb(struct cx231xx_dvb *dvb,
...
+		dev_warn(dev->dev,
 		       ...
 		       dev->name, result);
 		goto fail_adapter;
@@ -473,7 +473,7 @@ static int register_dvb(struct cx231xx_dvb *dvb,
 	/* register frontend */
...
+		dev_warn(dev->dev,
 			 "%s: dvb_dmx_init failed (errno = %d)\n",
 		       dev->name, result);
 		goto fail_dmx;
@@ -502,7 +502,7 @@ static int register_dvb(struct cx231xx_dvb *dvb,
...
+		dev_warn(dev->dev,
 			 ...
 			 dev->name, result);
 		goto fail_dmxdev;
@@ -511,7 +511,7 @@ static int register_dvb(struct cx231xx_dvb *dvb,
...
+		dev_warn(dev->dev,
 		       ...
 		       dev->name, result);
 		goto fail_fe_hw;
@@ -520,7 +520,7 @@ static int register_dvb(struct cx231xx_dvb *dvb,
...
+		dev_warn(dev->dev,
 			 ...
 			 dev->name, result);
 		goto fail_fe_mem;
@@ -528,7 +528,7 @@ static int register_dvb(struct cx231xx_dvb *dvb,
...
+		dev_warn(dev->dev,
 			 ...
 			 result);
 		goto fail_fe_conn;
@@ -589,7 +589,7 @@ static int dvb_init(struct cx231xx *dev)
...
@@ -612,7 +612,7 @@ static int dvb_init(struct cx231xx *dev)
 					demod_i2c);
...
+			dev_err(dev->dev,
 			...
 			result = -EINVAL;
 			goto out_free;
@@ -637,7 +637,7 @@ static int dvb_init(struct cx231xx *dev)
 					       demod_i2c);
...
+			dev_err(dev->dev,
 			...
 			result = -EINVAL;
 			goto out_free;
@@ -660,7 +660,7 @@ static int dvb_init(struct cx231xx *dev)
 					demod_i2c);
...
+			dev_err(dev->dev,
 			...
 			result = -EINVAL;
 			goto out_free;
@@ -684,7 +684,7 @@ static int dvb_init(struct cx231xx *dev)
 					       demod_i2c);
...
+			dev_err(dev->dev,
 			...
 			result = -EINVAL;
 			goto out_free;
@@ -702,7 +702,7 @@ static int dvb_init(struct cx231xx *dev)
 		break;
...
@@ -711,7 +711,7 @@ static int dvb_init(struct cx231xx *dev)
 						tuner_i2c);
...
+			dev_err(dev->dev,
 			...
 			result = -EINVAL;
 			goto out_free;
@@ -733,7 +733,7 @@ static int dvb_init(struct cx231xx *dev)
 			);
...
+			dev_err(dev->dev,
 			...
 			result = -EINVAL;
 			goto out_free;
@@ -766,7 +766,7 @@ static int dvb_init(struct cx231xx *dev)
 			);
...
+			dev_err(dev->dev,
 			...
 			result = -EINVAL;
 			goto out_free;
@@ -811,7 +811,7 @@ static int dvb_init(struct cx231xx *dev)
...
@@ -820,7 +820,7 @@ static int dvb_init(struct cx231xx *dev)
 						demod_i2c);
...
+			dev_err(dev->dev,
 			...
 			result = -EINVAL;
 			goto out_free;
@@ -835,26 +835,26 @@ static int dvb_init(struct cx231xx *dev)
 		break;
...
index 7ded7df764fe..4b46f10985f4 100644
...
@@ -1653,7 +1653,7 @@ static int cx231xx_v4l2_open(struct file *filp)
 
 	fh = kzalloc(sizeof(struct cx231xx_fh), GFP_KERNEL);
 	if (!fh) {
-		dev_err(&dev->udev->dev, "cx231xx-video.c: Out of memory?!\n");
+		dev_err(dev->dev, "cx231xx-video.c: Out of memory?!\n");
 		return -ENOMEM;
 	}
 	if (mutex_lock_interruptible(&dev->lock)) {
@@ -1739,7 +1739,7 @@ void cx231xx_release_analog_resources(struct cx231xx *dev)
 		dev->radio_dev = NULL;
 	}
 	if (dev->vbi_dev) {
...
@@ -1748,7 +1748,7 @@ void cx231xx_release_analog_resources(struct cx231xx *dev)
 		dev->vbi_dev = NULL;
 	}
 	if (dev->vdev) {
-		dev_info(&dev->udev->dev, "V4L2 device %s deregistered\n",
+		dev_info(dev->dev, "V4L2 device %s deregistered\n",
 		...
 		if (dev->board.has_417)
@@ -2083,7 +2083,7 @@ int cx231xx_register_analog_devices(struct cx231xx *dev)
 {
 	int ret;
...
@@ -2121,7 +2121,7 @@ int cx231xx_register_analog_devices(struct cx231xx *dev)
...
@@ -2130,13 +2130,13 @@ int cx231xx_register_analog_devices(struct cx231xx *dev)
...
@@ -2147,7 +2147,7 @@ int cx231xx_register_analog_devices(struct cx231xx *dev)
...
@@ -2155,18 +2155,18 @@ int cx231xx_register_analog_devices(struct cx231xx *dev)
...
@@ -2174,11 +2174,11 @@ int cx231xx_register_analog_devices(struct cx231xx *dev)
