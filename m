...
Received: from web94907.mail.in2.yahoo.com ([203.104.17.160]:23134 "HELO
	web94907.mail.in2.yahoo.com" rhost-flags-OK-OK-OK-OK)
	by vger.kernel.org with SMTP id S1753040Ab0HYVUS convert rfc822-to-8bit
	(ORCPT <rfc822;linux-media@vger.kernel.org>);
	Wed, 25 Aug 2010 17:20:18 -0400
Message-ID: <676453.10169.qm@web94907.mail.in2.yahoo.com>
Date: Thu, 26 Aug 2010 02:50:13 +0530 (IST)
...
In-Reply-To: <1282201557.14489.148.camel@masi.mnp.nokia.com>
MIME-Version: 1.0
...
--- On Thu, 19/8/10, Matti J. Aaltonen <matti.j.aaltonen@nokia.com> wrote:

> From: Matti J. Aaltonen <matti.j.aaltonen@nokia.com>
> Subject: Re: [PATCH v4 2/5] MFD: WL1273 FM Radio: MFD driver for the FM radio.
> To: "ext Pavan Savoy" <pavan_savoy@yahoo.co.in>
> Cc: "Mauro Carvalho Chehab" <mchehab@redhat.com>, "linux-media@vger.kernel.org" <linux-media@vger.kernel.org>, "hverkuil@xs4all.nl" <hverkuil@xs4all.nl>, "Valentin Eduardo (Nokia-MS/Helsinki)" <eduardo.valentin@nokia.com>
> Date: Thursday, 19 August, 2010, 12:35 PM
> Hello.
> 
> 
> On Wed, 2010-08-18 at 20:14 +0200, ext Pavan Savoy wrote:
> > --- On Wed, 7/7/10, Pavan Savoy <pavan_savoy@ti.com>
> wrote:
> > > 
...
> > > > > @@ -0,0 +1,616 @@
> > > > > +/*
> > > > > + * MFD driver for wl1273 FM radio and
> audio
> > > codec
> > > > submodules.
> > > > > + *
...
> > > > > +MODULE_PARM_DESC(radio_band, "Band:
> 0=Japan,
> > > > 1=USA-Europe*");
> > > > > +
> > > > > +/*
> > > > > + * static unsigned int rds_buf - the
> number of
> > > RDS
> > > > buffer blocks used.
> > > > > + *
...
> > > > > +MODULE_PARM_DESC(rds_buf, "RDS buffer
> entries:
> > > > *100*");
> > > > > +
> > > > > +int wl1273_fm_read_reg(struct
> wl1273_core *core,
> > > u8
> > > > reg, u16 *value)
> > > > > +{
...
> > > > > +
> > > > > +    r =
> > > > i2c_smbus_read_i2c_block_data(client, reg,
> 2, b);
> > > > > +    if (r != 2) {
> > > > > +
> > > > dev_err(&client->dev, "%s: Read: %d
> fails.\n",
> > > > __func__, reg);
> > > > > +        return
> > > > -EREMOTEIO;
...
> > > > > +
> > > > > +int wl1273_fm_write_cmd(struct
> wl1273_core
> > > *core, u8
> > > > cmd, u16 param)
> > > > > +{
...
> > > > > +
> > > > > +    r =
> > > > i2c_smbus_write_i2c_block_data(client, cmd,
> 2, buf);
> > > > > +    if (r) {
> > > > > +
> > > > dev_err(&client->dev, "%s: Cmd: %d
> fails.\n",
> > > > __func__, cmd);
> > > > > +        return r;
> > > > > +    }
...
> > > > > + *
> > > > > + * Audio modes are
> WL1273_AUDIO_DIGITAL and
> > > > WL1273_AUDIO_ANALOG.
> > > > > + */
> > > > > +int wl1273_fm_set_audio(struct
> wl1273_core
> > > *core,
...
> value.
> > > > > + */
> > > > > +int wl1273_fm_set_volume(struct
> wl1273_core
> > > *core,
> > > > unsigned int volume)
> > > > > +{
...
> > > > 3)
> > > > > +#define
> > > > WL1273_RDS_UNCORRECTABLE_ERROR   
> (1 <<
> > > > 4)
> > > > > +
> > > > > +static int wl1273_fm_rds(struct
> wl1273_core
> > > *core)
> > > > > +{
> > > > > +    struct i2c_client
...
> > > >
> > > >
> > > > msleep on an irq handler? You shouldn't be
> doing it!
> > > You're
> > > > not allowed to sleep
> > > > during IRQ time. Kernel can panic here.
> You'll
> > > probably
...
> > > wl1273_driver_id_table);
> > > > > +
> > > > > +static int wl1273_core_remove(struct
> i2c_client
> > > > *client)
> > > > > +{
> > > > > +    struct wl1273_core *core
...
> > > > > +    if (!pdata) {
> > > > > +
> > > > dev_err(&client->dev, "No platform
> data.\n");
> > > > > +        return
> > > > -EINVAL;
> > > > > +    }
...
> > > > > + *
> > > > > + * Some definitions for the wl1273
> radio
...
> > > > 96
> > > > > +#define
> > > > WL1273_TX_AUDIO_LEVEL_TEST_THRESHOLD 
>   73
> > > > > +#define
> > > > WL1273_TX_AUDIO_INPUT_LEVEL_RANGE_SET 
>   54
...
> > > >     88
> > > > > +#define
> > > > WL1273_REF_ERR_CALIB_PERIODICITY_SET 
>   89
...
> > > >     85
> > > > > +#define
> > > > WL1273_RSSI_BLOCK_SCAN_FREQ_SET   
> 95
> > > > > +#define
> > > > WL1273_RSSI_BLOCK_SCAN_START   
> 97
> > > > > +#define
> > > > WL1273_RSSI_BLOCK_SCAN_DATA_GET 
>    5
> > > > > +#define
...
> > > > 0x78b8
> > > > > +
> > > > > +/* I2S protocol, left channel first,
> data width
> > > 16
> > > > bits */
...
> > > > > +
> > > > > +/* Allowed modes */
> > > > > +#define WL1273_RX_ALLOWED   
> 0x01
> > > > > +#define WL1273_TX_ALLOWED   
> 0x02
