...
Received: from devils.ext.ti.com ([198.47.26.153]:52102 "EHLO
	devils.ext.ti.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1754758Ab0KHOzy (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Mon, 8 Nov 2010 09:55:54 -0500
...
Date: Mon,  8 Nov 2010 20:25:32 +0530
Message-Id: <1289228132-5105-1-git-send-email-manjunath.hadli@ti.com>
...
+	if (pdata->setup_clock(VPBE_MODE_SDTV) < 0)
+		return -EINVAL;
+
+	enableDigitalOutput(sd, 0);
+
...
+	/* Setup clock at VPSS & VENC for SD */
+	vpss_enable_clock(VPSS_VENC_CLOCK_SEL, 1);
+	if (venc->pdata->setup_clock(VPBE_MODE_SDTV) < 0)
+		return -EINVAL;
+
+	enableDigitalOutput(sd, 0);
+
...
+
+	/* Setup clock at VPSS & VENC for SD */
+	if (pdata->setup_clock(VPBE_MODE_EDTV) < 0)
+		return -EINVAL;
+
+	enableDigitalOutput(sd, 0);
+
...
+}
+
+static int venc_set_if_config(struct v4l2_subdev *sd, int irq, void *pdata)
+{
+	struct vpbe_if_params  *if_params = (struct vpbe_if_params *)pdata;
+	struct venc_state *venc = to_state(sd);
+	unsigned int val = 0;
+	int ret = 0;
+
+	v4l2_dbg(debug, 2, sd, "venc_set_if_config\n");
+
+	switch (if_params->if_type) {
+	case VPBE_DIGITAL_IF_YCC8:
+		val = VENC_VMOD_VDMD_YCBCR8 << VENC_VMOD_VDMD_SHIFT;
+		if (if_params->bt656)
+			venc_modify(sd, VENC_YCCCTL, 1, 1);
+		break;
+	case VPBE_DIGITAL_IF_YCC16:
+		val = VENC_VMOD_VDMD_YCBCR16 << VENC_VMOD_VDMD_SHIFT;
+		if (if_params->bt656)
+			venc_modify(sd, VENC_YCCCTL, 1, 1);
+		break;
+	case VPBE_DIGITAL_IF_SRGB:
+		val = VENC_VMOD_VDMD_RGB666 << VENC_VMOD_VDMD_SHIFT;
+		break;
+	case VPBE_DIGITAL_IF_PRGB:
+		val = VENC_VMOD_VDMD_RGB666 << VENC_VMOD_VDMD_SHIFT;
+		break;
+	case VPBE_ANALOG_TV_IF:
+		/* do nothing */
+		break;
+	default:
+		ret = -EINVAL;
...
+	}
+	if (!ret) {
+		venc_modify(sd, VENC_VMOD, val, VENC_VMOD_VDMD);
+		venc->if_params = if_params;
+	}
+	return ret;
+}
+
...
+static int venc_g_register(struct v4l2_subdev *sd,
+			   struct v4l2_dbg_register *reg)
+{
+	__iomem void *reg_base;
+
+	if (reg->match.type == 2)
+		reg->val = venc_read(sd, reg->reg);
+	else if (reg->match.type == 3) {
+		reg_base = ioremap_nocache(0x01c70800, 0x80);
+		if (reg_base == NULL)
+			return -EINVAL;
+		reg->val = __raw_readl(reg_base + reg->reg);
+		iounmap(reg_base);
+	} else {
+		reg_base = ioremap_nocache(0x01c70000, 0x80);
+		if (reg_base == NULL)
+			return -1;
+		reg->val = __raw_readl(reg_base + reg->reg);
+		iounmap(reg_base);
...
+ * @next: used internally by the venc driver to maintain a liked list of
+ *        callbacks
...
+};
+
+struct v4l2_subdev_vpbe_venc_ops {
+	int (*register_callback)(struct v4l2_subdev *sd,
+				 struct venc_callback *callback);
+	int (*unregister_callback)(struct v4l2_subdev *sd,
+				   struct venc_callback *callback);
+	int (*configure_venc)(struct v4l2_subdev *sd,
+			      struct vpbe_enc_mode_info *mode);
+	int (*is_second_field)(struct v4l2_subdev *sd);
+};
+*/
+struct venc_platform_data {
+	enum vpbe_types venc_type;
+	int (*setup_pinmux)(enum vpbe_if_types if_type,
+			    enum vpbe_rgb_modes rgb_mode,
+			    int field);
+	int (*setup_clock)(enum vpbe_display_modes mode);
