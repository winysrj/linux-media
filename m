...
Received: from smtp-vbr13.xs4all.nl ([194.109.24.33]:4049 "EHLO
	smtp-vbr13.xs4all.nl" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1757126Ab0E2OoQ (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Sat, 29 May 2010 10:44:16 -0400
Message-Id: <c49399d8e4f204c5fc3a3768ae406ff097909c6f.1275143672.git.hverkuil@xs4all.nl>
In-Reply-To: <cover.1275143672.git.hverkuil@xs4all.nl>
References: <cover.1275143672.git.hverkuil@xs4all.nl>
From: Hans Verkuil <hverkuil@xs4all.nl>
Date: Sat, 29 May 2010 16:45:58 +0200
Subject: [PATCH 03/15] [RFCv4] Documentation: add v4l2-controls.txt documenting the new controls API.
...
+private data.
+
+The v4l2_ctrl_config struct also has fields to set the is_private, is_volatile
+and is_uninitialized flags.
+
...
+boundaries at this stage. Or obtaining those details may require expensive
+hardware accesses.
+
+In that case you can specify a .init op in struct v4l2_ctrl_ops. This will be
+called on the first use of the control and can be used to setup the boundaries
+of the control. Besides adding a .init op you also need to set the
+is_uninitialized flag when adding the control. If this flag is set, then the
+framework will know that it has to call the .init op on the first use of the
+control. The flag will be cleared automatically once the control has been
+initialized.
+
+The initialization will only be done when the control is referenced by an
+application. Uninitialized controls will be ignored by v4l2_ctrl_handler_setup()
+and v4l2_ctrl_handler_log_status() and will result in a warning when accessed
+by v4l2_ctrl_g_ctrl() and v4l2_ctrl_s_ctrl().
+
+When you first add the control you should fill in sensible default values for
+the fields that you will initialize later.
+
+If one or more controls are marked uninitialized when you combine them into
+a cluster, then an attempt to access any control in the cluster (uninitialized
+or not) will lead to a call to the .init op of the cluster's master control.
+
+Since initializing the control probably requires hardware accesses the
+initialization can fail. A failure here should not normally happen, but poor
+hardware or firmware design, or of course hardware failure can result in this
+situation.
+
+For that reason the init op can return an error (usually this will be -EIO
+to signify a hardware problem). The framework will return that error to
+userspace and the control remains uninitialized.
+
+In case of an error the driver can also choose to call v4l2_ctrl_init_disable().
+This will mark the control (actually the whole control cluster) as being
+initialized but it also sets V4L2_CTRL_FLAG_DISABLED. This allows the
+application to ignore the error and continue instead with this control disabled.
+
+This only makes sense in cases where the hardware/firmware is known to be
+flaky as is the case with some UVC webcams.
+
+
