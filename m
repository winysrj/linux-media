...
Received: from ganesha.gnumonks.org ([213.95.27.120]:52709 "EHLO
	ganesha.gnumonks.org" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S932320Ab1CINoz (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Wed, 9 Mar 2011 08:44:55 -0500
...
Subject: [PATCH v2 6/8] media: MFC: Add MFC v5.1 V4L2 driver
Date: Wed,  9 Mar 2011 22:16:05 +0900
Message-Id: <1299676567-14194-7-git-send-email-jtp.park@samsung.com>
...
---
 ...
 29 files changed, 9182 insertions(+), 0 deletions(-)
...
index 986019f..59ba5e8 100644
--- a/drivers/media/video/Kconfig
+++ b/drivers/media/video/Kconfig
@@ -1025,4 +1025,23 @@ config  VIDEO_SAMSUNG_S5P_FIMC
...
+	default n
+	help
+	  This ia a V4L2 driver for the S5P MFC
+
+choice
+	prompt "VB2 memory allocator for MFC"
+	depends on VIDEO_SAMSUNG_S5P_MFC
+	default S5P_MFC_VB2_DMA_POOL
+
+config S5P_MFC_VB2_DMA_POOL
+	bool "DMA Pool"
+	select VIDEOBUF2_DMA_POOL
+	help
+	  use videobuf2 DMA pool allocator for MFC
+endchoice
+
 endif # V4L_MEM2MEM_DRIVERS
diff --git a/drivers/media/video/Makefile b/drivers/media/video/Makefile
index 12fca52..467d1b7 100644
...
+/* FIXME */
+//#define S5P_FIMV_H264_NB_IP_SIZE		(32 * 1024)
+//#define S5P_FIMV_H264_VERT_NB_MV_SIZE		(16 * 1024)
+
...
+}
+
+/* FIXME: consider 'is really need overall operation?'*/
...
+	u8 num_b_frame;
+	enum v4l2_codec_h264_profile profile;
+	u8 level;
+	enum v4l2_codec_switch interlace;
+	enum v4l2_codec_h264_loop_filter loop_filter_mode;
+	s8 loop_filter_alpha;
+	s8 loop_filter_beta;
+	enum v4l2_codec_h264_entropy_mode entropy_mode;
+	u8 max_ref_pic;
+	u8 num_ref_pic_4p;
+	enum v4l2_codec_switch _8x8_transform;
+	enum v4l2_codec_switch rc_mb;
+	u32 rc_framerate;
+	u8 rc_frame_qp;
+	u8 rc_min_qp;
+	u8 rc_max_qp;
+	enum v4l2_codec_switch_inv rc_mb_dark;
+	enum v4l2_codec_switch_inv rc_mb_smooth;
+	enum v4l2_codec_switch_inv rc_mb_static;
+	enum v4l2_codec_switch_inv rc_mb_activity;
+	u8 rc_p_frame_qp;
+	u8 rc_b_frame_qp;
+	enum v4l2_codec_switch ar_vui;
+	u8 ar_vui_idc;
+	u16 ext_sar_width;
+	u16 ext_sar_height;
+	enum v4l2_codec_switch open_gop;
+	u16 open_gop_size;
+};
+
+/**
...
+	/* MPEG4 Only */
+	u8 num_b_frame;
+	enum v4l2_codec_mpeg4_profile profile;
+	u8 level;
+	enum v4l2_codec_switch quarter_pixel;
+	u16 vop_time_res;
+	u16 vop_frm_delta;
+	u8 rc_b_frame_qp;
...
+
+	u16 gop_size;
+	enum v4l2_codec_multi_slice_mode slice_mode;
+	u16 slice_mb;
+	u32 slice_bit;
+	u16 intra_refresh_mb;
+	enum v4l2_codec_switch pad;
+	u8 pad_luma;
+	u8 pad_cb;
+	u8 pad_cr;
+	enum v4l2_codec_switch rc_frame;
+	u32 rc_bitrate;
+	u16 rc_reaction_coeff;
+
+	u16 vbv_buf_size;
+	enum v4l2_codec_seq_hdr_mode seq_hdr_mode;
+	enum v4l2_codec_frame_skip_mode frame_skip_mode;
+	enum v4l2_codec_switch fixed_target_bit;
+
+	union {
...
+
+	int frame_count;
+	enum v4l2_codec_frame_type frame_type;
+	enum v4l2_codec_force_frame_type force_frame_type;
+
...
+	},
+	{
+		.id = V4L2_CID_CODEC_MULTI_SLICE_MODE,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_MULTI_SLICE_MB,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_MULTI_SLICE_BIT,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_INTRA_REFRESH_MB,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_PAD_CTRL_ENABLE,
+		.type = V4L2_CTRL_TYPE_BOOLEAN,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_PAD_LUMA_VALUE,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_PAD_CB_VALUE,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_PAD_CR_VALUE,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_RC_FRAME_ENABLE,
+		.type = V4L2_CTRL_TYPE_BOOLEAN,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_RC_REACTION_COEFF,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_FORCE_FRAME_TYPE,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_VBV_BUF_SIZE,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_SEQ_HDR_MODE,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_FRAME_SKIP_MODE,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_RC_FIXED_TARGET_BIT,
+		.type = V4L2_CTRL_TYPE_BOOLEAN,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_B_FRAMES,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_PROFILE,
+		.type = V4L2_CTRL_TYPE_INTEGER,
+		.name = "H264 profile",
+		.minimum = 0,
+		.maximum = 2,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_INTERLACE,
+		.type = V4L2_CTRL_TYPE_BOOLEAN,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_LOOP_FILTER_MODE,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_LOOP_FILTER_ALPHA,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_LOOP_FILTER_BETA,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_ENTROPY_MODE,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_MAX_REF_PIC,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_NUM_REF_PIC_4P,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_8X8_TRANSFORM,
+		.type = V4L2_CTRL_TYPE_BOOLEAN,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_RC_MB_ENABLE,
+		.type = V4L2_CTRL_TYPE_BOOLEAN,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_RC_FRAME_RATE,
+		.type = V4L2_CTRL_TYPE_INTEGER,
+		.name = "Frame rate",
+		.minimum = 1,
+		.maximum = (1 << 30) - 1,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_RC_FRAME_QP,
+		.type = V4L2_CTRL_TYPE_INTEGER,
+		.name = "Frame QP value",
+		.minimum = 0,
+		.maximum = 51,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_RC_MIN_QP,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+		.id = V4L2_CID_CODEC_H264_RC_MAX_QP,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_RC_MB_DARK,
+		.type = V4L2_CTRL_TYPE_BOOLEAN,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_RC_MB_SMOOTH,
+		.type = V4L2_CTRL_TYPE_BOOLEAN,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_RC_MB_STATIC,
+		.type = V4L2_CTRL_TYPE_BOOLEAN,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_RC_MB_ACTIVITY,
+		.type = V4L2_CTRL_TYPE_BOOLEAN,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_RC_P_FRAME_QP,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_RC_B_FRAME_QP,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_AR_VUI_ENABLE,
+		.type = V4L2_CTRL_TYPE_BOOLEAN,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_AR_VUI_IDC,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_EXT_SAR_WIDTH,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_EXT_SAR_HEIGHT,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_OPEN_GOP,
+		.type = V4L2_CTRL_TYPE_BOOLEAN,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H264_I_PERIOD,
+		.type = V4L2_CTRL_TYPE_INTEGER,
+		.name = "H264 I period",
+		.minimum = 0,
+		.maximum = (1 << 16) - 1,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_MPEG4_B_FRAMES,
+		.type = V4L2_CTRL_TYPE_INTEGER,
+		.name = "The number of B frames",
+		.minimum = 0,
+		.maximum = 2,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_MPEG4_PROFILE,
+		.type = V4L2_CTRL_TYPE_INTEGER,
+		.name = "MPEG4 profile",
+		.minimum = 0,
+		.maximum = 1,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_MPEG4_RC_FRAME_QP,
+		.type = V4L2_CTRL_TYPE_INTEGER,
+		.name = "Frame QP value",
+		.minimum = 1,
+		.maximum = 31,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_MPEG4_RC_MIN_QP,
+		.type = V4L2_CTRL_TYPE_INTEGER,
+		.name = "Minimum QP value",
+		.minimum = 1,
+		.maximum = 31,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_MPEG4_RC_MAX_QP,
+		.type = V4L2_CTRL_TYPE_INTEGER,
+		.name = "Minimum QP value",
+		.minimum = 1,
+		.maximum = 31,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_MPEG4_QUARTER_PIXEL,
+		.type = V4L2_CTRL_TYPE_BOOLEAN,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_MPEG4_RC_P_FRAME_QP,
+		.type = V4L2_CTRL_TYPE_INTEGER,
+		.name = "P frame QP value",
+		.minimum = 1,
+		.maximum = 31,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_MPEG4_RC_B_FRAME_QP,
+		.type = V4L2_CTRL_TYPE_INTEGER,
+		.name = "B frame QP value",
+		.minimum = 1,
+		.maximum = 31,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_MPEG4_VOP_TIME_RES,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_MPEG4_VOP_FRM_DELTA,
+		.type = V4L2_CTRL_TYPE_INTEGER,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H263_RC_FRAME_RATE,
+		.type = V4L2_CTRL_TYPE_INTEGER,
+		.name = "Frame rate",
+		.minimum = 1,
+		.maximum = (1 << 30) - 1,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H263_RC_FRAME_QP,
+		.type = V4L2_CTRL_TYPE_INTEGER,
+		.name = "Frame QP value",
+		.minimum = 1,
+		.maximum = 31,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H263_RC_MIN_QP,
+		.type = V4L2_CTRL_TYPE_INTEGER,
+		.name = "Minimum QP value",
+		.minimum = 1,
+		.maximum = 31,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H263_RC_MAX_QP,
+		.type = V4L2_CTRL_TYPE_INTEGER,
+		.name = "Minimum QP value",
+		.minimum = 1,
+		.maximum = 31,
...
+	},
+	{
+		.id = V4L2_CID_CODEC_H263_RC_P_FRAME_QP,
+		.type = V4L2_CTRL_TYPE_INTEGER,
+		.name = "P frame QP value",
+		.minimum = 1,
+		.maximum = 31,
...
+
+	if (p->seq_hdr_mode == V4L2_CODEC_SEQ_HDR_MODE_SEQ) {
+		spin_lock_irqsave(&dev->irqlock, flags);
+
...
+
+	switch (ctrl->id) {
+	case V4L2_CID_CODEC_STREAM_SIZE:
+		/* FIXME: return valid stream size based on SFR */
+		ctrl->value = ctx->enc_dst_buf_size;
+		break;
+	case V4L2_CID_CODEC_FRAME_COUNT:
+		ctrl->value = ctx->frame_count;
+		break;
+	case V4L2_CID_CODEC_FRAME_TYPE:
...
+
+	switch (ctrl->id) {
+	case V4L2_CID_CODEC_GOP_SIZE:
+		p->gop_size = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_MULTI_SLICE_MODE:
+		p->slice_mode = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_MULTI_SLICE_MB:
+		p->slice_mb = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_MULTI_SLICE_BIT:
+		p->slice_bit = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_INTRA_REFRESH_MB:
+		p->intra_refresh_mb = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_PAD_CTRL_ENABLE:
+		p->pad = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_PAD_LUMA_VALUE:
+		p->pad_luma = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_PAD_CB_VALUE:
+		p->pad_cb = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_PAD_CR_VALUE:
+		p->pad_cr = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_RC_FRAME_ENABLE:
+		p->rc_frame = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_RC_BIT_RATE:
+		p->rc_bitrate = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_RC_REACTION_COEFF:
+		p->rc_reaction_coeff = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_FORCE_FRAME_TYPE:
+		ctx->force_frame_type = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_VBV_BUF_SIZE:
+		p->vbv_buf_size = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_SEQ_HDR_MODE:
+		p->seq_hdr_mode = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_FRAME_SKIP_MODE:
+		p->frame_skip_mode = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_RC_FIXED_TARGET_BIT:
+		p->fixed_target_bit = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_B_FRAMES:
+		p->codec.h264.num_b_frame = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_PROFILE:
+		p->codec.h264.profile = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_LEVEL:
+		p->codec.h264.level = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_INTERLACE:
+		p->codec.h264.interlace = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_LOOP_FILTER_MODE:
+		p->codec.h264.loop_filter_mode = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_LOOP_FILTER_ALPHA:
+		p->codec.h264.loop_filter_alpha = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_LOOP_FILTER_BETA:
+		p->codec.h264.loop_filter_beta = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_ENTROPY_MODE:
+		p->codec.h264.entropy_mode = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_MAX_REF_PIC:
+		p->codec.h264.max_ref_pic = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_NUM_REF_PIC_4P:
+		p->codec.h264.num_ref_pic_4p = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_8X8_TRANSFORM:
+		p->codec.h264._8x8_transform = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_RC_MB_ENABLE:
+		p->codec.h264.rc_mb = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_RC_FRAME_RATE:
+		p->codec.h264.rc_framerate = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_RC_FRAME_QP:
+		p->codec.h264.rc_frame_qp = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_RC_MIN_QP:
+		p->codec.h264.rc_min_qp = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_RC_MAX_QP:
+		p->codec.h264.rc_max_qp = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_RC_MB_DARK:
+		p->codec.h264.rc_mb_dark = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_RC_MB_SMOOTH:
+		p->codec.h264.rc_mb_smooth = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_RC_MB_STATIC:
+		p->codec.h264.rc_mb_static = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_RC_MB_ACTIVITY:
+		p->codec.h264.rc_mb_activity = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_RC_P_FRAME_QP:
+		p->codec.h264.rc_p_frame_qp = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_RC_B_FRAME_QP:
+		p->codec.h264.rc_b_frame_qp = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_AR_VUI_ENABLE:
+		p->codec.h264.ar_vui = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_AR_VUI_IDC:
+		p->codec.h264.ar_vui_idc = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_EXT_SAR_WIDTH:
+		p->codec.h264.ext_sar_width = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_EXT_SAR_HEIGHT:
+		p->codec.h264.ext_sar_height = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_OPEN_GOP:
+		p->codec.h264.open_gop = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H264_I_PERIOD:
+		p->codec.h264.open_gop_size = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_MPEG4_B_FRAMES:
+		p->codec.mpeg4.num_b_frame = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_MPEG4_PROFILE:
+		p->codec.mpeg4.profile = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_MPEG4_LEVEL:
+		p->codec.mpeg4.level = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_MPEG4_RC_FRAME_QP:
+		p->codec.mpeg4.rc_frame_qp = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_MPEG4_RC_MIN_QP:
+		p->codec.mpeg4.rc_min_qp = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_MPEG4_RC_MAX_QP:
+		p->codec.mpeg4.rc_max_qp = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_MPEG4_QUARTER_PIXEL:
+		p->codec.mpeg4.quarter_pixel = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_MPEG4_RC_P_FRAME_QP:
+		p->codec.mpeg4.rc_p_frame_qp = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_MPEG4_RC_B_FRAME_QP:
+		p->codec.mpeg4.rc_b_frame_qp = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_MPEG4_VOP_TIME_RES:
+		p->codec.mpeg4.vop_time_res = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_MPEG4_VOP_FRM_DELTA:
+		p->codec.mpeg4.vop_frm_delta = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H263_RC_FRAME_RATE:
+		p->codec.mpeg4.rc_framerate = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H263_RC_FRAME_QP:
+		p->codec.mpeg4.rc_frame_qp = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H263_RC_MIN_QP:
+		p->codec.mpeg4.rc_min_qp = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H263_RC_MAX_QP:
+		p->codec.mpeg4.rc_max_qp = ctrl->value;
+		break;
+	case V4L2_CID_CODEC_H263_RC_P_FRAME_QP:
+		p->codec.mpeg4.rc_p_frame_qp = ctrl->value;
+		break;
+	default:
+		v4l2_err(&dev->v4l2_dev, "Invalid control\n");
...
+	if (p->slice_mode == V4L2_CODEC_MULTI_SLICE_MODE_MACROBLOCK_COUNT) {
+		WRITEL(p->slice_mb, S5P_FIMV_ENC_MSLICE_MB);
+	} else if (p->slice_mode == V4L2_CODEC_MULTI_SLICE_MODE_BIT_COUNT) {
...
+
+	/* bit rate */
+	if (p->rc_frame == V4L2_CODEC_SW_ENABLE)
+		WRITEL(p->rc_bitrate,
...
+	/** vbv buffer size */
+	if (p->frame_skip_mode == V4L2_CODEC_FRAME_SKIP_MODE_VBV_BUF_SIZE) {
+		shm &= ~(0xFFFF << 16);
...
+	/** height */
+	if (p_264->interlace == V4L2_CODEC_SW_ENABLE)
...
+	if (p_264->rc_mb == V4L2_CODEC_SW_ENABLE) {
...
+	}
+
+	if ((p->rc_frame == V4L2_CODEC_SW_DISABLE) &&
+	    (p_264->rc_mb == V4L2_CODEC_SW_DISABLE)) {
...
+	s5p_mfc_write_shm(ctx, shm, EXT_ENC_CONTROL);
+
+	if (p_264->ar_vui == V4L2_CODEC_SW_ENABLE) {
+		/* aspect ration IDC */
...
+	/** value */
+	if (p_264->open_gop == V4L2_CODEC_SW_ENABLE) {
+		shm &= ~(0xFFFF);
...
+		s5p_mfc_cache_clean((void *)(ctx->shm + ofs), 4);	\
+	} while(0)
+
+static inline u32 s5p_mfc_read_shm(struct s5p_mfc_ctx *ctx, unsigned int ofs)
+{
+	s5p_mfc_cache_inv((void *)(ctx->shm + ofs), 4);
