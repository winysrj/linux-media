...
Received: from mail.gmx.net ([213.165.64.20]:52544 "HELO mail.gmx.net"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with SMTP
	id S1751858AbZIBMdx (ORCPT <rfc822;linux-media@vger.kernel.org>);
	Wed, 2 Sep 2009 08:33:53 -0400
Date: Wed, 2 Sep 2009 14:33:50 +0200 (CEST)
...
Subject: [PATCH 3/3] soc-camera: convert to the new imagebus API
In-Reply-To: <Pine.LNX.4.64.0909021416520.6326@axis700.grange>
Message-ID: <Pine.LNX.4.64.0909021430590.6326@axis700.grange>
...
PXA270, i.MX31, sh7722, mt9m001, mt9t031, mt9v022, ov772x, tw9910 run-time 
tested, ap325rxa, mx1_camera and soc_camera_platform only compile-tested. 
As I said in the introduction mail, I think, the latter is broken.
 ...
 drivers/media/video/pxa_camera.c           |  263 +++++++++++-----------
 ...
 15 files changed, 940 insertions(+), 861 deletions(-)

diff --git a/arch/sh/boards/board-ap325rxa.c b/arch/sh/boards/board-ap325rxa.c
index 48ab77e..2f389e2 100644
--- a/arch/sh/boards/board-ap325rxa.c
+++ b/arch/sh/boards/board-ap325rxa.c
@@ -314,8 +314,8 @@ static struct soc_camera_platform_info camera_info = {
 	.format_name = "UYVY",
 	.format_depth = 16,
 	.format = {
...
+	    mt9m111->code == V4L2_IMGBUS_FMT_SBGGR10_2X8)
 		is_raw_format = 1;
 	else
 		is_raw_format = 0;
...
+	case V4L2_IMGBUS_FMT_SBGGR10_2X8:
...
+		imgf->code == V4L2_IMGBUS_FMT_SBGGR10_2X8;
...
index 7722ee4..efd38f8 100644
--- a/drivers/media/video/pxa_camera.c
+++ b/drivers/media/video/pxa_camera.c
@@ -182,16 +182,12 @@ struct pxa_cam_dma {
 /* buffer for one video frame */
 struct pxa_buffer {
 	/* common v4l buffer stuff -- must be first */
-	struct videobuf_buffer vb;
...
@@ -242,11 +238,15 @@ static int pxa_videobuf_setup(struct videobuf_queue *vq, unsigned int *count,
 			      unsigned int *size)
 {
 	struct soc_camera_device *icd = vq->priv_data;
+	int bytes_per_line = v4l2_imgbus_bytes_per_line(icd->user_width,
...
+	*size = bytes_per_line * icd->user_height;
 	...
 	if (0 == *count)
 		*count = 32;
@@ -432,6 +432,11 @@ static int pxa_videobuf_prepare(struct videobuf_queue *vq,
...
+		return bytes_per_line;
 	...
 	dev_dbg(dev, "%s (vb=0x%p) 0x%08lx %d\n", __func__,
 		vb, vb->baddr, vb->bsize);
@@ -455,18 +460,18 @@ static int pxa_videobuf_prepare(struct videobuf_queue *vq,
 	 */
 	buf->inwork = 1;
...
+	vb->size = bytes_per_line * vb->height;
 	if (0 != vb->baddr && vb->bsize < vb->size) {
 		ret = -EINVAL;
 		goto out;
@@ -1156,9 +1161,15 @@ static int pxa_camera_set_bus_param(struct soc_camera_device *icd, __u32 pixfmt)
 	struct soc_camera_host *ici = to_soc_camera_host(icd->dev.parent);
...
@@ -1222,59 +1233,50 @@ static int pxa_camera_try_bus_param(struct soc_camera_device *icd,
...
@@ -1288,45 +1290,40 @@ static int pxa_camera_get_formats(struct soc_camera_device *icd, int idx,
 		cam = icd->host_priv;
 	}
...
+		xlate++;
 	...
 	return formats;
@@ -1338,11 +1335,11 @@ static void pxa_camera_put_formats(struct soc_camera_device *icd)
 	icd->host_priv = NULL;
 }
...
@@ -1357,9 +1354,9 @@ static int pxa_camera_set_crop(struct soc_camera_device *icd,
...
@@ -1376,27 +1373,23 @@ static int pxa_camera_set_crop(struct soc_camera_device *icd,
 		return ret;
 	}
...
+		if (pxa_camera_check_frame(imgf.width, imgf.height)) {
 			...
 			return -EINVAL;
@@ -1413,10 +1406,10 @@ static int pxa_camera_set_crop(struct soc_camera_device *icd,
...
@@ -1428,14 +1421,13 @@ static int pxa_camera_set_fmt(struct soc_camera_device *icd,
 	struct pxa_camera_dev *pcdev = ici->priv;
 	struct device *dev = icd->dev.parent;
 	struct v4l2_subdev *sd = soc_camera_to_subdev(icd);
...
+	struct v4l2_imgbus_framefmt imgf;
 	...
 	xlate = soc_camera_xlate_by_fourcc(icd, pix->pixelformat);
@@ -1444,24 +1436,27 @@ static int pxa_camera_set_fmt(struct soc_camera_device *icd,
 		return -EINVAL;
 	}
...
@@ -1473,10 +1468,13 @@ static int pxa_camera_set_fmt(struct soc_camera_device *icd,
 		recalculate_fifo_timeout(pcdev, sense.pixel_clock);
 	}
...
@@ -1484,17 +1482,16 @@ static int pxa_camera_set_fmt(struct soc_camera_device *icd,
...
@@ -1508,22 +1505,34 @@ static int pxa_camera_try_fmt(struct soc_camera_device *icd,
 			      &pix->height, 32, 2048, 0,
...
@@ -745,7 +755,8 @@ static int sh_mobile_ceu_set_bus_param(struct soc_camera_device *icd,
 	return 0;
 }
...
+				       unsigned char buswidth)
 {
 	struct soc_camera_host *ici = to_soc_camera_host(icd->dev.parent);
 	struct sh_mobile_ceu_dev *pcdev = ici->priv;
@@ -754,48 +765,79 @@ static int sh_mobile_ceu_try_bus_param(struct soc_camera_device *icd)
...
@@ -813,13 +855,13 @@ static int sh_mobile_ceu_get_formats(struct soc_camera_device *icd, int idx,
 	if (!idx)
 		cam->extra_fmt = NULL;
...
@@ -830,32 +872,31 @@ static int sh_mobile_ceu_get_formats(struct soc_camera_device *icd, int idx,
...
+			xlate->host_fmt->name);
 	...
 	return formats;
@@ -1035,17 +1076,15 @@ static int client_s_crop(struct v4l2_subdev *sd, struct v4l2_crop *crop,
...
@@ -1060,32 +1099,29 @@ static int get_camera_subwin(struct soc_camera_device *icd,
 	if (!ceu_rect->width) {
 		struct v4l2_subdev *sd = soc_camera_to_subdev(icd);
 		struct device *dev = icd->dev.parent;
-		struct v4l2_format f;
...
@@ -1102,13 +1138,12 @@ static int get_camera_subwin(struct soc_camera_device *icd,
 	return 0;
 }
...
+	unsigned int width = imgf->width, height = imgf->height, tmp_w, tmp_h;
 	...
 	int ret;
@@ -1122,29 +1157,29 @@ static int client_s_fmt(struct soc_camera_device *icd, struct v4l2_format *f,
...
@@ -1162,27 +1197,26 @@ static int client_s_fmt(struct soc_camera_device *icd, struct v4l2_format *f,
  */
...
@@ -1191,10 +1225,10 @@ static int client_scale(struct soc_camera_device *icd, struct v4l2_rect *rect,
...
@@ -1258,8 +1292,7 @@ static int sh_mobile_ceu_set_crop(struct soc_camera_device *icd,
...
+	struct v4l2_imgbus_framefmt imgf;
 	...
 	u32 capsr, cflcr;
@@ -1308,25 +1341,24 @@ static int sh_mobile_ceu_set_crop(struct soc_camera_device *icd,
...
+	ret = client_scale(icd, cam_rect, rect, ceu_rect, &imgf,
 	...
 	dev_geo(dev, "6-9: %d\n", ret);
@@ -1374,8 +1406,7 @@ static int sh_mobile_ceu_set_fmt(struct soc_camera_device *icd,
 	struct sh_mobile_ceu_dev *pcdev = ici->priv;
 	struct sh_mobile_ceu_cam *cam = icd->host_priv;
 	struct v4l2_pix_format *pix = &f->fmt.pix;
-	struct v4l2_format cam_f = *f;
...
+	struct v4l2_imgbus_framefmt imgf;
 	...
 	struct device *dev = icd->dev.parent;
 	__u32 pixfmt = pix->pixelformat;
@@ -1444,9 +1475,10 @@ static int sh_mobile_ceu_set_fmt(struct soc_camera_device *icd,
...
+	imgf.field	= pix->field;
 	...
 	switch (pixfmt) {
 	case V4L2_PIX_FMT_NV12:
@@ -1459,11 +1491,10 @@ static int sh_mobile_ceu_set_fmt(struct soc_camera_device *icd,
 		image_mode = false;
 	}
...
@@ -1471,20 +1502,20 @@ static int sh_mobile_ceu_set_fmt(struct soc_camera_device *icd,
...
@@ -1498,10 +1529,9 @@ static int sh_mobile_ceu_set_fmt(struct soc_camera_device *icd,
...
@@ -1515,6 +1545,7 @@ static int sh_mobile_ceu_try_fmt(struct soc_camera_device *icd,
 	const struct soc_camera_format_xlate *xlate;
 	struct v4l2_pix_format *pix = &f->fmt.pix;
 	struct v4l2_subdev *sd = soc_camera_to_subdev(icd);
+	struct v4l2_imgbus_framefmt imgf;
 	...
 	int width, height;
 	int ret;
@@ -1533,18 +1564,24 @@ static int sh_mobile_ceu_try_fmt(struct soc_camera_device *icd,
 	width = pix->width;
 	height = pix->height;
...
+
 	...
 	case V4L2_PIX_FMT_NV12:
 	case V4L2_PIX_FMT_NV21:
@@ -1556,7 +1593,7 @@ static int sh_mobile_ceu_try_fmt(struct soc_camera_device *icd,
...
+			ret = v4l2_subdev_call(sd, video, try_imgbus_fmt, &imgf);
 			if (ret < 0) {
 				...
 				dev_err(icd->dev.parent,
@@ -1662,7 +1699,7 @@ static int sh_mobile_ceu_set_ctrl(struct soc_camera_device *icd,
...
-
 	 ...
 	 */
@@ -917,14 +909,25 @@ static struct v4l2_subdev_core_ops tw9910_subdev_core_ops = {
 #endif
 };
...
@@ -159,23 +156,13 @@ static inline struct v4l2_subdev *soc_camera_to_subdev(
...
@@ -183,9 +170,8 @@ struct soc_camera_data_format {
  * format setup.
  */
