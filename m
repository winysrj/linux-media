...
Received: from mail.gmx.net ([213.165.64.20]:50086 "HELO mail.gmx.net"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with SMTP
	id S1754481AbZDUJ05 (ORCPT <rfc822;linux-media@vger.kernel.org>);
	Tue, 21 Apr 2009 05:26:57 -0400
Date: Tue, 21 Apr 2009 11:26:53 +0200 (CEST)
From: Guennadi Liakhovetski <g.liakhovetski@gmx.de>
To: Linux Media Mailing List <linux-media@vger.kernel.org>
cc: Hans Verkuil <hverkuil@xs4all.nl>,
	Magnus Damm <magnus.damm@gmail.com>,
	Robert Jarzmik <robert.jarzmik@free.fr>
Subject: [PATCH/RFC v1] soc-camera: (partially) convert to v4l2-(sub)dev API
Message-ID: <Pine.LNX.4.64.0904211102400.6551@axis700.grange>
MIME-Version: 1.0
...
This is based on my earlier patches from today, specifically, it requires 
the "v4l2-subdev: add a v4l2_i2c_new_dev_subdev() function" patch. 
Obviously, this is also based on my "slightly" outdated current 
repository, which I will be rebasing now. I'll upload my updated stack too 
for those eager to test:-)

diff --git a/arch/arm/mach-mx3/pcm037.c b/arch/arm/mach-mx3/pcm037.c
index 8e2fee6..e6a81a8 100644
--- a/arch/arm/mach-mx3/pcm037.c
+++ b/arch/arm/mach-mx3/pcm037.c
@@ -283,7 +283,7 @@ static struct at24_platform_data board_eeprom = {
 	.flags = AT24_FLAG_ADDR16,
 };
 
-static int pcm037_camera_power(struct device *dev, int on)
+static int pcm037_camera_power(struct soc_camera_link *link, int on)
 {
 	/* disable or enable the camera in X7 or X8 PCM970 connector */
 	gpio_set_value(IOMUX_TO_GPIO(MX31_PIN_CSI_D5), !on);
@@ -301,6 +301,7 @@ static struct soc_camera_link iclink = {
...
index d4eb0c7..828e7c4 100644
--- a/arch/arm/mach-pxa/em-x270.c
+++ b/arch/arm/mach-pxa/em-x270.c
@@ -824,7 +824,7 @@ struct pxacamera_platform_data em_x270_camera_platform_data = {
 	.mclk_10khz = 2600,
 };
 
-static int em_x270_sensor_power(struct device *dev, int on)
+static int em_x270_sensor_power(struct soc_camera_link *link, int on)
 {
 	int ret;
 	int is_on = regulator_is_enabled(em_x270_camera_ldo);
@@ -858,6 +858,7 @@ static struct soc_camera_link iclink = {
...
index 204263d..6702bfc 100644
--- a/arch/arm/mach-pxa/mioa701.c
+++ b/arch/arm/mach-pxa/mioa701.c
@@ -737,6 +737,7 @@ static struct soc_camera_link iclink = {
...
index 619b90e..a1ae436 100644
--- a/arch/arm/mach-pxa/pcm990-baseboard.c
+++ b/arch/arm/mach-pxa/pcm990-baseboard.c
@@ -452,6 +452,7 @@ static struct soc_camera_link iclink[] = {
...
@@ -459,6 +460,7 @@ static struct soc_camera_link iclink[] = {
 		.query_bus_param	= pcm990_camera_query_bus_param,
 		.set_bus_param		= pcm990_camera_set_bus_param,
 		.free_bus		= pcm990_camera_free_bus,
+		.module_name		= "mt9m001",
...
index 37c9139..1edd1ec 100644
--- a/arch/sh/boards/board-ap325rxa.c
+++ b/arch/sh/boards/board-ap325rxa.c
@@ -249,7 +249,7 @@ static unsigned char camera_ncm03j_magic[] =
 	0x63, 0xD4, 0x64, 0xEA, 0xD6, 0x0F,
 };
 
-static int ap325rxa_camera_power(struct device *dev, int on)
+static int ap325rxa_camera_power(struct soc_camera_link *link, int on)
 {
 	gpio_set_value(GPIO_PTZ5, on); /* RST_CAM/RSTB */
 	mdelay(10);
@@ -305,6 +305,7 @@ static struct soc_camera_platform_info camera_info = {
...
index b8cb246..238cbff 100644
--- a/arch/sh/boards/mach-migor/setup.c
+++ b/arch/sh/boards/mach-migor/setup.c
@@ -331,7 +331,7 @@ static void camera_power_off(void)
 	mutex_unlock(&camera_lock);
 }
 
-static int ov7725_power(struct device *dev, int mode)
+static int ov7725_power(struct soc_camera_link *link, int mode)
 {
 	if (mode)
 		camera_power_on(0);
@@ -341,7 +341,7 @@ static int ov7725_power(struct device *dev, int mode)
 	return 0;
 }
 
-static int tw9910_power(struct device *dev, int mode)
+static int tw9910_power(struct soc_camera_link *link, int mode)
 {
 	if (mode)
 		camera_power_on(1);
@@ -419,9 +419,10 @@ static struct i2c_board_info migor_camera_i2c[] = {
...
@@ -429,9 +430,10 @@ static struct tw9910_video_info tw9910_info = {
 	.buswidth = SOCAM_DATAWIDTH_8,
...
+	videobuf_queue_dma_contig_init(q, &mx1_videobuf_ops, icd->dev.parent,
 					...
 					V4L2_FIELD_NONE,
@@ -543,14 +543,13 @@ static int mx1_camera_set_bus_param(struct soc_camera_device *icd, __u32 pixfmt)
...
@@ -741,7 +740,7 @@ static int __init mx1_camera_probe(struct platform_device *pdev)
...
@@ -1148,7 +1134,7 @@ static int mx3_camera_probe(struct platform_device *pdev)
...
-
 	if (!try_module_get(ici->ops->owner)) {
 		...
 		ret = -EINVAL;
@@ -366,17 +351,29 @@ static int soc_camera_open(struct file *file)
 		if (ret < 0)
 			goto eiufmt;
...
+
 		...
 		if (ret < 0)
@@ -393,20 +390,23 @@ static int soc_camera_open(struct file *file)
 	return 0;
...
@@ -809,15 +804,18 @@ static void scan_add_host(struct soc_camera_host *ici)
...
+	struct v4l2_subdev *subdev;
 	...
 	if (!adap) {
 		ret = -ENODEV;
@@ -827,42 +825,62 @@ static int soc_camera_probe(struct device *dev)
...
-	if (!client) {
+	subdev = v4l2_i2c_new_dev_subdev(adap, icl->module_name,
+					 icl->board_info, &ici->v4l2_dev);
+	if (!subdev) {
 		ret = -ENOMEM;
 		goto ei2cnd;
 	}
...
+eadd:
 ...
 ei2cga:
 	return ret;
@@ -888,6 +906,7 @@ static int soc_camera_remove(struct device *dev)
 		struct i2c_client *client =
...
@@ -940,6 +959,7 @@ static void dummy_release(struct device *dev)
...
+	int ret;
 	...
 	if (!ici || !ici->ops ||
 	    !ici->ops->try_fmt ||
@@ -952,18 +972,20 @@ int soc_camera_host_register(struct soc_camera_host *ici)
 	    !ici->ops->add ||
 	    !ici->ops->remove ||
 	    !ici->ops->poll || 
-	    !ici->dev)
...
+		goto edevreg;
 	...
 	mutex_unlock(&list_lock);
@@ -971,6 +994,10 @@ int soc_camera_host_register(struct soc_camera_host *ici)
 	scan_add_host(ici);
...
@@ -984,7 +1011,7 @@ void soc_camera_host_unregister(struct soc_camera_host *ici)
 	list_del(&ici->list);
 
 	list_for_each_entry(icd, &devices, list) {
-		if (icd->dev.parent == ici->dev) {
+		if (icd->dev.parent == ici->v4l2_dev.dev) {
 			/* The bus->remove will be called */
 			device_unregister(&icd->dev);
 			/* Not before device_unregister(), .remove
@@ -999,7 +1026,7 @@ void soc_camera_host_unregister(struct soc_camera_host *ici)
...
@@ -1079,7 +1106,6 @@ static int video_dev_create(struct soc_camera_device *icd)
...
@@ -1100,46 +1126,33 @@ static int video_dev_create(struct soc_camera_device *icd)
...
-EXPORT_SYMBOL(soc_camera_video_stop);
 ...
 {
@@ -1178,7 +1195,7 @@ static int __devexit soc_camera_pdrv_remove(struct platform_device *pdev)
 	return 0;
 }
 
-static struct platform_driver soc_camera_pdrv = {
+static struct platform_driver soc_camera_pdriver = {
 	.remove  = __exit_p(soc_camera_pdrv_remove),
 	.driver  = {
 		.name = "soc-camera-pdrv",
@@ -1195,7 +1212,7 @@ static int __init soc_camera_init(void)
 	if (ret)
 		goto edrvr;
 
-	ret = platform_driver_probe(&soc_camera_pdrv, soc_camera_pdrv_probe);
+	ret = platform_driver_probe(&soc_camera_pdriver, soc_camera_pdrv_probe);
 	if (ret)
 		goto epdr;
...
+	platform_driver_unregister(&soc_camera_pdriver);
 	driver_unregister(&ic_drv);
 	bus_unregister(&soc_camera_bus_type);
 }
diff --git a/drivers/media/video/soc_camera_platform.c b/drivers/media/video/soc_camera_platform.c
index 021f9f1..b9036d8 100644
...
+static struct soc_camera_platform_priv *to_priv(const struct i2c_client *client)
 {
-	struct i2c_client *client = to_i2c_client(to_soc_camera_control(icd));
-	struct soc_camera_platform_info *p = i2c_to_info(client);
-
...
+			    struct soc_camera_platform_priv, subdev);
 }
...
+	struct soc_camera_platform_priv *priv = to_priv(client);
+	return priv->info;
 }
...
@@ -89,16 +67,10 @@ static int soc_camera_platform_set_crop(struct soc_camera_device *icd,
 	return 0;
 }
...
@@ -110,7 +82,7 @@ static int soc_camera_platform_try_fmt(struct soc_camera_device *icd,
...
@@ -129,15 +101,20 @@ static void soc_camera_platform_video_remove(struct soc_camera_device *icd)
 	icd->ops = NULL;
 }
...
@@ -169,7 +146,8 @@ static int soc_camera_platform_probe(struct i2c_client *client,
 		return -ENOMEM;
 
 	priv->info = p;
-	i2c_set_clientdata(client, priv);
+
+	v4l2_i2c_subdev_init(&priv->subdev, client, &platform_subdev_ops);
 
 	icd->ops	= &soc_camera_platform_ops;
 	icd->width_min	= 0;
@@ -189,7 +167,7 @@ static int soc_camera_platform_probe(struct i2c_client *client,
...
@@ -848,25 +815,36 @@ static void tw9910_video_remove(struct soc_camera_device *icd)
 }
...
+static struct v4l2_subdev_tuner_ops tw9910_subdev_tuner_ops = {
+	.s_std		= tw9910_s_std,
+};
+
...
+
  ...
  * i2c_driver function
  */
@@ -907,7 +885,8 @@ static int tw9910_probe(struct i2c_client *client,
 		return -ENOMEM;
...
@@ -946,7 +925,7 @@ static int tw9910_probe(struct i2c_client *client,
...
@@ -46,7 +47,6 @@ struct soc_camera_device {
...
-	struct module *owner;
 	...
 	int use_count;
@@ -59,8 +59,8 @@ struct soc_camera_file {
 };
...
-	struct device *dev;
 	...
 	void *priv;
 	const char *drv_name;
@@ -98,10 +98,11 @@ struct soc_camera_link {
...
+	int (*power)(struct soc_camera_link *, int);
+	int (*reset)(struct soc_camera_link *);
 	/*
 	 * some platforms may support different data widths than the sensors
 	 * native ones due to different data line routing. Let the board code
@@ -119,7 +120,9 @@ static inline struct soc_camera_device *to_soc_camera_dev(struct device *dev)
...
@@ -135,9 +138,6 @@ static inline struct device *to_soc_camera_control(struct soc_camera_device *icd
 int soc_camera_host_register(struct soc_camera_host *ici);
...
@@ -168,28 +168,15 @@ struct soc_camera_format_xlate {
 };
