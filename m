...
Received: from smtp-vbr8.xs4all.nl ([194.109.24.28]:3520 "EHLO
	smtp-vbr8.xs4all.nl" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1752827AbZKONQE (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Sun, 15 Nov 2009 08:16:04 -0500
From: Hans Verkuil <hverkuil@xs4all.nl>
To: santiago.nunez@ridgerun.com
Subject: Re: [PATCH 3/4 v7] TVP7002 driver for DM365
Date: Sun, 15 Nov 2009 14:16:00 +0100
...
In-Reply-To: <1257889836-19208-1-git-send-email-santiago.nunez@ridgerun.com>
MIME-Version: 1.0
Content-Type: text/plain;
  charset="iso-8859-15"
Content-Transfer-Encoding: 7bit
Content-Disposition: inline
Message-Id: <200911151416.00674.hverkuil@xs4all.nl>
Sender: linux-media-owner@vger.kernel.org
List-ID: <linux-media.vger.kernel.org>

On Tuesday 10 November 2009 22:50:36 santiago.nunez@ridgerun.com wrote:
...
The four defines above all refer to an interlaced format, right?
If so, then I suggest changing 1080 to 1080I.

Also, is there no 1080P50 support? I see 720P50 and 1080I60. Just
wondering.
...
Pointless check: we *know* that we never get a NULL pointer here.
...
Why not merge tvp7002_map_set_preset and tvp7002_set_fmt_parms? I would also
suggest creating a small static const table mapping a preset to the register
params. It's cleaner code that way.
...
Even better: merge them all into this function. You make things unnecessarily
complex by splitting this up into three functions. After all, all that this
really does is a table lookup and then set the registers.
...
Hmm, I suggest that you make a static const table along these lines:

static const struct tvp7002_preset {
	u32 dv_preset;
	const struct i2c_reg_value *fmt_regs;
...
So you can use INDEX_* as the index into this table, and use a simple for-loop
(inline function?) to match a dv_preset to this index.

This whole function can then be replaced by a simple table lookup.
...
Ditto for this scanmode function.
...
You can remove this check: this is something that the bridge driver will
check.
...
This debug message seems of limited value to me. It's up to you, though.
...
Hmm, the previous version of this function was actually more readable. All
those gotos do not help here.

What about this:

static const struct tvp7002_dv_check {
	u16 progressive;
	u16 lines_per_frame;
	u16 cpl_min;
	u16 cpl_max;
} tvp7002_dv_check_tbl[] = {
	/* INDEX_720P60 */
	{ 1, TVP7002_LINES_720, TVP7002_CPL_720P_60_LOWER, TVP7002_CPL_720P_60_UPPER },
	...
	/* INDEX_576P50 */
	{ 1, 625, 0, 0xffff },
	{ 0, 0, 0, 0 }
...
and do this:
	...
	const struct tvp7002_dv_check *chk = tvp7002_dv_check_tbl;

	for (index = 0; chk->lines_per_frame; index++, chk++) {
		if (chk->progressive != progressive ||
		    chk->lines_per_frame != lines_per_frame)
			continue;
		/* You need to check if this works OK with the SD presets,
		   clocks_per_line should perhaps become a u16 as well. */
		if (clocks_per_line >= chk->cpl_min &&
		    clocks_per_line <= chk->cpl_max)
			break;
	}
	if (!chk->lines_per_frame) {
...
And of course, the data in tvp7002_dv_check can actually go to the global
preset table I suggested earlier.
...
fmtdesc->index is unsigned, so no need to check for neg numbers.
...
First of all, are you really, really sure that this is the way to program this
...
print 'No standard found.' and return 0. After all, that IS the status of the
