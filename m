...
Received: from mailout3.samsung.com ([203.254.224.33]:29071 "EHLO
	mailout3.samsung.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1751310Ab1LFPEW (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Tue, 6 Dec 2011 10:04:22 -0500
Received: from epcpsbgm2.samsung.com (mailout3.samsung.com [203.254.224.33])
 by mailout3.samsung.com
 (Oracle Communications Messaging Exchange Server 7u4-19.01 64bit (built Sep  7
 2010)) with ESMTP id <0LVS000RNF784ZO0@mailout3.samsung.com> for
 linux-media@vger.kernel.org; Wed, 07 Dec 2011 00:04:20 +0900 (KST)
Received: from AMDN157 ([106.116.48.215])
 by mmp2.samsung.com (Oracle Communications Messaging Exchange Server 7u4-19.01
 64bit (built Sep  7 2010)) with ESMTPA id <0LVS00AS7F72KE10@mmp2.samsung.com>
 for linux-media@vger.kernel.org; Wed, 07 Dec 2011 00:04:20 +0900 (KST)
...
In-reply-to: <20111206143538.GD938@valkosipuli.localdomain>
Subject: RE: [RFC] Resolution change support in video codecs in v4l2
Date: Tue, 06 Dec 2011 16:03:33 +0100
Message-id: <00da01ccb428$3c9522c0$b5bf6840$%debski@samsung.com>
MIME-version: 1.0
...
I agree. This memory could be used by other application in that time. Look
at our CMA memory manager. The memory that is not used by the hardware can
be used by user processes.
...
> >
> > The decision of stopping the streaming should be taken by the driver. API
> needs
> > to properly support it and provide some way for userspace to re-start
> streaming
...
Second that - in memory-to-memory devices streaming is one of the 
requirements that are needed to do processing. You need to have sufficient
number of input and buffers queued. Streaming is usually just a flag that
enables processing.
...
> >
> > OK, but userspace applications may use REQBUFS instead of CREATE_BUFFERS,
> as REQBUFS
> > is part of the API.
> 
...
Another thing that we were talking about is the need to use old buffers 
after new get allocated. If we use REQBUFS and theses are MMAP buffers then
they will get lost.
This was really useful in GStreamer, where some buffer could be used later in
the pipeline. They just can't disappear.
...
> > >frames, simply.
> >
> > Ok, but let's not add unneeded things at the API if you're not sure. If we
> have
