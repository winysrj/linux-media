...
Received: from mx1.redhat.com ([209.132.183.28]:57162 "EHLO mx1.redhat.com"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
	id S1751852Ab1BDA6m (ORCPT <rfc822;linux-media@vger.kernel.org>);
	Thu, 3 Feb 2011 19:58:42 -0500
Message-ID: <4D4B4F3D.50705@redhat.com>
Date: Thu, 03 Feb 2011 22:58:37 -0200
...
Subject: Re: [RFC PATCH] tuner-core: fix broken G_TUNER call when tuner is
 in standby
References: <AANLkTim3=Xuyantq2zKJ0W8C+-objnBQNbYNaqb9pgc-@mail.gmail.com>	<4D497693.5020403@redhat.com> <AANLkTikXKxdPDQZjdi_LwqGSUpiYbDvR3=EXnJS6u0Ok@mail.gmail.com>
In-Reply-To: <AANLkTikXKxdPDQZjdi_LwqGSUpiYbDvR3=EXnJS6u0Ok@mail.gmail.com>
...
This will change the behaviour for all tuners. Changing it 
is not a good idea, as applications may break.

With respect to the firmware load timing, the proper fix would be to honour
the O_NONBLOCK, e. g. if apps open device with O_NONBLOCK, the firmware load
should be asynchronous and ioctl calls should return an error code to indicate
that the operation is still happening. Otherwise, it should be ok to have
a long delay. A fix like that is probably not easy to implement, though.
...
Michael knows more about that part, as he wrote the patch that replaced is_stere with get_status,
but from the above grep, it seems that we can just remove is_stereo() from dvb_frontend.h and
from tuner_core.c.
> 
>>> +                     u32 tuner_status;
>>> +
...
has_signal() was introduced earlier. get_rf_strength() replaced it, AFAIK. Only
tda8290 is still using the old call. Again, Michael is the right guy for checking,
because he did the changes at the core, and he is maintainin
> 
...
check_mode() should be doing such changes. It is there to ensure that
the driver is on the right mode when tuner is called.

One of the major problems that tuner-core tries to address is the
switch between the different modes (radio, analog, digital?).

Btw, I just checked that we still have some V4L1 mess inside it.
I'll write a few cleanup patches and test with some devices here.
...
I see your point, but why some application would try to access a
power-down tuner, if it doesn't have the intention to power it up?

> I welcome any alternative patch which makes G_TUNER work per the spec
> for tuners in a standby state.

I'll try to write a patch for it.
