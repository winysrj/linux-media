...
Received: from mx1.redhat.com ([209.132.183.28]:14557 "EHLO mx1.redhat.com"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
	id S1754727Ab2DRTSo (ORCPT <rfc822;linux-media@vger.kernel.org>);
	Wed, 18 Apr 2012 15:18:44 -0400
Message-ID: <4F8F136E.7030800@redhat.com>
Date: Wed, 18 Apr 2012 16:18:06 -0300
...
References: <201204071924.44679.hfvogt@gmx.net> <4F8ECD00.70207@redhat.com> <4F8ED65D.5090105@iki.fi>
...
> 
> Is that checking needed and why? If there is no rc_device why we even call poll for it? Better to fix some core routines if that is true.
> 
> Also rc_keydown() takes 2nd param as int, but in that case it does not matter. Anyhow, 3rd param is toggle which is used by RC5/6. IIRC I have never implemented RC5 or RC6 remote receiver, so I am not sure if it is needed and in which case.

It is better to implement the toggle, when it is available/known, as the core will use it to detect when
the same key was pressed quickly twice, or if someone just kept it pressed by a long time.

When this is not implemented and someone presses the same key quickly twice (a "double click"),
the second click will be ignored, if the time is lower than REP_DELAY (by default, 500 ms).

Not all protocols/decoders can detect it though. NEC protocol can't. RC-5/RC-6 can do it.
...
> 
> This looks odd for my eyes. Maybe x = (y == z); is better. Checkpatch didn't complain it?
...
>>> +        }
> 
> I hate to default some random remote controller keytable. Use EMPTY map, it is for that.
> 
>>> +    }
>>> +
>>>       return 0;
...
> 
> Does this mean we promise userspace we can do both NEC and RC6? Does it mean we should offer method to change protocol in that case?
> I suspect it is not even possible to switch from remote protocol to other unless eeprom change or firmware hack.
