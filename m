...
Received: from mail-pg1-f195.google.com ([209.85.215.195]:32959 "EHLO
        ...
        with ESMTP id S1754551AbeGJBlV (ORCPT
        <rfc822;linux-media@vger.kernel.org>); Mon, 9 Jul 2018 21:41:21 -0400
Received: by mail-pg1-f195.google.com with SMTP id r5-v6so1592235pgv.0
        for <linux-media@vger.kernel.org>; Mon, 09 Jul 2018 18:41:21 -0700 (PDT)
MIME-Version: 1.0
In-Reply-To: <20180618145843.14631-17-paul.kocialkowski@bootlin.com>
References: <20180618145843.14631-1-paul.kocialkowski@bootlin.com> <20180618145843.14631-17-paul.kocialkowski@bootlin.com>
From: Ezequiel Garcia <ezequiel@vanguardiasur.com.ar>
Date: Mon, 9 Jul 2018 22:41:19 -0300
Message-ID: <CAAEAJfCr3G5SXyNS3okpJCU9n=KgzVu+cL3x35GeYtoYhmmOgA@mail.gmail.com>
Subject: Re: [linux-sunxi] [PATCH v4 16/19] media: platform: Add Sunxi-Cedrus
 VPU decoder driver
...
> diff --git a/drivers/media/platform/sunxi/cedrus/Kconfig b/drivers/media/=
platform/sunxi/cedrus/Kconfig
> new file mode 100644
...
> diff --git a/drivers/media/platform/sunxi/cedrus/Makefile b/drivers/media=
/platform/sunxi/cedrus/Makefile
> new file mode 100644
...
> +
> +cedrus-y =3D cedrus.o cedrus_video.o cedrus_hw.o cedrus_dec.o cedrus_mpe=
g2.o
> diff --git a/drivers/media/platform/sunxi/cedrus/cedrus.c b/drivers/media=
/platform/sunxi/cedrus/cedrus.c
> new file mode 100644
...
> +               .id             =3D V4L2_CID_MPEG_VIDEO_MPEG2_SLICE_HEADE=
R,
> +               .elem_size      =3D sizeof(struct v4l2_ctrl_mpeg2_slice_h=
eader),
> +       },
> +};
> +
...
> +       if (ret) {
> +               dev_err(&pdev->dev, "Failed to initialize media entity pa=
ds\n");
> +               return ret;
> +       }
> +
...
> +       snprintf(vfd->name, sizeof(vfd->name), "%s", cedrus_video_device.=
name);
> +
...
> diff --git a/drivers/media/platform/sunxi/cedrus/cedrus.h b/drivers/media=
/platform/sunxi/cedrus/cedrus.h
> new file mode 100644
...
> +
> +#endif
> diff --git a/drivers/media/platform/sunxi/cedrus/cedrus_dec.c b/drivers/m=
edia/platform/sunxi/cedrus/cedrus_dec.c
> new file mode 100644
...
> +       }
> +
> +       buffer_entry =3D list_last_entry(&ctx->src_list, struct cedrus_bu=
ffer, list);
...
> +
> +       buffer_entry =3D list_last_entry(&ctx->dst_list, struct cedrus_bu=
ffer, list);
...
I believe it's a mem2mem core nasty bug if you can't get a source or destin=
ation
...
> +       if (!run.dst) {
> +               v4l2_err(&dev->v4l2_dev, "No destination buffer to prepar=
e\n");
> +               return;
> +       }
> +
...
> +               }
> +
> +               run.mpeg2.hdr =3D get_ctrl_ptr(ctx, CEDRUS_CTRL_DEC_MPEG2=
_SLICE_HEADER);
...
> +
> +       if (!ctx->job_abort) {
> +               if (ctx->vpu_src_fmt->fourcc =3D=3D V4L2_PIX_FMT_MPEG2_SL=
ICE)
...
> +       /*
> +        * V4L2 M2M and request API cleanup is done here while hardware s=
tate
> +        * cleanup is done in the interrupt context. Doing all the cleanu=
p in
> +        * the interrupt context is a bit risky, since the job_abort call=
 might
> +        * originate from the release hook, where interrupts have already=
 been
> +        * disabled.
> +        */
> +
...
> +       v4l2_m2m_job_finish(ctx->dev->m2m_dev, ctx->fh.m2m_ctx);
> +}
> diff --git a/drivers/media/platform/sunxi/cedrus/cedrus_dec.h b/drivers/m=
edia/platform/sunxi/cedrus/cedrus_dec.h
> new file mode 100644
...
> +
> +#endif
> diff --git a/drivers/media/platform/sunxi/cedrus/cedrus_hw.c b/drivers/me=
dia/platform/sunxi/cedrus/cedrus_hw.c
> new file mode 100644
...
> +
> +int cedrus_engine_enable(struct cedrus_dev *dev, enum cedrus_engine engi=
ne)
> +{
> +       u32 reg =3D 0;
> +
...
> +               src_buffer->state =3D dst_buffer->state =3D VB2_BUF_STATE=
_ERROR;
> +       else
> +               src_buffer->state =3D dst_buffer->state =3D VB2_BUF_STATE=
_DONE;
...
> +
> +       /*
> +        * The VPU is only able to handle bus addresses so we have to sub=
tract
...
> +       of_reserved_mem_device_release(dev->dev);
> +}
> diff --git a/drivers/media/platform/sunxi/cedrus/cedrus_hw.h b/drivers/me=
dia/platform/sunxi/cedrus/cedrus_hw.h
> new file mode 100644
...
> +
> +#endif
> diff --git a/drivers/media/platform/sunxi/cedrus/cedrus_mpeg2.c b/drivers=
/media/platform/sunxi/cedrus/cedrus_mpeg2.c
> new file mode 100644
...
> +{
> +       struct cedrus_dev *dev =3D ctx->dev;
> +       const struct v4l2_ctrl_mpeg2_slice_header *frame_hdr =3D run->mpe=
g2.hdr;
> +
...
> +       dma_addr_t fwd_luma =3D 0, fwd_chroma =3D 0, bwd_luma =3D 0, bwd_=
chroma =3D 0;
> +
...
> +               fwd_chroma =3D vb2_dma_contig_plane_dma_addr(fwd_vb2_buf,=
 1);
> +       }
> +
...
> +               bwd_chroma =3D vb2_dma_contig_plane_dma_addr(bwd_vb2_buf,=
 1);
> +       }
> +
...
> +       dst_luma_addr =3D vb2_dma_contig_plane_dma_addr(&run->dst->vb2_bu=
f, 0);
> +       dst_chroma_addr =3D vb2_dma_contig_plane_dma_addr(&run->dst->vb2_=
buf, 1);
...
> +       src_buf_addr =3D vb2_dma_contig_plane_dma_addr(&run->src->vb2_buf=
, 0);
> +       cedrus_write(dev, VE_MPEG_VLD_ADDR, VE_MPEG_VLD_ADDR_VAL(src_buf_=
addr));
...
> +}
> diff --git a/drivers/media/platform/sunxi/cedrus/cedrus_mpeg2.h b/drivers=
/media/platform/sunxi/cedrus/cedrus_mpeg2.h
> new file mode 100644
...
> +
> +#endif
> diff --git a/drivers/media/platform/sunxi/cedrus/cedrus_regs.h b/drivers/=
media/platform/sunxi/cedrus/cedrus_regs.h
> new file mode 100644
...
> +
> +/*
> + * For more information, consult http://linux-sunxi.org/VE_Register_guid=
e
> + */
> +
> +/* VE_MPEG_CTRL:
...
> + * The bits 24 to 26 are used to select the input format (1 for MPEG1, 2=
 for
> + *                           MPEG2, 4 for MPEG4)
> + * The bit 21 (0x8) is used to disable bitstream error handling
> + *
> + * In MPEG4 the w*h value is somehow used for an offset, unknown but nee=
ded
> + */
...
> +
> +#endif
> diff --git a/drivers/media/platform/sunxi/cedrus/cedrus_video.c b/drivers=
/media/platform/sunxi/cedrus/cedrus_video.c
> new file mode 100644
...
> +               v4l2_err(&dev->v4l2_dev, "Invalid destination format: %08=
x\n",
...
> +
> +               sizes[0] =3D round_up(ctx->dst_fmt.plane_fmt[0].sizeimage=
, 8);
> +               sizes[1] =3D sizes[0];
> +               break;
> +
...
> +}
> diff --git a/drivers/media/platform/sunxi/cedrus/cedrus_video.h b/drivers=
/media/platform/sunxi/cedrus/cedrus_video.h
> new file mode 100644
