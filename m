...
Received: from smtp-vbr14.xs4all.nl ([194.109.24.34]:4627 "EHLO
	...
	with ESMTP id S1755020AbZFILri (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Tue, 9 Jun 2009 07:47:38 -0400
From: Hans Verkuil <hverkuil@xs4all.nl>
To: Eduardo Valentin <eduardo.valentin@nokia.com>
Subject: Re: [PATCHv6 4 of 7] FMTx: si4713: Add files to handle si4713 i2c device
Date: Tue, 9 Jun 2009 13:47:12 +0200
Cc: "ext Mauro Carvalho Chehab" <mchehab@infradead.org>,
	"ext Douglas Schilling Landgraf" <dougsland@gmail.com>,
	Linux-Media <linux-media@vger.kernel.org>,
	"Nurkkala Eero.An (EXT-Offcode/Oulu)" <ext-Eero.Nurkkala@nokia.com>
References: <1244449087-5543-1-git-send-email-eduardo.valentin@nokia.com> <1244449087-5543-4-git-send-email-eduardo.valentin@nokia.com> <1244449087-5543-5-git-send-email-eduardo.valentin@nokia.com>
In-Reply-To: <1244449087-5543-5-git-send-email-eduardo.valentin@nokia.com>
MIME-Version: 1.0
Content-Type: text/plain;
  charset="iso-8859-15"
Content-Transfer-Encoding: 7bit
Content-Disposition: inline
Message-Id: <200906091347.12775.hverkuil@xs4all.nl>
Sender: linux-media-owner@vger.kernel.org
List-ID: <linux-media.vger.kernel.org>

On Monday 08 June 2009 10:18:04 Eduardo Valentin wrote:
> From: Eduardo Valentin <eduardo.valentin@nokia.com>
> 
> # HG changeset patch
> # User Eduardo Valentin <eduardo.valentin@nokia.com>
> # Date 1243414606 -10800
> # Node ID 786afca68fc9fd35b18ee5cb4166b491613b13a5
> # Parent  ccae4c3150d272235cc2b19af8e9adb2e8b2e5f5
...
OK, let me start this review with some comments on the sysfs interface.

I don't think this should be included in the driver. The reason for this is
that I will be working on a new framework for control handling based around
v4l2_device and v4l2_subdev. This should move most of the hard work into the
v4l2 core and when that is in I want to let the core create the sysfs nodes.

Since sysfs is a public API it can cause problems if the core implementation
will be different from your implementation with respect to naming and in what
directory the nodes are. Once in, it is very hard to change it.

Regardless of this I suggest merging si4713-subdev.c into si4713.c. I don't
think it makes much sense to have this as two separate sources.
...
> 
> diff -r ccae4c3150d2 -r 786afca68fc9 linux/drivers/media/radio/si4713-subdev.c
...
Suggest renaming this to si4713_to_v4l2 and vice versa.
...
Huh? Can kradio handle a transmitter device? And anyway, why supply these
controls if they are not implemented? It seems more like a driver workaround
...
Just remove unused ops. Don't initialize them to NULL, the compiler
...
Should be g_modulator. Just add that to v4l2_subdev.
...
There is no audmode for g_modulator. You should use txsubchans instead and
set that to either V4L2_TUNER_SUB_MONO or V4L2_TUNER_SUB_STEREO.
...
Hmm, v4l2_modulator has no signal field. Do we need this? Or should this be
implemented as a read-only control?
...
Please don't use struct v4l2_i2c_driver_data: that's only for i2c drivers
that have to support older kernels (pre-2.6.26). Instead write this as a
regular i2c driver (see e.g. adv7343.c). Eventually v4l2_i2c_driver_data
will be phased out.
...
> diff -r ccae4c3150d2 -r 786afca68fc9 linux/drivers/media/radio/si4713.c
> --- /dev/null	Thu Jan 01 00:00:00 1970 +0000
...
Where is this set_power function defined? This platform_data is something
specific to this driver, and so it must be declared in an
include/media/si4713.h header.

Note that I still think the notifier in v4l2_device should be used for this.
...
Why does this return -ENODEV while other set functions return just 0?
...
I can't help thinking that all these get/set functions can be simplified.
It would probably be easier to see how that can be done if the sysfs part was
removed and the two sources are merged.

> diff -r ccae4c3150d2 -r 786afca68fc9 linux/drivers/media/radio/si4713.h
> --- /dev/null	Thu Jan 01 00:00:00 1970 +0000
