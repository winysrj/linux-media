...
Received: from moutng.kundenserver.de ([212.227.17.9]:50533 "EHLO
	moutng.kundenserver.de" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1752594Ab2JAKI5 (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Mon, 1 Oct 2012 06:08:57 -0400
Date: Mon, 1 Oct 2012 12:08:54 +0200 (CEST)
...
In-Reply-To: <1348840048-21423-1-git-send-email-twang13@marvell.com>
Message-ID: <Pine.LNX.4.64.1210011053500.3573@axis700.grange>
...
I don't see clk[1] amd clk[2] used anywhere in your patches. If they are 
indeed needed and unless you really want to address them as an array, it 
might be better to use a descriptive name for each clock instead of the 
array.

>  	int irq;
>  };
>  
> @@ -90,15 +107,7 @@ static struct mmp_camera *mmpcam_find_device(struct platform_device *pdev)
>  	return NULL;
>  }
>  
> -
...
> +}
> +#else
> +/* setup the camera clk gate in APMU */
> +static void mmpcam_island_clk(struct mmp_camera *cam, int on)
> +{
> +	if (on)
...
I would remove this function and inline clk_enable() / clk_disable() 
directly below.
...
Ouch, this looks like an abuse to me. The pad API isn't yet supported by 
soc-camera, nor by the ccic driver, AFAICS. Your use of struct 
v4l2_subdev_frame_interval::pad here doesn't seem valid. I think using 
struct v4l2_fract directly for this function would be better. In patch 2/4 
in mcam_camera_set_fmt() you assign
...
so, looks like you don't need to pass .mclk_min to mmpcam_calc_dphy() at 
all. As for calling subdevice .g_frame_interval() method with that struct

+	ret = v4l2_subdev_call(sd, video, g_frame_interval, &inter);

that's an abuse too - the clock frequency has nothing to do with the pad 
number. The clock frequency should be passed to the sensor by different 
means. I think, this is an example of the problem, that has been discussed 
at the media mini-summit a month ago. The correct way to solve it would be 
to have your bridge driver export a clock for the sensor. If you cannot do 
that yet, we have to think about a different way to achieve this, please, 
let us know.
...
You are jeeping the clock frequency value around, so, you probably assume, 
that it will stay constant. However, as soon as you release the clock by 
calling clk_put() its frequency can be changed by another user. So, I 
think you should hold a reference to the clock for as long as you're using 
it.
...
>  }
>  
> @@ -312,6 +452,7 @@ static int mmpcam_platform_remove(struct platform_device *pdev)
>  
>  	if (cam == NULL)
>  		return -ENODEV;
> +
