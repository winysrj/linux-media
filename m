...
Received: from metis.ext.pengutronix.de ([92.198.50.35]:35210 "EHLO
	metis.ext.pengutronix.de" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1761596AbaGRKXR (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Fri, 18 Jul 2014 06:23:17 -0400
...
Subject: [PATCH v2 08/11] [media] coda: split userspace interface into encoder and decoder device
Date: Fri, 18 Jul 2014 12:22:42 +0200
Message-Id: <1405678965-10473-9-git-send-email-p.zabel@pengutronix.de>
...
@@ -2289,11 +2304,6 @@ static int coda_start_streaming(struct vb2_queue *q, unsigned int count)
 		}
...
-			ctx->inst_type = CODA_INST_DECODER;
 		...
 		if (count < 1)
 			return -EINVAL;
@@ -2715,7 +2725,7 @@ static void coda_stop_streaming(struct vb2_queue *q)
 	}
 }
...
@@ -2867,35 +2877,55 @@ static int coda_ctrls_setup(struct coda_ctx *ctx)
...
@@ -2909,8 +2939,10 @@ static int coda_next_free_instance(struct coda_dev *dev)
 	return idx;
 }
...
+			  struct vb2_queue *dst_vq);
 	...
 	char *name;
@@ -2932,6 +2964,7 @@ static int coda_open(struct file *file)
...
@@ -2965,8 +2998,11 @@ static int coda_open(struct file *file)
 		goto err_clk_ahb;
...
+
 ...
 {
 	struct coda_dev *dev = video_drvdata(file);
@@ -3081,9 +3127,18 @@ static int coda_release(struct file *file)
 	return 0;
 }
...
@@ -3622,15 +3688,6 @@ static void coda_fw_callback(const struct firmware *fw, void *context)
 			return;
 	}
...
@@ -3643,13 +3700,28 @@ static void coda_fw_callback(const struct firmware *fw, void *context)
 		goto rel_ctx;
 	}
...
@@ -3883,7 +3955,8 @@ static int coda_remove(struct platform_device *pdev)
 {
