...
Received: from smtp-vbr11.xs4all.nl ([194.109.24.31]:1813 "EHLO
	...
	with ESMTP id S1751823AbZDRMoU (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Sat, 18 Apr 2009 08:44:20 -0400
From: Hans Verkuil <hverkuil@xs4all.nl>
To: Chaithrika U S <chaithrika@ti.com>
Subject: Re: [PATCH v2 3/4] ARM: DaVinci: DM646x Video: Add VPIF display driver
Date: Sat, 18 Apr 2009 14:44:13 +0200
Cc: linux-media@vger.kernel.org,
	davinci-linux-open-source@linux.davincidsp.com,
	Manjunath Hadli <mrh@ti.com>, Brijesh Jadav <brijesh.j@ti.com>
References: <1239189533-19961-1-git-send-email-chaithrika@ti.com>
In-Reply-To: <1239189533-19961-1-git-send-email-chaithrika@ti.com>
MIME-Version: 1.0
Content-Type: text/plain;
  charset="iso-8859-15"
Content-Transfer-Encoding: 7bit
Content-Disposition: inline
Message-Id: <200904181444.13823.hverkuil@xs4all.nl>
...
There is no need for this global. It's only used when registering the video
devices and there it is much simpler to use 'j ? 3 : 2' than refer to this
global.
...
NTSC/PAL. After all, that's really what it is about. DM646X_V4L2_STD should
be updated as well.
...
Notice how this 'else' part is identical to the process_progressive_mode()
...
Note that the return value isn't checked. Actually, wouldn't it make sense to
merge vpif_get_mode_info with this one?

And if the standard isn't found, shouldn't this function return an error as
well?
...
No check here whether the standard is supported. Also I think that this code
...
If buftype isn't V4L2_BUF_TYPE_VIDEO_OUTPUT, then an error should be reported
at the start of this function since we only support this buffer type here.
...
Note that the default g_std as is provided by video_ioctl2 only works with
drivers that have only video devices. It will not work when vbi devices are
added. So if you intend to add vbi devices in the future, then it is better
...
This isn't right. If the device was allocated and registered, then only call
video_unregister_device(). If the device was allocated but not registered,
...
I expect that the next round of patches will be ready for merging. The code
