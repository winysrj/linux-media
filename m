...
Received: from mx1.redhat.com ([209.132.183.28]:44779 "EHLO mx1.redhat.com"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
	id S1753178Ab1HYUUE (ORCPT <rfc822;linux-media@vger.kernel.org>);
	Thu, 25 Aug 2011 16:20:04 -0400
Message-ID: <4E56AF04.2020705@redhat.com>
Date: Thu, 25 Aug 2011 22:22:28 +0200
From: Hans de Goede <hdegoede@redhat.com>
MIME-Version: 1.0
To: Hans Verkuil <hverkuil@xs4all.nl>
CC: linux-media <linux-media@vger.kernel.org>
Subject: Re: [RFC PATCH] Modify volatile auto cluster handling as per earlier
 discussions
References: <201108091840.09884.hverkuil@xs4all.nl>
In-Reply-To: <201108091840.09884.hverkuil@xs4all.nl>
Content-Type: multipart/mixed;
 boundary="------------020108080509090800080203"
Sender: linux-media-owner@vger.kernel.org
List-ID: <linux-media.vger.kernel.org>

This is a multi-part message in MIME format.
--------------020108080509090800080203
...
Content-Type: text/plain;
 name="0001-Modify-volatile-auto-cluster-handling-as-per-earlier.patch"
Content-Transfer-Encoding: 7bit
Content-Disposition: attachment;
 filename*0="0001-Modify-volatile-auto-cluster-handling-as-per-earlier.pa";
 filename*1="tch"

>From fd6d6c8987e06aafcff56544cdeb4ea664b77a7d Mon Sep 17 00:00:00 2001
From: Hans Verkuil <hverkuil@xs4all.nl>
Date: Tue, 9 Aug 2011 18:40:09 +0200
Subject: [PATCH] Modify volatile auto cluster handling as per earlier
 discussions
...
+	int ret;
 	...
 	if (pdev->auto_white_balance->is_new) {
 		ret = pwc_set_u8_ctrl(pdev, SET_CHROM_CTL,
@@ -639,52 +641,35 @@ static int pwc_set_awb(struct pwc_device *pdev)
 		if (ret)
 			return ret;
...
+	int ret;
 	...
 	if (pdev->autogain->is_new) {
 		ret = pwc_set_u8_ctrl(pdev, SET_LUM_CTL,
@@ -692,27 +677,28 @@ static int pwc_set_autogain(struct pwc_device *pdev)
 				      pdev->autogain->val ? 0 : 0xff);
 		if (ret)
 			return ret;
+
...
+	int ret;
 	...
 	if (pdev->exposure_auto->is_new) {
@@ -721,27 +707,28 @@ static int pwc_set_exposure_auto(struct pwc_device *pdev)
 				      is_auto ? 0 : 0xff);
 		if (ret)
 			return ret;
+
...
+	int ret;
 	...
 	if (pdev->autogain->is_new) {
 		ret = pwc_set_u8_ctrl(pdev, SET_LUM_CTL,
@@ -749,35 +736,32 @@ static int pwc_set_autogain_expo(struct pwc_device *pdev)
 				      pdev->autogain->val ? 0 : 0xff);
 		if (ret)
 			return ret;
+
...
@@ -878,10 +862,6 @@ static int pwc_s_ctrl(struct v4l2_ctrl *ctrl)
 					pdev->autocontour->val ? 0 : 0xff);
 		}
 		if (ret == 0 && pdev->contour->is_new) {
-			if (pdev->autocontour->val) {
...
-			}
 			ret = pwc_set_u8_ctrl(pdev, SET_LUM_CTL,
 					      PRESET_CONTOUR_FORMATTER,
 					      pdev->contour->val);
@@ -1099,6 +1079,14 @@ static int pwc_enum_frameintervals(struct file *file, void *fh,
 	return 0;
 }
...
+
 static long pwc_default(struct file *file, void *fh, bool valid_prio,
 			int cmd, void *arg)
 {
@@ -1122,6 +1110,7 @@ const struct v4l2_ioctl_ops pwc_ioctl_ops = {
 	.vidioc_dqbuf			    = pwc_dqbuf,
 	.vidioc_streamon		    = pwc_streamon,
 	.vidioc_streamoff		    = pwc_streamoff,
+	.vidioc_log_status		    = pwc_log_status,
