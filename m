...
Received: from smtp-vbr7.xs4all.nl ([194.109.24.27]:3553 "EHLO
	smtp-vbr7.xs4all.nl" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1755976Ab0E2OoF (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Sat, 29 May 2010 10:44:05 -0400
Message-Id: <f50926a1bbadf47ec3b45ca2965495ae60675c14.1275143672.git.hverkuil@xs4all.nl>
In-Reply-To: <cover.1275143672.git.hverkuil@xs4all.nl>
References: <cover.1275143672.git.hverkuil@xs4all.nl>
From: Hans Verkuil <hverkuil@xs4all.nl>
Date: Sat, 29 May 2010 16:45:52 +0200
Subject: [PATCH 01/15] [RFCv4] v4l2: Add new control handling framework
...
---
 ...
 drivers/media/video/v4l2-ctrls.c  | 1931 +++++++++++++++++++++++++++++++++++++
 include/media/v4l2-ctrls.h        |  495 ++++++++++
 include/media/v4l2-dev.h          |    4 +
 include/media/v4l2-device.h       |    4 +
 include/media/v4l2-subdev.h       |    3 +
 7 files changed, 2443 insertions(+), 475 deletions(-)
...
+
+static int v4l2_ctrl_initialize(struct v4l2_ctrl *ctrl)
+{
+	struct v4l2_ctrl *master = ctrl->cluster[0];
+	int err = 0;
+
+	if (likely(!ctrl->is_uninitialized && !master->cluster_is_uninitialized))
+		return 0;
+	v4l2_ctrl_lock(ctrl);
+	if (ctrl->is_uninitialized || master->cluster_is_uninitialized) {
+		enum v4l2_ctrl_type type = master->type;
+		s32 maximum = master->maximum;
+		int i;
+
+		err = master->ops->init(master);
+
+		if (!err) {
+			master->cluster_is_uninitialized = 0;
+			for (i = 0; i < master->ncontrols; i++)
+				master->cluster[i]->is_uninitialized = 0;
+
+			/* Sanity check (for the master control only). */
+			WARN_ON(type != master->type);
+			WARN_ON(type == V4L2_CTRL_TYPE_STRING && maximum < master->maximum);
+		}
+	}
...
+}
+
+/* If the initialization of a control fails, then the driver can decide to
+   disable the control's cluster by calling this function. */
+void v4l2_ctrl_init_disable(struct v4l2_ctrl *ctrl)
+{
+	struct v4l2_ctrl *master = ctrl->cluster[0];
+	int i;
+
+	WARN_ON(!mutex_is_locked(&master->handler->lock));
+
+	master->cluster_is_uninitialized = 0;
+	for (i = 0; i < master->ncontrols; i++) {
+		ctrl = master->cluster[i];
+
+		ctrl->is_uninitialized = 0;
+		/* This control could not be initialized,
+		   so disable it instead. */
+		ctrl->flags |= V4L2_CTRL_FLAG_DISABLED;
+	}
+}
+EXPORT_SYMBOL(v4l2_ctrl_init_disable);
+
...
+		ctrl->is_uninitialized = cfg->is_uninitialized;
+	}
+	return ctrl;
+}
...
+
+	/* If at least one control is marked uninitialized, then mark the
+	   cluster as being uninitialized. Because whenever one of the controls
+	   of the cluster is used, then the cluster must be initialized. */
+	for (i = 0; i < ncontrols; i++) {
+		if (controls[i]) {
+			if (controls[i]->is_uninitialized)
+				controls[0]->cluster_is_uninitialized = 1;
...
+		if (ctrl->is_uninitialized ||
+		    ctrl->cluster[0]->cluster_is_uninitialized)
+			continue;
...
+	if (!ref)
+		return -EINVAL;
+	err = v4l2_ctrl_initialize(ref->ctrl);
+	if (err)
+		return err;
+
+	ctrl = ref->ctrl;
...
+  *		minimum, maximum, default_value, step/menu_skip_mask,
...
+  *		expensive operation.
+  *		If this is a string control, then the maximum field may not be
+  *		increased. Contact the linux-media mailinglist if you need to
+  *		increase the maximum string length.
...
+	int (*init)(struct v4l2_ctrl *ctrl);
...
+  * @cluster_is_uninitialized: Internal flag: set for the master control of
+  *		the cluster if one or more controls in that cluster are
+  *		uninitialized.
...
+  *		this flag.
+  * @is_uninitialized: If set, then this control starts of as uninitialized.
+  *		The first time this controls is referenced the init op will
+  *		be called to initialize the control. Drivers can set this flag.
+  * @ops:	The control ops.
+  * @id:	The control ID.
...
+	unsigned int done:1;
+	unsigned int cluster_is_uninitialized:1;
+
+	unsigned int is_private:1;
+	unsigned int is_volatile:1;
+	unsigned int is_uninitialized:1;
+
+	const struct v4l2_ctrl_ops *ops;
+	u32 id;
+	const char *name;
...
+  * @is_uninitialized: If set, then this control starts of as uninitialized.
+  *		The first time this controls is referenced the init op will
+  *		be called to initialize the control.
+  */
...
+void v4l2_ctrl_cluster(unsigned ncontrols, struct v4l2_ctrl **controls);
+
+/** v4l2_ctrl_init_disable() - Mark all controls in the cluster as disabled.
+  * @ctrl: One of the controls of the cluster.
+  *
+  * If the initialization of a control fails, then the driver can decide to
+  * disable the control's cluster by calling this function.
+  * This is typically only done if it is known that the hardware/firmware of
+  * the device is flaky and allows the application to continue albeit with
+  * this control cluster missing.
+  */
+void v4l2_ctrl_init_disable(struct v4l2_ctrl *ctrl);
+
+
