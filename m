...
Received: from resqmta-po-06v.sys.comcast.net ([96.114.154.165]:50676 "EHLO
	resqmta-po-06v.sys.comcast.net" rhost-flags-OK-OK-OK-OK)
	by vger.kernel.org with ESMTP id S933186AbbFJOWD (ORCPT
...
Subject: [PATCH v3 2/2] sound/usb: Update ALSA driver to use media controller API
Date: Wed, 10 Jun 2015 08:21:57 -0600
Message-Id: <9deae05513f79661f511b2ed932908733fbd6310.1433904553.git.shuahkh@osg.samsung.com>
...
a specific device.  In this patch, media controller API is
enabled for AU0828 hw. snd_usb_create_quirk() will check
this new field, if set will create a media device using
media_device_get_devres() interface.

media_device_get_devres() will allocate a new media
device devres or return an existing one, if it finds
one.
...
A new media entity for ALSA is registered when a new
sub stream initialized in snd_usb_init_substream().
When this entity is registered, Media Controller API
interface media_device_register_entity() invokes the
ALSA entity's media_entity_operations hook
register_notify. In this hook, ALSA creates a link
from tuner entity to the newly registered ALSA entity.
This link is deleted from snd_usb_init_substream().

The state is kept to indicate if tuner is linked.
This is to account for case when tuner entity
doesn't exist. Media pipeline gets started to mark
the tuner busy from snd_usb_substream_capture_trigger()
in response to SNDRV_PCM_TRIGGER_START and pipeline
is stopped in response to SNDRV_PCM_TRIGGER_STOP.
snd_usb_pcm_close() stops pipeline to cover the case
when SNDRV_PCM_TRIGGER_STOP isn't issued. Pipeline start
and stop are done only when tuner_linked is set.
...
+}
+
+static int media_entity_alsa_link_init(struct media_entity *entity)
+{
+	struct snd_usb_substream *subs;
+	struct media_ctl *mctl;
+	struct media_entity *ptr;
+
+	if (media_entity_type(entity) != MEDIA_ENT_T_DEVNODE_ALSA)
+		return 0;
+
+	/* private and mctl should be valid for MEDIA_ENT_T_DEVNODE_ALSA */
+	subs = (struct snd_usb_substream *) entity->private;
+	mctl = (struct media_ctl *) subs->media_ctl;
+
+	if (mctl->tuner_linked)
+		return 0;
+
+	media_device_for_each_entity(ptr, mctl->media_dev) {
+		if (media_entity_type(ptr) == MEDIA_ENT_T_V4L2_SUBDEV_TUNER) {
+			int ret;
+
...
+	.register_notify = media_entity_alsa_link_init,
+};
+
...
+	mctl->media_entity.ops = &media_ops;
+	mctl->media_entity.private = (void *) subs;
