...
Received: from atrey.karlin.mff.cuni.cz ([195.113.26.193]:35049 "EHLO
        atrey.karlin.mff.cuni.cz" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1751476AbcJWUdU (ORCPT
        <rfc822;linux-media@vger.kernel.org>);
        Sun, 23 Oct 2016 16:33:20 -0400
Date: Sun, 23 Oct 2016 22:33:15 +0200
...
Message-ID: <20161023203315.GC6391@amd>
...
> > @@ -2,6 +2,7 @@ msp3400-objs	:=3D	msp3400-driver.o msp3400-kthreads.o
> >  obj-$(CONFIG_VIDEO_MSP3400) +=3D msp3400.o
> > =20
> >  obj-$(CONFIG_VIDEO_SMIAPP)	+=3D smiapp/
> > +obj-$(CONFIG_VIDEO_ET8EK8)	+=3D et8ek8/
> >  obj-$(CONFIG_VIDEO_CX25840) +=3D cx25840/
> >  obj-$(CONFIG_VIDEO_M5MOLS)	+=3D m5mols/
> >  obj-y				+=3D soc_camera/
> > diff --git a/drivers/media/i2c/et8ek8/Kconfig b/drivers/media/i2c/et8ek=
8/Kconfig
...
> > diff --git a/drivers/media/i2c/et8ek8/Makefile b/drivers/media/i2c/et8e=
k8/Makefile
> > new file mode 100644
...
> > @@ -0,0 +1,2 @@
> > +et8ek8-objs			+=3D et8ek8_mode.o et8ek8_driver.o
> > +obj-$(CONFIG_VIDEO_ET8EK8)	+=3D et8ek8.o
...
> > +};
> > +
> > +#define to_et8ek8_sensor(sd)	container_of(sd, struct et8ek8_sensor, su=
bdev)
> > +
> > +enum et8ek8_versions {
> > +	ET8EK8_REV_1 =3D 0x0001,
> > +	ET8EK8_REV_2,
> > +};
> > +
> > +/*
...
> > + */
> > +static int et8ek8_i2c_read_reg(struct i2c_client *client, u16 data_len=
gth,
> > +			       u16 reg, u32 *val)
> > +{
> > +	int r;
...
> > +	if (!client->adapter)
> > +		return -ENODEV;
> > +	if (data_length !=3D ET8EK8_REG_8BIT && data_length !=3D ET8EK8_REG_1=
6BIT)
> > +		return -EINVAL;
> > +
...
> > +	data[0] =3D (u8) (reg >> 8);
> > +	data[1] =3D (u8) (reg & 0xff);
> > +	r =3D i2c_transfer(client->adapter, &msg, 1);
> > +	if (r < 0)
> > +		goto err;
> > +
> > +	msg.len =3D data_length;
...
> > +	if (data_length =3D=3D ET8EK8_REG_8BIT)
> > +		*val =3D data[0];
> > +	else
> > +		*val =3D (data[0] << 8) + data[1];
> > +
> > +	return 0;
> > +
> > +err:
...
> > +}
> > +
> > +static void et8ek8_i2c_create_msg(struct i2c_client *client, u16 len, =
u16 reg,
> > +				  u32 val, struct i2c_msg *msg,
> > +				  unsigned char *buf)
> > +{
> > +	msg->addr =3D client->addr;
> > +	msg->flags =3D 0; /* Write */
> > +	msg->len =3D 2 + len;
> > +	msg->buf =3D buf;
> > +
> > +	/* high byte goes out first */
> > +	buf[0] =3D (u8) (reg >> 8);
> > +	buf[1] =3D (u8) (reg & 0xff);
> > +
> > +	switch (len) {
> > +	case ET8EK8_REG_8BIT:
> > +		buf[2] =3D (u8) (val) & 0xff;
> > +		break;
> > +	case ET8EK8_REG_16BIT:
> > +		buf[2] =3D (u8) (val >> 8) & 0xff;
> > +		buf[3] =3D (u8) (val & 0xff);
> > +		break;
> > +	default:
...
> > +		 */
> > +		while (next->type !=3D ET8EK8_REG_TERM &&
> > +		       next->type !=3D ET8EK8_REG_DELAY) {
> > +			/*
...
> > +		r =3D et8ek8_i2c_buffered_write_regs(client, regs, cnt);
> > +
...
> > +		 */
> > +		if (next->type =3D=3D ET8EK8_REG_DELAY) {
> > +			msleep(next->val);
> > +			/*
> > +			 * ZZZ ...
...
> > + * Returns zero if successful, or non-zero otherwise.
> > + */
> > +static int et8ek8_i2c_write_reg(struct i2c_client *client, u16 data_le=
ngth,
> > +				u16 reg, u32 val)
> > +{
> > +	int r;
...
> > +	else
> > +		r =3D 0; /* on success i2c_transfer() returns messages trasfered */
> > +
> > +	return r;
> > +}
> > +
...
> > +		u16 type)
> > +{
> > +	struct et8ek8_reglist **next =3D &meta->reglist[0].ptr;
> > +
> > +	while (*next) {
> > +		if ((*next)->type =3D=3D type)
> > +			return *next;
> > +
> > +		next++;
> > +	}
...
> > +		struct v4l2_mbus_framefmt *fmt)
> > +{
> > +	struct et8ek8_reglist **list =3D et8ek8_reglist_first(meta);
> > +	struct et8ek8_reglist *best_match =3D NULL;
> > +	struct et8ek8_reglist *best_other =3D NULL;
> > +	struct v4l2_mbus_framefmt format;
> > +	unsigned int max_dist_match =3D (unsigned int)-1;
> > +	unsigned int max_dist_other =3D (unsigned int)-1;
> > +
> > +	/*
...
> > +		unsigned int dist;
> > +
> > +		if ((*list)->type !=3D ET8EK8_REGLIST_MODE)
> > +			continue;
> > +
> > +		et8ek8_reglist_to_mbus(*list, &format);
> > +
> > +		dist =3D min(fmt->width, format.width)
> > +		     * min(fmt->height, format.height);
> > +		dist =3D format.width * format.height
> > +		     + fmt->width * fmt->height - 2 * dist;
> > +
> > +
> > +		if (fmt->code =3D=3D format.code) {
> > +			if (dist < max_dist_match || !best_match) {
> > +				best_match =3D *list;
> > +				max_dist_match =3D dist;
> > +			}
> > +		} else {
> > +			if (dist < max_dist_other || !best_other) {
> > +				best_other =3D *list;
> > +				max_dist_other =3D dist;
> > +			}
> > +		}
> > +	}
> > +
...
> > +{
> > +	int fps =3D TIMEPERFRAME_AVG_FPS(*timeperframe);
> > +	struct et8ek8_reglist **list =3D et8ek8_reglist_first(meta);
> > +	struct et8ek8_mode *current_mode =3D &current_reglist->mode;
> > +
> > +	for (; *list; list++) {
> > +		struct et8ek8_mode *mode =3D &(*list)->mode;
> > +
> > +		if ((*list)->type !=3D ET8EK8_REGLIST_MODE)
> > +			continue;
> > +
> > +		if (mode->window_width !=3D current_mode->window_width ||
> > +		    mode->window_height !=3D current_mode->window_height)
> > +			continue;
> > +
> > +		if (TIMEPERFRAME_AVG_FPS(mode->timeperframe) =3D=3D fps)
> > +			return *list;
> > +	}
> > +
> > +	return NULL;
...
> > +{
> > +	const struct et8ek8_reglist **list1 =3D (const struct et8ek8_reglist =
**)a,
> > +		**list2 =3D (const struct et8ek8_reglist **)b;
> > +
> > +	/* Put real modes in the beginning. */
> > +	if ((*list1)->type =3D=3D ET8EK8_REGLIST_MODE &&
> > +	    (*list2)->type !=3D ET8EK8_REGLIST_MODE)
> > +		return -1;
> > +	if ((*list1)->type !=3D ET8EK8_REGLIST_MODE &&
> > +	    (*list2)->type =3D=3D ET8EK8_REGLIST_MODE)
> > +		return 1;
> > +
> > +	/* Descending width. */
...
> > +
> > +		list =3D meta->reglist[nlists].ptr;
> > +
> > +		dev_dbg(&client->dev,
...
> > + */
> > +static int et8ek8_exposure_rows_to_us(struct et8ek8_sensor *sensor, in=
t rows)
> > +{
...
> > + * rounds and clamps the given value and updates the V4L2 control valu=
e.
...
> > +	r =3D et8ek8_i2c_write_reg(client, ET8EK8_REG_8BIT,
> > +				0x124a, new.analog >> 8);
> > +	if (r)
> > +		return r;
> > +	r =3D et8ek8_i2c_write_reg(client, ET8EK8_REG_8BIT,
...
> > +}
> > +
> > +static int et8ek8_set_test_pattern(struct et8ek8_sensor *sensor, s32 m=
ode)
> > +{
> > +	struct i2c_client *client =3D v4l2_get_subdevdata(&sensor->subdev);
...
> > +	}
> > +
> > +	rval =3D et8ek8_i2c_write_reg(client, ET8EK8_REG_8BIT, 0x111B,
> > +				    tp_mode << 4);
> > +	if (rval)
> > +		return rval;
> > +
> > +	rval =3D et8ek8_i2c_write_reg(client, ET8EK8_REG_8BIT, 0x1121,
> > +				    cbh_mode << 7);
> > +	if (rval)
> > +		return rval;
> > +
> > +	rval =3D et8ek8_i2c_write_reg(client, ET8EK8_REG_8BIT, 0x1124,
> > +				    cbv_mode << 7);
> > +	if (rval)
> > +		return rval;	=09
> > +
> > +	rval =3D et8ek8_i2c_write_reg(client, ET8EK8_REG_8BIT, 0x112C, din_sw=
);
> > +	if (rval)
> > +		return rval;
> > +
> > +	rval =3D et8ek8_i2c_write_reg(client, ET8EK8_REG_8BIT, 0x1420, r1420);
> > +	return rval;
> > +}
> > +
> > +/* -------------------------------------------------------------------=
...
> > +
> > +	if (ctrl->id =3D=3D V4L2_CID_EXPOSURE)
> > +		rows =3D et8ek8_exposure_us_to_rows(sensor, (u32 *)&ctrl->val);
> > +
> > +	switch (ctrl->id) {
> > +	case V4L2_CID_GAIN:
...
> > +
> > +	sensor->subdev.ctrl_handler =3D &sensor->ctrl_handler;
> > +
> > +	return 0;
> > +}
> > +
...
> > +{
> > +	struct v4l2_ctrl *ctrl =3D sensor->exposure;
> > +	struct et8ek8_mode *mode =3D &sensor->current_reglist->mode;
> > +	u32 min, max, pixel_rate;
> > +	static const int S =3D 8;
> > +
> > +	min =3D et8ek8_exposure_rows_to_us(sensor, 1);
> > +	max =3D et8ek8_exposure_rows_to_us(sensor, mode->max_exp);
> > +
> > +	/*
...
> > +	 */
> > +	pixel_rate =3D ((mode->pixel_clock + (1 << S) - 1) >> S) + mode->widt=
h;
...
> > +	int rval;
> > +
> > +	rval =3D et8ek8_i2c_write_regs(client, sensor->current_reglist->regs);
> > +	if (rval)
> > +		goto fail;
> > +
...
> > +	 */
> > +	rval =3D v4l2_ctrl_handler_setup(&sensor->ctrl_handler);
> > +	if (rval)
> > +		goto fail;
> > +
> > +	return 0;
...
> > +
> > +	ret =3D et8ek8_configure(sensor);
> > +	if (ret < 0)
> > +		return ret;
> > +
...
> > +	int val, rval;
> > +
> > +	rval =3D regulator_enable(sensor->vana);
> > +	if (rval) {
...
> > +
> > +	if (sensor->current_reglist)
> > +		xclk_freq =3D sensor->current_reglist->mode.ext_clock;
> > +	else
> > +		xclk_freq =3D sensor->xclk_freq;
> > +
> > +	rval =3D clk_set_rate(sensor->ext_clk, xclk_freq);
> > +	if (rval < 0) {
> > +		dev_err(&client->dev, "unable to set extclk clock freq to %u\n",
> > +			xclk_freq);
> > +		goto out;
> > +	}
> > +	rval =3D clk_prepare_enable(sensor->ext_clk);
> > +	if (rval < 0) {
...
> > +
> > +	rval =3D et8ek8_i2c_reglist_find_write(client, &meta_reglist,
> > +					     ET8EK8_REGLIST_POWERON);
> > +	if (rval)
> > +		goto out;
> > +
> > +#ifdef USE_CRC
> > +	rval =3D et8ek8_i2c_read_reg(client, ET8EK8_REG_8BIT, 0x1263, &val);
> > +	if (rval)
> > +		goto out;
...
> > +	val &=3D ~BIT(4);
> > +#endif
> > +	rval =3D et8ek8_i2c_write_reg(client, ET8EK8_REG_8BIT, 0x1263, val);
> > +	if (rval)
> > +		goto out;
> > +#endif
> > +
...
> > +{
> > +	struct et8ek8_reglist **list =3D
> > +			et8ek8_reglist_first(&meta_reglist);
> > +	u32 pixelformat[MAX_FMTS];
> > +	int npixelformat =3D 0;
> > +
> > +	if (code->index >=3D MAX_FMTS)
> > +		return -EINVAL;
> > +
> > +	for (; *list; list++) {
> > +		struct et8ek8_mode *mode =3D &(*list)->mode;
...
> > +			et8ek8_reglist_first(&meta_reglist);
> > +	struct v4l2_mbus_framefmt format;
> > +	int cmp_width =3D INT_MAX;
> > +	int cmp_height =3D INT_MAX;
> > +	int index =3D fse->index;
> > +
> > +	for (; *list; list++) {
> > +		if ((*list)->type !=3D ET8EK8_REGLIST_MODE)
> > +			continue;
> > +
> > +		et8ek8_reglist_to_mbus(*list, &format);
> > +		if (fse->code !=3D format.code)
> > +			continue;
> > +
> > +		/* Assume that the modes are grouped by frame size. */
> > +		if (format.width =3D=3D cmp_width && format.height =3D=3D cmp_height)
> > +			continue;
> > +
> > +		cmp_width =3D format.width;
> > +		cmp_height =3D format.height;
> > +
> > +		if (index-- =3D=3D 0) {
> > +			fse->min_width =3D format.width;
> > +			fse->min_height =3D format.height;
> > +			fse->max_width =3D format.width;
> > +			fse->max_height =3D format.height;
> > +			return 0;
> > +		}
> > +	}
> > +
...
> > +
> > +		et8ek8_reglist_to_mbus(*list, &format);
> > +		if (fie->code !=3D format.code)
> > +			continue;
> > +
> > +		if (fie->width !=3D format.width || fie->height !=3D format.height)
> > +			continue;
> > +
> > +		if (index-- =3D=3D 0) {
> > +			fie->interval =3D mode->timeperframe;
> > +			return 0;
> > +		}
> > +	}
> > +
...
> > +	struct v4l2_mbus_framefmt *format;
> > +
> > +	format =3D __et8ek8_get_pad_format(sensor, cfg, fmt->pad, fmt->which);
> > +	if (!format)
> > +		return -EINVAL;
> > +
...
> > +	fi->interval =3D sensor->current_reglist->mode.timeperframe;
> > +
> > +	return 0;
> > +}
> > +
...
> > +	struct et8ek8_reglist *reglist;
> > +
> > +	reglist =3D et8ek8_reglist_find_mode_ival(&meta_reglist,
...
> > +		return -EINVAL;
> > +
> > +	if (sensor->current_reglist->mode.ext_clock !=3D reglist->mode.ext_cl=
ock)
> > +		return -EINVAL;
> > +
> > +	sensor->current_reglist =3D reglist;
...
> > +	struct et8ek8_sensor *sensor =3D to_et8ek8_sensor(subdev);
> > +	struct i2c_client *client =3D v4l2_get_subdevdata(subdev);
> > +	unsigned int length =3D ET8EK8_PRIV_MEM_SIZE;
> > +	unsigned int offset =3D 0;
> > +	u8 *ptr  =3D sensor->priv_mem;
> > +	int rval =3D 0;
> > +
...
> > +		rval =3D et8ek8_i2c_write_reg(client, ET8EK8_REG_8BIT, 0x0001,
> > +					    0xe0 | (offset >> 3));
> > +		if (rval < 0)
> > +			return rval;
> > +
...
> > +			u32 status;
> > +
> > +			rval =3D et8ek8_i2c_read_reg(client, ET8EK8_REG_8BIT,
> > +						   0x0003, &status);
> > +			if (rval < 0)
> > +				return rval;
> > +			if (!(status & 0x08))
...
> > +		ofs =3D offset & 0x07;	/* Offset within this window */
> > +		bytes =3D length + ofs > 8 ? 8-ofs : length;
> > +		msg.addr =3D client->addr;
> > +		msg.flags =3D 0;
> > +		msg.len =3D 2;
> > +		msg.buf =3D buffer;
> > +		ofs +=3D PRIV_MEM_START_REG;
> > +		buffer[0] =3D (u8)(ofs >> 8);
> > +		buffer[1] =3D (u8)(ofs & 0xFF);
> > +
> > +		rval =3D i2c_transfer(client->adapter, &msg, 1);
> > +		if (rval < 0)
> > +			return rval;
> > +
...
> > +
> > +	rval =3D et8ek8_power_on(sensor);
> > +	if (rval) {
...
> > +	}
> > +
> > +	sensor->version =3D (rev_h << 8) + rev_l;
> > +	if (sensor->version !=3D ET8EK8_REV_1 && sensor->version !=3D ET8EK8_=
REV_2)
> > +		dev_info(&client->dev,
> > +			 "unknown version 0x%x detected, continuing anyway\n",
> > +			 sensor->version);
> > +
> > +	rval =3D et8ek8_reglist_import(client, &meta_reglist);
> > +	if (rval) {
> > +		dev_err(&client->dev,
...
> > +	}
> > +
> > +	sensor->current_reglist =3D et8ek8_reglist_find_type(&meta_reglist,
> > +							   ET8EK8_REGLIST_MODE);
> > +	if (!sensor->current_reglist) {
> > +		dev_err(&client->dev,
...
> > +		goto out_poweroff;
> > +	}
> > +	rval =3D et8ek8_stream_on(sensor); /* Needed to be able to read EEPRO=
M */
> > +	if (rval)
> > +		goto out_poweroff;
> > +	rval =3D et8ek8_g_priv_mem(subdev);
> > +	if (rval)
> > +		dev_warn(&client->dev,
> > +			"can not read OTP (EEPROM) memory from sensor\n");
> > +	rval =3D et8ek8_stream_off(sensor);
> > +	if (rval)
> > +		goto out_poweroff;
> > +
> > +	rval =3D et8ek8_power_off(sensor);
> > +	if (rval)
> > +		goto out_poweroff;
> > +
> > +	return 0;
...
> > +		     char *buf)
> > +{
> > +	struct v4l2_subdev *subdev =3D i2c_get_clientdata(to_i2c_client(dev));
> > +	struct et8ek8_sensor *sensor =3D to_et8ek8_sensor(subdev);
> > +
...
> > +
> > +	rval =3D device_create_file(&client->dev, &dev_attr_priv_mem);
> > +	if (rval) {
> > +		dev_err(&client->dev, "could not register sysfs entry\n");
> > +		return rval;
> > +	}
> > +
> > +	rval =3D et8ek8_dev_init(subdev);
> > +	if (rval)
> > +		goto err_file;
> > +
> > +	rval =3D et8ek8_init_controls(sensor);
> > +	if (rval) {
> > +		dev_err(&client->dev, "controls initialization failed\n");
> > +		goto err_file;
> > +	}
> > +
> > +	format =3D __et8ek8_get_pad_format(sensor, NULL, 0,
...
> > +		ret =3D __et8ek8_set_power(sensor, !!on);
> > +		if (ret < 0)
> > +			goto done;
> > +	}
> > +
...
fh)
> > +{
> > +	struct et8ek8_sensor *sensor =3D to_et8ek8_sensor(sd);
> > +	struct v4l2_mbus_framefmt *format;
> > +	struct et8ek8_reglist *reglist;
> > +
> > +	reglist =3D et8ek8_reglist_find_type(&meta_reglist, ET8EK8_REGLIST_MO=
DE);
> > +	format =3D __et8ek8_get_pad_format(sensor, fh->pad, 0,
...
> > +}
> > +
> > +static int et8ek8_close(struct v4l2_subdev *sd, struct v4l2_subdev_fh =
*fh)
> > +{
> > +	return et8ek8_set_power(sd, false);
> > +}
> > +
> > +static const struct v4l2_subdev_video_ops et8ek8_video_ops =3D {
> > +	.s_stream =3D et8ek8_s_stream,
> > +	.g_frame_interval =3D et8ek8_get_frame_interval,
> > +	.s_frame_interval =3D et8ek8_set_frame_interval,
> > +};
> > +
> > +static const struct v4l2_subdev_core_ops et8ek8_core_ops =3D {
> > +	.s_power =3D et8ek8_set_power,
> > +};
> > +
> > +static const struct v4l2_subdev_pad_ops et8ek8_pad_ops =3D {
> > +	.enum_mbus_code =3D et8ek8_enum_mbus_code,
> > +	.enum_frame_size =3D et8ek8_enum_frame_size,
> > +	.enum_frame_interval =3D et8ek8_enum_frame_ival,
> > +	.get_fmt =3D et8ek8_get_pad_format,
> > +	.set_fmt =3D et8ek8_set_pad_format,
> > +};
> > +
> > +static const struct v4l2_subdev_ops et8ek8_ops =3D {
> > +	.core =3D &et8ek8_core_ops,
> > +	.video =3D &et8ek8_video_ops,
> > +	.pad =3D &et8ek8_pad_ops,
> > +};
> > +
> > +static const struct v4l2_subdev_internal_ops et8ek8_internal_ops =3D {
> > +	.registered =3D et8ek8_registered,
> > +	.open =3D et8ek8_open,
> > +	.close =3D et8ek8_close,
> > +};
> > +
> > +/* -------------------------------------------------------------------=
-------
...
> > +	}
> > +
> > +	sensor->vana =3D devm_regulator_get(dev, "vana");
> > +	if (IS_ERR(sensor->vana)) {
> > +		dev_err(&client->dev, "could not get regulator for vana\n");
> > +		return PTR_ERR(sensor->vana);
> > +	}
> > +
> > +	sensor->ext_clk =3D devm_clk_get(dev, NULL);
...
> > +	ret =3D media_entity_pads_init(&sensor->subdev.entity, 1, &sensor->pa=
d);
> > +	if (ret < 0) {
...
> > +};
> > +
> > +static const struct i2c_device_id et8ek8_id_table[] =3D {
> > +	{ ET8EK8_NAME, 0 },
> > +	{ }
> > +};
...
> > +MODULE_LICENSE("GPL");
> > diff --git a/drivers/media/i2c/et8ek8/et8ek8_mode.c b/drivers/media/i2c=
/et8ek8/et8ek8_mode.c
> > new file mode 100644
...
> > +static struct et8ek8_reglist mode1_poweron_mode2_16vga_2592x1968_12_07=
fps =3D {
> > +/* (without the +1)
> > + * SPCK       =3D 80 MHz
> > + * CCP2       =3D 640 MHz
...
> > +		/* Frequency of SPCK setting (SPCK=3DMRCK) */
...
> > +static struct et8ek8_reglist mode3_4vga_1296x984_29_99fps_dpcm10_8 =3D=
 {
> > +/* (without the +1)
> > + * SPCK       =3D 96.5333333333333 MHz
> > + * CCP2       =3D 579.2 MHz
> > + * VCO        =3D 579.2 MHz
> > + * VCOUNT     =3D 84 (2016)
> > + * HCOUNT     =3D 133 (3192)
...
> > +		.window_origin_x =3D 0,
> > +		.window_origin_y =3D 0,
> > +		.window_width =3D 1296,
> > +		.window_height =3D 984,
> > +		.pixel_clock =3D 96533333,
> > +		.ext_clock =3D 9600000,
> > +		.timeperframe =3D {
> > +			.numerator =3D 100,
> > +			.denominator =3D 3000
...
> > +static struct et8ek8_reglist mode4_svga_864x656_29_88fps =3D {
> > +/* (without the +1)
> > + * SPCK       =3D 80 MHz
> > + * CCP2       =3D 320 MHz
> > + * VCO        =3D 640 MHz
...
> > +static struct et8ek8_reglist mode5_vga_648x492_29_93fps =3D {
> > +/* (without the +1)
> > + * SPCK       =3D 80 MHz
> > + * CCP2       =3D 320 MHz
> > + * VCO        =3D 640 MHz
...
> > +static struct et8ek8_reglist mode2_16vga_2592x1968_3_99fps =3D {
> > +/* (without the +1)
> > + * SPCK       =3D 80 MHz
> > + * CCP2       =3D 640 MHz
> > + * VCO        =3D 640 MHz
...
> > +
> > +/* Mode_648x492_5fps */
> > +static struct et8ek8_reglist mode_648x492_5fps =3D {
> > +/* (without the +1)
> > + * SPCK       =3D 13.3333333333333 MHz
> > + * CCP2       =3D 53.3333333333333 MHz
> > + * VCO        =3D 640 MHz
> > + * VCOUNT     =3D 84 (2016)
> > + * HCOUNT     =3D 221 (5304)
> > + * CKREF_DIV  =3D 2
...
> > +static struct et8ek8_reglist mode3_4vga_1296x984_5fps =3D {
> > +/* (without the +1)
> > + * SPCK       =3D 49.4 MHz
> > + * CCP2       =3D 395.2 MHz
> > + * VCO        =3D 790.4 MHz
...
> > +static struct et8ek8_reglist mode_4vga_1296x984_25fps_dpcm10_8 =3D {
> > +/* (without the +1)
> > + * SPCK       =3D 84.2666666666667 MHz
> > + * CCP2       =3D 505.6 MHz
> > + * VCO        =3D 505.6 MHz
> > + * VCOUNT     =3D 88 (2112)
> > + * HCOUNT     =3D 133 (3192)
...
> > +	}
> > +};
> > diff --git a/drivers/media/i2c/et8ek8/et8ek8_reg.h b/drivers/media/i2c/=
et8ek8/et8ek8_reg.h
> > new file mode 100644
...
iEYEARECAAYFAlgNHosACgkQMOfwapXb+vJV8gCeMRnE8OuhFVxgQZr/0U9mTRA4
3IQAnj43zxnDRBl6lcERa708DbPGhIYm
=jb3G
-----END PGP SIGNATURE-----
