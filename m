...
Received: from comal.ext.ti.com ([198.47.26.152]:44026 "EHLO comal.ext.ti.com"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
	id S1753449Ab1DTPar convert rfc822-to-8bit (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Wed, 20 Apr 2011 11:30:47 -0400
...
Date: Wed, 20 Apr 2011 21:00:08 +0530
Subject: RE: [PATCH v16 01/13] davinci vpbe: V4L2 display driver for DM644X
 SoC
Message-ID: <B85A65D85D7EB246BE421B3FB0FBB593024BCEF72E@dbde02.ent.ti.com>
In-Reply-To: <201104071358.21451.laurent.pinchart@ideasonboard.com>
...
Hi Laurent,
 Thank you for you very valuable and detailed comments. I have fixed a lot of your suggestions and there are a few questions I need more explanation for.
I will send the fixed and updated patches as a follow-up after your clarifications.
...
This is the minimum number of buffers to be allocated by the system as there is no scatter-gather mechanism in Davinci. To make sure of availability of a minimum numbers of buffers for the system which may not be available otherwise due to fragmentation, these are used.
>
...
Ok. We are now not using the numbers.
>
...
Addressed. v4l2_capability structure is filled in vpbe_display_querycap()
>
> > +static u8 layer_first_int[VPBE_DISPLAY_MAX_DEVICES];
>
> This belongs to your vpbe_display_obj object, not to a global variable.
addressed- layer_first_int is made part of struct vpbe_layer instead of
global variable.
...
I would like to get to videobuf2 as a next set of changes. I want to get the Dm6446 driver fisrt, add it with Dm365 and do the videobuf2 later.  I hope it is okay.
>
...
Not sure what you mean here. We get the FIRST or second field event from the hardware so we need to check the register value rather than pix_fmt.field.
>
...
OSD here refers to both the IP block which hosts the 4 windows or layers and the 2 RGB based windows/layers as part of the same. "osd_device->ops" is the collection of all the functions which deal with all the 4 "layers" and in general everything that is in the OSD block.
...
Fixed. The name of vpbe_display_obj is changed to vpbe_layer
>
> > +
> > +           ret = osd_device->ops.enable_layer(osd_device,
...
Fixed. min() used for both cfg->xpos and cfg->ypos.
>
> > +   v4l2_dbg(1, debug, &vpbe_dev->v4l2_dev,
> > +           "new xpos = %d, ypos = %d\n",
> > +           cfg->xpos, cfg->ypos);
...
I am Ok either ways. When debugging is enabled, it is just one of the data points since there are multiple windows.
>
...
>
> directly.
Wel,. In the past patches we had a large amount of multiple indirections, and as part of a suggestion on open source I removed all of them with a general rule of not having more than 2 indirections. What is your take on this? How many indirection levels do you allow?
>
> > +   } else {
...
Not sure about this. The entire driver is under V4L2 lock per layer handle and it would not allow another call to come here once in. Can you elaborate how is it a race condition?
>
...
It was a safety check. Removed as there is no need for it.
>
> > +           }
> > +   }
> > +
...
It can return an error if the driver is set to a DV_PRESET mode. timings_type would tell whether is is an SD standard or a DV_PRESET.
>
> > +}
> > +
> > +/**
...
I do not think anyone would attempt to open the device so soon as the boot process is not complete yet. Also, if the irq is registered before, the interrupts start and the driver crashes for lack of initialized structure variables.
>
...
This is a common IP for DM644x, Dm355 and Dm365 for which the patches will follow after this set. So I think it is OK.
>
> > +MODULE_LICENSE("GPL");
...
venc_is_second_field fn declaration is removed.
>
