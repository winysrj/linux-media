...
Received: from ams-iport-1.cisco.com ([144.254.224.140]:5164 "EHLO
	ams-iport-1.cisco.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1753827Ab2G3Mmj (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Mon, 30 Jul 2012 08:42:39 -0400
...
Date: Mon, 30 Jul 2012 14:41:47 +0000
...
Message-Id: <201207301441.47667.kradlow@cisco.com>
...
> > +};
> > +
> > +/* struct to encapsulate an array of all defined Alternative Frequencies
> > for a channel */
...
> > +
> > +	/** state information **/
> > +	uint32_t decode_information;	/* defines the state of the decoding
> > process */
> > +	uint32_t valid_fields;	/* defines the RDS info fields that are valid
> > atm */
> > +
> > +	/** RDS info fields **/
> > +	bool is_rbds; 		/* use RBDS standard version of LUTs */
...
> 
> > +	bool ptyn_ab_flag;	/* PTYN A/B flag (toggled), to signal change of
> > program type */
> > +	uint8_t rt_pos;
> > +	uint8_t rt_length;
> > +	uint8_t rt[64];
...
> Same remark as for ps.
> 
> > +	bool rt_ab_flag;	/* RT A/B flag (toggled), to signal transmission 
of
> > new RT */
> > +	bool ta;
> > +	bool tp;
...
> > + * @reset_statistics:	true = set all statistic values to 0, false = 
keep
> > them untouched */
...
> > *handle);
> > +LIBV4L_PUBLIC const char *v4l2_rds_get_language_str(const struct
> > v4l2_rds *handle);
> > +LIBV4L_PUBLIC const char *v4l2_rds_get_country_str(const struct v4l2_rds
> > *handle);
> > +LIBV4L_PUBLIC const char *v4l2_rds_get_coverage_str(const struct
> > v4l2_rds *handle);
> > +
