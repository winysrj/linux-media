...
Received: from mailout2.w1.samsung.com ([210.118.77.12]:16863 "EHLO
	mailout2.w1.samsung.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S932473Ab3BSMAt (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Tue, 19 Feb 2013 07:00:49 -0500
Received: from eucpsbgm2.samsung.com (unknown [203.254.199.245])
 by mailout2.w1.samsung.com
 (Oracle Communications Messaging Server 7u4-24.01(7.0.4.24.0) 64bit (built Nov
 17 2011)) with ESMTP id <0MIG00G0BUNBNU70@mailout2.w1.samsung.com> for
 linux-media@vger.kernel.org; Tue, 19 Feb 2013 12:00:47 +0000 (GMT)
...
Subject: [PATCH 2/2] media: added max77693-led driver
Date: Tue, 19 Feb 2013 13:00:21 +0100
Message-id: <1361275221-6595-3-git-send-email-a.hajda@samsung.com>
...
---
 ...
 drivers/media/i2c/max77693-led.c                   |  651 ++++++++++++++++++++
 drivers/mfd/max77693.c                             |   55 +-
 include/linux/mfd/max77693.h                       |   33 +-
 6 files changed, 805 insertions(+), 20 deletions(-)
...
+}
+
+static const u32 max77693_torch_timeouts[] = {262000, 524000, 786000, 1048000,
+	1572000, 2096000, 2620000, 3144000, 4193000, 5242000, 6291000, 7340000,
...
+}
+
+static void clamp_u32(u32 *v, u32 min, u32 max, u32 step)
+{
+	if (*v < min)
+		*v = min;
+	else if (*v > max)
...
+
+	for (i = 0; i < 2; ++i)
+		clamp_u32(&p->boost_mode[i], MAX77693_LED_BOOST_NONE,
...
+
+	clamp_u32(&p->iout[FLASH1], MAX77693_FLASH_IOUT_MIN,
+		  max, MAX77693_FLASH_IOUT_STEP);
+	clamp_u32(&p->iout[FLASH2], MAX77693_FLASH_IOUT_MIN,
+		  max, MAX77693_FLASH_IOUT_STEP);
+	clamp_u32(&p->iout[TORCH1], MAX77693_TORCH_IOUT_MIN,
+		  MAX77693_TORCH_IOUT_MAX, MAX77693_TORCH_IOUT_STEP);
+	clamp_u32(&p->iout[TORCH2], MAX77693_TORCH_IOUT_MIN,
+		  MAX77693_TORCH_IOUT_MAX, MAX77693_TORCH_IOUT_STEP);
+
+	for (i = 0; i < 4; ++i)
+		clamp_u32(&p->trigger[i], 0, 7, 1);
+	for (i = 0; i < 2; ++i)
+		clamp_u32(&p->trigger_type[i], MAX77693_LED_TRIG_TYPE_EDGE,
+			  MAX77693_LED_TRIG_TYPE_LEVEL, 1);
+
+	clamp_u32(&p->timeout[FLASH], MAX77693_FLASH_TIMEOUT_MIN,
+		  MAX77693_FLASH_TIMEOUT_MAX, MAX77693_FLASH_TIMEOUT_STEP);
+
+	if (p->timeout[TORCH]) {
+		clamp_u32(&p->timeout[TORCH], MAX77693_TORCH_TIMEOUT_MIN,
+			  MAX77693_TORCH_TIMEOUT_MAX, 1);
+		p->timeout[TORCH] = max77693_torch_timeout_from_reg(
+			      max77693_torch_timeout_to_reg(p->timeout[TORCH]));
+	}
+
+	clamp_u32(&p->boost_vout, MAX77693_FLASH_VOUT_MIN,
+		  MAX77693_FLASH_VOUT_MAX, MAX77693_FLASH_VOUT_STEP);
+
+	if (p->low_vsys) {
+		clamp_u32(&p->low_vsys, MAX77693_FLASH_VSYS_MIN,
...
+
+	if (pdev->dev.of_node) {
+		p = kzalloc(sizeof(*flash->pdata), GFP_KERNEL);
+		if (!p)
+			return -ENOMEM;
+		max77693_led_parse_dt(p, pdev->dev.of_node);
+	} else {
+		p = dev_get_platdata(&pdev->dev);
+		if (!p)
+			return -ENODEV;
+	}
+	flash->pdata = p;
...
+
+	dev_info(&pdev->dev, "Iout[uA]=%d,%d,%d,%d; Triggers=%d,%d,%d,%d;"
...
+	ret = max77693_led_get_platform_data(flash, pdev);
+	if (ret < 0)
+		goto done;
+
+	sd = &flash->subdev;
