...
Received: from gateway09.websitewelcome.com ([67.18.52.11]:60817 "EHLO
	gateway09.websitewelcome.com" rhost-flags-OK-OK-OK-OK)
	by vger.kernel.org with ESMTP id S1751973AbaBKXv1 (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Tue, 11 Feb 2014 18:51:27 -0500
Received: from gator3086.hostgator.com (ns6171.hostgator.com [50.87.144.121])
	by gateway09.websitewelcome.com (Postfix) with ESMTP id EE3EF845B6FAF
	for <linux-media@vger.kernel.org>; Tue, 11 Feb 2014 16:56:31 -0600 (CST)
From: Dean Anderson <linux-dev@sensoray.com>
To: hverkuil@xs4all.nl, linux-dev@sensoray.com,
	linux-media@vger.kernel.org
Subject: [PATCH] s2255drv: upgrade to videobuf2
Date: Tue, 11 Feb 2014 14:56:24 -0800
Message-Id: <1392159384-30088-1-git-send-email-linux-dev@sensoray.com>
...
@@ -706,93 +692,45 @@ static int buffer_prepare(struct videobuf_queue *vq, struct videobuf_buffer *vb,
...
@@ -820,8 +758,7 @@ static int vidioc_enum_fmt_vid_cap(struct file *file, void *priv,
...
@@ -844,8 +781,7 @@ static int vidioc_try_fmt_vid_cap(struct file *file, void *priv,
 {
 	const struct s2255_fmt *fmt;
 	enum v4l2_field field;
-	struct s2255_fh *fh = priv;
...
@@ -905,14 +841,13 @@ static int vidioc_try_fmt_vid_cap(struct file *file, void *priv,
...
+	ret = vidioc_try_fmt_vid_cap(file, vc, f);
 	...
 	if (ret < 0)
 		return ret;
@@ -922,24 +857,16 @@ static int vidioc_s_fmt_vid_cap(struct file *file, void *priv,
 	if (fmt == NULL)
 		return -EINVAL;
...
@@ -984,44 +911,9 @@ static int vidioc_s_fmt_vid_cap(struct file *file, void *priv,
 		mode.restart = 1;
 	vc->mode = mode;
...
@@ -1199,24 +1091,11 @@ static int s2255_cmd_status(struct s2255_vc *vc, u32 *pstatus)
 	return res;
 }
...
-	}
 	...
 	vc->bad_payload = 0;
 	vc->cur_frame = 0;
@@ -1225,48 +1104,40 @@ static int vidioc_streamon(struct file *file, void *priv, enum v4l2_buf_type i)
...
@@ -1287,22 +1158,17 @@ static int vidioc_s_std(struct file *file, void *priv, v4l2_std_id i)
...
+	struct s2255_vc *vc = video_drvdata(file);
 	...
 	*i = vc->std;
 	return 0;
@@ -1318,8 +1184,7 @@ static int vidioc_g_std(struct file *file, void *priv, v4l2_std_id *i)
...
@@ -1404,8 +1269,7 @@ static int s2255_s_ctrl(struct v4l2_ctrl *ctrl)
...
@@ -1416,8 +1280,8 @@ static int vidioc_g_jpegcomp(struct file *file, void *priv,
...
@@ -1428,9 +1292,9 @@ static int vidioc_s_jpegcomp(struct file *file, void *priv,
...
@@ -1464,8 +1328,7 @@ static int vidioc_g_parm(struct file *file, void *priv,
...
+	struct s2255_vc *vc = video_drvdata(file);
 	...
 	int fdec = FDEC_1;
 	__u32 def_num, def_dem;
@@ -1473,8 +1336,8 @@ static int vidioc_s_parm(struct file *file, void *priv,
 		return -EINVAL;
 	mode = vc->mode;
...
@@ -1518,8 +1381,7 @@ static const struct v4l2_frmsize_discrete pal_sizes[] = {
...
@@ -1537,8 +1399,7 @@ static int vidioc_enum_framesizes(struct file *file, void *priv,
...
+	struct s2255_vc *vc = video_drvdata(file);
 	...
 	int is_ntsc = vc->std & V4L2_STD_525_60;
@@ -1570,16 +1431,18 @@ static int vidioc_enum_frameintervals(struct file *file, void *priv,
 	return 0;
 }
...
@@ -1720,59 +1537,14 @@ static void s2255_destroy(struct s2255_dev *dev)
 	kfree(dev);
 }
...
@@ -1781,17 +1553,17 @@ static const struct v4l2_ioctl_ops s2255_ioctl_ops = {
...
@@ -1847,6 +1619,8 @@ static int s2255_probe_v4l(struct s2255_dev *dev)
 	int i;
 	int cur_nr = video_nr;
 	struct s2255_vc *vc;
+	struct vb2_queue *q;
+
 	...
 	if (ret)
 		return ret;
@@ -1879,8 +1653,24 @@ static int s2255_probe_v4l(struct s2255_dev *dev)
...
@@ -2029,7 +1819,7 @@ static int save_frame(struct s2255_dev *dev, struct s2255_pipeinfo *pipe_info)
 	idx = vc->cur_frame;
...
+	if (!vb2_is_streaming(&vc->vb_vidq)) {
 		...
 		return -EINVAL;
@@ -2073,7 +1863,7 @@ static int save_frame(struct s2255_dev *dev, struct s2255_pipeinfo *pipe_info)
...
+		if (vb2_is_streaming(&vc->vb_vidq))
 		...
 		vc->frame_count++;
 		frm->ulState = S2255_READ_IDLE;
@@ -2222,7 +2012,6 @@ static int s2255_board_init(struct s2255_dev *dev)
...
-		vc->b_acquire = 0;
 		...
 		if (dev->pid == 0x2257 && j > 1)
 			vc->mode.color |= (1 << 16);
@@ -2249,7 +2038,7 @@ static int s2255_board_shutdown(struct s2255_dev *dev)
...
@@ -2397,7 +2186,6 @@ static int s2255_stop_acquire(struct s2255_vc *vc)
 	if (res != 0)
...
-	vc->b_acquire = 0;
 	...
 	return res;
@@ -2503,6 +2291,8 @@ static int s2255_probe(struct usb_interface *interface,
 		vc->dev = dev;
...
@@ -2548,7 +2338,6 @@ static int s2255_probe(struct usb_interface *interface,
