...
Received: from mail-io0-f178.google.com ([209.85.223.178]:34016 "EHLO
        mail-io0-f178.google.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1750768AbdGYTSn (ORCPT
        <rfc822;linux-media@vger.kernel.org>);
        Tue, 25 Jul 2017 15:18:43 -0400
Received: by mail-io0-f178.google.com with SMTP id l7so59947340iof.1
        for <linux-media@vger.kernel.org>; Tue, 25 Jul 2017 12:18:42 -0700 (PDT)
Message-ID: <1501010315.15645.8.camel@ndufresne.ca>
...
Date: Tue, 25 Jul 2017 15:18:35 -0400
...
References: <1501004422-8294-1-git-send-email-thierry.escande@collabora.com>
         <1501004422-8294-2-git-send-email-thierry.escande@collabora.com>
...
> +	unsigned long
> changed_mask[BITS_TO_LONGS(GO2001_PARAM_CHANGE_MAX)];
> +};
> +
...
> +};
> +
> +static inline struct go2001_buffer *vb_to_go2001_buf(struct
> vb2_buffer *vb)
> +{
> +	return container_of(vb, struct go2001_buffer, vb);
> +}
> +
> +static inline struct go2001_ctx *hw_inst_to_ctx(struct
> go2001_hw_inst *hw_inst)
> +{
...
> +{
> +	return container_of(ctrl->handler, struct go2001_ctx,
> ctrl_handler);
> +}
> +
...
> +	dev_err(&(gdev)->pdev->dev, "%s:%d " fmt, __func__,
> __LINE__, ##args)
> +
> +#define go2001_info(gdev, fmt, args...) \
> +	dev_info(&(gdev)->pdev->dev, "%s:%d " fmt, __func__,
> __LINE__, ##args)
> +
...
> diff --git a/drivers/media/pci/go2001/go2001_driver.c
...
> +MODULE_PARM_DESC(go2001_debug_level, " verbosity level for debug
> messages.");
> +
> +module_param(go2001_fw_debug_level, uint, 0644);
> +MODULE_PARM_DESC(go2001_fw_debug_level, " verbosity level for
...
> +}
> +
> +struct vb2_queue *go2001_get_vq(struct go2001_ctx *ctx, enum
> v4l2_buf_type type)
> +{
> +	return V4L2_TYPE_IS_OUTPUT(type) ? &ctx->src_vq : &ctx-
> >dst_vq;
> +}
> +
...
> +}
> +
> +static int go2001_queue_setup(struct vb2_queue *q, unsigned int
> *num_buffers,
...
> +
> +	f = V4L2_TYPE_IS_OUTPUT(q->type) ? ctx->src_fmt : ctx-
> >dst_fmt;
> +	if (!f) {
...
> +			*num_buffers =
> GO2001_MIN_NUM_BITSTREAM_BUFFERS;
> +		*num_planes = 1;
...
> +			go2001_err(gdev, "Plane address/size not
> aligned %d/%zu\n",
> +				   sgt->sgl->offset,
...
> +			go2001_err(gdev, "Failed allocating HW
> memory map\n");
> +
> +			ret = -ENOMEM;
> +			goto err;
> +		}
> +
> +		go2001_dbg(gdev, 3, "Plane %d: mmap list size:
> %zu\n", plane,
> +			   dma_desc->list_size);
> +
> +		mmap_list = dma_desc->mmap_list;
> +		for_each_sg(sgt->sgl, sg, dma_desc->num_entries,
> sgi) {
> +			dma_addr = sg_dma_address(sg);
> +			dma_len = sg_dma_len(sg);
> +
> +			mmap_list[sgi].dma_addr =
> cpu_to_le64(dma_addr);
> +			mmap_list[sgi].size = cpu_to_le32(dma_len);
> +
> +			go2001_dbg(gdev, 4, "Chunk %d: 0x%08llx,
> size %d\n",
...
> +	if (ret) {
> +		go2001_err(ctx->gdev, "Failed mapping buffer in
> HW\n");
> +		goto err;
> +	}
> +
...
> +
> +	/*
> +	 * If this is a CAPTURE encoder buffer, we need to
> reassemble
...
> +
> +	/*
> +	 * ith partition resides at partition_off[i] from start of
> the buffer
> +	 * and is of size partition_size[i]. As there may be gaps
> between
...
> >
> +		    plane_size) {
> +			go2001_err(ctx->gdev, "Invalid partition %d
> info, off: 0x%zx, size: %zu, plane_size: %zu\n",
...
> +
> +	/* Clean up regardless of whether unmap in HW succeeded or
> not. */
> +	for (plane = 0; plane < vb->num_planes; ++plane) {
> +		dma_desc = &gbuf->dma_desc[plane];
> +		if (dma_desc->mmap_list)
...
> +}
> +
> +static int go2001_start_streaming(struct vb2_queue *q, unsigned int
> count)
> +{
> +	struct go2001_ctx *ctx = vb2_get_drv_priv(q);
> +	unsigned long flags;
...
> +		   V4L2_TYPE_IS_OUTPUT(q->type) ? "OUTPUT" :
> "CAPTURE");
> +
> +	spin_lock_irqsave(&ctx->qlock, flags);
> +
> +	if (ctx->state == ERROR) {
> +		go2001_dbg(ctx->gdev, 1, "Instance %p in error
> state\n", ctx);
> +
> +		spin_unlock_irqrestore(&ctx->qlock, flags);
> +		return -EIO;
...
> +	if (ret) {
> +		go2001_err(ctx->gdev, "Failed to start
> streaming\n");
> +
> +		if (V4L2_TYPE_IS_OUTPUT(q->type)) {
> +			go2001_buffer_done_queued(ctx, &ctx-
> >src_buf_q);
> +			go2001_buffer_done_queued(ctx, &ctx-
> >src_resume_q);
> +		} else {
...
> +};
> +
> +static int go2001_init_vb2_queue(struct vb2_queue *q, struct
> go2001_ctx *ctx,
> +				 enum v4l2_buf_type type)
> +{
> +	q->type = type;
...
> +		ctx->pending_rt_params.enc_params.bitrate = ctrl-
> >val;
> +		go2001_dbg(ctx->gdev, 2, "Bitrate changed to %d
> bps.\n",
> +			   ctx-
> >pending_rt_params.enc_params.bitrate);
> +		break;
> +
> +	case V4L2_CID_MPEG_VIDEO_FORCE_KEY_FRAME:
> +		set_bit(GO2001_KEYFRAME_REQUESTED,
> +			ctx->pending_rt_params.changed_mask);
> +		ctx->pending_rt_params.enc_params.request_keyframe =
> true;
> +		go2001_dbg(ctx->gdev, 2, "Keyframe requested.\n");
> +		break;
> +
> +	default:
> +		go2001_dbg(ctx->gdev, 1, "Unsupported control id=%d,
> ignoring.\n",
> +			   ctrl->id);
> +		break;
> +	}
...
> +				  V4L2_CID_MPEG_VIDEO_FRAME_RC_ENABL
> E,
...
> GO2001_DEF_BITRATE);
> +		v4l2_ctrl_new_std(hdl, &go2001_enc_ctrl_ops,
> +				  V4L2_CID_MPEG_VIDEO_FORCE_KEY_FRAM
> E,
> +				  0, 0, 0, 0);
> +	}
> +
...
> +}
> +
> +static int go2001_init_ctx(struct go2001_dev *gdev, struct
> go2001_ctx *ctx,
> +			   struct file *file, struct video_device
> *vdev,
...
> +	if (ctx->codec_mode == CODEC_MODE_DECODER) {
> +		ctx->src_fmt = go2001_find_fmt(ctx,
> V4L2_PIX_FMT_VP8);
> +		ctx->dst_fmt = go2001_find_fmt(ctx,
> V4L2_PIX_FMT_RGB32);
> +	} else {
> +		ctx->src_fmt = go2001_find_fmt(ctx,
> V4L2_PIX_FMT_NV12M);
...
> +				    V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLA
> NE);
> +	if (ret)
> +		return ret;
> +
...
> +
> +	if (!(req_events & (POLLOUT | POLLWRNORM | POLLIN |
> POLLRDNORM)))
> +		return rc;
> +
> +	if ((!vb2_is_streaming(src_q) || list_empty(&src_q-
> >queued_list)) &&
> +	    (!vb2_is_streaming(dst_q) || list_empty(&dst_q-
> >queued_list))) {
> +		rc |= POLLERR;
> +		return rc;
> +	}
...
> +}
> +
> +static int go2001_mmap(struct file *file, struct vm_area_struct
> *vma)
> +{
> +	struct go2001_ctx *ctx = fh_to_ctx(file->private_data);
...
> +	int ret = 0;
> +
> +	list_for_each_entry(hw_inst, &gdev->new_inst_list,
> inst_entry) {
...
> +
> +	if (!ctx) {
> +		go2001_err(gdev, "No ctx awaiting VM_INIT_*,
> dropping\n");
> +		return -EIO;
> +	}
> +
...
> +		go2001_init_hw_inst(&ctx->hw_inst, reply-
> >session_id);
> +		list_add_tail(&ctx->hw_inst.inst_entry, &ctx->gdev-
> >inst_list);
> +		break;
> +
...
> +}
> +
> +static void go2001_handle_release_instance_reply(struct go2001_ctx
> *ctx)
> +{
> +	unsigned long flags;
> +
...
> +}
> +
> +static void go2001_calc_finfo(struct go2001_ctx *ctx, struct
> go2001_fmt *fmt,
> +			      struct go2001_frame_info *finfo,
...
> +	finfo->coded_width = round_up(finfo->width,
> GO2001_VPX_MACROBLOCK_SIZE);
> +	finfo->coded_height = round_up(finfo->height,
> +				       GO2001_VPX_MACROBLOCK_SIZE);
> +
> +	go2001_dbg(ctx->gdev, 2, "Visible (coded) resolution: %ux%u
> (%ux%u)\n",
...
> +			(finfo->coded_width * fmt-
> >plane_row_depth[i]) / 8;
> +		finfo->plane_size[i] = (finfo->coded_width  *
> +					finfo->coded_height *
> +					fmt->plane_depth[i]) / 8;
> +		go2001_dbg(ctx->gdev, 2, "Plane %d: bpl: %u, size:
> %zu\n",
> +			   i, finfo->bytesperline[i], finfo-
> >plane_size[i]);
> +	}
> +}
> +
...
> +
> +	go2001_dbg(ctx->gdev, 2, "HW reports new resolution: %ux%u
> (%ux%u)\n",
...
> +		go2001_err(ctx->gdev, "Invalid resolution from the
> HW\n");
> +		return -EINVAL;
> +	}
> +
...
> +
> +		memset(gbuf->partition_off, 0, sizeof(gbuf-
> >partition_off));
> +		memset(gbuf->partition_size, 0, sizeof(gbuf-
> >partition_size));
> +
> +		/*
> +		 * Partitions may have gaps between them, we will
> have to
> +		 * reassemble the bitstream later in buf_finish. For
> now just
> +		 * save the size and offset of each partition and
> set the
> +		 * payload for the buffer to the sum of sizes
> (returned in
...
> +
> +		for (i = 0; i < VP8_MAX_NUM_PARTITIONS; ++i) {
> +			gbuf->partition_off[i] = enc_reply-
> >partition_off[i];
> +			gbuf->partition_size[i] = enc_reply-
> >partition_size[i];
> +		}
> +
> +		dst_vbuf->flags = 0;
> +		if (enc_reply->frame_type ==
> +					GO2001_EMPTY_BUF_ENC_FRAME_K
> EYFRAME)
...
> +			.u.src_change.changes =	V4L2_EVENT_SR
> C_CH_RESOLUTION,
> +		};
...
> +		/*
> +		 * We will retry this frame after reallocating
> output buffers.
> +		 * Add the source buffer to the resume queue, which
> will
> +		 * be spliced in front of the source queue after
> reallocation.
> +		 */
...
> +
> +	dev_info(&gdev->pdev->dev, "GO2001 ver: %d/%d, VP8 decoder:
> %d/%d VP8 encoder: %d/%d, VP9 decoder: %d/%d\n",
...
> +}
> +
> +static inline int go2001_handle_reply_default(struct go2001_dev
> *gdev, u32 type,
> +					      u32 status)
> +{
> +	int ret = 0;
...
> +}
> +
> +static struct go2001_hw_inst *find_hw_inst_by_id_locked(struct
> go2001_dev *gdev,
> +							u32
> session_id)
> +{
...
> +	if (!hw_inst) {
> +		go2001_err(gdev, "Got reply for an invalid instance
> id %d\n",
> +			   hdr->session_id);
> +		ret = -EIO;
> +		goto out;
...
> +			go2001_err(gdev, "Unexpected reply without a
> request\n");
> +			ret = -EIO;
> +			goto out;
> +		}
...
> +				msecs_to_jiffies(GO2001_WATCHDOG_TIM
> EOUT_MS));
> +
...
> +
> +		WARN_ON(hw_inst->last_reply_seq_id + 1 != hdr-
> >sequence_id);
...
> +		ret = go2001_handle_init_reply(gdev, reply, hdr-
> >status);
> +		break;
> +
...
> +		ret = go2001_handle_reply_default(gdev, hdr->type,
> hdr->status);
> +		break;
> +
...
> +		ret = go2001_handle_get_info_reply(ctx, reply, hdr-
> >status);
> +		break;
> +
...
> +		struct go2001_event_assert_reply *a =
> msg_to_param(reply);
> +
> +		a->filename[ARRAY_SIZE(a->filename) - 1] = '\0';
> +		a->funcname[ARRAY_SIZE(a->funcname) - 1] = '\0';
> +		a->expr[ARRAY_SIZE(a->expr) - 1] = '\0';
> +		go2001_err(gdev, "FW ASSERT at %s:%d in %s,
> executing %s\n",
> +			   a->filename, a->line_no, a->funcname, a-
> >expr);
> +		ret = -EIO;
> +		break;
> +	}
...
> +		struct go2001_event_log_reply *l =
> msg_to_param(reply);
> +
...
> +		struct go2001_set_log_level_reply *l =
> msg_to_param(reply);
> +
...
> +
> +	default:
> +		go2001_err(gdev, "Unexpected reply [%d:%d],
> type=0x%x\n",
> +			   hdr->session_id, hdr->sequence_id, hdr-
> >type);
> +		ret = -EIO;
> +		break;
> +	}
...
> +out:
> +	if (ret) {
> +		go2001_err(gdev, "Error %d for reply [%d:%d]
> type=0x%x, status=0x%x\n",
> +			   ret, hdr->session_id, hdr->sequence_id,
> hdr->type,
> +			   hdr->status);
> +		if (ctx)
> +			go2001_ctx_error_locked(ctx);
...
> +		go2001_schedule_frames(ctx);
> +	/*
> +	 * Critical failure only on EIO, otherwise we may be able to
> continue
...
> +			   enum go2001_fmt_type type, struct
> v4l2_fmtdesc *f)
> +{
> +	struct go2001_fmt *fmt;
> +	int num_matched = 0;
...
> +	for (i = 0; i < ARRAY_SIZE(formats); ++i) {
> +		fmt = &formats[i];
> +		if (!(codec_mode & fmt->codec_modes) || fmt->type !=
> type)
> +			continue;
> +
...
> +
> +	if (ctx->codec_mode == CODEC_MODE_DECODER)
> +		return go2001_enum_fmt(ctx->codec_mode,
> FMT_TYPE_RAW, f);
> +
...
> +}
> +
> +static int fill_v4l2_format_raw(struct v4l2_format *f, struct
> go2001_fmt *gfmt,
> +				struct go2001_frame_info *finfo)
> +{
...
> +		pix_mp->plane_fmt[i].bytesperline = finfo-
> >bytesperline[i];
> +		pix_mp->plane_fmt[i].sizeimage = finfo-
> >plane_size[i];
> +	}
> +
> +	return 0;
...
> +		go2001_err(ctx->gdev, "Frame info not available
> yet\n");
> +		return -EINVAL;
> +	}
> +
...
> +	}
> +
> +	fill_v4l2_format_coded(f, ctx->src_fmt, ctx-
> >bitstream_buf_size);
> +
> +	return 0;
> +}
...
> +	}
> +
> +	fill_v4l2_format_coded(f, ctx->dst_fmt, ctx-
> >bitstream_buf_size);
> +
> +	return 0;
> +}
> +
> +static struct go2001_fmt *__go2001_dec_try_fmt_out(struct file
> *file, void *fh,
> +						   struct
> v4l2_format *f)
> +{
...
> +				  struct v4l2_format *f)
> +{
> +	struct go2001_fmt *fmt = __go2001_dec_try_fmt_out(file, fh,
> f);
> +
> +	return fmt ? 0 : -EINVAL;
> +}
> +
> +static struct go2001_fmt *__go2001_dec_try_fmt_cap(struct file
> *file, void *fh,
> +						   struct
> v4l2_format *f)
> +{
...
> +	 * resolution is set after parsing headers and comes from
> the HW.
> +	 */
> +	fmt = go2001_find_fmt(ctx, pix_mp->pixelformat);
...
> +				  struct v4l2_format *f)
> +{
> +	struct go2001_fmt *fmt = __go2001_dec_try_fmt_cap(file, fh,
> f);
> +
> +	return fmt ? 0 : -EINVAL;
> +}
> +
> +static struct go2001_fmt *__go2001_enc_try_fmt_out(struct file
> *file, void *fh,
> +						struct v4l2_format
> *f,
> +						struct
> go2001_frame_info *finfo)
> +{
> +	struct go2001_ctx *ctx = fh_to_ctx(fh);
> +	struct v4l2_pix_format_mplane *pix_mp = &f->fmt.pix_mp;
> +	struct go2001_fmt *fmt;
...
> +		return NULL;
> +
> +	go2001_calc_finfo(ctx, fmt, finfo, pix_mp->width, pix_mp-
> >height);
...
> +{
> +	struct go2001_frame_info finfo;
> +	struct go2001_fmt *fmt = __go2001_enc_try_fmt_out(file, fh,
> f, &finfo);
> +
> +	return fmt ? 0 : -EINVAL;
> +}
> +
> +static struct go2001_fmt *__go2001_enc_try_fmt_cap(struct file
> *file, void *fh,
> +						   struct
> v4l2_format *f)
> +{
...
> +				  struct v4l2_format *f)
> +{
> +	struct go2001_fmt *fmt = __go2001_enc_try_fmt_cap(file, fh,
> f);
> +
> +	return fmt ? 0 : -EINVAL;
> +}
...
> +		go2001_err(ctx->gdev, "Format cannot be set in this
> state\n");
> +		return -EBUSY;
> +	}
> +
...
> +		return -EINVAL;
> +
> +	/* At 1280x720 and below go2001 can do three reference
> frames. */
> +	ctx->enc_params.multi_ref_frame_mode =
> +				finfo.width * finfo.height <= 1280 *
> 720;
> +
> +	ctx->src_fmt = fmt;
> +	ctx->finfo = finfo;
...
> +		go2001_err(ctx->gdev, "G_CROP on CAPTURE for encoder
> unsupported\n");
> +		return -EINVAL;
> +	}
> +
...
> +		go2001_dbg(ctx->gdev, 2, "Crop: (%d, %d) -> (%u,
> %u)\n",
> +			s->r.left, s->r.top, s->r.width, s-
> >r.height);
> +		break;
> +	default:
> +		return -EINVAL;
...
> +
> +	if (!V4L2_TYPE_IS_OUTPUT(s->type)) {
> +		go2001_err(ctx->gdev, "Crop can only be set on the
> OUTPUT queue\n");
> +		return -EINVAL;
> +	}
> +
...
> +
> +	if (!go2001_has_frame_info(ctx)) {
> +		go2001_err(ctx->gdev, "Crop must be set after
> setting format\n");
> +		return -EINVAL;
> +	}
> +
...
> +
> +	if (finfo->width != s->r.width || finfo->height != s-
> >r.height) {
> +		go2001_dbg(ctx->gdev, 1, "Adjusted crop from (%d,
> %d) to (%d, %d)\n",
> +			   s->r.width, s->r.height, aligned_w,
> aligned_h);
> +	}
> +
> +	if (s->r.left != 0 || s->r.top != 0 || aligned_w > finfo-
> >coded_width ||
> +	    aligned_h > finfo->coded_height) {
> +		go2001_err(ctx->gdev, "Invalid crop (%d, %d) -> (%u,
> %u) for coded size %ux%u\n",
> +			   s->r.left, s->r.top, aligned_w,
> aligned_h,
...
> +
> +	if (V4L2_TYPE_IS_OUTPUT(s->type)) {
> +		go2001_err(ctx->gdev, "G_SEL on OUTPUT for decoder
> unsupported\n");
> +		return -EINVAL;
> +	}
> +
...
> >denominator == 0) {
> +		go2001_err(ctx->gdev, "Invalid values for
> timeperframe\n");
> +		return -EINVAL;
> +	}
> +
...
> +	set_bit(GO2001_FRAMERATE_CHANGE, ctx-
> >pending_rt_params.changed_mask);
...
> +		go2001_err(ctx->gdev, "No frame info available
> yet\n");
> +		return -EINVAL;
> +	}
> +
...
> +		if (ret) {
> +			go2001_err(ctx->gdev, "Failed initializing
> codec\n");
> +			return ret;
> +		}
> +
...
> +
> +	/*
> +	 * The buffer which triggered the resolution change, and any
> buffers
> +	 * which might have followed it and were sent by us to the
> hardware
> +	 * before we got the resolution change notification, have to
> be
> +	 * processed again, in the same order.
> +	 * Move them from the resume queue to the front of the
> source
> +	 * queue, and reinitialize for processing.
> +	 *
> +	 * First, make a copy of the resume list, so we can release
> +	 * the lock to be able to call go2001_prepare_gbuf() on each
> element.
> +	 */
...
> +
> +	/*
> +	 * Finally, splice the lists so that the resume list is
> added in
...
> +
> +	if (ret)
> +		go2001_err(ctx->gdev, "REQBUFS for type %d
> failed\n", rb->type);
> +	else
> +		go2001_dbg(ctx->gdev, 2, "Allocated %d buffers for
> type %d\n",
> +			   rb->count, rb->type);
> +
> +	return ret;
> +}
> +
> +static int go2001_querybuf(struct file *file, void *fh, struct
> v4l2_buffer *b)
> +{
> +	struct go2001_ctx *ctx = fh_to_ctx(file->private_data);
> +	int ret;
...
> +}
> +
> +static int go2001_qbuf(struct file *file, void *fh, struct
> v4l2_buffer *b)
> +{
> +	struct go2001_ctx *ctx = fh_to_ctx(file->private_data);
...
> +
> +	if (ctx->state == ERROR) {
> +		go2001_dbg(ctx->gdev, 1, "Context %p in error
> state\n", ctx);
> +		return -EIO;
> +	}
> +
...
> +}
> +
> +static int go2001_dqbuf(struct file *file, void *fh, struct
> v4l2_buffer *b)
> +{
> +	struct go2001_ctx *ctx = fh_to_ctx(file->private_data);
> +	struct vb2_queue *vq = go2001_get_vq(ctx, b->type);
...
> +}
> +
> +static int go2001_streamon(struct file *file, void *fh, enum
> v4l2_buf_type type)
> +{
> +	struct go2001_ctx *ctx = fh_to_ctx(file->private_data);
...
> +	if (!fmt) {
> +		go2001_dbg(ctx->gdev, 1, "Unsupported pixelformat
> %d\n",
> +			   fsize->pixel_format);
> +		return -EINVAL;
> +	}
...
> +		go2001_dbg(ctx->gdev, 1, "Only supported for coded
> formats\n");
> +		return -EINVAL;
> +	}
> +
...
> +};
> +
> +static int go2001_probe(struct pci_dev *pdev, const struct
> pci_device_id *id)
> +{
> +	int ret;
> +	struct go2001_dev *gdev;
...
> +	}
> +
> +	ret = request_irq(pdev->irq, go2001_irq, 0, DRIVER_NAME,
> gdev);
> +	if (ret) {
...
> +	if (ret) {
> +		go2001_err(gdev, "Failed registering V4L2
> device\n");
> +		goto free_irq;
> +	}
> +
...
> +	struct go2001_dev *gdev = container_of(v4l2_dev, struct
> go2001_dev,
> +					       v4l2_dev);
> +
...
> +
> +	ret = wait_event_timeout(gdev->reply_wq, gdev-
> >msgs_in_flight == 0,
> +				 msecs_to_jiffies(GO2001_WATCHDOG_TI
> MEOUT_MS));
> +	if (ret == 0)
> +		go2001_err(gdev, "Timed out waiting for HW to become
> idle\n");
> +
> +	mutex_lock(&gdev->lock);
> +	go2001_cancel_all_contexts(gdev);
...
> +#endif
> +
> +static SIMPLE_DEV_PM_OPS(go2001_pm_ops, go2001_suspend,
> go2001_resume);
> +
...
> diff --git a/drivers/media/pci/go2001/go2001_hw.c
...
> +				void __iomem *ctrl_addr, size_t
> ctrl_size,
> +				void __iomem *load_addr, size_t
> max_size,
> +				u32 entry_addr)
> +{
...
> +
> +	/*
> +	 * Calculate a checksum of the header (excluding checksum
> field itself).
> +	 */
> +	chksum_ptr = (u8 *)&hdr;
...
> +
> +	/*
> +	 * The next interrupt after resuming will be the "firmware
> loaded"
...
> +			       offsetof(struct go2001_boot_hdr,
> entry_addr));
...
> checksum));
> +	/*
> +	 * Write signature last, as this is what the device is
> spinning on
...
> +	}
> +
> +	ret = request_firmware(&boot_fw, GO2001_BOOT_FW_NAME, &gdev-
> >pdev->dev);
> +	if (ret) {
...
> +	}
> +
> +	ret = request_firmware(&fw, GO2001_FW_NAME, &gdev->pdev-
> >dev);
> +	if (ret) {
> +		go2001_err(gdev, "Unable to open firmware %s\n",
> +			   GO2001_FW_NAME);
...
> +		go2001_err(gdev, "Timed out waiting for firmware to
> start\n");
> +		ret = -ENODEV;
> +		goto out_release_fw;
> +	}
...
> GO2001_BOOT_FW_NAME);
> +
> +	ret = go2001_load_fw_image(gdev, fw, ctrl_iomem,
> ctrl_iomem_size,
> +				   fw_iomem + GO2001_FW_OFF,
> +				   fw_iomem_size - GO2001_FW_OFF,
> +				   GO2001_FW_ENTRY_BASE +
> GO2001_FW_OFF);
> +	if (ret) {
> +		go2001_err(gdev, "Failed loading firmware %s\n",
> +			   GO2001_FW_NAME);
...
> == 0) {
> +		go2001_err(gdev, "Timed out waiting for
> firmware\n");
> +		ret = -ENODEV;
> +		goto out_release_fw;
> +	}
...
> +		return -ENODEV;
> +
> +	if (len < GO2001_MSG_RING_MEM_OFFSET +
> GO2001_MSG_RING_MEM_SIZE)
> +		return -ENODEV;
> +
...
> +	go2001_dbg(gdev, 1, "Ring %p: [0x%x-0x%x], msg size: %d,
> capacity: %d, wr_off: 0x%x, rd_off: 0x%x\n",
> +		   ring->desc_iomem, desc->start_off, desc->end_off,
> +		   desc->msg_size,
> +		   (desc->end_off - desc->start_off) / desc-
> >msg_size,
> +		   desc->wr_off, desc->rd_off);
> +
> +	if (WARN_ON(desc->msg_size == 0 || desc->end_off <= desc-
> >start_off ||
...
> +				offsetof(struct
> go2001_msg_ring_desc, wr_off));
> +	r->desc.rd_off = ioread32(r->desc_iomem +
> +				offsetof(struct
> go2001_msg_ring_desc, rd_off));
> +}
> +
> +static inline bool go2001_ring_is_full_locked(struct go2001_msg_ring
> *r)
> +{
> +	u32 next_off;
> +
...
> +}
> +
> +static inline bool go2001_ring_is_empty_locked(struct
> go2001_msg_ring *r)
> +{
> +	go2001_update_ring_locked(r);
...
> +}
> +
> +static void free_msg(struct go2001_dev *gdev, struct go2001_msg
> *msg)
> +{
...
> +}
> +
> +static struct go2001_hw_inst *get_next_ready_hw_inst(struct
> go2001_dev *gdev)
> +{
> +	struct go2001_hw_inst *hw_inst;
> +
...
> +		gdev->curr_hw_inst = list_first_entry(&gdev-
> >inst_list,
> +					struct go2001_hw_inst,
> inst_entry);
> +	}
...
> +	list_for_each_entry_continue(hw_inst, &gdev->inst_list,
> inst_entry) {
...
> +}
> +
> +static struct go2001_msg *go2001_get_pending_msg_locked(struct
> go2001_dev *gdev)
> +{
> +	struct go2001_hw_inst *hw_inst;
> +	struct go2001_msg_hdr *hdr;
...
> +	msg = list_first_entry(&hw_inst->pending_list, struct
> go2001_msg,
> +			       list_entry);
> +	hdr = msg_to_hdr(msg);
> +	go2001_dbg(gdev, 5, "Found pending message for instance %d,
> seqid %d\n",
...
> +}
> +
> +static void go2001_drop_pending_for_hw_inst_locked(struct go2001_dev
> *gdev,
> +						struct
> go2001_hw_inst *hw_inst)
> +{
...
> +	assert_spin_locked(&gdev->irqlock);
> +
> +	list_for_each_entry_safe(msg, tmp_msg, &hw_inst-
> >pending_list,
> +				 list_entry) {
> +		list_del_init(&msg->list_entry);
> +		free_msg(gdev, msg);
...
> +
> +	list_for_each_entry(hw_inst, &gdev->inst_list, inst_entry)
> +		go2001_drop_pending_for_hw_inst_locked(gdev,
> hw_inst);
> +	INIT_LIST_HEAD(&gdev->inst_list);
> +
> +	list_for_each_entry(hw_inst, &gdev->new_inst_list,
...
> +}
> +
> +static void go2001_print_msg(struct go2001_dev *gdev, struct
> go2001_msg *msg,
> +			     const char *prefix)
> +{
> +	struct go2001_msg_hdr *hdr = msg_to_hdr(msg);
> +
> +	go2001_dbg(gdev, 4, "%s: MSG: [%d/%d], type=0x%x, size=%d,
> status=0x%x\n",
> +		   prefix, hdr->session_id, hdr->sequence_id, hdr-
> >type,
> +		   hdr->size, hdr->status);
> +
...
> +				   struct go2001_dma_desc *dma_desc)
> +{
> +	struct go2001_mmap_list_entry *mmap_list = dma_desc-
> >mmap_list;
> +	int i;
> +
> +	for (i = 0; i < dma_desc->num_entries; ++i)
> +		go2001_dbg(gdev, 4, "entry %02d: addr: 0x%08llx,
> size: %d\n",
> +			   i, mmap_list[i].dma_addr,
> mmap_list[i].size);
> +}
> +
...
> +			go2001_dbg(gdev, 5, "TX ring full, not
> sending.\n");
> +			break;
> +		}
> +
...
> +		hdr = msg_to_hdr(msg);
> +
> +		memcpy_toio(r->desc_iomem + desc->wr_off, hdr, hdr-
> >size);
...
> +			  + offsetof(struct go2001_msg_ring_desc,
> wr_off));
> +
> +		if (gdev->msgs_in_flight == 0)
> +			schedule_delayed_work(&gdev->watchdog_work,
...
> +
> +/*
> + * NOTE: msg must not be used by the caller after this functions
> returns;
> + * either we'll free it after sending it to HW, or it will be
> cleaned up by the
...
> +}
> +
> +static u32 go2001_queue_msg(struct go2001_ctx *ctx, struct
> go2001_msg *msg)
> +{
> +	struct go2001_dev *gdev = ctx->gdev;
> +	unsigned long flags;
...
> +	spin_unlock_irqrestore(&gdev->irqlock, flags);
> +	go2001_dbg(gdev, 5, "Queued message type 0x%x for inst %d,
> seqid: %d\n",
> +		   type, ctx->hw_inst.session_id, seqid);
> +
> +	/* The message is freed after being sent to hardware or on
> error. */
> +	go2001_send_pending(gdev);
> +
> +	return seqid;
...
> +}
> +
> +static int go2001_wait_for_reply(struct go2001_dev *gdev, u32
> session_id,
> +				 u32 sequence_id)
> +{
> +	int ret;
...
> +			go2001_sequence_passed(gdev, session_id,
> sequence_id),
> +			msecs_to_jiffies(GO2001_REPLY_TIMEOUT_MS));
> +	if (ret == 0) {
> +		go2001_err(gdev, "Timeout waiting for reply to
> %d/%d\n",
...
> +		(list_empty(&ctx->hw_inst.pending_list) && !ctx-
> >job.src_buf);
> +	spin_unlock_irqrestore(&ctx->qlock, flags2);
...
> +
> +	spin_lock_irqsave(&gdev->irqlock, flags);
> +	seqid = go2001_queue_msg_locked(gdev, &gdev->ctrl_inst,
> msg);
> +	spin_unlock_irqrestore(&gdev->irqlock, flags);
> +
...
> +		list_add_tail(&ctx->hw_inst.inst_entry, &gdev-
> >new_inst_list);
...
> +}
> +
> +int go2001_get_reply(struct go2001_dev *gdev, struct go2001_msg
> *msg)
> +{
...
> +	}
> +
> +	memcpy_fromio(hdr, r->desc_iomem + desc->rd_off, desc-
> >msg_size);
...
> +		   ctx->src_fmt->desc);
> +
> +	msg = prepare_msg(gdev, GO2001_VM_INIT_DECODER,
> sizeof(*param));
> +	if (!msg)
> +		return -ENOMEM;
> +
...
> +}
> +
> +static int go2001_s_ctrl(struct go2001_ctx *ctx, enum
> go2001_hw_ctrl_type type,
> +			 union go2001_hw_ctrl *ctrl)
> +{
> +	struct go2001_set_ctrl_param *param;
> +	struct go2001_msg *msg;
> +
> +	msg = prepare_msg(ctx->gdev, GO2001_VM_SET_CTRL,
> sizeof(*param));
> +	if (!msg)
> +		return -ENOMEM;
> +
...
> +	ctrl->interp_filter_type =
> GO2001_CODING_CTRL_INTERP_FILTER_BICUBIC;
> +	ctrl->deblock_filter_type =
> GO2001_CODING_CTRL_DEBLOCK_FILTER_NORMAL;
...
> +
> +	return go2001_s_ctrl(ctx, GO2001_HW_CTRL_TYPE_CODING,
> &hw_ctrl);
> +}
> +
...
> +	}
> +
> +	go2001_dbg(gdev, 2, "Initializing encoder for formats %s->%s 
> at %dx%d\n",
> +		   ctx->src_fmt->desc, ctx->dst_fmt->desc, ctx-
> >finfo.width,
> +		   ctx->finfo.height);
> +
> +	msg = prepare_msg(gdev, GO2001_VM_INIT_ENCODER,
> sizeof(*param));
> +	if (!msg)
> +		return -ENOMEM;
> +
...
> +
> +	param->session_id = 0;
> +	param->num_ref_frames = ctx->enc_params.multi_ref_frame_mode 
> ? 1 : 3;
...
> +	struct go2001_msg *msg;
> +
> +	go2001_dbg(gdev, 2, "Releasing session id %d, codec state
> %d\n",
...
> +	}
> +
> +	/* Regardless of whether we sent and/or got a reply or not,
> clean up. */
> +	go2001_release_hw_inst(gdev, &ctx->hw_inst);
> +}
> +
> +int go2001_map_buffer(struct go2001_ctx *ctx, struct go2001_buffer
> *buf)
> +{
> +	struct go2001_dev *gdev = ctx->gdev;
...
> +
> +		dma_desc->map_addr = dma_desc-
> >mmap_list[0].dma_addr;
> +
> +		list_desc->first_entry_dma_addr = dma_desc-
> >map_addr;
> +		list_desc->entry_count = dma_desc->num_entries;
> +		list_desc->mmap_list_addr = dma_desc->dma_addr;
> +
> +		go2001_dbg(gdev, 4, "Mapping plane %u: 0x%08llx,
> chunks: %d in HW\n",
> +			   i, dma_desc->map_addr, dma_desc-
> >num_entries);
...
> +}
> +
> +void go2001_unmap_buffer(struct go2001_ctx *ctx, struct
> go2001_buffer *buf)
> +{
...
> +
> +	msg = prepare_msg(gdev, GO2001_VM_RELEASE_MMAP,
> sizeof(*param));
> +	if (!msg)
> +		return;
> +
...
> +		go2001_dbg(gdev, 4, "Unmapping plane %u, 0x%08llx
> from HW\n",
...
> +		return 0;
> +
> +	msg = prepare_msg(ctx->gdev, GO2001_VM_RELEASE_MMAP,
> sizeof(*param));
> +	if (!msg)
> +		return -ENOMEM;
> +	param = msg_to_param(msg);
...
> +}
> +
> +static int go2001_build_dec_msg(struct go2001_ctx *ctx, struct
> go2001_msg *msg,
> +				struct go2001_buffer *src_buf,
> +				struct go2001_buffer *dst_buf)
> +{
...
> +	param->payload_size = vb2_get_plane_payload(&src_buf->vb,
> 0);
> +	if (dst_buf) {
...
> +			param->out_addr[i] = dst_buf-
> >dma_desc[i].map_addr;
> +			WARN_ON(!IS_ALIGNED(param->out_addr[i],
> 16));
> +		}
> +
> +		if (!ctx->dst_fmt) {
> +			go2001_err(gdev, "Destination format not
> set\n");
> +			return -EINVAL;
> +		}
...
> +
> +	if (test_and_clear_bit(GO2001_FRAMERATE_CHANGE,
> changed_mask)) {
> +		ctx->enc_params.framerate_num =	new_params-
> >framerate_num;
> +		ctx->enc_params.framerate_denom = new_params-
> >framerate_denom;
> +	}
> +
> +	if (test_and_clear_bit(GO2001_KEYFRAME_REQUESTED,
> changed_mask))
> +		ctx->enc_params.request_keyframe = new_params-
> >request_keyframe;
> +}
> +
> +static int go2001_build_enc_msg(struct go2001_ctx *ctx, struct
> go2001_msg *msg,
> +				struct go2001_buffer *src_buf,
> +				struct go2001_buffer *dst_buf)
> +{
...
> +	param = msg_to_param(msg);
> +
> +	if (WARN_ON(src_buf->vb.num_planes > ARRAY_SIZE(param-
> >in_addr)))
> +		return -EINVAL;
> +
...
> +		return -EINVAL;
> +
> +	param->time_increment = GO2001_MAX_FPS / ctx-
...
> +		param->ipf_frame_ctrl =
> GO2001_FRM_CTRL_REFERENCE_AND_REFRESH;
...
> +}
> +
> +static void go2001_flush(struct go2001_ctx *ctx, struct
> go2001_buffer *src_buf)
> +{
> +	assert_spin_locked(&ctx->qlock);
> +
...
> +
> +	/*
> +	 * Since VPX has no frame reordering and buffers are
> returned as
> +	 * soon as they are decoded, there is no need trigger
> anything on the
> +	 * destination queue. Just return the flush buffer to
> userspace.
> +	 */
> +	list_del(&src_buf->list);
...
> +}
> +
> +int go2001_prepare_gbuf(struct go2001_ctx *ctx, struct go2001_buffer
> *gbuf,
> +			bool is_src)
> +{
> +	struct go2001_msg *msg;
...
> +}
> +
> +void go2001_finish_gbuf(struct go2001_ctx *ctx, struct go2001_buffer
> *gbuf)
> +{
> +	if (gbuf->msg) {
...
Using empty payload to signal to drain/flush has been deprecated, and
produce a warning. It's fine to support, but new driver should
implement VIDIOC_ENCODER_CMD/V4L2_ENC_CMD_STOP and VIDIOC_DECODER_CMD/
V4L2_DEC_CMD_STOP .

> +		}
> +
> +		ret = go2001_build_dec_msg(ctx, msg, src_buf,
> dst_buf);
> +	} else {
> +		ret = go2001_build_enc_msg(ctx, msg, src_buf,
> dst_buf);
> +	}
> +	if (ret)
> +		goto out;
...
> +	struct go2001_msg *msg;
> +
> +	if (level != GO2001_LOG_LEVEL_DISABLED && level >
> GO2001_LOG_LEVEL_MAX)
> +		level = GO2001_LOG_LEVEL_MAX;
> +
> +	msg = prepare_msg(gdev, GO2001_VM_SET_LOG_LEVEL,
> sizeof(*param));
> +	if (!msg)
> +		return -ENOMEM;
> +
...
> +	list_add_tail(&gdev->ctrl_inst.inst_entry, &gdev-
> >inst_list);
...
> +	return 0;
> +}
> diff --git a/drivers/media/pci/go2001/go2001_hw.h
...
> diff --git a/drivers/media/pci/go2001/go2001_proto.h
...
> +	struct go2001_msg_hdr hdr;
> +	u8 param[GO2001_MAX_MSG_PAYLOAD_SIZE - sizeof(struct
> go2001_msg_hdr)];
> +} __packed;
> +
...
> +	u32 count;
> +	struct go2001_mmap_list_desc
> mmap_list_desc[GO2001_MMAP_MAX_ENTRIES];
> +} __packed;
> +
...
> +(((u32)'P') | (((u32)'C') << 8) | (((u32)'I') << 16) | (((u32)'E')
> << 24))
> +#define GO2001_FW_DONE_SIGNATURE \
> +(((u32)'D') | (((u32)'O') << 8) | (((u32)'N') << 16) | (((u32)'E')
> << 24))
> +
