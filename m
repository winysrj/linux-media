...
Received: from devils.ext.ti.com ([198.47.26.153]:36384 "EHLO
	devils.ext.ti.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1751308Ab1KLNTN (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Sat, 12 Nov 2011 08:19:13 -0500
Received: from dbdp20.itg.ti.com ([172.24.170.38])
	by devils.ext.ti.com (8.13.7/8.13.7) with ESMTP id pACDJ9PQ023255
	(version=TLSv1/SSLv3 cipher=DHE-RSA-AES256-SHA bits=256 verify=NO)
	for <linux-media@vger.kernel.org>; Sat, 12 Nov 2011 07:19:11 -0600
From: Manjunath Hadli <manjunath.hadli@ti.com>
To: LMML <linux-media@vger.kernel.org>
CC: dlos <davinci-linux-open-source@linux.davincidsp.com>,
	Manjunath Hadli <manjunath.hadli@ti.com>,
	Nagabhushana Netagunte <nagabhushana.netagunte@ti.com>
Subject: [RFC PATCH v4 4/8] davinci: vpfe: add support for CCDC hardware for dm365
Date: Sat, 12 Nov 2011 18:48:58 +0530
Message-ID: <1321103942-2778-5-git-send-email-manjunath.hadli@ti.com>
...
+		<< GAIN_INTEGER_SHIFT;
+	val |= ccdc_cfg.bayer.config_params.gain_offset.gain.r_ye.decimal &
+			GAIN_DECIMAL_MASK;
+	regw(val, CRGAIN);
+
+	val = (gain_off_ptr->gain.gr_cy.integer & GAIN_INTEGER_MASK) <<
+				GAIN_INTEGER_SHIFT;
+	val |= gain_off_ptr->gain.gr_cy.decimal & GAIN_DECIMAL_MASK;
+
+	regw(val, CGRGAIN);
+
+	val = (gain_off_ptr->gain.gb_g.integer & GAIN_INTEGER_MASK) <<
+				GAIN_INTEGER_SHIFT;
+	val |= gain_off_ptr->gain.gb_g.decimal & GAIN_DECIMAL_MASK;
+	regw(val, CGBGAIN);
+
+	val = (gain_off_ptr->gain.b_mg.integer & GAIN_INTEGER_MASK) <<
+				GAIN_INTEGER_SHIFT;
+	val |= gain_off_ptr->gain.b_mg.decimal & GAIN_DECIMAL_MASK;
+	regw(val, CBGAIN);
+
+	regw((gain_off_ptr->offset & OFFSET_MASK), COFSTA);
+}
+
...
+	      CCDC_INPUT_MASK) << CCDC_INPUT_SHIFT) | ((params->config_params.
+	      data_shift & CCDC_DATASFT_MASK) << CCDC_DATASFT_SHIFT);
+
+	regw(val, MODESET);
...
+	int err = -EINVAL;
+
+	if (gain_offset->gain_sdram_en || gain_offset->gain_ipipe_en ||
...
+		gain_offset->offset_h3a_en) && gain_offset->offset > 0xfff) {
+		dev_dbg(dev, "invalid  gain b_mg\n");
+		return err;
+	}
+
