...
Received: from ams-iport-2.cisco.com ([144.254.224.141]:29519 "EHLO
	ams-iport-2.cisco.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1753681Ab2IKNop (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Tue, 11 Sep 2012 09:44:45 -0400
From: Hans Verkuil <hverkuil@xs4all.nl>
To: Guennadi Liakhovetski <g.liakhovetski@gmx.de>
Subject: Re: [PATCH] media: add a VEU MEM2MEM format conversion and scaling driver
Date: Tue, 11 Sep 2012 15:44:29 +0200
Cc: Linux Media Mailing List <linux-media@vger.kernel.org>
References: <Pine.LNX.4.64.1209111459340.22084@axis700.grange>
In-Reply-To: <Pine.LNX.4.64.1209111459340.22084@axis700.grange>
MIME-Version: 1.0
Content-Type: Text/Plain;
  charset="iso-8859-1"
Content-Transfer-Encoding: 7bit
Message-Id: <201209111544.29596.hverkuil@xs4all.nl>
...
> +}
> +
> +static void sh_veu_process(struct sh_veu_file *veu_file,
...
> +
> +	if (src_buf && dst_buf)
> +		sh_veu_process(veu_file, src_buf, dst_buf);
> +}
> +
> +		/* ========== video ioctls ========== */
> +
...
> + */
> +static int sh_veu_init_stream(struct sh_veu_dev *veu, struct sh_veu_file *veu_file,
> +			      enum v4l2_buf_type type)
> +{
...
> +		return 0;
> +
> +	veu->m2m_ctx = v4l2_m2m_ctx_init(veu->m2m_dev, veu_file,
> +					 sh_veu_queue_init);
> +
...
> +{
> +	strncpy(cap->driver, "sh-veu", sizeof(cap->driver) - 1);
> +	strncpy(cap->card, "sh-mobile VEU", sizeof(cap->card) - 1);
> +	cap->bus_info[0] = '\0';

bus_info shouldn't be empty. Set it to the driver name for now.
...
> +	struct sh_veu_dev *veu = veu_file->veu_dev;
> +	struct vb2_queue *vq = v4l2_m2m_get_vq(veu->m2m_ctx, f->type);
...
colorspace not filled in: do it the same as the mem2mem_testdev.c driver does.

> +	dev_dbg(veu->dev, "%s(): type: %d, size %u\n", __func__,
> +		f->type, pix->sizeimage);
> +
> +	return 0;
> +}
> +
...
> +		break;
> +	default:
> +		/* Or silently change to NONE? */

TRY_FMT shouldn't generate errors, so just change to NONE.
...
This can't happen. Perhaps the default case should be right below the
'case V4L2_BUF_TYPE_VIDEO_OUTPUT:' line as a catch all.
...
> +{
> +	struct sh_veu_file *veu_file = priv;
> +	int ret = sh_veu_init_stream(veu_file->veu_dev, veu_file, f->type);
> +	if (ret < 0)
> +		return ret;
> +
> +	ret = sh_veu_try_fmt_vid_cap(file, priv, f);
> +	if (ret)
> +		return ret;
> +
...
> +		return ret;
> +
> +	ret = sh_veu_try_fmt_vid_out(file, priv, f);
> +	if (ret)
> +		return ret;
> +
> +	return sh_veu_s_fmt(priv, f);
...
> +	struct sh_veu_file *veu_file = priv;
> +
> +	if (!sh_veu_is_streamer(veu_file->veu_dev, veu_file, reqbufs->type))
> +		return -EPERM;
> +
> +	return v4l2_m2m_reqbufs(file, veu_file->veu_dev->m2m_ctx, reqbufs);
> +}
> +
...
> +{
> +	struct sh_veu_file *veu_file = priv;
> +	int ret = sh_veu_init_stream(veu_file->veu_dev, veu_file, type);
> +	if (ret < 0)
> +		return ret;
> +
...
> +
> +	.vidioc_enum_input	= sh_veu_enum_input,
> +	.vidioc_g_input		= sh_veu_g_input,
> +	.vidioc_s_input		= sh_veu_s_input,
> +};
> +
> +		/* ========== Queue operations ========== */
> +
...
> +{
> +	struct sh_veu_file *veu_file = vb2_get_drv_priv(vq);
> +	struct sh_veu_dev *veu = veu_file->veu_dev;
> +	struct sh_veu_vfmt *vfmt;
...
> +{
> +	struct sh_veu_file *veu_file = vb2_get_drv_priv(vb->vb2_queue);
> +	struct sh_veu_dev *veu = veu_file->veu_dev;
> +	struct sh_veu_vfmt *vfmt;
...
> +	v4l2_m2m_buf_queue(veu_file->veu_dev->m2m_ctx, vb);
> +}
> +
> +static void sh_veu_wait_prepare(struct vb2_queue *q)
> +{
> +	struct sh_veu_file *veu_file = vb2_get_drv_priv(q);
> +	sh_veu_unlock(veu_file);
> +}
> +
> +static void sh_veu_wait_finish(struct vb2_queue *q)
> +{
> +	struct sh_veu_file *veu_file = vb2_get_drv_priv(q);
> +	sh_veu_lock(veu_file);
> +}
> +
...
> +	struct sh_veu_file *veu_file = v4l2_m2m_get_curr_priv(veu->m2m_dev);
> +
> +	if (veu_file->xaction == MEM2MEM_DEF_TRANSLEN || veu_file->aborting) {
> +		v4l2_m2m_job_finish(veu->m2m_dev, veu->m2m_ctx);
> +		veu_file->xaction = 0;
> +	} else {
> +		sh_veu_device_run(veu_file);
> +	}
> +
> +	return IRQ_HANDLED;
> +}
...
> +
> +	veu_file = v4l2_m2m_get_curr_priv(veu->m2m_dev);
> +
...
> +	vdev->ctrl_handler = &veu->ctrl_handler;
> +
...
> +
> +	v4l2_ctrl_handler_init(&veu->ctrl_handler, 2);
> +	v4l2_ctrl_new_std(&veu->ctrl_handler, NULL,
> +			  V4L2_CID_MIN_BUFFERS_FOR_CAPTURE, 2, 2, 1, 2);
> +	v4l2_ctrl_new_std(&veu->ctrl_handler, NULL,
> +			  V4L2_CID_MIN_BUFFERS_FOR_OUTPUT, 2, 2, 1, 2);
> +	v4l2_ctrl_handler_setup(&veu->ctrl_handler);

You must check for an error here and free the control handler on error.

	if (veu->ctrl_handler.error) {
		...
	}
...
> +
> +	v4l2_ctrl_handler_free(&veu->ctrl_handler);
