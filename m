Return-path: <linux-media-owner@vger.kernel.org>
Received: from mx3.molgen.mpg.de ([141.14.17.11]:39653 "EHLO mx1.molgen.mpg.de"
        rhost-flags-OK-OK-OK-FAIL) by vger.kernel.org with ESMTP
        id S1729801AbeGZQaq (ORCPT <rfc822;linux-media@vger.kernel.org>);
        Thu, 26 Jul 2018 12:30:46 -0400
Subject: Re: [git:media_tree/master] media: uvcvideo: Add KSMedia 8-bit IR
 format support
To: Mauro Carvalho Chehab <mchehab+samsung@kernel.org>
References: <E1fihuV-00048q-Ev@www.linuxtv.org>
Cc: linux-media@vger.kernel.org,
        Laurent Pinchart <laurent.pinchart@ideasonboard.com>
From: Paul Menzel <pmenzel+linux-media@molgen.mpg.de>
Message-ID: <6b567864-f60b-6809-6ac8-a8cbdf824871@molgen.mpg.de>
Date: Thu, 26 Jul 2018 17:13:25 +0200
MIME-Version: 1.0
In-Reply-To: <E1fihuV-00048q-Ev@www.linuxtv.org>
Content-Type: multipart/signed; protocol="application/pkcs7-signature"; micalg=sha-256; boundary="------------ms080506090405050206070106"
Sender: linux-media-owner@vger.kernel.org
List-ID: <linux-media.vger.kernel.org>

This is a cryptographically signed message in MIME format.

--------------ms080506090405050206070106
Content-Type: text/plain; charset=utf-8
Content-Language: en-US
Content-Transfer-Encoding: quoted-printable

Dear Mauro,


On 07/26/18 17:03, Mauro Carvalho Chehab wrote:
> This is an automatic generated email to let you know that the following=
 patch were queued:
>=20
> Subject: media: uvcvideo: Add KSMedia 8-bit IR format support
> Author:  Laurent Pinchart <laurent.pinchart@ideasonboard.com>
> Date:    Wed Mar 21 05:16:40 2018 -0400
>=20
> Add support for the 8-bit IR format GUID defined in the Microsoft Kerne=
l
> Streaming Media API.
>=20
> Reported-by: Paul Menzel <pmenzel+linux-media@molgen.mpg.de>
> Signed-off-by: Laurent Pinchart <laurent.pinchart@ideasonboard.com>
> Tested-by: Paul Menzel <pmenzel+linux-media@molgen.mpg.de>
> Signed-off-by: Mauro Carvalho Chehab <mchehab+samsung@kernel.org>
>=20
>  drivers/media/usb/uvc/uvc_driver.c | 5 +++++
>  drivers/media/usb/uvc/uvcvideo.h   | 3 +++
>  2 files changed, 8 insertions(+)

[=E2=80=A6]

Thank you very much for accepting Laurent=E2=80=99s patch. Could you plea=
se tag
this for the stable series (Cc: stable@vger.kernel.org)?


Kind regards,

Paul


--------------ms080506090405050206070106
Content-Type: application/pkcs7-signature; name="smime.p7s"
Content-Transfer-Encoding: base64
Content-Disposition: attachment; filename="smime.p7s"
Content-Description: S/MIME Cryptographic Signature


--------------ms080506090405050206070106--
