...
Received: from mailout3.samsung.com ([203.254.224.33]:36246 "EHLO
        ...
        with ESMTP id S932965AbdCaJEt (ORCPT
        <rfc822;linux-media@vger.kernel.org>);
        Fri, 31 Mar 2017 05:04:49 -0400
...
Subject: [Patch v3 08/11] [media] s5p-mfc: Add VP9 decoder support
Date: Fri, 31 Mar 2017 14:36:37 +0530
Message-id: <1490951200-32070-9-git-send-email-smitha.t@samsung.com>
...
References: <1490951200-32070-1-git-send-email-smitha.t@samsung.com>
 <CGME20170331090447epcas1p414811c3067f22671a77144f80a07df65@epcas1p4.samsung.com>
...
+		break;
 	case S5P_MFC_CODEC_H264_ENC:
 		if (IS_MFCV10(dev)) {
 			mfc_debug(2, "Use min scratch buffer size\n");
@@ -337,6 +343,7 @@ static int s5p_mfc_alloc_instance_buffer_v6(struct s5p_mfc_ctx *ctx)
...
+	case S5P_MFC_CODEC_VP9_DEC:
 		...
 		break;
 	case S5P_MFC_CODEC_H264_ENC:
@@ -569,6 +576,14 @@ static int s5p_mfc_set_dec_frame_buffer_v6(struct s5p_mfc_ctx *ctx)
 		}
 	}
...
+
 	...
 	if (buf_size1 < 0) {
@@ -2274,6 +2289,18 @@ const struct s5p_mfc_regs *s5p_mfc_init_regs_v6_plus(struct s5p_mfc_dev *dev)
