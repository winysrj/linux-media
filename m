...
Received: from mailout2.samsung.com ([203.254.224.25]:41245 "EHLO
	mailout2.samsung.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1752897Ab2H0JOB (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Mon, 27 Aug 2012 05:14:01 -0400
Received: from epcpsbgm1.samsung.com (epcpsbgm1 [203.254.230.26])
 by mailout2.samsung.com
 (Oracle Communications Messaging Server 7u4-24.01(7.0.4.24.0) 64bit (built Nov
 17 2011)) with ESMTP id <0M9E00ABNPKPO3K0@mailout2.samsung.com> for
 linux-media@vger.kernel.org; Mon, 27 Aug 2012 18:13:59 +0900 (KST)
Received: from localhost.localdomain ([107.108.73.106])
 by mmp2.samsung.com (Oracle Communications Messaging Server 7u4-24.01
 (7.0.4.24.0) 64bit (built Nov 17 2011))
 with ESMTPA id <0M9E008UBPLLV3D0@mmp2.samsung.com> for
 linux-media@vger.kernel.org; Mon, 27 Aug 2012 18:13:58 +0900 (KST)
...
Subject: [PATCH v5 1/4] [media] s5p-mfc: Update MFCv5 driver for callback based
 architecture
Date: Mon, 27 Aug 2012 17:28:00 +0530
Message-id: <1346068683-31610-2-git-send-email-arun.kk@samsung.com>
...
---
 ...
 drivers/media/video/s5p-mfc/s5p_mfc_common.h |   41 +-
 drivers/media/video/s5p-mfc/s5p_mfc_ctrl.c   |   27 +-
 drivers/media/video/s5p-mfc/s5p_mfc_ctrl.h   |    1 +
 drivers/media/video/s5p-mfc/s5p_mfc_dec.c    |   53 +-
 ...
 drivers/media/video/s5p-mfc/s5p_mfc_opr.c    | 1401 +++-------------------
 drivers/media/video/s5p-mfc/s5p_mfc_opr.h    |  178 ++-
 drivers/media/video/s5p-mfc/s5p_mfc_opr_v5.c | 1694 ++++++++++++++++++++++++++
 ...
 20 files changed, 2421 insertions(+), 1623 deletions(-)
...
+	return s5p_mfc_hw_call(s5p_mfc_cmds, sys_init_cmd, dev);
 }
...
+	return s5p_mfc_hw_call(s5p_mfc_cmds, sleep_cmd, dev);
 }
...
+	return s5p_mfc_hw_call(s5p_mfc_cmds, wakeup_cmd, dev);
 }
...
-	}
-	return ret;
+	return s5p_mfc_hw_call(s5p_mfc_cmds, open_inst_cmd, ctx);
 }
...
+	int warn_start;
 ...
 /**
@@ -562,6 +598,9 @@ struct mfc_control {
 	__u8			is_volatile;
 };
...
@@ -1042,3 +1043,13 @@ void s5p_mfc_dec_ctrls_delete(struct s5p_mfc_ctx *ctx)
 		ctx->ctrls[i] = NULL;
 }
...
@@ -1832,3 +1834,13 @@ void s5p_mfc_enc_ctrls_delete(struct s5p_mfc_ctx *ctx)
...
+int s5p_mfc_alloc_dec_temp_buffers(struct s5p_mfc_ctx *ctx)
+{
+	return s5p_mfc_hw_call(s5p_mfc_ops, alloc_dec_temp_buffers, ctx);
 }
...
-		ctx->desc_buf = NULL;
-	}
+	s5p_mfc_hw_call(s5p_mfc_ops, release_dec_desc_buffer, ctx);
 }
...
-	}
-	return 0;
+	return s5p_mfc_hw_call(s5p_mfc_ops, alloc_codec_buffers, ctx);
 }
...
-		ctx->bank2_size = 0;
-	}
+	s5p_mfc_hw_call(s5p_mfc_ops, release_codec_buffers, ctx);
 }
...
-	}
-	return 0;
+	return s5p_mfc_hw_call(s5p_mfc_ops, alloc_instance_buffer, ctx);
 }
...
-		ctx->shm = NULL;
-	}
+	s5p_mfc_hw_call(s5p_mfc_ops, release_instance_buffer, ctx);
 }
...
+	return s5p_mfc_hw_call(s5p_mfc_ops, alloc_dev_context_buffer, dev);
 }
...
+	s5p_mfc_hw_call(s5p_mfc_ops, release_dev_context_buffer, dev);
 }
...
+void s5p_mfc_dec_calc_dpb_size(struct s5p_mfc_ctx *ctx)
 {
-	struct s5p_mfc_dev *dev = ctx->dev;
+	s5p_mfc_hw_call(s5p_mfc_ops, dec_calc_dpb_size, ctx);
+}
...
+{
+	s5p_mfc_hw_call(s5p_mfc_ops, enc_calc_src_size, ctx);
 }
...
-	return 0;
+	return s5p_mfc_hw_call(s5p_mfc_ops, set_dec_frame_buffer, ctx);
 }
...
-	return 0;
+	return s5p_mfc_hw_call(s5p_mfc_ops, set_enc_stream_buffer, ctx, addr,
+			size);
 }
...
+	s5p_mfc_hw_call(s5p_mfc_ops, set_enc_frame_buffer, ctx, y_addr,
+			c_addr);
 }
...
-							<< MFC_OFFSET_SHIFT);
+	s5p_mfc_hw_call(s5p_mfc_ops, get_enc_frame_buffer, ctx, y_addr,
+			c_addr);
 }
...
-	}
-	return 0;
+	return s5p_mfc_hw_call(s5p_mfc_ops, set_enc_ref_buffer, ctx);
 }
...
-	return 0;
+	return s5p_mfc_hw_call(s5p_mfc_ops, init_decode, ctx);
 }
...
-	s5p_mfc_write_shm(ctx, shm, EXT_ENC_CONTROL);
-	return 0;
+	return s5p_mfc_hw_call(s5p_mfc_ops, init_encode, ctx);
 }
...
-	s5p_mfc_write_shm(ctx, shm, EXT_ENC_CONTROL);
-	return 0;
+	return s5p_mfc_hw_call(s5p_mfc_ops, encode_one_frame, ctx);
 }
...
-	unsigned int reg;
-	unsigned int shm;
+	s5p_mfc_hw_call(s5p_mfc_ops, try_run, dev);
+}
...
+			struct vb2_queue *vq)
+{
+	s5p_mfc_hw_call(s5p_mfc_ops, cleanup_queue, lh, vq);
 }
...
+void s5p_mfc_clear_int_flags(struct s5p_mfc_dev *dev)
 {
-	struct s5p_mfc_dev *dev = ctx->dev;
+	s5p_mfc_hw_call(s5p_mfc_ops, clear_int_flags, dev);
+}
...
+			unsigned int ofs)
+{
+	s5p_mfc_hw_call(s5p_mfc_ops, write_info, ctx, data, ofs);
 }
...
-	struct s5p_mfc_dev *dev = ctx->dev;
-	unsigned int dpb;
+	return s5p_mfc_hw_call(s5p_mfc_ops, read_info, ctx, ofs);
+}
...
+{
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_dspl_y_adr, dev);
 }
...
+int s5p_mfc_get_dec_y_adr(struct s5p_mfc_dev *dev)
 {
-	struct s5p_mfc_dev *dev = ctx->dev;
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_dec_y_adr, dev);
+}
...
+{
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_dspl_status, dev);
 }
...
+int s5p_mfc_get_dec_status(struct s5p_mfc_dev *dev)
 {
-	struct s5p_mfc_dev *dev = ctx->dev;
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_dec_status, dev);
+}
...
+{
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_dec_frame_type, dev);
 }
...
-		(ctx->inst_no), S5P_FIMV_SI_CH0_INST_ID);
-	return 0;
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_disp_frame_type, ctx);
 }
...
-	int new_ctx;
-	int cnt;
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_consumed_stream, dev);
+}
...
+{
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_int_reason, dev);
 }
...
+int s5p_mfc_get_int_err(struct s5p_mfc_dev *dev)
 {
-	struct s5p_mfc_dev *dev = ctx->dev;
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_int_err, dev);
+}
...
-	unsigned long flags;
-	unsigned int index;
+	return s5p_mfc_hw_call(s5p_mfc_ops, err_dec, err);
+}
...
+{
+	return s5p_mfc_hw_call(s5p_mfc_ops, err_dspl, err);
 }
...
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_img_width, dev);
+}
...
+{
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_img_height, dev);
 }
...
-	unsigned long flags;
-	struct s5p_mfc_buf *temp_vb;
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_dpb_count, dev);
+}
...
+{
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_mv_count, dev);
 }
...
-	unsigned int dst_size;
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_inst_no, dev);
+}
...
+{
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_enc_strm_size, dev);
 }
...
-	struct s5p_mfc_buf *temp_vb;
-	int ret;
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_enc_slice_type, dev);
+}
...
+{
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_enc_dpb_count, dev);
 }
...
-	int new_ctx;
-	unsigned int ret = 0;
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_enc_pic_count, dev);
+}
...
+{
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_sei_avail_status, ctx);
+}
...
+{
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_mvc_num_views, dev);
+}
...
+{
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_mvc_view_id, dev);
 }
 
+unsigned int s5p_mfc_get_pic_type_top(struct s5p_mfc_ctx *ctx)
+{
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_pic_type_top, ctx);
+}
...
-	int i;
+	return s5p_mfc_hw_call(s5p_mfc_ops, get_pic_type_bot, ctx);
+}
