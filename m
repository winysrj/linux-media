...
Received: from devils.ext.ti.com ([198.47.26.153]:38998 "EHLO
	devils.ext.ti.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1752914AbZDIEZJ convert rfc822-to-8bit (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Thu, 9 Apr 2009 00:25:09 -0400
...
Date: Thu, 9 Apr 2009 09:53:26 +0530
Subject: RE: [PATCH v2 3/4] ARM: DaVinci: DM646x Video: Add VPIF display
 	driver
Message-ID: <EAF47CD23C76F840A9E7FCE10091EFAB02A8764C4E@dbde02.ent.ti.com>
References: <1239189533-19961-1-git-send-email-chaithrika@ti.com>,<208cbae30904080610k6b4c0fbexaf5fb30c778f4d37@mail.gmail.com>
In-Reply-To: <208cbae30904080610k6b4c0fbexaf5fb30c778f4d37@mail.gmail.com>
...
> Hello,
> may i make small comment?
>
> On Wed, Apr 8, 2009 at 3:18 PM, Chaithrika U S <chaithrika@ti.com>
> wrote:
...
> > +       {
> > +               "NTSC", 720, 480, 30, 0, 1, 268, 1440, 1, 23, 263,
> 266,
> > +               286, 525, 525, 0, 1, 0, V4L2_STD_NTSC,
> > +       },
> > +       {
> > +               "PAL", 720, 576, 25, 0, 1, 280, 1440, 1, 23, 311,
> 313,
...
> > +               if (!vb->baddr) {
> > +                       v4l2_err(&vpif_obj.v4l2_dev, "buffer_address
> is 0\n");
> > +                       return -EINVAL;
> > +               }
> > +
...
> > +                               do_gettimeofday(&common->cur_frm-
> >ts);
> > +                               common->cur_frm->state =
> VIDEOBUF_DONE;
> > +                               wake_up_interruptible(&common-
> >cur_frm->done);
> > +                               /* Make cur_frm pointing to next_frm
> */
> > +                               common->cur_frm = common->next_frm;
...
> > +       ch->usrs--;
> > +       /* If this file handle has initialize encoder device, reset
> it */
> > +       if (fh->initialized)
> > +               ch->initialized = 0;
> > +
...
> > +{
> > +       if (fmt->index != 0) {
> > +               v4l2_err(&vpif_obj.v4l2_dev, "Invalid format
> index\n");
> > +               return -EINVAL;
> > +       }
> > +
...
> > +
> > +       /* bufferqueue is empty store buffer address in VPIF
> registers */
> > +       mutex_lock(&common->buffer_queue.vb_lock);
> > +       buf1 = common->buffer_queue.bufs[tbuf.index];
> > +       if (buf1->memory != tbuf.memory) {
> > +               v4l2_err(&vpif_obj.v4l2_dev, "invalid buffer
> type\n");
> > +               goto qbuf_exit;
> > +       }
> > +
...
> > +       mutex_lock_interruptible(&common->lock);
> > +       ret = v4l2_device_call_until_err(&vpif_obj.v4l2_dev, 1,
> video,
> > +                                               s_std_output,
> *std_id);
> > +       if (ret < 0) {
...
> > +               /* Calculate the offset for Y and C data  in the
> buffer */
...
> > +                       enable_channel3(1);
> > +               }
> > +               channel_first_int[VPIF_VIDEO_INDEX][ch->channel_id] =
> 1;
> > +       }
> > +
> > +streamon_exit:
...
> > +                   config_params.min_bufsize[VPIF_CHANNEL3_VIDEO];
> > +
> > +       config_params.numbuffers[VPIF_CHANNEL2_VIDEO] =
> ch2_numbuffers;
> > +
> > +       if (ch2_numbuffers) {
...
> > +                                                       ch2_bufsize;
> > +       }
> > +       config_params.numbuffers[VPIF_CHANNEL3_VIDEO] =
> ch3_numbuffers;
> > +
> > +       if (ch3_numbuffers) {
...
> > +}
>
> Imagine this kmalloc
>
> vpif_obj.sd = kmalloc(sizeof(struct v4l2_subdev *) * subdev_count,
> GFP_KERNEL);
>
>  fails and we do cleanup after this. We set err = -ENOMEM and go to
> probe_out;
> In the cleanup we change err= -EBUSY; (see that?) before
> video_dev_alloc_exit
> and finally we return EBUSY error instead of ENOMEM. Right?
> Looks that this happens after
> v4l2_device_register_subdev when we go to probe_subdev_out.
> Probably this can be redesigned.
>

Good catch. This has to be corrected. Will do it in next update.
