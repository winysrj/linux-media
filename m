...
Received: from mail-fx0-f46.google.com ([209.85.161.46]:52057 "EHLO
	mail-fx0-f46.google.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1751768Ab1EVSUr (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Sun, 22 May 2011 14:20:47 -0400
Received: by fxm17 with SMTP id 17so3512861fxm.19
        for <linux-media@vger.kernel.org>; Sun, 22 May 2011 11:20:45 -0700 (PDT)
From: "Christoph Pinkl" <christoph.pinkl@gmail.com>
To: "'Marko Ristola'" <marko.ristola@kolumbus.fi>,
	"'Adrian C.'" <anrxc@sysphere.org>, "'Lou'" <tuxoholic@hotmail.de>
Cc: <linux-media@vger.kernel.org>, <abraham.manu@gmail.com>
References: <alpine.LNX.2.00.1105040038430.10167@flfcurer.bet> <4DC431C6.1010605@kolumbus.fi> <alpine.LNX.2.00.1105102329290.12340@flfcurer.bet> <4dcd3ef7.dc06df0a.52b1.5d88@mx.google.com> <alpine.LNX.2.00.1105210314230.26477@flfcurer.bet> <alpine.LNX.2.00.1105210922290.31652@flfcurer.bet> <4DD7E5F5.4090503@kolumbus.fi>
In-Reply-To: <4DD7E5F5.4090503@kolumbus.fi>
Subject: AW: AW: Remote control not working for Terratec Cinergy C (2.6.37 Mantis driver)
Date: Sun, 22 May 2011 20:20:54 +0200
Message-ID: <4dd953fc.118fdf0a.36d9.fffff063@mx.google.com>
MIME-Version: 1.0
Content-Type: multipart/mixed;
	boundary="----=_NextPart_000_001D_01CC18BD.C1F66DE0"
...
------=_NextPart_000_001D_01CC18BD.C1F66DE0
...
index 05cbb9d..8eca749 100644=0A=
...
+		tasklet_schedule(&mantis->uart_tasklet);=0A=
 	...
 	if (stat & MANTIS_INT_OCERR) {=0A=
 		dprintk(MANTIS_DEBUG, 0, "<%s>", label[3]);=0A=
@@ -180,6 +181,14 @@ static int __devinit mantis_pci_probe(struct =
pci_dev *pdev, const struct pci_dev=0A=
 	config->irq_handler	=3D &mantis_irq_handler;=0A=
 	mantis->hwconfig	=3D config;=0A=
 =0A=
+	if (mantis->hwconfig->config_init !=3D NULL) {=0A=
+		dprintk(MANTIS_ERROR, 1,=0A=
+			"Mantis-subsystem: vendor:0x%04x, device:0x%04x\n",=0A=
+			mantis->pdev->subsystem_vendor,=0A=
+			mantis->pdev->subsystem_device);=0A=
+		mantis->hwconfig->config_init(mantis);=0A=
+	}=0A=
+=0A=
 	err =3D mantis_pci_init(mantis);=0A=
 	if (err) {=0A=
 		dprintk(MANTIS_ERROR, 1, "ERROR: Mantis PCI initialization failed =
<%d>", err);=0A=
@@ -215,21 +224,32 @@ static int __devinit mantis_pci_probe(struct =
pci_dev *pdev, const struct pci_dev=0A=
...
@@ -257,6 +277,7 @@ static void __devexit mantis_pci_remove(struct =
pci_dev *pdev)=0A=
 	if (mantis) {=0A=
 =0A=
 		mantis_uart_exit(mantis);=0A=
...
index bd400d2..a61046d 100644=0A=
...
+#include <media/rc-core.h>=0A=
 ...
 =0A=
@@ -91,8 +94,12 @@ struct mantis_hwconfig {=0A=
...
*fe);=0A=
+	void (*config_init)(struct mantis_pci *mantis);=0A=
 =0A=
 	u8			power;=0A=
 	u8			reset;=0A=
@@ -100,6 +107,12 @@ struct mantis_hwconfig {=0A=
...
+=0A=
 ...
 =0A=
@@ -167,13 +180,11 @@ struct mantis_pci {=0A=
 =0A=
...
index db6d54d..615429d 100644=0A=
...
index 38a436c..9b72fdc 100644=0A=
...
@@ -343,6 +346,19 @@ static int vp1041_frontend_init(struct mantis_pci =
*mantis, struct dvb_frontend *=0A=
 	return 0;=0A=
 }=0A=
 =0A=
+static void vp1041_config_init(struct mantis_pci *mantis) {=0A=
+	struct mantis_hwconfig *config =3D mantis->hwconfig;=0A=
+=0A=
+	switch (mantis->pdev->subsystem_vendor) {=0A=
+	case TWINHAN_TECHNOLOGIES:=0A=
+		if (mantis->pdev->subsystem_device =3D=3D MANTIS_VP_1041_DVB_S2)=0A=
+                        config->ir_codes =3D RC_MAP_TWINHAN_DTV_CAB_CI;=0A=
+		break;=0A=
+	default:=0A=
+		config->ir_codes =3D RC_MAP_TERRATEC_CINERGY_S2_HD;=0A=
+	}=0A=
+}=0A=
+=0A=
 struct mantis_hwconfig vp1041_config =3D {=0A=
 	.model_name	=3D MANTIS_MODEL_NAME,=0A=
 	.dev_type	=3D MANTIS_DEV_TYPE,=0A=
@@ -352,6 +368,9 @@ struct mantis_hwconfig vp1041_config =3D {=0A=
 	.parity		=3D MANTIS_PARITY_NONE,=0A=
 	.bytes		=3D 0,=0A=
 =0A=
+	.ir_work	=3D mantis_input_process,=0A=
+=0A=
+	.config_init	=3D vp1041_config_init,=0A=
