...
Received: from mga02.intel.com ([134.134.136.20]:18152 "EHLO mga02.intel.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1751691AbdJLBC6 (ORCPT <rfc822;linux-media@vger.kernel.org>);
        Wed, 11 Oct 2017 21:02:58 -0400
...
Date: Thu, 12 Oct 2017 01:02:54 +0000
Message-ID: <C193D76D23A22742993887E6D207B54D1AE28D72@ORSMSX106.amr.corp.intel.com>
...
In-Reply-To: <20171010074543.xmqavghypbnv25xr@valkosipuli.retiisi.org.uk>
...
We encountered some issue to build with 64bit previously, will fix in next update.
> 
> > +	select V4L2_FWNODE
...
> > +
> > +/*
> > + * cio2_find_format - lookup color format by fourcc or/and media bus
> code
> > + * @pixelformat: fourcc to match, ignored if null
> > + * @mbus_code: media bus code to match, ignored if null
> > + */
> > +static const struct ipu3_cio2_fmt *cio2_find_format(const u32
> *pixelformat,
> > +			const u32 *mbus_code)
> > +{
> > +	unsigned int i;
...
> > +
> > +	q->fbpt = dma_alloc_coherent(dev, CIO2_FBPT_SIZE, &q-
> >fbpt_bus_addr,
> > +					GFP_KERNEL);
> > +	if (!q->fbpt)
> > +		return -ENOMEM;
...
> > +}
> > +
> > +static int cio2_fbpt_rearrange(struct cio2_device *cio2, struct cio2_queue
> *q)
> > +{
...
Will attempt the pre-allocation approach (without in-place swapping) first then.
...
> > +		if (b) {
> > +			unsigned int bytes =
> entry[1].second_entry.num_of_bytes;
> > +
...
> > +			b->vbb.sequence = atomic_read(&q-
> >frame_sequence);
> > +			if (b->vbb.vb2_buf.planes[0].length != bytes)
> > +				dev_warn(dev, "buffer length is %d
> received %d\n",
...
> > +		.u.frame_sync.frame_sequence = atomic_read(&q-
> >frame_sequence),
> > +	};
> > +
...
> > +	 * does not get an opportunity to queue new buffers to the CIO2
> DMA
> > +	 * engine. When the DMA engine encounters an FBPT entry without
> the
...
> > +	for (i = 0; i < CIO2_MAX_LOPS; i++) {
> > +		if (b->lop[i])
> > +			dma_free_coherent(&cio2->pci_dev->dev,
> CIO2_PAGE_SIZE,
> > +					  b->lop[i], b->lop_bus_addr[i]);
> > +	}
> > +}
...
> > +		q->subdev_fmt.quantization =
> sensor_fmt.format.quantization;
...
> > +	}
> > +
> > +	ret = cio2_subdev_link_validate_get_format(link->source,
> &source_fmt);
> > +	if (ret < 0)
> > +		return 0;
> > +
...
> > +	}
> > +
> > +	if (!cio2_find_format(&q->format.pixelformat,
...
> > +	q->format.plane_fmt[0].sizeimage = q-
...
We can move the cio2_fbpt_init_dummy(cio2) earlier in the calling order, the cio2_queues_init() and cio2_notifer_init() have to happen after cio2->v4l2_dev being registered. 
...
Yes, IPU3 is capable of supporting 4 sensors, will change CIO2_QUEUES to 4.
...
Ack, will make either changes as suggested.
> 
> > +	};
