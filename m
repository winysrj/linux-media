...
Received: from mail.kernel.org (mail.kernel.org [198.145.29.99])
	by smtp.lore.kernel.org (Postfix) with ESMTP id 0CE6CC43381
	for <linux-media@archiver.kernel.org>; Fri,  8 Mar 2019 05:51:18 +0000 (UTC)
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.kernel.org (Postfix) with ESMTP id 8978720684
	for <linux-media@archiver.kernel.org>; Fri,  8 Mar 2019 05:51:17 +0000 (UTC)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1725372AbfCHFvQ (ORCPT <rfc822;linux-media@archiver.kernel.org>);
        Fri, 8 Mar 2019 00:51:16 -0500
Received: from mailgw01.mediatek.com ([210.61.82.183]:10561 "EHLO
        mailgw01.mediatek.com" rhost-flags-OK-FAIL-OK-FAIL) by vger.kernel.org
        with ESMTP id S1726324AbfCHFvQ (ORCPT
        <rfc822;linux-media@vger.kernel.org>); Fri, 8 Mar 2019 00:51:16 -0500
X-UUID: 0ecce3b9c6b94a4eb18a27cae0a52f46-20190308
X-UUID: 0ecce3b9c6b94a4eb18a27cae0a52f46-20190308
Received: from mtkcas06.mediatek.inc [(172.21.101.30)] by mailgw01.mediatek.com
        (envelope-from <daoyuan.huang@mediatek.com>)
        (mhqrelay.mediatek.com ESMTP with TLS)
        with ESMTP id 1130546329; Fri, 08 Mar 2019 13:50:09 +0800
Received: from MTKMBS06N2.mediatek.inc (172.21.101.130) by
 mtkmbs08n2.mediatek.inc (172.21.101.56) with Microsoft SMTP Server (TLS) id
 15.0.1395.4; Fri, 8 Mar 2019 13:50:07 +0800
Received: from mtkcas07.mediatek.inc (172.21.101.84) by
 mtkmbs06n2.mediatek.inc (172.21.101.130) with Microsoft SMTP Server (TLS) id
 15.0.1395.4; Fri, 8 Mar 2019 13:50:07 +0800
Received: from mtkslt306.mediatek.inc (10.21.14.136) by mtkcas07.mediatek.inc
 (172.21.101.73) with Microsoft SMTP Server id 15.0.1395.4 via Frontend
 Transport; Fri, 8 Mar 2019 13:50:07 +0800
...
Subject: [RFC v1 4/4] media: platform: mtk-mdp3: Add Mediatek MDP3 driver
Date:   Fri, 8 Mar 2019 13:49:20 +0800
Message-ID: <1552024160-33055-5-git-send-email-daoyuan.huang@mediatek.com>
...
X-TM-SNTS-SMTP: 7061BE174DA630CBA073E425634C8513DF345519C9E8A6532BA6BE94E0713C392000:8
X-MTK:  N
...
---
 ...
 24 files changed, 5809 insertions(+)
...
+
+#define MM_REG_WRITE_MASK(cmd, base, ofst, val, mask, ...) \
+	cmdq_pkt_write_mask(cmd->pkt, cmd->clt_base, \
+		(base) + (ofst), (val), (mask), ##__VA_ARGS__)
+#define MM_REG_WRITE(cmd, base, ofst, val, mask, ...) \
+	MM_REG_WRITE_MASK(cmd, base, ofst, val, \
...
+
+#define MM_REG_POLL_MASK(cmd, base, ofst, val, mask, ...) \
+	cmdq_pkt_poll(cmd->pkt, cmd->clt_base, \
+		(base) + (ofst), (val), (mask), ##__VA_ARGS__)
+#define MM_REG_POLL(cmd, base, ofst, val, mask, ...) \
+	MM_REG_POLL_MASK(cmd, base, ofst, val, \
+		(((mask) & (ofst##_MASK)) == (ofst##_MASK)) ? \
+			(0xffffffff) : (mask), ##__VA_ARGS__)
+
...
+	if (subfrm->mutex_mod) {
+		/* Set mutex modules */
+		MM_REG_WRITE(cmd, mm_mutex, MM_MUTEX_MOD,
+			     subfrm->mutex_mod, 0x07FFFFFF);
+		MM_REG_WRITE(cmd, mm_mutex, MM_MUTEX_SOF,
...
+
+		/* Enable the mutex */
+		MM_REG_WRITE(cmd, mm_mutex, MM_MUTEX_EN, 0x1, 0x00000001);
+
...
+		set = &ctrl->sets[index];
+		MM_REG_WRITE_MASK(cmd, mmsys, set->reg, set->value, 0xFFFFFFFF);
+	}
...
+	for (index = 0; index < ctrl->num_sets; index++) {
+		set = &ctrl->sets[index];
+		MM_REG_WRITE_MASK(cmd, mmsys, set->reg, 0, 0xFFFFFFFF);
+	}
+	return 0;
+}
+
...
+		MM_REG_WRITE(cmd, prz1->reg_base, PRZ_ENABLE,
+			     0x00000000, 0x00000001);
+#endif
+	/* Reset RDMA */
+	MM_REG_WRITE(cmd, base, MDP_RDMA_RESET, 0x00000001, 0x00000001);
+	MM_REG_POLL(cmd, base, MDP_RDMA_MON_STA_1, 0x00000100, 0x00000100);
+	MM_REG_WRITE(cmd, base, MDP_RDMA_RESET, 0x00000000, 0x00000001);
+	return 0;
+}
+
...
+	if (block10bit)
+		MM_REG_WRITE(cmd, base, MDP_RDMA_RESV_DUMMY_0,
...
+
+	/* Setup smi control */
+	MM_REG_WRITE(cmd, base, MDP_RDMA_GMCIF_CON,
+		     (1 <<  0) +
+		     (7 <<  4) + //burst type to 8
...
+	MM_REG_WRITE(cmd, base, MDP_RDMA_SRC_CON, rdma->src_ctrl, 0x03C8FE0F);
+#if RDMA_SUPPORT_10BIT
+	if (en_ufo) {
+		/* Setup source buffer base */
+		MM_REG_WRITE(cmd, base, MDP_RDMA_UFO_DEC_LENGTH_BASE_Y,
+			     rdma->ufo_dec_y, 0xFFFFFFFF);
+		MM_REG_WRITE(cmd, base, MDP_RDMA_UFO_DEC_LENGTH_BASE_C,
+			     rdma->ufo_dec_c, 0xFFFFFFFF);
+		/* Set 10bit source frame pitch */
+		if (block10bit)
+			MM_REG_WRITE(cmd, base, MDP_RDMA_MF_BKGD_SIZE_IN_PXL,
+				     rdma->mf_bkgd_in_pxl, 0x001FFFFF);
+	}
+#endif
+	MM_REG_WRITE(cmd, base, MDP_RDMA_CON, rdma->control, 0x00001110);
+	/* Setup source buffer base */
+	MM_REG_WRITE(cmd, base, MDP_RDMA_SRC_BASE_0, rdma->iova[0], 0xFFFFFFFF);
+	MM_REG_WRITE(cmd, base, MDP_RDMA_SRC_BASE_1, rdma->iova[1], 0xFFFFFFFF);
+	MM_REG_WRITE(cmd, base, MDP_RDMA_SRC_BASE_2, rdma->iova[2], 0xFFFFFFFF);
+	/* Setup source buffer end */
+	MM_REG_WRITE(cmd, base, MDP_RDMA_SRC_END_0,
+		     rdma->iova_end[0], 0xFFFFFFFF);
+	MM_REG_WRITE(cmd, base, MDP_RDMA_SRC_END_1,
+		     rdma->iova_end[1], 0xFFFFFFFF);
+	MM_REG_WRITE(cmd, base, MDP_RDMA_SRC_END_2,
+		     rdma->iova_end[2], 0xFFFFFFFF);
+	/* Setup source frame pitch */
+	MM_REG_WRITE(cmd, base, MDP_RDMA_MF_BKGD_SIZE_IN_BYTE,
+		     rdma->mf_bkgd, 0x001FFFFF);
+	MM_REG_WRITE(cmd, base, MDP_RDMA_SF_BKGD_SIZE_IN_BYTE,
+		     rdma->sf_bkgd, 0x001FFFFF);
+	/* Setup color transform */
+	MM_REG_WRITE(cmd, base, MDP_RDMA_TRANSFORM_0,
...
+
+	/* Enable RDMA */
+	MM_REG_WRITE(cmd, base, MDP_RDMA_EN, 0x00000001, 0x00000001);
+
+	/* Set Y pixel offset */
+	MM_REG_WRITE(cmd, base, MDP_RDMA_SRC_OFFSET_0,
+		     subfrm->offset[0], 0xFFFFFFFF);
+#if RDMA_SUPPORT_10BIT
+	/* Set 10bit UFO mode */
+	if (block10bit && en_ufo)
+		MM_REG_WRITE(cmd, base, MDP_RDMA_SRC_OFFSET_0_P,
+			     subfrm->offset_0_p, 0xFFFFFFFF);
+#endif
+	/* Set U pixel offset */
+	MM_REG_WRITE(cmd, base, MDP_RDMA_SRC_OFFSET_1,
+		     subfrm->offset[1], 0xFFFFFFFF);
+	/* Set V pixel offset */
+	MM_REG_WRITE(cmd, base, MDP_RDMA_SRC_OFFSET_2,
+		     subfrm->offset[2], 0xFFFFFFFF);
+	/* Set source size */
+	MM_REG_WRITE(cmd, base, MDP_RDMA_MF_SRC_SIZE, subfrm->src, 0x1FFF1FFF);
+	/* Set target size */
+	MM_REG_WRITE(cmd, base, MDP_RDMA_MF_CLIP_SIZE,
+		     subfrm->clip, 0x1FFF1FFF);
+	/* Set crop offset */
+	MM_REG_WRITE(cmd, base, MDP_RDMA_MF_OFFSET_1,
...
+	/* Disable RDMA */
+	MM_REG_WRITE(cmd, base, MDP_RDMA_EN, 0x00000000, 0x00000001);
+	return 0;
+}
+
...
+
+	/* Reset RSZ */
+	MM_REG_WRITE(cmd, base, PRZ_ENABLE, 0x00010000, 0x00010000);
+	MM_REG_WRITE(cmd, base, PRZ_ENABLE, 0x00000000, 0x00010000);
+	/* Enable RSZ */
+	MM_REG_WRITE(cmd, base, PRZ_ENABLE, 0x00000001, 0x00000001);
+	return 0;
+}
+
...
+		/* Disable RSZ */
+		MM_REG_WRITE(cmd, base, PRZ_ENABLE, 0x00000000, 0x00000001);
+		return 0;
+	}
+
+	MM_REG_WRITE(cmd, base, PRZ_CONTROL_1, rsz->control1, 0x03FFFDF3);
+	MM_REG_WRITE(cmd, base, PRZ_CONTROL_2, rsz->control2, 0x0FFFC290);
+	MM_REG_WRITE(cmd, base, PRZ_HORIZONTAL_COEFF_STEP,
+		     rsz->coeff_step_x, 0x007FFFFF);
+	MM_REG_WRITE(cmd, base, PRZ_VERTICAL_COEFF_STEP,
...
+	phys_addr_t base = ctx->comp->reg_base;
+
+	MM_REG_WRITE(cmd, base, PRZ_CONTROL_2, subfrm->control2, 0x00003800);
+	MM_REG_WRITE(cmd, base, PRZ_INPUT_IMAGE, subfrm->src, 0xFFFFFFFF);
+#if RSZ_DISABLE_DCM_SMALL_TILE
+	if ((csf->in.right - csf->in.left + 1) <= 16)
+		MM_REG_WRITE(cmd, base, PRZ_CONTROL_1, 1 << 27, 1 << 27);
+#endif
+	MM_REG_WRITE(cmd, base, PRZ_LUMA_HORIZONTAL_INTEGER_OFFSET,
+		     csf->luma.left, 0x0000FFFF);
+	MM_REG_WRITE(cmd, base, PRZ_LUMA_HORIZONTAL_SUBPIXEL_OFFSET,
+		     csf->luma.left_subpix, 0x001FFFFF);
+	MM_REG_WRITE(cmd, base, PRZ_LUMA_VERTICAL_INTEGER_OFFSET,
+		     csf->luma.top, 0x0000FFFF);
+	MM_REG_WRITE(cmd, base, PRZ_LUMA_VERTICAL_SUBPIXEL_OFFSET,
+		     csf->luma.top_subpix, 0x001FFFFF);
+	MM_REG_WRITE(cmd, base, PRZ_CHROMA_HORIZONTAL_INTEGER_OFFSET,
+		     csf->chroma.left, 0x0000FFFF);
+	MM_REG_WRITE(cmd, base, PRZ_CHROMA_HORIZONTAL_SUBPIXEL_OFFSET,
+		     csf->chroma.left_subpix, 0x001FFFFF);
+
+	MM_REG_WRITE(cmd, base, PRZ_OUTPUT_IMAGE, subfrm->clip, 0xFFFFFFFF);
+
+	return 0;
+}
+
...
+
+	if ((csf->in.right - csf->in.left + 1) <= 16)
+		MM_REG_WRITE(cmd, base, PRZ_CONTROL_1, 0, 1 << 27);
+#endif
+	return 0;
+}
+
...
+#endif
+	/* Reset WROT */
+	MM_REG_WRITE(cmd, base, VIDO_SOFT_RST, 0x01, 0x00000001);
+	MM_REG_POLL(cmd, base, VIDO_SOFT_RST_STAT, 0x01, 0x00000001);
+	MM_REG_WRITE(cmd, base, VIDO_SOFT_RST, 0x00, 0x00000001);
+	MM_REG_POLL(cmd, base, VIDO_SOFT_RST_STAT, 0x00, 0x00000001);
+	return 0;
+}
+
...
+	MM_REG_WRITE(cmd, base, VIDO_BASE_ADDR,   wrot->iova[0], 0xFFFFFFFF);
+	MM_REG_WRITE(cmd, base, VIDO_BASE_ADDR_C, wrot->iova[1], 0xFFFFFFFF);
+	MM_REG_WRITE(cmd, base, VIDO_BASE_ADDR_V, wrot->iova[2], 0xFFFFFFFF);
+	/* Write frame related registers */
+	MM_REG_WRITE(cmd, base, VIDO_CTRL, wrot->control, 0xF131510F);
+	/* Write frame Y pitch */
+	MM_REG_WRITE(cmd, base, VIDO_STRIDE, wrot->stride[0], 0x0000FFFF);
+	/* Write frame UV pitch */
+	MM_REG_WRITE(cmd, base, VIDO_STRIDE_C, wrot->stride[1], 0x0000FFFF);
+	MM_REG_WRITE(cmd, base, VIDO_STRIDE_V, wrot->stride[2], 0x0000FFFF);
+	/* Write matrix control */
+	MM_REG_WRITE(cmd, base, VIDO_MAT_CTRL, wrot->mat_ctrl, 0x000000F3);
+
+	/* Set the fixed ALPHA as 0xFF */
+	MM_REG_WRITE(cmd, base, VIDO_DITHER, 0xFF000000, 0xFF000000);
+	/* Set VIDO_EOL_SEL */
+	MM_REG_WRITE(cmd, base, VIDO_RSV_1, 0x80000000, 0x80000000);
+	/* Set VIDO_FIFO_TEST */
+	if (wrot->fifo_test != 0)
+		MM_REG_WRITE(cmd, base, VIDO_FIFO_TEST,
+			     wrot->fifo_test, 0x00000FFF);
+
+#if WROT_FILTER_CONSTRAINT
+	/* Filter enable */
+	MM_REG_WRITE(cmd, base, VIDO_MAIN_BUF_SIZE, wrot->filter, 0x00000077);
+#endif
+
+	return 0;
+}
...
+	MM_REG_WRITE(cmd, base, VIDO_OFST_ADDR,
+		     subfrm->offset[0], 0x0FFFFFFF);
+	/* Write U pixel offset */
+	MM_REG_WRITE(cmd, base, VIDO_OFST_ADDR_C,
+		     subfrm->offset[1], 0x0FFFFFFF);
+	/* Write V pixel offset */
+	MM_REG_WRITE(cmd, base, VIDO_OFST_ADDR_V,
+		     subfrm->offset[2], 0x0FFFFFFF);
+	/* Write source size */
+	MM_REG_WRITE(cmd, base, VIDO_IN_SIZE, subfrm->src, 0x1FFF1FFF);
+	/* Write target size */
+	MM_REG_WRITE(cmd, base, VIDO_TAR_SIZE, subfrm->clip, 0x1FFF1FFF);
+	MM_REG_WRITE(cmd, base, VIDO_CROP_OFST, subfrm->clip_ofst, 0x1FFF1FFF);
+
+	MM_REG_WRITE(cmd, base, VIDO_MAIN_BUF_SIZE,
+		     subfrm->main_buf, 0x1FFF7F00);
+
+	/* Enable WROT */
+	MM_REG_WRITE(cmd, base, VIDO_ROT_EN, 0x01, 0x00000001);
+
+	return 0;
+}
+
...
+#endif
+	/* Disable WROT */
+	MM_REG_WRITE(cmd, base, VIDO_ROT_EN, 0x00, 0x00000001);
+
+	return 0;
+}
+
...
+
+	/* Reset WDMA */
+	MM_REG_WRITE(cmd, base, WDMA_RST, 0x1, 0x00000001);
+	MM_REG_POLL(cmd, base, WDMA_FLOW_CTRL_DBG, 0x01, 0x00000001);
+	MM_REG_WRITE(cmd, base, WDMA_RST, 0x0, 0x00000001);
+	return 0;
+}
+
...
+	phys_addr_t base = ctx->comp->reg_base;
+
+	MM_REG_WRITE(cmd, base, WDMA_BUF_CON2, 0x10101050, 0xFFFFFFFF);
+
+	/* Setup frame information */
+	MM_REG_WRITE(cmd, base, WDMA_CFG, wdma->wdma_cfg, 0x0F01B8F0);
+	/* Setup frame base address */
+	MM_REG_WRITE(cmd, base, WDMA_DST_ADDR,   wdma->iova[0], 0xFFFFFFFF);
+	MM_REG_WRITE(cmd, base, WDMA_DST_U_ADDR, wdma->iova[1], 0xFFFFFFFF);
+	MM_REG_WRITE(cmd, base, WDMA_DST_V_ADDR, wdma->iova[2], 0xFFFFFFFF);
+	/* Setup Y pitch */
+	MM_REG_WRITE(cmd, base, WDMA_DST_W_IN_BYTE,
+		     wdma->w_in_byte, 0x0000FFFF);
+	/* Setup UV pitch */
+	MM_REG_WRITE(cmd, base, WDMA_DST_UV_PITCH,
+		     wdma->uv_stride, 0x0000FFFF);
+	/* Set the fixed ALPHA as 0xFF */
+	MM_REG_WRITE(cmd, base, WDMA_ALPHA, 0x800000FF, 0x800000FF);
+
+	return 0;
+}
+
...
+
+	/* Write Y pixel offset */
+	MM_REG_WRITE(cmd, base, WDMA_DST_ADDR_OFFSET,
+		     subfrm->offset[0], 0x0FFFFFFF);
+	/* Write U pixel offset */
+	MM_REG_WRITE(cmd, base, WDMA_DST_U_ADDR_OFFSET,
+		     subfrm->offset[1], 0x0FFFFFFF);
+	/* Write V pixel offset */
+	MM_REG_WRITE(cmd, base, WDMA_DST_V_ADDR_OFFSET,
+		     subfrm->offset[2], 0x0FFFFFFF);
+	/* Write source size */
+	MM_REG_WRITE(cmd, base, WDMA_SRC_SIZE, subfrm->src, 0x3FFF3FFF);
+	/* Write target size */
+	MM_REG_WRITE(cmd, base, WDMA_CLIP_SIZE, subfrm->clip, 0x3FFF3FFF);
+	/* Write clip offset */
+	MM_REG_WRITE(cmd, base, WDMA_CLIP_COORD, subfrm->clip_ofst, 0x3FFF3FFF);
+
+	/* Enable WDMA */
+	MM_REG_WRITE(cmd, base, WDMA_EN, 0x01, 0x00000001);
+
+	return 0;
+}
+
...
+	/* Disable WDMA */
+	MM_REG_WRITE(cmd, base, WDMA_EN, 0x00, 0x00000001);
+	return 0;
+}
+
...
+
+	/* CCORR enable */
+	MM_REG_WRITE(cmd, base, MDP_CCORR_EN, 0x1, 0x1);
+	/* Relay mode */
+	MM_REG_WRITE(cmd, base, MDP_CCORR_CFG, 0x1, 0x1);
+	return 0;
+}
+
...
+	MM_REG_WRITE(cmd, base, MDP_CCORR_SIZE,
...
+		MM_REG_WRITE(cmd, mmsys, MMSYS_SW0_RST_B,    0x0, 0x00000008);
+		MM_REG_WRITE(cmd, mmsys, MMSYS_SW0_RST_B, 1 << 3, 0x00000008);
+		/* Reset MDP_DL_ASYNC_RX */
+		/* Bit  10: MDP_DL_ASYNC_RX */
+		MM_REG_WRITE(cmd, mmsys, MMSYS_SW1_RST_B,     0x0, 0x00000400);
+		MM_REG_WRITE(cmd, mmsys, MMSYS_SW1_RST_B, 1 << 10, 0x00000400);
+
+		/* Enable sof mode */
+		MM_REG_WRITE(cmd, mmsys, ISP_RELAY_CFG_WD, 0 << 31, 0x80000000);
+	}
+
...
+		MM_REG_WRITE(cmd, mmsys, MMSYS_SW0_RST_B,    0x0, 0x00000010);
+		MM_REG_WRITE(cmd, mmsys, MMSYS_SW0_RST_B, 1 << 4, 0x00000010);
+		/* Reset MDP_DL_ASYNC2_RX */
+		/* Bit  11: MDP_DL_ASYNC2_RX */
+		MM_REG_WRITE(cmd, mmsys, MMSYS_SW1_RST_B,     0x0, 0x00000800);
+		MM_REG_WRITE(cmd, mmsys, MMSYS_SW1_RST_B, 1 << 11, 0x00000800);
+
+		/* Enable sof mode */
+		MM_REG_WRITE(cmd, mmsys, IPU_RELAY_CFG_WD, 0 << 31, 0x80000000);
+	}
+
+	return 0;
+}
...
+	MM_REG_WRITE_MASK(cmd, base, 0x2890, isp->smxi_iova[0], 0xFFFFFFFF);
+	MM_REG_WRITE_MASK(cmd, base, 0x27D0, isp->smxi_iova[0], 0xFFFFFFFF);
+	/* DIP_X_SMX2I_BASE_ADDR, DIP_X_SMX2O_BASE_ADDR */
+	MM_REG_WRITE_MASK(cmd, base, 0x28C0, isp->smxi_iova[1], 0xFFFFFFFF);
+	MM_REG_WRITE_MASK(cmd, base, 0x2800, isp->smxi_iova[1], 0xFFFFFFFF);
+	/* DIP_X_SMX3I_BASE_ADDR, DIP_X_SMX3O_BASE_ADDR */
+	MM_REG_WRITE_MASK(cmd, base, 0x28F0, isp->smxi_iova[2], 0xFFFFFFFF);
+	MM_REG_WRITE_MASK(cmd, base, 0x2830, isp->smxi_iova[2], 0xFFFFFFFF);
+	/* DIP_X_SMX4I_BASE_ADDR, DIP_X_SMX4O_BASE_ADDR */
+	MM_REG_WRITE_MASK(cmd, base, 0x2920, isp->smxi_iova[3], 0xFFFFFFFF);
+	MM_REG_WRITE_MASK(cmd, base, 0x2860, isp->smxi_iova[3], 0xFFFFFFFF);
+
+	switch (isp->cq_idx) {
+	case ISP_DRV_DIP_CQ_THRE0:
+		MM_REG_WRITE_MASK(cmd, base, 0x2208,
+				  isp->cq_iova, 0xFFFFFFFF);
+		break;
+	case ISP_DRV_DIP_CQ_THRE1:
+		MM_REG_WRITE_MASK(cmd, base, 0x2214,
+				  isp->cq_iova, 0xFFFFFFFF);
+		break;
+	case ISP_DRV_DIP_CQ_THRE2:
+		MM_REG_WRITE_MASK(cmd, base, 0x2220,
+				  isp->cq_iova, 0xFFFFFFFF);
+		break;
+	case ISP_DRV_DIP_CQ_THRE3:
+		MM_REG_WRITE_MASK(cmd, base, 0x222C,
+				  isp->cq_iova, 0xFFFFFFFF);
+		break;
+	case ISP_DRV_DIP_CQ_THRE4:
+		MM_REG_WRITE_MASK(cmd, base, 0x2238,
+				  isp->cq_iova, 0xFFFFFFFF);
+		break;
+	case ISP_DRV_DIP_CQ_THRE5:
+		MM_REG_WRITE_MASK(cmd, base, 0x2244,
+				  isp->cq_iova, 0xFFFFFFFF);
+		break;
+	case ISP_DRV_DIP_CQ_THRE6:
+		MM_REG_WRITE_MASK(cmd, base, 0x2250,
+				  isp->cq_iova, 0xFFFFFFFF);
+		break;
+	case ISP_DRV_DIP_CQ_THRE7:
+		MM_REG_WRITE_MASK(cmd, base, 0x225C,
+				  isp->cq_iova, 0xFFFFFFFF);
+		break;
+	case ISP_DRV_DIP_CQ_THRE8:
+		MM_REG_WRITE_MASK(cmd, base, 0x2268,
+				  isp->cq_iova, 0xFFFFFFFF);
+		break;
+	case ISP_DRV_DIP_CQ_THRE9:
+		MM_REG_WRITE_MASK(cmd, base, 0x2274,
+				  isp->cq_iova, 0xFFFFFFFF);
+		break;
+	case ISP_DRV_DIP_CQ_THRE10:
+		MM_REG_WRITE_MASK(cmd, base, 0x2280,
+				  isp->cq_iova, 0xFFFFFFFF);
+		break;
+	case ISP_DRV_DIP_CQ_THRE11:
+		MM_REG_WRITE_MASK(cmd, base, 0x228C,
+				  isp->cq_iova, 0xFFFFFFFF);
+		break;
+	case ISP_DRV_DIP_CQ_THRE12:
+		MM_REG_WRITE_MASK(cmd, base, 0x2298,
+				  isp->cq_iova, 0xFFFFFFFF);
+		break;
+	case ISP_DRV_DIP_CQ_THRE13:
+		MM_REG_WRITE_MASK(cmd, base, 0x22A4,
+				  isp->cq_iova, 0xFFFFFFFF);
+		break;
+	case ISP_DRV_DIP_CQ_THRE14:
+		MM_REG_WRITE_MASK(cmd, base, 0x22B0,
+				  isp->cq_iova, 0xFFFFFFFF);
+		break;
...
+	phys_addr_t base = ctx->comp->reg_base;
+
+	MM_REG_WRITE_MASK(cmd, base, 0x2304,
...
+	if (isp->dl_flags & (1 << MDP_CAMIN))
+		MM_REG_WRITE_MASK(cmd, base, 0x0030, 0x00000000, 0x00000200);
+	/* MDP2_DL_SEL: select MDP_CROP2 */
+	if (isp->dl_flags & (1 << MDP_CAMIN2))
+		MM_REG_WRITE_MASK(cmd, base, 0x0030, 0x00000000, 0x00000C00);
+
+	switch (isp->cq_idx) {
+	case ISP_DRV_DIP_CQ_THRE0:
+		MM_REG_WRITE_MASK(cmd, base, 0x2000, 0x0001, 0x00000001);
+		MM_REG_WAIT(cmd, ISP_P2_0_DONE);
+		break;
+	case ISP_DRV_DIP_CQ_THRE1:
+		MM_REG_WRITE_MASK(cmd, base, 0x2000, 0x0002, 0x00000002);
+		MM_REG_WAIT(cmd, ISP_P2_1_DONE);
+		break;
+	case ISP_DRV_DIP_CQ_THRE2:
+		MM_REG_WRITE_MASK(cmd, base, 0x2000, 0x0004, 0x00000004);
+		MM_REG_WAIT(cmd, ISP_P2_2_DONE);
+		break;
+	case ISP_DRV_DIP_CQ_THRE3:
+		MM_REG_WRITE_MASK(cmd, base, 0x2000, 0x0008, 0x00000008);
+		MM_REG_WAIT(cmd, ISP_P2_3_DONE);
+		break;
+	case ISP_DRV_DIP_CQ_THRE4:
+		MM_REG_WRITE_MASK(cmd, base, 0x2000, 0x0010, 0x00000010);
+		MM_REG_WAIT(cmd, ISP_P2_4_DONE);
+		break;
+	case ISP_DRV_DIP_CQ_THRE5:
+		MM_REG_WRITE_MASK(cmd, base, 0x2000, 0x0020, 0x00000020);
+		MM_REG_WAIT(cmd, ISP_P2_5_DONE);
+		break;
+	case ISP_DRV_DIP_CQ_THRE6:
+		MM_REG_WRITE_MASK(cmd, base, 0x2000, 0x0040, 0x00000040);
+		MM_REG_WAIT(cmd, ISP_P2_6_DONE);
+		break;
+	case ISP_DRV_DIP_CQ_THRE7:
+		MM_REG_WRITE_MASK(cmd, base, 0x2000, 0x0080, 0x00000080);
+		MM_REG_WAIT(cmd, ISP_P2_7_DONE);
+		break;
+	case ISP_DRV_DIP_CQ_THRE8:
+		MM_REG_WRITE_MASK(cmd, base, 0x2000, 0x0100, 0x00000100);
+		MM_REG_WAIT(cmd, ISP_P2_8_DONE);
+		break;
+	case ISP_DRV_DIP_CQ_THRE9:
+		MM_REG_WRITE_MASK(cmd, base, 0x2000, 0x0200, 0x00000200);
+		MM_REG_WAIT(cmd, ISP_P2_9_DONE);
+		break;
+	case ISP_DRV_DIP_CQ_THRE10:
+		MM_REG_WRITE_MASK(cmd, base, 0x2000, 0x0400, 0x00000400);
+		MM_REG_WAIT(cmd, ISP_P2_10_DONE);
+		break;
+	case ISP_DRV_DIP_CQ_THRE11:
+		MM_REG_WRITE_MASK(cmd, base, 0x2000, 0x0800, 0x00000800);
+		MM_REG_WAIT(cmd, ISP_P2_11_DONE);
+		break;
+	case ISP_DRV_DIP_CQ_THRE12:
+		MM_REG_WRITE_MASK(cmd, base, 0x2000, 0x1000, 0x00001000);
+		MM_REG_WAIT(cmd, ISP_P2_12_DONE);
+		break;
+	case ISP_DRV_DIP_CQ_THRE13:
+		MM_REG_WRITE_MASK(cmd, base, 0x2000, 0x2000, 0x00002000);
+		MM_REG_WAIT(cmd, ISP_P2_13_DONE);
+		break;
+	case ISP_DRV_DIP_CQ_THRE14:
+		MM_REG_WRITE_MASK(cmd, base, 0x2000, 0x4000, 0x00004000);
...
+		MM_REG_WRITE(cmd, base, MDP_ASYNC_CFG_WD,
+			     (isp_dl_h << 16) + isp_dl_w, 0x3FFF3FFF);
+#endif
+#ifdef ISP_RELAY_CFG_WD
+		MM_REG_WRITE(cmd, base, ISP_RELAY_CFG_WD,
+			     (isp_dl_h << 16) + isp_dl_w, 0x3FFF3FFF);
+#endif
+	} else {
+#ifdef MDP_ASYNC_IPU_CFG_WD
+		MM_REG_WRITE(cmd, base, MDP_ASYNC_IPU_CFG_WD,
+			     (isp_dl_h << 16) + isp_dl_w, 0x3FFF3FFF);
+#endif
+#ifdef IPU_RELAY_CFG_WD
+		MM_REG_WRITE(cmd, base, IPU_RELAY_CFG_WD,
+			     (isp_dl_h << 16) + isp_dl_w, 0x3FFF3FFF);
+#endif
+	}
+
...
+	mdp->id_count = 0;
+
+	mdp->cmdq_base = cmdq_register_device(dev);
