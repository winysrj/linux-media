...
Received: from mx07-00178001.pphosted.com ([62.209.51.94]:58335 "EHLO
	mx07-00178001.pphosted.com" rhost-flags-OK-OK-OK-OK)
	by vger.kernel.org with ESMTP id S1753103AbbD0P4z (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Mon, 27 Apr 2015 11:56:55 -0400
From: Fabien Dessenne <fabien.dessenne@st.com>
To: <linux-media@vger.kernel.org>
CC: Benjamin Gaignard <benjamin.gaignard@linaro.org>
Subject: [PATCH 2/3] [media] bdisp: 2D blitter driver using v4l2 mem2mem framework
Date: Mon, 27 Apr 2015 17:56:43 +0200
Message-ID: <1430150204-22944-3-git-send-email-fabien.dessenne@st.com>
...
diff --git a/drivers/media/platform/bdisp/Kconfig b/drivers/media/platform/bdisp/Kconfig
new file mode 100644
index 0000000..3f6de4f
--- /dev/null
...
+	 be called bdisp.ko.
diff --git a/drivers/media/platform/bdisp/Makefile b/drivers/media/platform/bdisp/Makefile
new file mode 100644
index 0000000..ae347a4
--- /dev/null
...
diff --git a/drivers/media/platform/bdisp/bdisp-filter.h b/drivers/media/platform/bdisp/bdisp-filter.h
new file mode 100644
...
+};
diff --git a/drivers/media/platform/bdisp/bdisp-hw.c b/drivers/media/platform/bdisp/bdisp-hw.c
new file mode 100644
index 0000000..9c288ac
--- /dev/null
+++ b/drivers/media/platform/bdisp/bdisp-hw.c
@@ -0,0 +1,783 @@
+/*
+ * Copyright (C) STMicroelectronics SA 2014
+ * Authors: Fabien Dessenne <fabien.dessenne@st.com> for STMicroelectronics.
...
+	return 0;
+}
diff --git a/drivers/media/platform/bdisp/bdisp-reg.h b/drivers/media/platform/bdisp/bdisp-reg.h
new file mode 100644
index 0000000..e7e1a42
--- /dev/null
+++ b/drivers/media/platform/bdisp/bdisp-reg.h
@@ -0,0 +1,235 @@
+/*
+ * Copyright (C) STMicroelectronics SA 2014
+ * Authors: Fabien Dessenne <fabien.dessenne@st.com> for STMicroelectronics.
...
diff --git a/drivers/media/platform/bdisp/bdisp-v4l2.c b/drivers/media/platform/bdisp/bdisp-v4l2.c
new file mode 100644
index 0000000..c8ccfd3
--- /dev/null
+++ b/drivers/media/platform/bdisp/bdisp-v4l2.c
@@ -0,0 +1,1477 @@
+/*
+ * Copyright (C) STMicroelectronics SA 2014
+ * Authors: Fabien Dessenne <fabien.dessenne@st.com> for STMicroelectronics.
...
+
+	if (WARN(!ctx || !ctx->m2m_ctx, "Null hardware context\n"))
+		return;
+
+	dev_dbg(ctx->bdisp_dev->dev, "%s\n", __func__);
+
...
+
+		v4l2_m2m_job_finish(ctx->bdisp_dev->m2m.m2m_dev, ctx->m2m_ctx);
+	}
+}
+
...
+	ctx->m2m_ctx = v4l2_m2m_ctx_init(bdisp->m2m.m2m_dev, ctx, queue_init);
...
+}
+
+static unsigned int bdisp_poll(struct file *file,
...
+}
+
+static int bdisp_mmap(struct file *file, struct vm_area_struct *vma)
+{
+	struct bdisp_ctx *ctx = fh_to_ctx(file->private_data);
+	struct bdisp_dev *bdisp = ctx->bdisp_dev;
+	int ret;
...
+}
+
+static int bdisp_cropcap(struct file *file, void *fh,
...
+		dev_dbg(ctx->bdisp_dev->dev, "Unsupported crop type\n");
+		return -EINVAL;
+	}
+
+	frame = ctx_get_frame(ctx, crop->type);
+	if (IS_ERR(frame)) {
+		dev_err(ctx->bdisp_dev->dev, "Invalid frame (%p)\n", frame);
+		return PTR_ERR(frame);
+	}
...
+	crop->bounds.left = 0;
+	crop->bounds.top = 0;
+	crop->bounds.width = frame->width;
+	crop->bounds.height = frame->height;
...
+}
+
+static int bdisp_g_crop(struct file *file, void *fh, struct v4l2_crop *crop)
+{
+	struct bdisp_frame *frame;
+	struct bdisp_ctx *ctx = fh_to_ctx(fh);
+
...
+}
+
+static int bdisp_s_crop(struct file *file, void *fh,
+			const struct v4l2_crop *crop)
+{
+	struct bdisp_frame *frame;
+	struct bdisp_ctx *ctx = fh_to_ctx(fh);
+	struct v4l2_crop out_crop = *crop;
...
+}
+
+static int bdisp_reqbufs(struct file *file, void *fh,
...
+}
+
+static int bdisp_create_bufs(struct file *file, void *fh,
...
+}
+
+static int bdisp_expbuf(struct file *file, void *fh,
...
+}
+
+static int bdisp_querybuf(struct file *file, void *fh, struct v4l2_buffer *buf)
+{
+	struct bdisp_ctx *ctx = fh_to_ctx(fh);
+
...
+}
+
+static int bdisp_qbuf(struct file *file, void *fh, struct v4l2_buffer *buf)
+{
+	struct bdisp_ctx *ctx = fh_to_ctx(fh);
+
...
+}
+
+static int bdisp_dqbuf(struct file *file, void *fh, struct v4l2_buffer *buf)
+{
+	struct bdisp_ctx *ctx = fh_to_ctx(fh);
+
...
+}
+
+static int bdisp_streamoff(struct file *file, void *fh, enum v4l2_buf_type type)
+{
+	struct bdisp_ctx *ctx = fh_to_ctx(fh);
+
...
+	.vidioc_cropcap                 = bdisp_cropcap,
+	.vidioc_g_crop                  = bdisp_g_crop,
+	.vidioc_s_crop                  = bdisp_s_crop,
+	.vidioc_reqbufs                 = bdisp_reqbufs,
+	.vidioc_create_bufs             = bdisp_create_bufs,
+	.vidioc_expbuf                  = bdisp_expbuf,
+	.vidioc_querybuf                = bdisp_querybuf,
+	.vidioc_qbuf                    = bdisp_qbuf,
+	.vidioc_dqbuf                   = bdisp_dqbuf,
+	.vidioc_streamon                = bdisp_streamon,
+	.vidioc_streamoff               = bdisp_streamoff,
...
+err_m2m:
+	video_device_release(bdisp->m2m.vdev);
+
+	return ret;
+}
+
...
+MODULE_LICENSE("GPL");
diff --git a/drivers/media/platform/bdisp/bdisp.h b/drivers/media/platform/bdisp/bdisp.h
new file mode 100644
index 0000000..291f79c
--- /dev/null
+++ b/drivers/media/platform/bdisp/bdisp.h
@@ -0,0 +1,190 @@
+/*
+ * Copyright (C) STMicroelectronics SA 2014
+ * Authors: Fabien Dessenne <fabien.dessenne@st.com> for STMicroelectronics.
