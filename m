...
Received: from mail-wm0-f65.google.com ([74.125.82.65]:33826 "EHLO
        ...
        with ESMTP id S1757386AbcHWIHf (ORCPT
        <rfc822;linux-media@vger.kernel.org>);
        Tue, 23 Aug 2016 04:07:35 -0400
Received: by mail-wm0-f65.google.com with SMTP id q128so16918459wma.1
        for <linux-media@vger.kernel.org>; Tue, 23 Aug 2016 01:07:34 -0700 (PDT)
From: Johan Fjeldtvedt <jaffe1@gmail.com>
To: linux-media@vger.kernel.org
Cc: Johan Fjeldtvedt <jaffe1@gmail.com>
Subject: [PATCH] cec-compliance: improve One Touch Play tests wrt. CTS 1.4b
Date: Tue, 23 Aug 2016 10:07:04 +0200
Message-Id: <1471939624-22360-1-git-send-email-jaffe1@gmail.com>
...
+			return false;
 	...
 	return true;
@@ -116,45 +117,51 @@ const unsigned power_status_subtests_size = ARRAY_SIZE(power_status_subtests);
...
-	node->remote[la].has_text_view_on = true;
 	...
 	if (interactive)
 		return 0;
@@ -162,21 +169,32 @@ static int one_touch_play_text_view_on(struct node *node, unsigned me, unsigned
 		return PRESUMED_OK;
 }
...
+		return ret;
 	...
 	fail_on_test(!transmit_timeout(node, &msg));
@@ -188,6 +206,20 @@ static int one_touch_play_active_source(struct node *node, unsigned me, unsigned
 		return PRESUMED_OK;
 }
...
+
 ...
 {
 	struct cec_msg msg = {};
@@ -205,7 +237,10 @@ static int one_touch_play_req_active_source(struct node *node, unsigned me, unsi
