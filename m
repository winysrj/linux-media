...
Received: from userp1040.oracle.com ([156.151.31.81]:27905 "EHLO
	userp1040.oracle.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1753939AbcCOOIV (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Tue, 15 Mar 2016 10:08:21 -0400
Date: Tue, 15 Mar 2016 17:07:15 +0300
...
Message-ID: <20160315140715.GB30221@mwanda>
...
Why is this in staging?  I spotted a few error handling bugs, and some
style issues but I don't see any reason why this couldn't be merge into
the kernel proper.
...
make it unsigned?  What does "i_" stand for?  The name should have
something to do with bits.

Update:  Aurgh!!!  Gar!  It stands for int.  No no no.  Remove all of
these.
...
I would have expected this to be related to size.
...
It probably looks nicer like this:

		*s-> = (*s->p << s->i_left) |
		       (i_bits >> (i_count - s->i_left));
...
These bogus initializations just turn off GCC's uninitialized variable
checking.  It's a good feature.  Don't fight against it.
...
Too many parenthesis.  You're thinking of LISP.
...
I don't understand what this limit is.  Just say / 4 instead of
shifting.  We used to have to do divides this way back in the 80s but
these days the compiler is smart.
...
This returns very specific error codes, but it's not documented what
they mean and no one checks.
...
This test will not work because we end with retries set to -1.  Change
it to --retries.
...
I've sort of been assuming that "remove all these initializers" is
implied.
...
OR operations goes on the first line.
...
Better to preserve the error code.

	err = pci_enable_device(pci_dv);
	if (err)
		goto pci_enable_fail;
...
devm_ means you don't have to do this free manually.  Leave it out.
...
We are copying more than 4 bytes here.  It should be 4 + 1 + nal_len
or something.
...
Btw, if you were True Enterprise then you would have to write everything
in three line stanzas.

#define MY_F_MASK (0xf)
#define MY_F_SHIFT (12)
#define MY_MASK_SHIFT (MY_F_MASK << MY_F_SHIFT)

That's how the infiniband people do it.
...
Ugh....  Just ignore checkpatch warnings here and go over the 80
character limit.
...
-1 is a magic number that stands for -EPERM.  Just use -EPERM directly.
...
It's normally really easy and mindless to write bug free error handling.

1) Unwind in the reverse order.
2) Choose a good label name that reflects what the goto does.  The
   labels names here are *all* based on the goto location which is a
   common mistake/anti-pattern:

	if (input_init_failed)
		goto input_init_fail;

   This is useless.  It looks like it provides useful information but
   actually you can already see from the line before that
   input_init_failed so it doesn't add any new information.  What you
   want to know as a reviewer is "Does this goto free the most recently
   allocated thing?"  So good names are like "goto free_dma;" and
   "goto inputs_fini;"  I've actually named functions called_from_frob()
   so I'm guilty of this same sort of bad naming based on the call site
   when it's hard to think of an a good name...  :P
3) Don't free things that haven't been allocated.  Since the goto name
   is the most recently allocated thing, this is straight forward.
   Where you normally run into trouble with this rule is if you have a
   "goto fail;" which tries to free everything.  Do everything style is
   the most bug prone way to do error handling.
...
This is the other reason that this naming style doesn't work for labels.
It doesn't make sense when there are two goto locations.  In the best
case, it adds no information but it's also common that it adds
misleading information like this.  v4l2_ctrl_fail failed earlier.
...
If we failed to allocate the ctrl then do we need to free this?

That stuff is weird and there is no documentation for the
v4l2_ctrl_new_custom() function...
...
Can we just remove this debug code?  Another option would be to write
two versions of this function, one normal one and one to print the debug
output.  The debug output makes it work different and makes it look
...
static int tw5864_is_motion_triggered(struct tw5864_h264_frame *frame)
{
	...
	for (i = 0; i < md_cells_hor * md_cells_vert; i++) {
		u16 thresh = input->md_threshold_grid_values[i];
		u16 metric = tw5864_md_metric_from_mvd(mv[i]);
...
These fit in 80 characters already.
