...
Received: from smtp-vbr15.xs4all.nl ([194.109.24.35]:3431 "EHLO
	smtp-vbr15.xs4all.nl" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1757043AbZHGIJB (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Fri, 7 Aug 2009 04:09:01 -0400
From: Hans Verkuil <hverkuil@xs4all.nl>
To: m-karicheri2@ti.com
Subject: Re: [PATCH v0 4/5] V4L: vpif_capture driver for DM6467
Date: Fri, 7 Aug 2009 10:08:50 +0200
Cc: linux-media@vger.kernel.org,
	davinci-linux-open-source@linux.davincidsp.com
References: <1249599957-21797-1-git-send-email-m-karicheri2@ti.com>
In-Reply-To: <1249599957-21797-1-git-send-email-m-karicheri2@ti.com>
MIME-Version: 1.0
Content-Type: text/plain;
  charset="iso-8859-15"
Content-Transfer-Encoding: 7bit
Content-Disposition: inline
Message-Id: <200908071008.51699.hverkuil@xs4all.nl>
Sender: linux-media-owner@vger.kernel.org
List-ID: <linux-media.vger.kernel.org>

Hi Muralidharan,

First of all I've reviewed the other patches in this series and they look OK
...
For this patch I have several comments, though.

On Friday 07 August 2009 01:05:57 m-karicheri2@ti.com wrote:
...
> ---
>  drivers/media/video/davinci/vpif_capture.c | 2189 ++++++++++++++++++++++++++++
...
> +{
> +	enum v4l2_field field = V4L2_FIELD_INTERLACED;
> +
> +	if (iface->if_type == VPIF_IF_RAW_BAYER)
...
This can be written as 'else if (2 == muxmode)'.
That saves this extra level of indentation.
...
Why all these superfluous parenthesis? That happens a lot in the source, BTW.
...
> +		vpif_dbg(1, debug, "other channel is using\n");

'is using'? Is using what?
...
I strongly recommend that instead of calling the subdev for the current input
indirectly using v4l2_device_call_until_err() you use the v4l2_subdev pointer
associated with the current input directly. So typically when the input is
changed you update a ch->curr_sd pointer to the associated struct v4l2_subdev.

After that you can just call v4l2_subdev_call(ch->sd, video, s_stream, 1).
Much more concise.

In addition the v4l2_device_call_until_err() macro will replace the ENOIOCTLCMD
error by 0. The idea behind this macro is that you have an unknown number of
subdevs (>= 0) that might be interested in this command, but if no one is, then
...
> +
> +	if (ret < 0) {
> +		vpif_dbg(1, debug, "cann't set vpif channel mode\n");

Typo: cann't -> "can't" or "cannot".
...
> +
> +	if (ret < 0) {
> +		vpif_dbg(1, debug, "cann't set video params\n");
...
vpif_g_std -> vpif_s_std

> +
> +	if (!(*std_id & ch->video_dev->tvnorms))
> +		return -EINVAL;

Already done in v4l2-ioctl.c
...
> +						   &sd_index);
> +	if (NULL == subdev_info) {
> +		vpif_dbg(1, debug, "couldn't lookup sub device for the"
> +			 "input index\n");
...
That way the string doesn't need to be broken up.
...
These fields are already cleared by v4l2-ioctl.c
...
never liked that v4l2-ioctl has default handling for g_std. It's a dangerous
...
> + *
> + * Allocate memory for data structures and intialize them
> + */
...
That's it from me. Nothing big as you can see.
