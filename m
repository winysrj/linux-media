...
Received: from shell.v3.sk ([90.176.6.54]:49570 "EHLO shell.v3.sk"
        ...
        id S1729512AbeKEQuB (ORCPT <rfc822;linux-media@vger.kernel.org>);
        Mon, 5 Nov 2018 11:50:01 -0500
...
Subject: [PATCH 10/11] [media] marvell-ccic: use async notifier to get the sensor
Date: Mon,  5 Nov 2018 08:30:53 +0100
Message-Id: <20181105073054.24407-11-lkundrak@v3.sk>
...
index ff1feca7bc9b..cafb55f2b057 100644
...
-#include <linux/i2c.h>
 ...
 #include <linux/slab.h>
@@ -317,6 +317,7 @@ static int mmpcam_probe(struct platform_device *pdev)
...
+	struct fwnode_handle *ep;
 	...
 	int ret;
=20
@@ -331,7 +332,6 @@ static int mmpcam_probe(struct platform_device *pdev)
...
@@ -375,15 +375,6 @@ static int mmpcam_probe(struct platform_device *pdev=
)
...
-	}
 	 ...
 	 * Sensor GPIO pins.
 	 */
@@ -406,6 +397,19 @@ static int mmpcam_probe(struct platform_device *pdev=
)
=20
 	mcam_init_clk(mcam);
=20
...
+
 	 ...
 	 */
@@ -415,6 +419,7 @@ static int mmpcam_probe(struct platform_device *pdev)
 	ret =3D mccic_register(mcam);
 	if (ret)
 		goto out_power_down;
+
