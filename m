...
Received: from mga05.intel.com ([192.55.52.43]:31411 "EHLO mga05.intel.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1751332AbeBWHuG (ORCPT <rfc822;linux-media@vger.kernel.org>);
        Fri, 23 Feb 2018 02:50:06 -0500
Date: Fri, 23 Feb 2018 09:50:01 +0200
...
Message-ID: <20180223075001.hboz4urcxfzbpnwv@paasikivi.fi.intel.com>
References: <20180222102338.28896-1-rui.silva@linaro.org>
 <20180222102338.28896-3-rui.silva@linaro.org>
MIME-Version: 1.0
Content-Type: text/plain; charset=us-ascii
Content-Disposition: inline
In-Reply-To: <20180222102338.28896-3-rui.silva@linaro.org>
Sender: linux-media-owner@vger.kernel.org
List-ID: <linux-media.vger.kernel.org>

Hi Rui,

On Thu, Feb 22, 2018 at 10:23:38AM +0000, Rui Miguel Silva wrote:
...
As the frame rate is specific to a mode rather than the other way around, I
think you should probably swap the array dimensions. You could even remove
this as frame rate selection clearly isn't implemented in the driver.
...
You shouldn't implement accessing 16-bit registers like this neither for
reading or writing unless there's a specific reason to do so. Just access
...
Even if you're using GPIO only right now, the power up and power down
...
Please merge the two above functions to ov2670_enable. That might be better
called e.g. ov2670_power_on. Likewise ov2670_disable could be
ov2670_power_off. These functions are implementing power on and power off
sequences after all.
...
Could you use ov2680_gain_set() from the caller instead? Perhaps rename it
...
I'd suppose you'll get interesting effects if the exposure latching point
happens to be in between these updates.

A single read would avoid that. Or is there a particular reason to perform
...
ov2680_mode_set_direct() is only used here. How about moving the contents
here?
...
You only support 30 frames per second. Even then, you need to modify the
request to match with what the driver supports, so the implementation can
...
The closest match would be better. Can you use this? I'm planning to send a
...
v4l2_ctrl_new_std() may return NULL. Please check gain and exposure are
non-NULL before dereferencing them. The error handling seems fine
otherwise.
...
I don't have the sensor datasheet, but practically everywhere you'll need
execute the power up sequence before accessing the sensor registers.
...
I believe the register lists are intended for a particular frequency and do
