...
Received: from mailout4.w1.samsung.com ([210.118.77.14]:42221 "EHLO
	mailout4.w1.samsung.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1755204Ab0EGJgl (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Fri, 7 May 2010 05:36:41 -0400
MIME-version: 1.0
Content-transfer-encoding: 7BIT
Content-type: text/plain; charset=us-ascii
Date: Fri, 07 May 2010 11:36:09 +0200
From: Sylwester Nawrocki <s.nawrocki@samsung.com>
Subject: RE: [PATCH v1 3/3] ARM: S5P: Add Camera interface (video
 postprocessor) driver
In-reply-to: <20100506062555.GP26401@trinity.fluff.org>
...
Reply-to: Sylwester Nawrocki <s.nawrocki@samsung.com>
Message-id: <000001caedc8$b997b5d0$2cc72170$%nawrocki@samsung.com>
Content-language: en-us
References: <1271673000-3020-1-git-send-email-s.nawrocki@samsung.com>
 <1271673000-3020-4-git-send-email-s.nawrocki@samsung.com>
 <20100506062555.GP26401@trinity.fluff.org>
...
> soc-owner@vger.kernel.org] On Behalf Of Ben Dooks
> Sent: Thursday, May 06, 2010 8:26 AM
> To: Sylwester Nawrocki
> Cc: linux-media@vger.kernel.org; linux-samsung-soc@vger.kernel.org;
> linux-arm-kernel@lists.infradead.org; p.osciak@samsung.com;
...
yes, s3c64xx are equipped with similar device, just an older revision,
so with a little work the driver could be adapted to s3c64xx too. 
I'm going to rename the files to something less single SOC specific then.
...
> > +	tristate "Samsung S5P Camera Interface (video postprocessor)
> driver"
...
> > +++ b/drivers/media/video/Makefile
> > @@ -159,6 +159,7 @@ obj-$(CONFIG_VIDEO_MX1)			+=
> mx1_camera.o
...
> >
> > diff --git a/drivers/media/video/samsung-fimc/Makefile
...
> > diff --git a/drivers/media/video/samsung-fimc/s5p_fimc.c
...
> > + * by the Free Software Foundiation. either version 2 of the
> License,
...
indeed, I have reviewed this include list and will
remove also few other ones which are irrelevant
...
I wasn't sure about that, will do
...
> > +}
> > +
> > +static int s5p_fimc_get_scaler_factor(u32 src, u32 tar, u32 *ratio,
> u32 *shift)
> > +{
> > +	if (src >= tar * 64) {
> > +		return -EINVAL;
...
all right, I don't mind changing that the other way around
...
> > +
> > +	if (sx <= 0 || sy <= 0) {
> > +		err("invalid source size: s: %d %d, t: %d %d", sx, sy, tx,
> ty);
> > +		return -EINVAL;
> > +	}
> > +
...
> > +	dbg("sc->hfactor= %d, sc->vfactor= %d", sc->hfactor, sc-
> >vfactor);
> > +
...
will change that, just wanted to emphasize we are dealing with a pointer
...
> > +			dst_frame->fmt->color,
> > +			dst_frame->dma_offset.y_h, dst_frame-
> >dma_offset.y_v);
> > +
...
> ||
> > +			src_frame->width > SCALER_MAX_HRATIO*dst_frame-
> >width) {
...
> > +}
> > +
> > +/* the color format (planes_cnt, buff_cnt) must be already
> configured */
> > +static int s5p_fimc_prepare_addr(struct fimc_ctx *ctx,
> > +		struct fimc_vid_buffer *buf, enum v4l2_buf_type type)
> > +{
...
> > +}
> > +
> > +static int s5p_fimc_buf_setup(struct videobuf_queue *vq, unsigned
> int *count,
> > +				unsigned int *size)
> > +{
...
> > +			v4l2_err(&ctx->fimc_dev->v4l2_dev,
> > +				"User-provided buffer too small (%d <
%d)\n",
> > +				 vb->bsize, frame->size);
> > +			WARN_ON(1);
> > +			return -EINVAL;
...
> > +}
> > +
> > +static int s5p_fimc_g_fmt(struct file *file, void *priv, struct
> v4l2_format *f)
> > +{
> > +	struct fimc_ctx *ctx = priv;
> > +	struct fimc_frame *frame;
...
> > +	fmt = find_format(f);
> > +	if (!fmt) {
> > +		v4l2_err(&dev->v4l2_dev, "Fourcc format (0x%08x)
> invalid.\n",
> > +			 pix->pixelformat);
> > +		return -EINVAL;
> > +	}
...
> > +}
> > +
> > +static int s5p_fimc_s_fmt(struct file *file, void *priv, struct
> v4l2_format *f)
> > +{
> > +	struct fimc_frame *frame;
...
> > +	}
> > +
> > +	dbg("depth=%d, bytesperline=%d", frame->fmt->depth, pix-
> >bytesperline);
> > +
...
> > +	dbg("f_width= %d, f_height= %d", frame->f_width, frame-
> >f_height);
> > +
> > +s_fmt_out:
...
> > +}
> > +
> > +static int s5p_fimc_g_crop(struct file *file, void *fh, struct
> v4l2_crop *cr)
> > +{
> > +	struct fimc_frame *frame;
> > +	struct fimc_ctx *ctx = file->private_data;
...
> > +}
> > +
> > +static int s5p_fimc_s_crop(struct file *file, void *fh, struct
> v4l2_crop *cr)
> > +{
> > +	struct fimc_frame *f;
> > +	struct fimc_ctx *ctx = file->private_data;
...
> > +	cr->c.left = ((cr->c.left + PIX_ALIGN_MASK - 1) &
> ~PIX_ALIGN_MASK);
...
> > +	struct fimc_dev *dev = ctx->fimc_dev;
> > +
> > +	videobuf_queue_dma_contig_init(vq, &s5p_fimc_qops, dev-
> >v4l2_dev.dev,
...
> > +
> > +
> > +static int s5p_fimc_mmap(struct file *file, struct vm_area_struct
> *vma)
> > +{
> > +	struct fimc_ctx *ctx = file->private_data;
...
> > +	}
> > +
> > +	dev->regs_res = request_mem_region(res->start,
> resource_size(res),
> > +			dev_name(&pdev->dev));
> > +	if (!dev->regs_res) {
...
> > +	if (IS_ERR(srclk)) {
> > +		dev_err(&pdev->dev, "failed to get source clock of
> fimc\n");
> > +		goto err_regs_unmap;
> > +	}
> 
...
Thanks for pointing this out, that's my oversight.
I consider moving all this clock selection code, except clk_enable/disable
to the platform's machine_init(). Would it the right direction?
...
> > +
> > +	snprintf(vfd->name, sizeof(vfd->name), "%s%d", MODULE_NAME, dev-
> >id);
> > +
...
> > +		v4l2_err(&dev->v4l2_dev, "Failed to init mem2mem
> device\n");
...
agreed, I will clean this up, until I really get the pm ops implemented
...
> > +	"S5PC Camera Interface V4L2 Driver, (c) 2010 Samsung
> Electronics\n";
> > +
...
> > +MODULE_LICENSE("GPL");
> > +
> > diff --git a/drivers/media/video/samsung-fimc/s5p_fimc.h
...
> > +};
> > +
> > +/* Y/Cb/Cr components order at DMA output for 1 plane YCbCr 4:2:2
> formats */
...
> > +
> > +/* Input Y/Cb/Cr components order for 1 plane YCbCr 4:2:2 color
> formats */
...
> > +
> > +/* Cb/Cr chrominance components order for 2 plane Y/CbCr 4:2:2
> formats */
...
> > +/* YCbCr data dynamic range selection for the RGB <-> YCBCR color
> space
...
> > +/**
> > + * struct fimc_scaler
> > + * @enabled:		flag indicating whether the scaler is used
or
> not
...
> > + * @copy_mode:		flag indicating one to one mode, i.e. no
> scaling
...
> > + * @updated:		flags variable to keep track of user
> configurable
> > + *			parameters (ioctl)
...
> > +
> > +#endif /* S5P_FIMC_H_ */
> > diff --git a/drivers/media/video/samsung-fimc/s5p_fimc_reg.c
...
> > @@ -0,0 +1,559 @@
> > +/*
> > + * Register interface file for Samsung Camera Interface (FIMC)
> driver
> > + *
> > + * Copyright (c) 2010 Samsung Electronics
> > + *
...
> > +
> > +	if ((S5P_CISTATUS_OVFIY | S5P_CISTATUS_OVFICB |
> S5P_CISTATUS_OVFICR)
> > +		& status) {
...
> > +		cfg =
> (S5P_CISCCTRL_CSCR2Y_NARROW|S5P_CISCCTRL_CSCY2R_NARROW);
> > +	else
...
> > +	struct fimc_dev *dev = ctx->fimc_dev;
> > +	int i = 0;
> > +	/* each of the four output register sets points to the same
> buffer */
...
I am going to update the driver to the last version of v4l2-mem2mem
framework,
rebase it and will post again.
