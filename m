...
Received: from perceval.ideasonboard.com ([95.142.166.194]:33837 "EHLO
	perceval.ideasonboard.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1751867Ab1DUKSF (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Thu, 21 Apr 2011 06:18:05 -0400
From: Laurent Pinchart <laurent.pinchart@ideasonboard.com>
To: "Hadli, Manjunath" <manjunath.hadli@ti.com>
Subject: Re: [PATCH v16 01/13] davinci vpbe: V4L2 display driver for DM644X SoC
Date: Thu, 21 Apr 2011 12:18:05 +0200
...
References: <B85A65D85D7EB246BE421B3FB0FBB593024BCEF72E@dbde02.ent.ti.com>
In-Reply-To: <B85A65D85D7EB246BE421B3FB0FBB593024BCEF72E@dbde02.ent.ti.com>
MIME-Version: 1.0
Content-Type: Text/Plain;
  charset="iso-8859-1"
Content-Transfer-Encoding: 7bit
Message-Id: <201104211218.06340.laurent.pinchart@ideasonboard.com>
...
> for. I will send the fixed and updated patches as a follow-up after your
> clarifications.
...
> availability of a minimum numbers of buffers for the system which may not
> be available otherwise due to fragmentation, these are used.
...
Otherwise the buffer timestamp will vary when the system clock is modified (as 
a result of an NTP time update for instance).
...
> 
> I would like to get to videobuf2 as a next set of changes. I want to get
> the Dm6446 driver fisrt, add it with Dm365 and do the videobuf2 later.  I
> hope it is okay.

We're trying to get rid of videobuf1, so accepting new drivers that make use 
of videobuf1 is a bit problematic. Have you had a look at videobuf2 ? How much 
time do you think it would take to convert this driver to videobuf2 ? Let's 
first address all the other issues, and then tackle that one.
...
> > > +           /*
> > > +           * If the display is non-interlaced, then we need to flag
...
> 
> Not sure what you mean here. We get the FIRST or second field event from
> the hardware so we need to check the register value rather than
> pix_fmt.field.

Interlacing is configured by userspace. When configured in interlaced mode, I 
expect the device to alternate fields. When configured in progressive mode, I 
expect it to always return the same field. If that's correct, the FIDST bit is 
only needed to identify the active field when configured in interlaced mode. 
pix_fmt.field can be use to check whether we're in interlaced or progressive 
mode.
...
> 
> I am Ok either ways. When debugging is enabled, it is just one of the data
> points since there are multiple windows.

You could leave it, but a debug call in the querycap handler doesn't look very 
useful to me.
...
> > directly.
> 
> Wel,. In the past patches we had a large amount of multiple indirections,
> and as part of a suggestion on open source I removed all of them with a
> general rule of not having more than 2 indirections. What is your take on
> this? How many indirection levels do you allow?

There's no hard rule for that. I usually use intermediate pointers when the 
number of indirection levels grow too high, except when I only need to perform 
the indirection once or twice, like in this case.
...
> > drivers. It might not be a requirement though, but I find it to make code
> > more readable.
> 
...
> 
> Not sure about this. The entire driver is under V4L2 lock per layer handle
> and it would not allow another call to come here once in.
...
> > 
> > If the requested format isn't NV12, cfg->pixfmt won't be modified. If it
> > has been set to NV12 by a previous S_FMT call, it won't become YUYV. Is
> > that intentional ?
> 
...
> > > +static int vpbe_display_cfg_layer_default(enum vpbe_display_device_id
...
It's how V4L2 drivers should behave. Some drivers might not follow that rule, 
...
> > before registering the V4L2 devices, otherwise userspace will be able to
> > open devices before you're done with the initialization.
> 
> I do not think anyone would attempt to open the device so soon as the boot
> process is not complete yet.

Unless you compile the driver as a module. In that case an application could 
open the device as soon as it gets registered. hal is known to do that.

> Also, if the irq is registered before, the interrupts start and the driver
> crashes for lack of initialized structure variables.
...
> 
> This is a common IP for DM644x, Dm355 and Dm365 for which the patches will
> follow after this set. So I think it is OK.
