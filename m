...
Received: from smtp.nokia.com ([192.100.122.230]:33103 "EHLO
	mgw-mx03.nokia.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1757920AbZFROEC (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Thu, 18 Jun 2009 10:04:02 -0400
...
Subject: [PATCHv8  7/9] FMTx: si4713: Add files to handle si4713 i2c device
Date: Thu, 18 Jun 2009 16:55:49 +0300
Message-Id: <1245333351-28157-8-git-send-email-eduardo.valentin@nokia.com>
In-Reply-To: <1245333351-28157-7-git-send-email-eduardo.valentin@nokia.com>
...
+static int si4713_g_modulator(struct v4l2_subdev *sd, struct v4l2_modulator *);
+/*
...
+	rval |= si4713_write_econtrol_integers(sdev, &ctrl);
+
+	ctrl.id = V4L2_CID_RDS_TX_ENABLED;
+	ctrl.value = tmp->rds_info.enabled;
+	rval |= si4713_write_econtrol_integers(sdev, &ctrl);
+
...
+}
+
+/* si4713_ioctl - deal with private ioctls (only rssi for now) */
+long si4713_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
+{
+	struct si4713_device *sdev = to_si4713_device(sd);
+	struct si4713_rssi *rssi = arg;
+	u16 frequency;
+	int rval = 0;
+
+	if (!arg)
...
+	mutex_lock(&sdev->mutex);
+	switch (cmd) {
+	case SI4713_IO_MEASURE_RSSI:
+		frequency = v4l2_to_si4713(rssi->frequency);
+
+		if (sdev->power_state) {
...
+			goto unlock;
+
+		sdev->stereo = get_stereo_status(sdev->stereo);
+	}
+
+	if (sdev->stereo)
+		vm->txsubchans = V4L2_TUNER_SUB_STEREO;
