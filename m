...
Received: from mx1.redhat.com ([209.132.183.28]:25966 "EHLO mx1.redhat.com"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
	id S965104Ab3DPR0Z (ORCPT <rfc822;linux-media@vger.kernel.org>);
	Tue, 16 Apr 2013 13:26:25 -0400
Message-ID: <516D89B3.5090103@redhat.com>
Date: Tue, 16 Apr 2013 14:26:11 -0300
From: Mauro Carvalho Chehab <mchehab@redhat.com>
MIME-Version: 1.0
To: Antti Palosaari <crope@iki.fi>
CC: LMML <linux-media@vger.kernel.org>
Subject: Re: [RFC v2013-04-11] SDR API REQUIREMENT SPECIFICATION
References: <5166CF3A.5040603@iki.fi> <20130416050928.6cf5a875@redhat.com> <516D61FE.9060601@iki.fi>
In-Reply-To: <516D61FE.9060601@iki.fi>
...
>
> I tried to study and list all requirements, without a even knowing very well what is
> possible using V4L2 API, as you has gone now totally to the implementation side.
>  Surely most those implementations you mentioned are best to implement as you said.
...
>> field there for it.
>
> I am not sure about that too, but I have feeling that could be correct. Needs confirming (not algorithm, but use in SDR ADC/DAC).
>
>
...
> Smaller numbers transferred? Is there is some very common user scenario application likes to see drift rather than actual frequency?
...
> Returning drift extends possible numbers to negative too. Also app needs to do some trivial calculation in order to get real value.

Yes.

> Basically HAM radio style application will read that value very often when it updates freq seen in UI.
>
> Also, returning it as a drift means app needs to know what is frequency set, which it surely usually knows, but not always (makes dependency to value set & permission to set value first).
...
>
> 268GHz is enough and 1Hz step is not needed that high frequencies. 62.5 Hz is not practical for smaller frequencies, lets say when you go to the ~500 kHz or under.
>Dunno what are target radio channel practical requirements for transmitting data very low frequencies but 63 Hz off-by from target channel sounds too much.
...
>
> Could you study requirements here? Some examples from existing radios could be nice.

See the cx2388x datasheet for an oversampling example. Those chips have 2 10
...
Most analog video A/D do the same trick, in order to reduce the quantization
noise and reduce image artifacts.

Btw, it could be possible to use the cx88 driver for SDR too. It is a matter
...
>
> At the fist I would like to drop it if possible ever and try to use driver
>specific logic to determine suitable values from target RF, filters and gains.
>Maybe better to add it only if there appears some case what we could not resolve without.

That doesn't seem to be possible. While most tuners use the central frequency as
the reference, there are others that use the initial frequency. For example, xc3028,
xc4000, xc5000 and r820t uses the initial frequency as its tuning frequency
(I think that there are more cases like that at the tree). So, in order to get
the IF, you need to know the standard on those tuners, in order to calculate
the frequency offset between the initial frequency and the "center" frequency[1].
...
So, the better seems to simply pass the type of the RF standard, for the tuner
to adjust for it.
...
>demodulator (SW layer). Could you point any SDR HW which detects pilot carrier? Isn't that demodulator...
