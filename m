...
Received: from mx3.redhat.com (mx3.redhat.com [172.16.48.32])
	by int-mx1.corp.redhat.com (8.13.1/8.13.1) with ESMTP id m6ELrWFB030892
	for <video4linux-list@redhat.com>; Mon, 14 Jul 2008 17:53:32 -0400
Received: from smtp-vbr15.xs4all.nl (smtp-vbr15.xs4all.nl [194.109.24.35])
	by mx3.redhat.com (8.13.8/8.13.8) with ESMTP id m6ELqsvR026763
	for <video4linux-list@redhat.com>; Mon, 14 Jul 2008 17:52:55 -0400
From: Hans Verkuil <hverkuil@xs4all.nl>
To: Adrian Bunk <bunk@kernel.org>
Date: Mon, 14 Jul 2008 23:51:13 +0200
References: <200807081750.39305.hverkuil@xs4all.nl>
	<1215634133.8970.379.camel@pasglop>
	<20080714204550.GE31553@cs181140183.pp.htv.fi>
In-Reply-To: <20080714204550.GE31553@cs181140183.pp.htv.fi>
MIME-Version: 1.0
Content-Type: text/plain;
  charset="utf-8"
Content-Transfer-Encoding: 7bit
Content-Disposition: inline
Message-Id: <200807142351.13673.hverkuil@xs4all.nl>
Cc: Benjamin Herrenschmidt <benh@kernel.crashing.org>,
	v4l <video4linux-list@redhat.com>, mlan@cpu.lu,
	Mauro Carvalho Chehab <mchehab@infradead.org>
Subject: Re: [2.6 patch] remove the broken planb driver
...
On Monday 14 July 2008 22:45:50 Adrian Bunk wrote:
> On Thu, Jul 10, 2008 at 06:08:53AM +1000, Benjamin Herrenschmidt 
wrote:
> > On Wed, 2008-07-09 at 13:00 +0300, Adrian Bunk wrote:
> > > On Tue, Jul 08, 2008 at 05:50:39PM +0200, Hans Verkuil wrote:
> > > > Hi all,
> > > >
> > > > The PlanB driver ("PlanB Video-In on PowerMac") has been marked
> > > > as broken for ages. No one seems to care about it. I propose to
> > > > mark this driver for removal in 2.6.28 as well.
> > >
> > > It's broken for ages, and if there's agreement about the removal
> > > there's no need to wait for 2.6.28.
> > >
> > > > Comments?
> > >
> > > When I proposed removing removing it last year Benjamin
> > > Herrenschmidt (Cc'ed) said he'll have a look. [1]
> > >
> > > Benjamin?
> >
...
I agree, there is nothing like that warm fuzzy feeling you get when your 
linecount for the day is negative :-)

> > Oh well, I suppose you can. I never found time to look into it,
> > but I still -do- have a machine somewhere with that HW (though
> > whether it still boots is to be verified).
> >
> > Remove it for now, It's broken anyway. I can always re-submit it if
> > I ever fix it.
>
> Thanks, patch below.
>
...
> b/drivers/media/video/Kconfig index 5ccb0ae..8e362e4 100644
...
> b/drivers/media/video/Makefile index ecbbfaa..6805468 100644
> --- a/drivers/media/video/Makefile
> +++ b/drivers/media/video/Makefile
> @@ -55,7 +55,6 @@ obj-$(CONFIG_VIDEO_ZORAN_DC30) += zr36050.o
> zr36016.o obj-$(CONFIG_VIDEO_ZORAN_ZR36060) += zr36060.o
>
...
> diff --git a/drivers/media/video/planb.c
> b/drivers/media/video/planb.c deleted file mode 100644
...
> -
> -    Based largely on the bttv driver by Ralph Metzler
> (rjkm@thp.uni-koeln.de) -
> -    Additional debugging and coding by Takashi Oe
> (toe@unlserve.unl.edu) -
...
> -MODULE_PARM_DESC(def_norm, "Default startup norm (0=PAL, 1=NTSC,
> 2=SECAM)"); -module_param(video_nr, int, 0);
> -MODULE_LICENSE("GPL");
> -
> -
> -/* ------------------ PlanB Exported Functions ------------------ */
> -static long planb_write(struct video_device *, const char *,
> unsigned long, int); -static long planb_read(struct video_device *,
> char *, unsigned long, int); -static int planb_open(struct
> video_device *, int);
> -static void planb_close(struct video_device *);
> -static int planb_ioctl(struct video_device *, unsigned int, void *);
> -static int planb_init_done(struct video_device *);
> -static int planb_mmap(struct video_device *, const char *, unsigned
> long); -static void release_planb(void);
...
> -static inline void tab_cmd_dbdma(volatile struct dbdma_cmd *,
> unsigned short, -	unsigned int);
> -static inline void tab_cmd_store(volatile struct dbdma_cmd *,
> unsigned int, -	unsigned int);
> -static inline void tab_cmd_gen(volatile struct dbdma_cmd *, unsigned
> short, -	unsigned short, unsigned int, unsigned int);
> -static int init_planb(struct planb *);
> -static int find_planb(void);
> -static void planb_pre_capture(int, int, struct planb *);
> -static volatile struct dbdma_cmd *cmd_geo_setup(volatile struct
> dbdma_cmd *, -					int, int, int, int, int, struct planb *);
...
> -	}
> -	if (i-- < npage) {
> -		printk(KERN_DEBUG "PlanB: init_grab: grab buffer not
> allocated\n"); -		for (; i > 0; i--) {
...
> -}
> -
> -static void saa_set(unsigned char addr, unsigned char val, struct
> planb *pb) -{
...
> -}
> -
> -static unsigned int saa_geo_setup(int width, int height, int
> interlace, int bpp, -	struct planb *pb)
> -{
...
> -	saa_regs[norm][SAA7196_HFILT] = (saa_regs[norm][SAA7196_HFILT] &
> ~0x3) -						| (width >> 8 & 0x3);
...
> -/***************************/
> -
> -static inline void planb_dbdma_restart(volatile struct dbdma_regs
> *ch) -{
...
> -	volatile struct dbdma_cmd *c1, int width, int height, int
> interlace, -	int bpp, int clip, struct planb *pb)
> -{
...
> -	int	i, size;
> -
> -	/* allocate memory for two plus alpha command buffers (size: max
> lines, -	   plus 40 commands handling, plus 1 alignment), plus dummy
> command buf, -	   plus clipmask buffer, plus frame grabbing status */
...
> -	}
> -	pb->frame_stat=(volatile unsigned int
> *)(pb->pre_cmd[MAX_GBUFFERS-1] -						+ PLANB_DUMMY);
...
> -	unsigned int caddr =
> (unsigned)in_le32(&pb->planb_base->ch1.cmdptr); -
...
> -	jump = virt_to_bus(c1+16); /* 14 by cmd_geo_setup() and 2 for
> padding */ -	if((c1 = cmd_geo_setup(c1, pb->win.width,
> pb->win.height, interlace, -					bpp, 1, pb)) == NULL) {
...
> -		return;
> -	}
> -	tab_cmd_store(c1++, addr_com, (unsigned)(DBDMA_NOP | BR_ALWAYS) <<
> 16); -	tab_cmd_store(c1++, addr_dep, jump);
...
> -							PLANB_SET(DMA_ABORT));
> -
> -	base = (pb->frame_buffer_phys + pb->offset + pb->win.y *
> (pb->win.bpl -					+ pb->win.pad) + pb->win.x * bpp);
> -
> -	if (interlace) {
> -		stepsize = 2;
...
> -	/* odd field mask: */
> -/* C */	tab_cmd_store(c2++,
> (unsigned)(&pb->planb_base_phys->ch2.br_sel),
> -							PLANB_SET(DMA_ABORT));
> -	/* this points to pos. B */
...
> -		return -EINVAL;
> -	if (mp->height * mp->width * format > PLANB_MAX_FBUF) /* format =
> bpp */ -		return -EINVAL;
> -
> -	planb_lock(pb);
...
> -
> -	tab_cmd_dbdma(c1++, DBDMA_NOP, 0);
> -	if((c1 = cmd_geo_setup(c1, pb->gwidth[fr], pb->gheight[fr],
> interlace, -						bpp, 0, pb)) == NULL) {
...
> -}
> -
> -static volatile struct dbdma_cmd *setup_grab_cmd(int fr, struct
> planb *pb) -{
> -	int		i, bpp, count, nlines, stepsize, interlace;
...
> -	/* Preamble commands: */
> -	tab_cmd_dbdma(c1++, DBDMA_NOP, 0);
> -	tab_cmd_dbdma(c1, DBDMA_NOP | BR_ALWAYS, virt_to_bus(c1 + 16));
> c1++; -	if((c1 = cmd_geo_setup(c1, pb->gwidth[fr], pb->gheight[fr],
> interlace, -						bpp, 0, pb)) == NULL) {
> -		printk(KERN_WARNING "PlanB: encountered serious problems\n");
...
> -}
> -
> -static long planb_read(struct video_device *v, char *buf, unsigned
> long count, -				int nonblock)
> -{
...
> -}
> -
> -static int planb_ioctl(struct video_device *dev, unsigned int cmd,
> void *arg) -{
> -	struct planb *pb=(struct planb *)dev;
> -
> -	switch (cmd)
...
> -}
> -
> -static int planb_mmap(struct vm_area_struct *vma, struct
> video_device *dev, const char *adr, unsigned long size) -{
> -	int i;
> -	struct planb *pb = (struct planb *)dev;
...
> saa_rev); -	/* Initialize the SAA registers in memory and on chip */
> -	saa_init_regs (pb);
> -
...
> -	enable_irq(pb->irq);
> -
> -	if(video_register_device(&pb->video_dev, VFL_TYPE_GRABBER,
> video_nr)<0) -		return -1;
> -
> -	return 0;
> -}
> -
> -/*
> - *	Scan for a PlanB controller, request the irq and map the io
> memory - */
> -
> -static int find_planb(void)
> -{
...
> -err_out:
> -	/* FIXME handle error */   /* comment moved from pci_find_slot,
> above */ -	pci_dev_put(pdev);
> -	return 0;
> -}
> -
...
> diff --git a/drivers/media/video/planb.h
> b/drivers/media/video/planb.h deleted file mode 100644
...
> -#define PLANBIOCGSAAREGS	_IOWR('v', BASE_VIDIOCPRIVATE, struct
> planb_saa_regs)	/* Read a saa7196 reg value */ -#define
> PLANBIOCSSAAREGS	_IOW('v', BASE_VIDIOCPRIVATE + 1, struct
> planb_saa_regs)	/* Set a saa7196 reg value */ -#define
> PLANBIOCGSTAT		_IOR('v', BASE_VIDIOCPRIVATE + 2, struct
> planb_stat_regs)	/* Read planb status */ -#define PLANB_TV_MODE		1
> -#define PLANB_VTR_MODE		2
> -#define PLANBIOCGMODE		_IOR('v', BASE_VIDIOCPRIVATE + 3, int)	/* Get
> TV/VTR mode */ -#define PLANBIOCSMODE		_IOW('v', BASE_VIDIOCPRIVATE +
> 4, int)	/* Set TV/VTR mode */ -
> -#ifdef PLANB_GSCANLINE
> -#define PLANBG_GRAB_BPL		_IOR('v', BASE_VIDIOCPRIVATE + 5, int)	/* #
> of bytes per scanline in grab buffer */ -#endif
> -
...
> -#define PLANB_INV_REGS		_IOWR('v', BASE_VIDIOCPRIVATE + 21, struct
> planb_any_regs) -
> -#ifdef __KERNEL__
> -
...
> -#define PLANB_DUMMY 40	/* # of command buf's allocated for
> pre-capture seq. */ -	int gwidth[MAX_GBUFFERS],
> gheight[MAX_GBUFFERS];
