...
Received: from devils.ext.ti.com ([198.47.26.153]:33674 "EHLO
	devils.ext.ti.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1755482AbZDVJOT (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Wed, 22 Apr 2009 05:14:19 -0400
Received: from dflp53.itg.ti.com ([128.247.5.6])
	by devils.ext.ti.com (8.13.7/8.13.7) with ESMTP id n3M9EDaq012459
	for <linux-media@vger.kernel.org>; Wed, 22 Apr 2009 04:14:18 -0500
...
References: <1239189533-19961-1-git-send-email-chaithrika@ti.com> <200904181444.13823.hverkuil@xs4all.nl>
In-Reply-To: <200904181444.13823.hverkuil@xs4all.nl>
Subject: RE: [PATCH v2 3/4] ARM: DaVinci: DM646x Video: Add VPIF display driver
Date: Wed, 22 Apr 2009 14:44:16 +0530
Message-ID: <007d01c9c32a$b5cc07e0$216417a0$@com>
MIME-Version: 1.0
...
> Sent: Saturday, April 18, 2009 6:14 PM
> To: Chaithrika U S
> Cc: linux-media@vger.kernel.org; davinci-linux-open-
> source@linux.davincidsp.com; Manjunath Hadli; Brijesh Jadav
> Subject: Re: [PATCH v2 3/4] ARM: DaVinci: DM646x Video: Add VPIF
> display driver
> 
> On Wednesday 08 April 2009 13:18:53 Chaithrika U S wrote:
...
> 
> There is no need for this global. It's only used when registering the
> video
> devices and there it is much simpler to use 'j ? 3 : 2' than refer to
> this
> global.
> 
Point taken, will update.
...
> 
> I think this should be V4L2_STD_525_60 and V4L2_STD_625_50 rather than
> NTSC/PAL. After all, that's really what it is about. DM646X_V4L2_STD
> should
> be updated as well.
> 
OK. 
...
> > +}
> > +
> > +static void process_interlaced_mode(int fid, struct common_obj
> *common)
> > +{
> > +	unsigned long addr = 0;
> > +
...
> > +
> 
> Notice how this 'else' part is identical to the
> process_progressive_mode()
> function! So you can just call that directly.
> 
Very true, hadn't noticed it!!
...
2);
> > +				/* If fid does not match with stored field
id
> */
> > +				if (fid != ch->field_id) {
> > +					/* Make them in sync */
...
> 
> This should be 'config->stdid & std_info->stdid'. These standards are
> bitmasks.
> 
OK. 
...
> > +	}
> 
> And just a 'return 0;' at the end.
> 
OK
...
> > +	struct vpif_channel_config_params *std_info = &vpifparams-
> >std_info;
> > +	int ret;
> > +
...
> 
> Note that the return value isn't checked. Actually, wouldn't it make
> sense to
> merge vpif_get_mode_info with this one?
>
 
> And if the standard isn't found, shouldn't this function return an
> error as
> well?
> 
Will look into the above points and improve the implementation.
...
> > +	}
> > +
> > +	ch->vpifparams.video_params.stdid = ch-
> >vpifparams.std_info.stdid;
> > +}
> > +
...
> 
> I recommend making a few macros for these info/warn/err/dbg defines.
> The first argument can be part of the macro so you will end up with
> something like: 'vpif_err("invalid field format\n");'.
> 
OK. This will be a good improvement. Thanks!
...
> 
> Just 'return' directly, no need for the err variable that way.
> 
OK
...
> > +
> > +	if (common->started)
> > +		err = videobuf_poll_stream(filep, &common->buffer_queue,
> wait);
> 
> Ditto.
OK
...
> > +	if (err < 0) {
> 
> You will never get here since err is always 0.
> 
Hmm, I think this is from the older versions of the code. Will update this.
...
> > +	ch->usrs++;
> 
> I think this should be an atomic action (using atomic_t).
> 
OK.
...
> > +	if (fh != NULL)
> 
> This test is obviously not needed.
> 
OK
...
> 
> No need for duplicate parenthesis.
OK.
...
> > +	vpif_get_std_info(ch);
> 
> No check here whether the standard is supported. Also I think that this
> code
> should go to the beginning of this function, before the calls to the
> subdevices are made.
> 
OK. vpif_get_mode_info() does the checking. Will that suffice?
...
> 
OK. Will replace it with 
	return videobuf_dqbuf(&common->buffer_queue, p, file->flags &
O_NONBLOCK);
...
> 
> If buftype isn't V4L2_BUF_TYPE_VIDEO_OUTPUT, then an error should be
> reported
> at the start of this function since we only support this buffer type
> here.
> 
OK
...
> > +		/* clock settings */
> > +		ret =
> > +		 vpif_config_data->set_clock(ch-
> >vpifparams.std_info.ycmux_mode,
> > +
...
> > +	if (buftype == V4L2_BUF_TYPE_VIDEO_OUTPUT) {
> 
> Same as above: if it isn't this type, then just report an error.
> 
OK
...
> 
> Note that the default g_std as is provided by video_ioctl2 only works
> with
> drivers that have only video devices. It will not work when vbi devices
> are
> added. So if you intend to add vbi devices in the future, then it is
> better
> to implement g_std yourself.
> 
OK. Is it OK if this is added as an improvement later?

> > +	.vidioc_enum_output		= vpif_enum_output,
> > +	.vidioc_s_output		= vpif_s_output,
> 
> I'm missing a g_output here.
OK.
...
> > +	vpif_obj.sd = kmalloc(sizeof(struct v4l2_subdev *) *
> subdev_count,
> > +								GFP_KERNEL);
> > +	if (vpif_obj.sd == NULL) {
> > +		v4l2_err(&vpif_obj.v4l2_dev,
...
> > +		video_device_release(ch->video_dev);
> 
> This isn't right. If the device was allocated and registered, then only
> call
> video_unregister_device(). If the device was allocated but not
> registered,
> then call video_device_release().
> 
...
> 
> There is a v4l2_device_unregister missing here.
> 
Yes, missed it. I will update this.
...
> > +struct vbi_obj {
> > +	int num_services;
> > +	struct vpif_vbi_params vbiparams;	/* vpif parameters for the
> raw
> > +						 * vbi data */
> > +};
> > +
...
> > +	u8 *fbuffers[VIDEO_MAX_FRAME];		/* List of buffer
> pointers for
> > +						 * storing frames */
> > +	u32 numbuffers;				/* number of buffers */
> > +	struct videobuf_buffer *cur_frm;	/* Pointer pointing to
> current
> > +						 * videobuf_buffer */
...
performing
> > +						 * IO */
> > +	u8 started;				/* Indicates whether
streaming
> > +						 * started */
> > +	u32 ytop_off;				/* offset of Y top from the
> > +						 * starting of the buffer */
> > +	u32 ybtm_off;				/* offset of Y bottom from
> the
> > +						 * starting of the buffer */
> > +	u32 ctop_off;				/* offset of C top from the
> > +						 * starting of the buffer */
> > +	u32 cbtm_off;				/* offset of C bottom from
> the
> > +						 * starting of the buffer */
...
> > +	struct video_device *video_dev;	/* Identifies video device
> for
> > +					 * this channel */
...
> > +					 * opened device */
> > +	u8 io_allowed[VPIF_NUMOBJECTS];	/* Indicates whether this
> file handle
> > +					 * is doing IO */
> > +	enum v4l2_priority prio;	/* Used to keep track priority of
> > +					 * this instance */
> > +	u8 initialized;			/* Used to keep track of whether
> this
...
> 
> I expect that the next round of patches will be ready for merging. The
> code
> has improved a lot. Thank you for all your hard work!
> 
Thanks. Your reviews and suggestions have been a great help in improving
this patch set and taking it forward.
> Regards,
> 
> 	Hans
> 
