...
Received: from hqemgate14.nvidia.com ([216.228.121.143]:14616 "EHLO
	hqemgate14.nvidia.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1755681AbbHYNqK (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Tue, 25 Aug 2015 09:46:10 -0400
Date: Tue, 25 Aug 2015 15:44:46 +0200
...
Message-ID: <20150825134444.GH14034@ulmo.nvidia.com>
...
>=20
> That's why I put bypass mode here. And bypass mode can be set in device t=
ree
> or from v4l2-ctrls.
...
This doesn't seem to match the TRM, which has the following values:
	 ...
	 2 =3D RD_DONE
	 3 =3D REG_WR_SAFE
	 4 =3D VI_MWA_REQ_DONE
	 5 =3D VI_MWB_REQ_DONE
	 6 =3D VI_MWA_ACK_DONE
	 7 =3D VI_MWB_ACK_DONE
	 8 =3D VI_ISPA_DONE
	 9 =3D VI_CSI_PPA_FRAME_START
	10 =3D VI_CSI_PPB_FRAME_START
	11 =3D VI_CSI_PPA_LINE_START
	12 =3D VI_CSI_PPB_LINE_START
	13 =3D VI_VGP0_RCVD
	14 =3D VI_VGP1_RCVD
	15 =3D VI_ISPB_DONE
...
and then use them together with the above macro:
	...
	value =3D VI_CFG_VI_INCR_SYNCPT_COND(VI_CSI_PP_FRAME_START(port)) |
...
> >lanes =3D=3D 4, which will effectively never happen. So at the very leas=
t I
...
> Sure, I forget to fix this. lanes should get from DT and for TPG mode I w=
ill
...
> by 2 channels. like CSIA and CSIB, CSIC and CSID, CSIE and CSIF. So we ha=
ve
> 3 groups.
...
> I also got those magic numbers from internal source. Some of them are not=
 in
...
>=20
> CSI have 6 channels but just 3 PHYs. If a channel want to use 4 data lane=
s,
> then it has to be CSIA, CSIC and CSIE. And CSIB, CSID and CSIF channels c=
an
...
> PHY register value and clear the port related bit and set that bit. Then =
it
> won't touch the setting of the other port. It means when we setup a chann=
el
> it should not change the other channel which sharing PHY register with the
> current one.
>=20
> In your case, you cleared the other port's bit and set the current port b=
it.
...
> Then when we try to enable CSIB, we should write 0x0101 to the PHY regist=
er
> but not 0x0100.
...
> >the SMMU?
> I tested this code in downstream kernel with SMMU. Do we fully support SM=
MU
...
> >>+	 * the minimum and maximum values, clamp the requested width and conv=
ert
> >>+	 * it back to pixels.
> >>+	 */
...
With your above calculation you'll end up with:
	...
	align =3D lcm(64, 4) =3D 64;
	width =3D rounddown(2 * 4 =3D 8, 64) =3D 0;
...
> >>+	 * line value is zero, the module doesn't support user configurable l=
ine
...
use
> case to use the surface 1 and surface 2. So I just added this parameter f=
or
> future usage.
>=20
...
> >>diff --git a/drivers/media/platform/tegra/tegra-core.h b/drivers/media/=
platform/tegra/tegra-core.h
...
> >>+
> >>+/* Minimum and maximum width and height common to Tegra video input de=
vice. */
...
> >>+/* UHD 4K resolution as default resolution for all Tegra video input d=
evice. */
...
> >>diff --git a/include/dt-bindings/media/tegra-vi.h b/include/dt-bindings=
/media/tegra-vi.h
> >[...]
...

YqSuqRQXwS+f/l/KFBHN
=RbYQ
-----END PGP SIGNATURE-----
