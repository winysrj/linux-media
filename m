...
Received: from lb2-smtp-cloud3.xs4all.net ([194.109.24.26]:32917 "EHLO
	lb2-smtp-cloud3.xs4all.net" rhost-flags-OK-OK-OK-OK)
	by vger.kernel.org with ESMTP id S1750956AbbCTSIU (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Fri, 20 Mar 2015 14:08:20 -0400
Message-ID: <550C6208.6080504@xs4all.nl>
Date: Fri, 20 Mar 2015 19:08:08 +0100
...
References: <1426870363-18839-1-git-send-email-k.debski@samsung.com> <1426870363-18839-6-git-send-email-k.debski@samsung.com>
In-Reply-To: <1426870363-18839-6-git-send-email-k.debski@samsung.com>
...
> [k.debski@samsung.com: add promiscuous mode]
...
I would actually expect that USB dongles read out the EDID from the source.
...
You probably need to add some documentation regarding cec_claim_log_addrs()
and how drivers can use it. Also, while logical addresses are being claimed,
are drivers or userspace allowed to transmit/receive other messages? Or just
...
This needs better documentation. The cec_post_event() function isn't
described either. Did I ever do something with this? I have a suspicion
that this can be removed altogether.
...
> +network devices. In the normal mode messages not directed to the device
> +(differentiated by the logical address of the CEC device) are not
> +forwarded to the userspace. Same rule applies to the messages containing
> +remote control key codes. When promiscuous mode is enabled all messages
> +can be read by userspace. Processing of the messages is still done, thus
> +key codes will be both interpreted by the framework and available as an
> +input device, but also raw messages containing these codes are sent to
> +the userspace.

Will messages that are processed by the driver or cec framework also be
relayed to userspace in promiscuous mode? Will userspace be able to tell
that it has been processed already?

> +
> +Switching on the promiscuous mode is done with a special ioctl.
> +
> +#define CEC_G_PROMISCUOUS	_IOR('a', 12, int)
> +#define CEC_S_PROMISCUOUS	_IOW('a', 13, int)
> +
...
> +		res = cec_transmit_msg(adap, &tx_data, false);
> +		if (promiscuous)
> +			break;
> +		return res;
> +
...
As mentioned, I think that this function and associated data structures
...
> +		if (copy_to_user(parg, &adap->promiscuous,
> +		    sizeof(adap->promiscuous)))
> +			return -EFAULT;
> +		break;
> +	}
> +
...
> +		u8 promiscuous;
> +
> +		if (!(adap->capabilities & CEC_CAP_PROMISCUOUS))
> +			return -ENOTTY;
> +		if (copy_from_user(&promiscuous, parg, sizeof(promiscuous)))
> +			return -EFAULT;
> +		adap->promiscuous = promiscuous;
> +		break;
> +	}
> +
> +	default:
...
> +
> +#define CEC_G_PROMISCUOUS	_IOR('a', 12, __u8)
> +#define CEC_S_PROMISCUOUS	_IOW('a', 13, __u8)
> +
> +#endif
> 
