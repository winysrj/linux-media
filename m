...
Received: from lb2-smtp-cloud9.xs4all.net ([194.109.24.26]:39210 "EHLO
        lb2-smtp-cloud9.xs4all.net" rhost-flags-OK-OK-OK-OK)
        by vger.kernel.org with ESMTP id S1750810AbdGaKRn (ORCPT
        <rfc822;linux-media@vger.kernel.org>);
        Mon, 31 Jul 2017 06:17:43 -0400
...
Message-ID: <dbb77604-5f9f-975c-6213-ed6449587ef3@xs4all.nl>
Date: Mon, 31 Jul 2017 12:17:37 +0200
MIME-Version: 1.0
...
You must return pending buffers in this error path as well.
...
You can use this handler_init for both encoder and decoder. The last argument
is just a hint and it is used to determine how many buckets to use for the
hash. It's the same number for 1 or 3 controls, so just move this before the
if and use 3 as the argument.
...
This can't be right. TGT_CROP should return the current crop, the other
two return the maximum crop (i.e. in this case the width/height of the
output format).
...
It's a bit difficult to tell, but this doesn't change what VIDIOC_G_FMT will
return, does it? G_FMT(OUTPUT) describes the output buffer. The TGT_CROP
describes which part of that buffer will be DMAed to the hardware.

So S_SELECTION(OUTPUT, TGT_CROP) should not change the output buffer format.
...
This is probably the reason for the failing v4l2-compliance.

There is no such thing as 'No format set' in V4L2. The driver always
has to fill in a format. Typically some default formats are used when
the m2m video device is opened. See e.g. the coda driver.

v4l2-compliance explicitly tests that there are no empty formats or
...
Please use the v4l2_m2m_ioctl_* helper functions here as much as possible.
