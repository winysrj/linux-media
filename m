...
Received: from smtp-vbr15.xs4all.nl ([194.109.24.35]:4708 "EHLO
	smtp-vbr15.xs4all.nl" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1753959Ab3LGKMX (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Sat, 7 Dec 2013 05:12:23 -0500
Message-ID: <52A2F47F.7010206@xs4all.nl>
Date: Sat, 07 Dec 2013 11:12:15 +0100
From: Hans Verkuil <hverkuil@xs4all.nl>
MIME-Version: 1.0
To: Sylwester Nawrocki <sylvester.nawrocki@gmail.com>
CC: Linux Media Mailing List <linux-media@vger.kernel.org>
Subject: Re: RFC: Properties, Configuration Storage, Selections and Matrices
References: <52862D55.8010406@xs4all.nl> <528B1CEC.5060308@xs4all.nl> <5290B6BB.7080102@xs4all.nl> <52A2630E.6010909@gmail.com>
In-Reply-To: <52A2630E.6010909@gmail.com>
...
My choice of words was too strong, the actual implementation has no restrictions
and any control/property may be used in a store.

See also my "Another update" section at the top.
...
I've dropped this mechanism in the latest version. Using a store that's too large
you will just get -EINVAL. You can't really give back a maximum store number since
controls can have different number of stores in the current implementation.
...
That feature has also been removed. It did not really match what a store is about.
Just like you can't destroy shadow registers, stores are also always there.

I quickly found out that this idea was bad when I tried to implement it :-)
...
Another unsolved problem is what to do if you have to deal with varying number
of stores. E.g. 4 hardware registers are grouped and have two shadow registers
each (so 4 properties with two stores each), and another 2 registers have four
shadow registers each (so 2 properties with four stores each).

Now you want to select store 1 for the first set and store 3 for the second set
of properties. The control framework isn't the problem, that can handle it fine,
but how do you specify that in a public API?

You may have noticed that this RFC does not really go in-depth into that part of
the story. It's something I need to experiment with.
...
I would choose commit "Add frame_size target." as the head. The later patches
are work-in-progress and vivi might not work as expected there.

I also have an updated v4l-utils that adds property/matrix support to v4l2-ctl:

http://git.linuxtv.org/hverkuil/v4l-utils.git/shortlog/refs/heads/propapi

Note that these trees are for experimentation and are not a clean series of
patches. Before I'll post anything it will have to be reorganized completely.
The code itself is clean though, it's just the journey to get there isn't
clean :-)

My current focus is to add atomic crop/compose support to vivi using the new
crop and compose properties. If that works out I'll start addressing various
TODOs in the code and experiment with how to tell the system what store to
apply when.
