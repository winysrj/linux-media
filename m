...
Received: from smtp-gw21.han.skanova.net ([81.236.55.21]:48691 "EHLO
	smtp-gw21.han.skanova.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1755816Ab0IJIyr (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Fri, 10 Sep 2010 04:54:47 -0400
Message-ID: <4C89F248.5000800@pelagicore.com>
Date: Fri, 10 Sep 2010 10:54:32 +0200
...
Subject: Re: [RESEND][PATCH 1/2] media: Add timberdale video-in driver
References: <1283428587.23309.25.camel@debian> <4C87E565.9020603@redhat.com>
In-Reply-To: <4C87E565.9020603@redhat.com>
...
>
> Hmm... why are you using a constant here? Does this device provide the standard UYVY fourcc
...
> If it uses the standard format, just use the standard definition. Otherwise, we need to add a
> new format to the V4L2 API, to make sure that userspace applications and libv4l will properly
> handle it.
>
>> +
...
>> +		.fps			= 26
>
> Hmm... 26? This sounds weird to me for a 60Hz video standard. It should be about 30fps.

You are right, it's weird. Should be 30!
...
>
> Hmm... there's something wrong here: You're enumerating all existing video
> formats, but you allow setting only a few PAL and one NTSC standard.
>
> If the device is for just camera devices, the better is to use
> V4L2_STD_525_60 instead of V4L2_STD_NTSC, and V4L2_STD_625_50 instead
> of V4L2_STD_PAL, at timblogiw_tvnorms[] array.

I will change it to logically or all standards in the tvnorms-array. which will reflect the 
standards in the template pointed out below.

>
> Is this device just for camera, or does it work with a video decoder?

It works with a video decoder, so there might be other sources than just camera
...
>
> Should match whatever you're using at VIDIOC_ENUMSTD.

Done by oring all standars in the video-norms array.
...
>
> An open may happen just after registering the device (and, in fact, it occurs, if
> you're using udev). So, I think you need to move those two calls to happen before
> registering the device, to avoid race conditions.
