...
Received: from na3sys009aog118.obsmtp.com ([74.125.149.244]:60029 "EHLO
	na3sys009aog118.obsmtp.com" rhost-flags-OK-OK-OK-OK)
	by vger.kernel.org with ESMTP id S1753472Ab2JHHHv (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Mon, 8 Oct 2012 03:07:51 -0400
Received: by mail-lb0-f174.google.com with SMTP id n3so2708647lbo.19
        for <linux-media@vger.kernel.org>; Mon, 08 Oct 2012 00:07:48 -0700 (PDT)
Message-ID: <1349680065.3227.25.camel@deskari>
...
Date: Mon, 08 Oct 2012 10:07:45 +0300
...
Content-Type: multipart/signed; micalg="pgp-sha1"; protocol="application/pgp-signature";
	boundary="=-Yb37yp3hJ6qoogsoe+TZ"
Mime-Version: 1.0
Sender: linux-media-owner@vger.kernel.org
List-ID: <linux-media.vger.kernel.org>


--=-Yb37yp3hJ6qoogsoe+TZ
...
> +	};
> +
> +Every property also supports the use of ranges, so the commonly used dat=
asheet
...
> +		hactive =3D <1920>;
> +		vactive =3D <1080>;
> +		hsync-len =3D <0 44 60>;
> +		hfront-porch =3D <80 88 95>;
> +		hback-porch =3D <100 148 160>;
> +		vfront-porch =3D <0 4 6>;
> +		vback-porch =3D <0 36 50>;
> +		vsync-len =3D <0 5 6>;
> +	};
> +
> +Usage in backend
...
> +
> +A backend driver may choose to use the display-timings directly and conv=
ert the timing
> +ranges to a suitable mode. Or it may just use the conversion of the disp=
lay timings
...
> +to conveniently get the respective mode representation from the devicetr=
ee.
> +
> +Conversion to videomode
> +=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D
> +
> +As device drivers normally work with some kind of video mode, the timing=
s can be
> +converted (may be just a simple copying of the typical value) to a gener=
ic videomode
...
> +
> +The generic videomode read in by the driver can be converted to the foll=
owing
...
> @@ -11,3 +11,4 @@ obj-$(CONFIG_OF_MDIO)	+=3D of_mdio.o
>  obj-$(CONFIG_OF_PCI)	+=3D of_pci.o
>  obj-$(CONFIG_OF_PCI_IRQ)  +=3D of_pci_irq.o
>  obj-$(CONFIG_OF_MTD)	+=3D of_mtd.o
> +obj-$(CONFIG_OF_DISPLAY_TIMINGS) +=3D of_display_timings.o
> diff --git a/drivers/of/of_display_timings.c b/drivers/of/of_display_timi=
ngs.c
> new file mode 100644
...
sentence with a capital letter =3D).
...
> +	int ret;
> +
> +	prop =3D of_find_property(np, name, &length);
> +	if (!prop) {
> +		pr_err("%s: could not find property %s\n", __func__, name);
> +		return -EINVAL;
> +	}
> +
> +	cells =3D length / sizeof(u32);
> +
> +	if (cells =3D=3D 1)
> +		ret =3D of_property_read_u32_array(np, name, &result->typ, cells);
> +	else if (cells =3D=3D 3)
> +		ret =3D of_property_read_u32_array(np, name, &result->min, cells);
> +	else {
...
> +	}
> +
> +	ret |=3D parse_property(np, "hback-porch", &st->hback_porch);
> +	ret |=3D parse_property(np, "hfront-porch", &st->hfront_porch);
> +	ret |=3D parse_property(np, "hactive", &st->hactive);
> +	ret |=3D parse_property(np, "hsync-len", &st->hsync_len);
> +	ret |=3D parse_property(np, "vback-porch", &st->vback_porch);
> +	ret |=3D parse_property(np, "vfront-porch", &st->vfront_porch);
> +	ret |=3D parse_property(np, "vactive", &st->vactive);
> +	ret |=3D parse_property(np, "vsync-len", &st->vsync_len);
> +	ret |=3D parse_property(np, "clock", &st->pixelclock);
> +
> +	st->vsync_pol_active_high =3D of_property_read_bool(np, "vsync-active-h=
igh");
> +	st->hsync_pol_active_high =3D of_property_read_bool(np, "hsync-active-h=
igh");
> +	st->de_pol_active_high =3D of_property_read_bool(np, "de-active-high");
> +	st->pixelclk_pol_inverted =3D of_property_read_bool(np, "pixelclk-inver=
ted");
> +	st->interlaced =3D of_property_read_bool(np, "interlaced");
> +	st->doublescan =3D of_property_read_bool(np, "doublescan");
> +
> +	if (ret) {
...
> +
> +struct display_timings *of_get_display_timing_list(struct device_node *n=
p)
> +{
...
> +	}
> +
> +	disp =3D kzalloc(sizeof(*disp), GFP_KERNEL);
> +
> +	entry =3D of_parse_phandle(timings_np, "default-timing", 0);
> +
> +	if (!entry) {
...
> +
> +	default_timing =3D (char *)entry->full_name;
...
> +	disp->timings =3D kzalloc(sizeof(struct signal_timing *)*disp->num_timi=
ngs,
> +				GFP_KERNEL);
> +
> +	disp->num_timings =3D 0;
...
> +		struct signal_timing *st;
> +
> +		st =3D of_get_display_timing(entry);
> +
> +		if (!st)
> +			continue;
> +
> +		if (strcmp(default_timing, entry->full_name) =3D=3D 0)
> +			disp->default_timing =3D disp->num_timings;
...
> +		disp->timings[disp->num_timings] =3D st;
> +		disp->num_timings++;
> +	}
> +
> +
...
> +		return -EINVAL;
> +
> +	default_np =3D of_parse_phandle(np, "default-timing", 0);
> +
> +	if (default_np)
> +		return 0;
> +
...
> diff --git a/include/linux/of_display_timings.h b/include/linux/of_displa=
y_timings.h
> new file mode 100644
...
> +static inline u32 signal_timing_get_value(struct timing_entry *te, int i=
ndex)
> +{
> +	return te->typ;
> +}
...
> +}
> +
> +static inline struct signal_timing *display_timings_get(struct display_t=
imings *disp,
> +							 int index)
> +{
> +	struct signal_timing *st;
...

w7Ca54jQIkRRd/JGyBoh
=p7xz
-----END PGP SIGNATURE-----
