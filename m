...
Received: from relay8-d.mail.gandi.net ([217.70.183.201]:41571 "EHLO
        relay8-d.mail.gandi.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1751285AbeDENGg (ORCPT
        <rfc822;linux-media@vger.kernel.org>); Thu, 5 Apr 2018 09:06:36 -0400
Date: Thu, 5 Apr 2018 15:06:25 +0200
...
Subject: Re: [PATCH v13 2/2] rcar-csi2: add Renesas R-Car MIPI CSI-2 receiver
 driver
Message-ID: <20180405130622.GJ20945@w540>
...
> > +	{ .code =3D MEDIA_BUS_FMT_RGB888_1X24,	.datatype =3D 0x24, .bpp =3D 2=
4 },
> > +	{ .code =3D MEDIA_BUS_FMT_UYVY8_1X16,	.datatype =3D 0x1e, .bpp =3D 16=
 },
> > +	{ .code =3D MEDIA_BUS_FMT_UYVY8_2X8,	.datatype =3D 0x1e, .bpp =3D 16 =
},
> > +	{ .code =3D MEDIA_BUS_FMT_YUYV10_2X10,	.datatype =3D 0x1e, .bpp =3D 1=
6 },
>
...
> > +}
> > +
> > +static inline struct rcar_csi2 *notifier_to_csi2(struct v4l2_async_not=
ifier *n)
> > +{
...
> > +}
> > +
> > +static void rcar_csi2_write(struct rcar_csi2 *priv, unsigned int reg, =
u32 data)
> > +{
...
> > +	int pad;
> > +
> > +	pad =3D media_entity_get_fwnode_pad(&subdev->entity, asd->match.fwnod=
e,
> > +					  MEDIA_PAD_FL_SOURCE);
> > +	if (pad < 0) {
...
> > +}
> > +
> > +static void rcar_csi2_notify_unbind(struct v4l2_async_notifier *notifi=
er,
> > +				       struct v4l2_subdev *subdev,
> > +				       struct v4l2_async_subdev *asd)
> > +{
...
> > +}
> > +
> > +static const struct v4l2_async_notifier_operations rcar_csi2_notify_op=
s =3D {
...

LiHKmO9CXRjY0vSfSxo1
=ZzH0
-----END PGP SIGNATURE-----
