...
Received: from perceval.ideasonboard.com ([213.167.242.64]:54004 "EHLO
        perceval.ideasonboard.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1751079AbeD1S2T (ORCPT
        <rfc822;linux-media@vger.kernel.org>);
        Sat, 28 Apr 2018 14:28:19 -0400
...
References: <20180422223430.16407-1-laurent.pinchart+renesas@ideasonboard.com>
 <20180422223430.16407-7-laurent.pinchart+renesas@ideasonboard.com>
From: Kieran Bingham <kieran.bingham@ideasonboard.com>
Message-ID: <b324b7b9-5566-9ccf-2016-19991a2b216c@ideasonboard.com>
Date: Sat, 28 Apr 2018 19:28:14 +0100
MIME-Version: 1.0
In-Reply-To: <20180422223430.16407-7-laurent.pinchart+renesas@ideasonboard.com>
Content-Type: multipart/signed; micalg=pgp-sha512;
 protocol="application/pgp-signature";
 boundary="g1f1mNYlcFP77tV1t7gzXhFumLhSGCNHz"
Sender: linux-media-owner@vger.kernel.org
List-ID: <linux-media.vger.kernel.org>

This is an OpenPGP/MIME signed message (RFC 4880 and 3156)
--g1f1mNYlcFP77tV1t7gzXhFumLhSGCNHz
Content-Type: multipart/mixed; boundary="pzG5gE6JaApfHhc9m2p275K7K5akpgUV3";
...
--pzG5gE6JaApfHhc9m2p275K7K5akpgUV3
...
Registers check out against the data sheet, and I haven't spotted anythin=
g else
here on top of the comments Jacopo has highlighted.

I'll put a +1 on Jacopo's suggestion for handling the m3-w quirk though. =
It's an
adjustment to the width/height in the event the quirk is active - so adju=
sting
...
>=20
> diff --git a/drivers/media/platform/vsp1/Makefile b/drivers/media/platf=
orm/vsp1/Makefile
> index 596775f932c0..4bb4dcbef7b5 100644
> --- a/drivers/media/platform/vsp1/Makefile
> +++ b/drivers/media/platform/vsp1/Makefile
> @@ -5,6 +5,6 @@ vsp1-y					+=3D vsp1_rpf.o vsp1_rwpf.o vsp1_wpf.o
>  vsp1-y					+=3D vsp1_clu.o vsp1_hsit.o vsp1_lut.o
>  vsp1-y					+=3D vsp1_brx.o vsp1_sru.o vsp1_uds.o
>  vsp1-y					+=3D vsp1_hgo.o vsp1_hgt.o vsp1_histo.o
> -vsp1-y					+=3D vsp1_lif.o
> +vsp1-y					+=3D vsp1_lif.o vsp1_uif.o
> =20
>  obj-$(CONFIG_VIDEO_RENESAS_VSP1)	+=3D vsp1.o
> diff --git a/drivers/media/platform/vsp1/vsp1.h b/drivers/media/platfor=
m/vsp1/vsp1.h
...
> diff --git a/drivers/media/platform/vsp1/vsp1_drv.c b/drivers/media/pla=
tform/vsp1/vsp1_drv.c
...
>  /* -------------------------------------------------------------------=
----------
> @@ -409,6 +410,19 @@ static int vsp1_create_entities(struct vsp1_device=
 *vsp1)
>  		list_add_tail(&uds->entity.list_dev, &vsp1->entities);
>  	}
> =20
> +	for (i =3D 0; i < vsp1->info->uif_count; ++i) {
> +		struct vsp1_uif *uif;
> +
> +		uif =3D vsp1_uif_create(vsp1, i);
> +		if (IS_ERR(uif)) {
> +			ret =3D PTR_ERR(uif);
> +			goto done;
> +		}
...
> =20
> @@ -513,6 +527,9 @@ static int vsp1_device_init(struct vsp1_device *vsp=
1)
>  	for (i =3D 0; i < vsp1->info->uds_count; ++i)
...
> @@ -740,6 +757,7 @@ static const struct vsp1_device_info vsp1_device_in=
fos[] =3D {
>  		.features =3D VSP1_HAS_BRU | VSP1_HAS_WPF_VFLIP,
>  		.lif_count =3D 1,
>  		.rpf_count =3D 5,
> +		.uif_count =3D 1,
>  		.wpf_count =3D 2,
>  		.num_bru_inputs =3D 5,
>  	}, {
> @@ -749,6 +767,7 @@ static const struct vsp1_device_info vsp1_device_in=
fos[] =3D {
>  		.features =3D VSP1_HAS_BRS | VSP1_HAS_BRU,
>  		.lif_count =3D 1,
>  		.rpf_count =3D 5,
> +		.uif_count =3D 1,
>  		.wpf_count =3D 1,
>  		.num_bru_inputs =3D 5,
>  	}, {
> @@ -758,6 +777,7 @@ static const struct vsp1_device_info vsp1_device_in=
fos[] =3D {
>  		.features =3D VSP1_HAS_BRS | VSP1_HAS_BRU,
>  		.lif_count =3D 2,
>  		.rpf_count =3D 5,
...
> @@ -539,6 +539,10 @@ struct media_pad *vsp1_entity_remote_pad(struct me=
dia_pad *pad)
...
> @@ -567,6 +571,8 @@ static const struct vsp1_route vsp1_routes[] =3D {
...
>  };
> =20
> diff --git a/drivers/media/platform/vsp1/vsp1_regs.h b/drivers/media/pl=
atform/vsp1/vsp1_regs.h
...
> diff --git a/drivers/media/platform/vsp1/vsp1_uif.c b/drivers/media/pla=
tform/vsp1/vsp1_uif.c
...
> +}
> +static inline void vsp1_uif_write(struct vsp1_uif *uif, struct vsp1_dl=
_list *dl,
> +				  u32 reg, u32 data)
> +{
...
> + */
> +
> +static const unsigned int uif_codes[] =3D {
...
> +{
> +	struct vsp1_uif *uif =3D to_uif(subdev);
> +	struct v4l2_subdev_pad_config *config;
> +	struct v4l2_mbus_framefmt *format;
> +	int ret =3D 0;
> +
> +	if (sel->pad !=3D UIF_PAD_SINK)
> +		return -EINVAL;
> +
> +	mutex_lock(&uif->entity.lock);
> +
> +	config =3D vsp1_entity_get_pad_config(&uif->entity, cfg, sel->which);=
...
> +		format =3D vsp1_entity_get_pad_format(&uif->entity, config,
> +						    UIF_PAD_SINK);
> +		sel->r.left =3D 0;
> +		sel->r.top =3D 0;
...
> +		sel->r =3D *vsp1_entity_get_pad_selection(&uif->entity, config,
> +							sel->pad, sel->target);
> +		break;
> +
> +	default:
...
> +	format =3D vsp1_entity_get_pad_format(&uif->entity, config, UIF_PAD_S=
INK);
> +
> +	sel->r.left =3D clamp_t(unsigned int, sel->r.left, 0, format->width -=
 1);
> +	sel->r.top =3D clamp_t(unsigned int, sel->r.top, 0, format->height - =
1);
> +	sel->r.width =3D clamp_t(unsigned int, sel->r.width, UIF_MIN_SIZE,
> +			       format->width - sel->r.left);
> +	sel->r.height =3D clamp_t(unsigned int, sel->r.height, UIF_MIN_SIZE,
> +				format->height - sel->r.top);
> +
> +	/* Store the crop rectangle. */
> +	selection =3D vsp1_entity_get_pad_selection(&uif->entity, config,
...
> + */
> +
> +static const struct v4l2_subdev_pad_ops uif_pad_ops =3D {
> +	.init_cfg =3D vsp1_entity_init_cfg,
> +	.enum_mbus_code =3D uif_enum_mbus_code,
> +	.enum_frame_size =3D uif_enum_frame_size,
> +	.get_fmt =3D vsp1_subdev_get_pad_format,
> +	.set_fmt =3D uif_set_format,
> +	.get_selection =3D uif_get_selection,
> +	.set_selection =3D uif_set_selection,
> +};
> +
> +static const struct v4l2_subdev_ops uif_ops =3D {
> +	.pad    =3D &uif_pad_ops,
> +};
> +
> +/* -------------------------------------------------------------------=
...
> +{
> +	struct vsp1_uif *uif =3D to_uif(&entity->subdev);
...
> +	 */
> +	if (params !=3D VSP1_ENTITY_PARAMS_INIT)
> +		return;
> +
> +	vsp1_uif_write(uif, dl, VI6_UIF_DISCOM_DOCMPMR,
> +		       VI6_UIF_DISCOM_DOCMPMR_SEL(9));
> +
> +	crop =3D vsp1_entity_get_pad_selection(entity, entity->config,
...
> +}
> +
> +static const struct vsp1_entity_operations uif_entity_ops =3D {
> +	.configure =3D uif_configure,
> +};
> +
> +/* -------------------------------------------------------------------=
----------
...
> + */
> +
> +static const struct soc_device_attribute vsp1_r8a7796[] =3D {
> +	{ .soc_id =3D "r8a7796" },
> +	{ /* sentinel */ }
> +};
> +
> +struct vsp1_uif *vsp1_uif_create(struct vsp1_device *vsp1, unsigned in=
t index)
> +{
> +	struct vsp1_uif *uif;
> +	char name[6];
> +	int ret;
> +
> +	uif =3D devm_kzalloc(vsp1->dev, sizeof(*uif), GFP_KERNEL);
> +	if (uif =3D=3D NULL)
> +		return ERR_PTR(-ENOMEM);
> +
> +	if (soc_device_match(vsp1_r8a7796))
> +		uif->m3w_quirk =3D true;
> +
> +	uif->entity.ops =3D &uif_entity_ops;
> +	uif->entity.type =3D VSP1_ENTITY_UIF;
> +	uif->entity.index =3D index;
> +
> +	/* The datasheet names the two UIF instances UIF4 and UIF5. */
> +	sprintf(name, "uif.%u", index + 4);
> +	ret =3D vsp1_entity_init(vsp1, &uif->entity, name, 2, &uif_ops,
...
> +	return uif;
> +}
> diff --git a/drivers/media/platform/vsp1/vsp1_uif.h b/drivers/media/pla=
tform/vsp1/vsp1_uif.h
> new file mode 100644
...

=v3rr
-----END PGP SIGNATURE-----
