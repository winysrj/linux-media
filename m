...
Received: from mailout3.samsung.com ([203.254.224.33]:56170 "EHLO
	...
	with ESMTP id S1754125Ab1DFIjH convert rfc822-to-8bit (ORCPT
	<rfc822;linux-media@vger.kernel.org>); Wed, 6 Apr 2011 04:39:07 -0400
Received: from epmmp2 (mailout3.samsung.com [203.254.224.33])
 by mailout3.samsung.com
 (Oracle Communications Messaging Exchange Server 7u4-19.01 64bit (built Sep  7
 2010)) with ESMTP id <0LJ800GRH2P3XS50@mailout3.samsung.com> for
 linux-media@vger.kernel.org; Wed, 06 Apr 2011 17:39:03 +0900 (KST)
Received: from AMDN157 ([106.116.48.215])
 by mmp2.samsung.com (iPlanet Messaging Server 5.2 Patch 2 (built Jul 14 2004))
 with ESMTPA id <0LJ800DJ92OXPU@mmp2.samsung.com> for
 linux-media@vger.kernel.org; Wed, 06 Apr 2011 17:39:03 +0900 (KST)
Date: Wed, 06 Apr 2011 10:38:56 +0200
From: Kamil Debski <k.debski@samsung.com>
Subject: RE: [RFC] New controls for codec devices
In-reply-to: <007e01cbf420$a6cee9a0$f46cbce0$%park@samsung.com>
...
Message-id: <000001cbf436$146ed410$3d4c7c30$%debski@samsung.com>
MIME-version: 1.0
...
References: <ADF13DA15EB3FE4FBA487CCC7BEFDF3619110DAD1E@bssrvexch01>
 ...
 <007e01cbf420$a6cee9a0$f46cbce0$%park@samsung.com>
...
> To: 'Laurent Pinchart'; 'Kamil Debski'
> Cc: linux-media@vger.kernel.org; 'Kyungmin Park'; jaeryul.oh@samsung.com;
> hansverk@cisco.com; 'Marek Szyprowski'; jemings@samsung.com;
...
The thing I don't like about so many prefixes is that it would make
some controls really long.

For example:
V4L2_CID_MPEG_DECODER_H264_ADAPTIVE_RC_ACTIVITY is 47 characters long (as we have
agreed to skip _DISABLE, which would make it 55).

If I remember correct the discussion at the brainstorming meeting - the idea
was to have encoder controls without a prefix (e.g. V4L2_CID_MPEG_GOP_SIZE) and
to have decoder specific controls prefixed with DECODER (as there is much less
decoder specific controls). What to do with common controls? I would skip the prefix
too.
...
> > > kinds of input buffers and its behavior is determined by a register. So
> it
> > > is a read/write control.  In the first case, a single input buffer
> contains
...
> > > > DECODER_H264_DISPLAY_DELAY control ? A zero value would mean no
> display
> > > > delay.
> > >
...
> > > the decoder to determine how many OUTPUT buffers will it process before
> a
...
> > > ensures that after the CAPTURE buffer is dequeued it won't be used by
> the
> > > hardware.
> > >
> > > If the display delay is enabled then the decoder has to return an
> CAPTURE
> > > buffer after processing a certain number of OUTPUT buffers. If this
> number
> > > is low, then it may result in buffers not being dequeued in display
> order.
> > >
> > > Even worse, I have just discovered that the buffer can still be used by
> the
> > > hardware as a reference after it has been dequeued in case of low
> display
> > > delay. So I think that this feature is very specific to our hardware
> (thus
> > > I think it should be hw specific control) and application has to be
> aware
> > > of this.
> >
...
> > the
> > display delay control to return CAPTURE buffers sooner, making it possible
> to
> > display frames without an additional delay.
> >
> > If applications need to write to the buffers, they can't use that feature.
> > Otherwise, why shouldn't drivers always return frames to userspace ASAP ?
> We
> > would then need a single control to tell the driver whether the
> application
> > wants to write to the buffers (in which case they won't be dequeued until
> the
> > driver doesn't need them anymore), or only wants to read from them (in
> which
...
> > >
> 
> It may be very useful to make thumbnail image of movie. User set display
> delay to 0,
...
> source stream(output queue) for destination(capture queue) output frame
> data.
> 
> MFC 5.1 supports only display delay for H264 because H264 stream can has
> long display
> delay especially due to codec feature. But next version of MFC or other
> codec devices
> can support this feature every codec standard, then we can make this control
> as common.
> 
> Can we distinguish enable/disable by one control?
> -1 means disable display delay as a default.

I don't like the idea. Is it common in V4L2 to have controls that act like this
(-1 for disable, other value is a valid value)? If not then we should stick to having
...
> > > > >
> > > > > VUI aspect ratio IDC for H.264 encoding.  The value is defined in
> VUI
> > > >
> > > > Table
> > > >
...
As a separate plane? Or a separate set of controls?

I think I would go for H264_VUI_* as this provides more flexibility.
...
> > > > > number of buffers required as reference and the application would
> like
...
> > > > >
> > > > > This is the minimum number of buffers required for the capture
> queue.
> > > > > This option may be useful if the decoder requires a minimum number
> of
> > > > > buffers required as reference and the application would like to have
> N
...
> >
> > This feature is useful for non-codec devices as well, should it be a
> standard
...
I can imagine a hardware that has similar requirements. It could be used as
a hint for other drivers.
...
> > >
> > > I've been thinking hard about this for some time. My first thought was
> to
...
> > > In case of H264 the level is a number consisting of two digits (such as
> 1.1
> > > or 4.0) and level "1b". In case of MPEG4 we have a single digit 0, 1, 2,
> > > 3, 4, 5 and 0b and 3b levels.
> >
> > You could have a single menu control with different options depending on
> the
> > stream type.
...
> > >
> > > In case of profiles the naming differs quite a lot. For H264 we have a
> lot
...
> > [snip]
> 
> I think it seems better that level & profile control are not separated if
> possible.
> >
...
> >
> 
> As Kamil's reply, MFC 5.1 provides loop filter for MPEG4 decoding as post
> filter.

On second thought I think that the name maybe confusing. It is a deblocking filter.
* DECODER_MPEG4_DEBLOCK_FILTER. Because the loop for H264 means that the deblocking filter
is used in the encoder too to process the referenced images.
...
> > > >
> > > > Do we need 3 controls for that, or can we use a single one (similarly
> to
> > > > what
...
> >
> > It's less obvious, but it can be defined. Or we could define a COLOR
> control
...
A color control seems reasonable. It’s a subject for another RFC.
...
> > >
> > > This is used for encoding rate control. If you want to control the bit
> rate
> > > per second then you need to know how many frames are there per second.
> >
> > OK. Does the encoder simply divide the requested bitrate by the frame rate
> to
> > get a bit per frame value, and program the hardware with that, or is it
> more
> > complex ? If it does, we could have a per-frame bitrate instead of a per-
> > second bitrate, but that might be confusing.
> >
> 
> How about make this control like MPEG4 and integrate one control?
> (resolution/delta)
> Fixed-scale(x1000) method seems like too tightly coupled with MFC.

It could also be *FRAME_RATE_NOMINATOR and *FRAME_RATE_DENOMINATOR but
I am not sure it would be the same as the controls 50 and 51 described
Below.
