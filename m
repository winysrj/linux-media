...
Received: from casper.infradead.org ([85.118.1.10]:40706 "EHLO
	casper.infradead.org" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1757446Ab0JQRwq (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Sun, 17 Oct 2010 13:52:46 -0400
Message-ID: <4CBB37DC.6050507@infradead.org>
Date: Sun, 17 Oct 2010 15:52:28 -0200
...
Subject: Re: [GIT PATCHES FOR 2.6.37]  Support for NetUP Dual DVB-T/C CI RF
 card V.2
References: <201010171955.18806.liplianin@me.by>
In-Reply-To: <201010171955.18806.liplianin@me.by>
...
On Linux, long int doesn't mean 64 bits. If you want to use a data with 64 bits,
use, instead: u64, for unsigned (or s64, for signed). 

Also, it doesn't seem to be a good idea to define those two data static. The proper way
is to move those two data to some struct, for example, to astate. You may declare it as:
...
when you'll alloc it with kzconfig, stack will be initialized to zero already.

One advantage is that, if you have two devices, a FPGA init of one device wouldn't interfere
in the other, even if they happen in parallel.
...
Hmm... are you initializing proc_attributes at the loop bellow? Otherwise, the better
is to use kzalloc.
...
Huh? Haven't you replace the errors code already by standard Unix errors? You don't need this
error translation logic. Instead, just return the error code to the caller.
...
Hmm... global vars? This doesn't seem right... what happens if you have two devices
simultaneously using those vars? The better is to encapsulete them into some
per-device struct.
...
The better is to encapsulate all those global vars into an struct, like altera_config,
otherwise you'll have serious problems if more than one device is connected.
...
PS.: I noticed that you've declare a large number of symbols as global. You need to be sure
that those names won't conflict with other symbols in Linux, otherwise, kernel won't compile
with allyesconfig. What we generally do for driver symbols is to start all of them with the
driver name.

So, for example, a better name for jbi_init_jtag() would be altera_jbi_init_jtag().

Btw, checkpatch got two warnings:

WARNING: unnecessary cast may hide bugs, see http://c-faq.com/malloc/mallocnocast.html
#407: FILE: drivers/misc/stapl-altera/altera.c:322:
+		attrs = (char *)kmalloc((u32)sym_count, GFP_KERNEL);

WARNING: unnecessary cast may hide bugs, see http://c-faq.com/malloc/mallocnocast.html
#414: FILE: drivers/misc/stapl-altera/altera.c:329:
+		proc_attributes = (u8 *)kmalloc((u32)proc_count, GFP_KERNEL);
