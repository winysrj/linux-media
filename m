...
Received: from mx1.redhat.com ([209.132.183.28]:17812 "EHLO mx1.redhat.com"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
	id S935080Ab3DPS6T (ORCPT <rfc822;linux-media@vger.kernel.org>);
	Tue, 16 Apr 2013 14:58:19 -0400
Message-ID: <516D9F39.3090501@redhat.com>
Date: Tue, 16 Apr 2013 15:58:01 -0300
From: Mauro Carvalho Chehab <mchehab@redhat.com>
MIME-Version: 1.0
To: Antti Palosaari <crope@iki.fi>
CC: LMML <linux-media@vger.kernel.org>
Subject: Re: [RFC v2013-04-11] SDR API REQUIREMENT SPECIFICATION
References: <5166CF3A.5040603@iki.fi> <20130416050928.6cf5a875@redhat.com> <516D61FE.9060601@iki.fi> <516D89B3.5090103@redhat.com> <516D95B1.1030009@iki.fi>
In-Reply-To: <516D95B1.1030009@iki.fi>
...
>> feedback.
>
> Here is the one =) I have had license maybe ~15 years, first ~5 years technician class and after CW was removed from requirements license upgrades to unlimited. By a change I just returned to home from our uni. HAM club!
> I have to say I am not very experienced working with radio bands as I rarely do it nowadays. It is more radio technology which inspires, like modding old Nokia/Mobira mobile phones to HAM band.

The one I know more have dozens of different radio types and several antennas
with different lengths. He installed himself (sometimes with his son - also HAM),
...
>
> And the hell this is totally different issue. These are application layer problems, in SDR we are interested of only controlling radio layer.
>
> That "drift" is usually detected by demodulator from the carrier (it is called carrier frequency offset). Do not mix demod / app layer here.
>
>
...
>> frequency drift.
>
> I quite disagree you here! I don't see idea at all to return drift. It is stupid. Just return frequency where tuner is standing and let the app do what it likes. There is already remote radios on the net.
>
>
...
>
> I don't understand that. Why that oversampling should be exposed to the userpsace?
> What I think it is just devices issue to sample what the best result is.
>If it does oversampling (like takes 2 samples for one returned) why it should be in API?
>Why we should care what kind of sampling techniques there is used by hw - it is
>enough that we know actual sample rate that go out from the device.
>
> Look for example those new digital cameras. They has very large sensor, then DSP is
>responsible of oversampling and user gets low sampled photo back. User does not need
>to know if the result enhanced by oversampling or some other technique.

You completely missed the point here: you asked if the tuner filter should be
calculated from sampling rate.

For me, "calculate from sampling rate" means to use the Nyquist frequency.

What I'm answering is that, sometimes, application may want to oversample,
instead of just using the Nyquist frequency. I also pointed an example of
one application (ok, this is a hardware demod, but the same logic could be
done in software), that uses 2x the Nyquist frequency. There are other
analog decoders that use 4x the Nyquist frequency for the A/D sampling rate.

Oversampling can help to reduce the noise, at the cost of requiring a higher
DMA transfer bandwidth.
...
>> frequency[1].
>
> It is up to tuner select most suitable IF. From the SDR point of view it does not has such much meaning.

The tuner does it based on the standard. If it doesn't know, it can't select
it.

 From SDR PoV, different hardware should produce the same results. So, if
a given hardware requires to know the standard in order to adjust the IF
and its LO frequency, while others do it transparent (zero IF tuners), both
should tune to the same channel for a given frequency.

So, the standard needs to be known, at least for those non-zero IF tuners.

> Tuner selects suitable IF because of demodulator needs / used band / bw.

Tuners also use IF to centralize the channel on their band pass filters.
That's basically why ISDB-T and DVB-T at 6MHz use different IF's on
several tuners.
...
>
> So what? Maybe tuner could be programmed a little bit "wrong" frequency in case of ISDB-T, but again, it is not SDR hw problem. That kind of issues belongs to the upper layers, not for radio control.

It is a SDR problem, if, in one driver, it tunes at the right frequency,
while on another driver it doesn't.

The hole point on providing an API is to abstract the hardware differences.

Also, in this specific case, if the frequency is not properly centered,
the tuner band-pass filter will drop one of the ISDB-T segments, as it will
be located at the guard interval, where the rejection is in the order of
tens of dB's.
