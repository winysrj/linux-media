...
Received: from smtp-vbr8.xs4all.nl ([194.109.24.28]:1052 "EHLO
	smtp-vbr8.xs4all.nl" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1753441AbZCNONR (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Sat, 14 Mar 2009 10:13:17 -0400
From: Hans Verkuil <hverkuil@xs4all.nl>
To: davinci-linux-open-source@linux.davincidsp.com
Subject: Re: [RFC 2/7] ARM: DaVinci: DM646x Video: ADV7343 video encoder driver
Date: Sat, 14 Mar 2009 15:13:32 +0100
Cc: chaithrika@ti.com, linux-media@vger.kernel.org
References: <1236935319-32730-1-git-send-email-chaithrika@ti.com>
In-Reply-To: <1236935319-32730-1-git-send-email-chaithrika@ti.com>
MIME-Version: 1.0
Content-Type: text/plain;
  charset="iso-8859-15"
Content-Transfer-Encoding: 7bit
Content-Disposition: inline
Message-Id: <200903141513.33098.hverkuil@xs4all.nl>
...
On Friday 13 March 2009 10:08:39 chaithrika@ti.com wrote:
...
These arrays might be mixing user and device level concepts. See my comments
on that at the end. It's not clear to me whether it is indeed the case, I'll
leave it to you to take action if it's wrong and I'll review it the next
round.
...
Use v4l2_ctrl_query_fill instead. The fill_std function has been removed
(that was a bad idea).
...
Not a good idea. The v4l-dvb master repository adds a .querystd callback that
...
This v4l2_dbg doesn't add anything useful that v4l_info didn't already say.
...
I'm not doing an in-depth review of this header. You first need to split
it up in two parts: one adv7343_regs.h header that is included by the adv7343.c
driver and is in the same directory as that driver: that header contains all
the internal datastructures and defines (register addresses, etc.) that
it uses.

The media/adv7343.h contains only the defines that other drivers need to
interface with the adv7343 driver: in particular the routing information
for the s_routing ops goes there.

Once that's done I'll review it again.

One thing to keep in mind at all times is that you do not mix user-level
...
The s_routing operations only deal with the device-level. How you encode
the pins is purely device specific and goes into the media/adv7343.h
header. Depending on the i2c device you may have to specify both input and
output pins. E.g. the input pin would be the pin that the videoport sends
its data to, and the output pin is the pin that is connector to the physical
connector on the board.

Note that when the conversion of the legacy drivers is finished I want to
modify the s_routing function a bit: instead of using the awkward
v4l2_routing struct you can just specify the input and output fields directly
...
media/saa7115.h is an example of that where the output field is abused to
