...
Received: from atrey.karlin.mff.cuni.cz ([195.113.26.193]:34407 "EHLO
        atrey.karlin.mff.cuni.cz" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1751691AbcJWUEA (ORCPT
        <rfc822;linux-media@vger.kernel.org>);
        Sun, 23 Oct 2016 16:04:00 -0400
Date: Sun, 23 Oct 2016 22:03:55 +0200
...
Message-ID: <20161023200355.GA5391@amd>
MIME-Version: 1.0
...
---
=46rom v4 I did cleanups to coding style and removed various oddities.
...
+obj-$(CONFIG_VIDEO_ET8EK8)	+=3D et8ek8/
 ...
 obj-y				+=3D soc_camera/
diff --git a/drivers/media/i2c/et8ek8/Kconfig b/drivers/media/i2c/et8ek8/Kc=
onfig
...
diff --git a/drivers/media/i2c/et8ek8/Makefile b/drivers/media/i2c/et8ek8/M=
akefile
new file mode 100644
...
+	if (!client->adapter)
+		return -ENODEV;
+	if (data_length !=3D ET8EK8_REG_8BIT && data_length !=3D ET8EK8_REG_16BIT)
+		return -EINVAL;
+
...
+}
+
+static void et8ek8_i2c_create_msg(struct i2c_client *client, u16 len, u16 =
reg,
...
+{
+	const struct et8ek8_reglist **list1 =3D (const struct et8ek8_reglist **)a,
...
+		return -EINVAL;
+
+	if (sensor->current_reglist->mode.ext_clock !=3D reglist->mode.ext_clock)
+		return -EINVAL;
+
+	sensor->current_reglist =3D reglist;
+	et8ek8_update_controls(sensor);
...
+	if (sensor->version !=3D ET8EK8_REV_1 && sensor->version !=3D ET8EK8_REV_=
2)
+		dev_info(&client->dev,
...
+		goto out_poweroff;
+	}
+	rval =3D et8ek8_stream_on(sensor); /* Needed to be able to read EEPROM */
+	if (rval)
+		goto out_poweroff;
...
+	struct et8ek8_reglist *reglist;
+
+	reglist =3D et8ek8_reglist_find_type(&meta_reglist, ET8EK8_REGLIST_MODE);
...
iEUEARECAAYFAlgNF6sACgkQMOfwapXb+vL4mACfVFEnFDfgXx41tmSEdADBUJs4
UoYAmJq7RF47WnUPTFgS/8LA91YJ1m8=
=5wOE
-----END PGP SIGNATURE-----
