...
Received: from mail.gmx.net ([213.165.64.20]:59479 "HELO mail.gmx.net"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with SMTP
	id S1753758Ab0EYPei (ORCPT <rfc822;linux-media@vger.kernel.org>);
	Tue, 25 May 2010 11:34:38 -0400
Date: Tue, 25 May 2010 17:34:52 +0200 (CEST)
...
Message-ID: <Pine.LNX.4.64.1005241754220.2611@axis700.grange>
...
I know, you have it from the previous review, I'll just put it here again 
as a reminder for myself for the next time - verify, that this is gone, 
...
> +	 */
> +	videobuf_waiton(vb, 0, 1);

No, you should be careful with this. You changed from uninterruptible wait 
to interruptible wait. In this case you have to check the return code and 
process the interrupted wait accordingly.
...
> +
> +	spin_lock_irqsave(&pcdev->lock, flags);
> +	if (cpu_is_mx25() && vb->state == VIDEOBUF_QUEUED) {
> +		list_del(&vb->queue);
> +		vb->state = VIDEOBUF_ERROR;
> +	}

This is something quite different from v1 and from what I suggested. Now 
you do not dequeue buffers in this function on mx27 at all?

In this function you can select one of the two approaches: either you 
terminate all buffers, including currently active ones, for which you 
also have to stop DMA, immediately and return them to the user with an 
error, or you wait for active buffers to complete naturally and only 
forcedly terminate queued but inactive buffers. The former is, probably, 
more like what the user expects and should complete immediately, whereas 
the latter can be easier to implement, but can put the user to sleep until 
the buffer is complete. But it either case it is, probably, better to be 
consistent across hardware types.

And then, given the error in the datasheet, all the more a comment will be 
required.
...
Ok, already this hard-codes 16 bit RGB in and out... If there's no 
sufficient man-power resp. financing for a proper implementation of a 
dynamic eMMA configuration, how about checking for 16bpp in try_fmt / 
set_fmt?
...
"565" means 5 bits for red and blue, 6 bits for green. but 0x2ca0 I cannot 
understand either. This seems to mean: 44 bits offset to the red or Y 
component in the pixel, 10 bits to green / U (Cb) component, and 0 bits to 
the V (Cr) component... Ah, I see, error in the Figure:( So, it's 11 bits 
offset to red, 5 bits to green, 0 to blue, so, it's RGB565. This means, 
also this register has to be adjusted for other pixel formats, or at least 
other bits-per-pixel values, if no pixel format specific processing is 
performed.
...
You have to set pix->field, just setting mf.field doesn't bring much. 
Actually, I think, you have to set pix->field always after the respective 
call to the subdev, other soc-camera drivers could be improved in this 
respect.
...
You missed this location from Sascha's patch, this wasn't on purpose, was 
it?
...
Given Sascha's explanation, please, at least provide an explanatory 
comment, maybe as a "TODO."
