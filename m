...
Received: from bear.ext.ti.com ([192.94.94.41]:52452 "EHLO bear.ext.ti.com"
	rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
	id S1754758Ab0KHOyq (ORCPT <rfc822;linux-media@vger.kernel.org>);
	Mon, 8 Nov 2010 09:54:46 -0500
...
Subject: [PATCH 2/6] davinci vpbe: VPBE display driver
Date: Mon,  8 Nov 2010 20:24:25 +0530
Message-Id: <1289228065-4699-1-git-send-email-manjunath.hadli@ti.com>
...
+		if (!strcmp(mode,
+			vpbe_dev->cfg->outputs[curr_output].modes[i].name)) {
+			memcpy(&vpbe_dev->current_timings,
+			       &vpbe_dev->cfg->outputs[curr_output].modes[i],
...
+		return -EINVAL;
+
+	memcpy(mode_info, &vpbe_dev->current_timings,
+	       sizeof(vpbe_dev->current_timings));
+
+	return 0;
+}
...
+
+		if ((cfg->outputs[curr_output].modes[i].timings_type &
+			VPBE_ENC_DV_PRESET) &&
+		    (cfg->outputs[curr_output].modes[i].timings.dv_preset ==
+			dv_preset)) {
+			memcpy(&vpbe_dev->current_timings,
+			       &vpbe_dev->cfg->outputs[curr_output].modes[i],
+			       sizeof(vpbe_dev->current_timings));
...
+		if ((cfg->outputs[curr_output].modes[i].timings_type &
+			VPBE_ENC_STD) &&
+		    (cfg->outputs[curr_output].modes[i].timings.std_id &
+			std_id)) {
+			memcpy(&vpbe_dev->current_timings,
+			       &vpbe_dev->cfg->outputs[curr_output].modes[i],
+			       sizeof(vpbe_dev->current_timings));
...
+
+	for (i = 0; i < vpbe_dev->cfg->outputs[curr_output].num_modes; i++) {
+		if (!strcmp(cfg->outputs[curr_output].modes[i].name,
+			    std_name)) {
+			memcpy(&vpbe_dev->current_timings,
+			       &vpbe_dev->cfg->outputs[curr_output].modes[i],
+			       sizeof(vpbe_dev->current_timings));
...
+	if (!ret) {
+		memcpy(&vpbe_dev->current_timings,
+		       preset_mode, sizeof(*preset_mode));
+		osd_device->ops.set_left_margin(osd_device,
+			vpbe_dev->current_timings.left_margin);
+		osd_device->ops.set_top_margin(osd_device,
+			vpbe_dev->current_timings.upper_margin);
...
+}
+
+static int osd_device_get(struct device *dev, void *data)
+{
