...
Received: from devils.ext.ti.com ([198.47.26.153]:36844 "EHLO
	devils.ext.ti.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
	with ESMTP id S1753230Ab2IYKxP convert rfc822-to-8bit (ORCPT
	<rfc822;linux-media@vger.kernel.org>);
	Tue, 25 Sep 2012 06:53:15 -0400
...
Date: Tue, 25 Sep 2012 10:52:57 +0000
Message-ID: <F0866C1C5ECEAF4CB380B619BD733BEA41F094F7@DQHE02.ent.ti.com>
References: <1347963452.13371.15.camel@smile> <505A74A6.2070805@gmail.com>
 ...
 <20120922132515.GE12025@valkosipuli.retiisi.org.uk>
In-Reply-To: <20120922132515.GE12025@valkosipuli.retiisi.org.uk>
...
I've reviewed v4l2 flash APIs and I think it's also good to use v4l2 for our led driver chips. 
You wrote you have your internel version of the lm3554 driver, when will it be ready for publish?
...
Sent: Saturday, September 22, 2012 10:25 PM
To: Jeong, Daniel
Cc: Andy Shevchenko; gshark; linux-leds@vger.kernel.org; linux-media@vger.kernel.org
Subject: Re: led flash driver for lm3554/lm3556
...
> >> > And the indicator function in LM3554/6 can be controlled by SW. 
> >> > One of our customers controls the indicator using SW.
...
> >> > )
> >> Actually accordingly to the specs lm3554 and lm3555 is quite 
> >> different by hw configuration. It's better to keep them separate from my p.o.v.
> >
...
If it's only that, then why not, but often the details have so many differences it's less work to just write a new driver instead of cluttering up the old one. That applies IMHO to e.g. LM3554 and LM3555.
...
> >> > and torch.
> >> It doesn't require to split them. If you want to provide led 
> >> framework for that chip we could do the generic glue driver. And I 
> >> would like to do it.
> >> 
...
> 
> These chips are not only for Camera but also for Audio amp etc. You 
> guys are focusing Camera but Audio can use this chip, actually some 
...
V4L2 flash API supports flash controllers, including those with torch and indicator functionality. It's generic in the sense that it does not have hard-wired connections to cameras for instance.
...
> relevant to LED framework and it should split into three part. And 
> lm3555 don't need to merged with lm355x since there is as3645a driver.

Nothing prevents using the V4L2 flash API outside the context of cameras, it's fully generic in that sense in terms of type of hardware it's intended for, including the above LM chips. You can just open the flash device node and use it. No cameras involved.

However, I do reckon that the LED class is an established API used to control LEDs elsewhere, and there's some overlap between the two APIs. For instance, the flash strobe, timeout and precise current control is only handled by the V4L2 flash API whereas it currently does not recognise RGB LEDs, and quite possibly never will.
...
V4L2 flash API, so that LED devices implementing the V4L2 flash API would readily get LED class support, too.
