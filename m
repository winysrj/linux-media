...
Received: from lb3-smtp-cloud2.xs4all.net ([194.109.24.29]:50221 "EHLO
        ...
        by vger.kernel.org with ESMTP id S1752037AbdF3OfU (ORCPT
        <rfc822;linux-media@vger.kernel.org>);
        Fri, 30 Jun 2017 10:35:20 -0400
From: Hans Verkuil <hverkuil@xs4all.nl>
To: linux-media@vger.kernel.org
Cc: Maxime Ripard <maxime.ripard@free-electrons.com>,
        Eric Anholt <eric@anholt.net>,
        Hans Verkuil <hans.verkuil@cisco.com>
Subject: [RFC PATCH 09/12] cec-pin: add low-level pin hardware support
Date: Fri, 30 Jun 2017 16:35:06 +0200
Message-Id: <20170630143509.56029-10-hverkuil@xs4all.nl>
...
When the adapter is unconfigured and the low-level driver supports interrupts,
then the interrupt will be used to detect changes. This should be quite
accurate. But when the adapter is configured a hrtimer has to be used.
...
index 000000000000..276d5c8cd378
--- /dev/null
...
+ *	Tx Data 0 Low -> Tx Data 0 High ->
+ *   or:
+ *	Tx Data 1 Low -> Tx Data 1 High -> Tx Data 1 Pre Sample -> Tx Data 1 Post Sample ->
+ *
...
+	if (pin->timer_overruns) {
+		seq_printf(file, "timer overruns: %u of %u\n", pin->timer_overruns, pin->timer_cnt);
+		seq_printf(file, "max timer overrun: %u usecs\n", pin->timer_max_overrun);
+		seq_printf(file, "avg timer overrun: %u usecs\n", pin->timer_sum_overrun / pin->timer_overruns);
+	}
+	pin->timer_cnt = 0;
+	pin->timer_overruns = 0;
...
new file mode 100644
index 000000000000..79a4bc51527a
--- /dev/null
...
+ * cec_pin_changed from the interrupt.
+ */
+void cec_pin_changed(struct cec_adapter *adap, bool value);
+
+struct cec_adapter *cec_pin_allocate_adapter(const struct cec_pin_ops *pin_ops,
