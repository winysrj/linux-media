...
Received: from lb1-smtp-cloud3.xs4all.net ([194.109.24.22]:39398 "EHLO
        lb1-smtp-cloud3.xs4all.net" rhost-flags-OK-OK-OK-OK)
        by vger.kernel.org with ESMTP id S1751428AbdFGK0W (ORCPT
        <rfc822;linux-media@vger.kernel.org>);
        Wed, 7 Jun 2017 06:26:22 -0400
Subject: Re: [PATCH v2] [media] v4l2: Add support for go2001 PCI codec driver
To: Thierry Escande <thierry.escande@collabora.com>,
        Mauro Carvalho Chehab <mchehab@kernel.org>
References: <1495723346-14027-1-git-send-email-thierry.escande@collabora.com>
Cc: linux-media@vger.kernel.org, linux-kernel@vger.kernel.org
From: Hans Verkuil <hverkuil@xs4all.nl>
Message-ID: <66d3c6ce-b1ba-3cf1-1494-80d34f2c9905@xs4all.nl>
Date: Wed, 7 Jun 2017 12:26:17 +0200
MIME-Version: 1.0
In-Reply-To: <1495723346-14027-1-git-send-email-thierry.escande@collabora.com>
...
Don't use format descriptions. This will be filled in by the v4l2 core
in order to ensure consistent naming.
...
It's not good practice to define controls in this way.

More below where these arrays are used.
...
Use V4L2_CAP_VIDEO_M2M_MPLANE instead of combining capture and output.

> +	cap->capabilities = cap->device_caps | V4L2_CAP_DEVICE_CAPS;

Don't set the device_caps and capabilities fields here. Instead fill in
the device_caps field of struct video_device and drop it here.

These two fields will be filled in by the core for you based on the
...
I assume that this really should never happen? I would prefer a WARN_ON
followed by a return 0.

Returning an error in start_streaming would require you to change the
state of all pending buffers back to VB2_BUF_STATE_QUEUED. But that's
not worth the effort if this condition should never happen in the first
place.
...
If any of these non-0 returns can actually happen, then you must call
vb2_buffer_done with state VB2_BUF_STATE_QUEUED for any queued buffers
so they are all given back to the vb2 framework.
...
Are you sure that USERPTR works? When you call malloc() to allocate a buffer
in userspace, then the pointer you get is not page-aligned. Often hardware
requires that it is page-aligned, and in that case you cannot use VB2_USERPTR.
...
Rather than using these go2001_ctrl struct, just add the controls directly
using v4l2_ctrl_new_std and v4l2_ctrl_new_custom (are there private controls
at all?).
...
Can't you use v4l2_m2m_fop_poll?
...
The same for using v4l2_m2m_fop_mmap.
...
Filled in by the v4l2 core.

> +			f->pixelformat = fmt->pixelformat;
> +			if (type == FMT_TYPE_CODED)
> +				f->flags = V4L2_FMT_FLAG_COMPRESSED;

Also done by the v4l2 core.
...
The old crop ioctls are no longer supports. Use the selection ioctls instead.

Check carefully with other recent codec drivers that support selection
(drivers/media/platform/sti/delta/delta-v4l2.c for example) since these ioctls
can be tricky.
...
You really should be using the v4l2_m2m_ioctl_* helpers here.
...
Otherwise you will never get control events.
...
Will this firmware become available somewhere?
...
For the next version can you add the 'v4l2-compliance' output for both video nodes?
Use the master code https://git.linuxtv.org/v4l-utils.git/ when compiling v4l2-compliance
to make sure you use the latest v4l2-compliance code.
